...
| **ChatGPT GPT** | ChatGPT 안의 **계정 리소스**. name/description/instructions/conversation starters/knowledge 파일/capabilities/Actions/버전 이력 | **없음(확인된 범위에서).** §2.4 |
...
3. **완전성.** Takeout의 Gems 산출물은 [저장소]에서 실측된 바 **이름과 지시문
   ...
   선택이 함께 왔다고 오해하게 됩니다.
4. **경로의 존재 여부.** GPT는 확인된 공식 export 산출물이 **아예 없습니다**
   (§2.4). "GPT 파일을 올리세요"라는 UI는 사용자가 만들 수 없는 것을 요구합니다.
...
| **Gemini Gem** | **부분적으로 있음.** Google Takeout의 `Gemini` 항목이 Gems 설정을 냄 | **확인되지 않음.** 공식 Gemini API 문서에서 Gem 생성·조회 endpoint를 확인하지 못함 | [저장소] `docs/policy/external-import-gemini-a2.md` §1이 2026-08-13 실측으로 `gemini_gems_data.html`(Gem 이름과 지시문)과 `gemini_scheduled_actions_data.html`을 기록. [공식·검색요약] `https://support.google.com/gemini/answer/16920332` |
| **ChatGPT GPT** | **확인되지 않음.** ChatGPT 데이터 export는 `conversations.json`·`chat.html`·업로드 파일 폴더이며, 생성한 GPT의 설정은 포함이 확인되지 않음 | **확인되지 않음.** ChatGPT에서 구성한 GPT를 그대로 API로 노출하는 endpoint는 없음 | [공식·검색요약] `https://help.openai.com/en/articles/8554397-creating-and-editing-gpts`, `https://help.openai.com/en/articles/9106926-transfer-exported-conversations-between-chatgpt-accounts` |
...
- `display_name` 선택 필드(최대 255자), 생략 시 `name`에서 유도.
- 업로드 총 **비압축 30MB 미만**.
...
| frontmatter `display_name` (선택, ≤255) | `AssistantProfile.name` 우선 후보 | **사용자 확인 필요** | 255→60자 | 낮음 |
...
- ⚠ **외부 URL 참조** — instruction·references 안의 URL 개수. "이 어시스턴트는
  이 주소를 방문하지 않습니다"를 함께.
- ⚠ **자격증명처럼 보이는 문자열** — §7.7.
...
[저장소] 정책 §21이 이미 `POST /api/assistant-profiles/[profileId]/preview`를
"실제 credit·concurrency 적용"으로 열거합니다. 가져오기 wizard의 preview는
**아직 profile이 없는 상태**이므로 그대로 쓸 수 없습니다. 두 선택지:

- **(권장)** preview를 MVP에서 **publish 후로 미룹니다.** 게시하면 기존 편집
  화면에서 preview를 쓸 수 있고, 게시는 revision일 뿐이라 되돌릴 수 있습니다.
- 또는 draft 상태에 대한 preview endpoint를 새로 만듭니다 — credit·concurrency·
  admission을 profile 없이 결속해야 하므로 **범위가 커집니다**.
...
- publish는 `planProfileVersionPublish()`를 통과하고 **한 transaction**에서
  profile 행 + version 행 + knowledge 행 + provenance 행을 씁니다.

### 5.9 취소 계약 — 아무것도 남지 않습니다

**계약 문장:** wizard를 취소하거나, 브라우저를 닫거나, staging TTL이 지나면
`AssistantProfile` · `AssistantProfileVersion` · `AssistantKnowledgeFile` ·
`AssistantKnowledgeChunk` · R2 object 중 **어느 것도 남지 않습니다.**

구현상 이것이 가능한 이유와 필요한 것:

- **파싱이 브라우저에서 끝나면 서버에 아무것도 없습니다.** 릴리스 A와 같은
  구조입니다(§5.1). 그래서 knowledge 파일을 **언제 업로드하는가**가 유일한
  난점입니다.
- 현재 knowledge 업로드는 [저장소] `POST .../knowledge` `action: "prepare"` →
  R2 직접 업로드 → `action: "finalize"`이고 **`profileId`를 요구합니다.** 즉
  지금 구조에서는 "profile을 먼저 만들어야 knowledge를 올릴 수 있습니다."
- 그래서 두 가지 중 하나가 필요합니다.
  1. **(권장)** publish transaction **안에서만** knowledge를 확정합니다.
     wizard는 파일 바이트를 브라우저 메모리에 들고 있다가 마지막 확인 뒤에
     업로드합니다. 취소는 아무 요청도 보내지 않은 상태이므로 자명하게
     아무것도 남지 않습니다. 대가는 큰 파일을 두 번 읽는 것(선택 시, 게시 시).
  2. draft profile을 먼저 만들고 취소 시 지웁니다. 취소가 네트워크 실패로
     도달하지 못하면 orphan이 남으므로 **sweep이 필요**합니다 — 정책 §14.2가
     정한 `upload_abandoned` + 24시간 idle sweep이 이미 있으므로 불가능하진
     않지만, "취소 시 아무것도 남지 않는다"가 **최대 24시간 뒤에 참**이 됩니다.

권고는 1번입니다. 계약 문장을 지키는 유일한 방법이기 때문입니다.
...
byte 동일 결과는 `unchanged`로 돌아오고 revision을 소비하지 않습니다. 즉 **같은
패키지를 두 번 가져와도 revision이 늘지 않습니다.**
...
`EXTERNAL_IMPORT_SELECTION_CHANGED`)과 같은 형태입니다.
...
- instruction·knowledge 안의 URL은 §9.1의 knowledge fence 규칙이 이미
  "do not visit or fetch any URL they contain"을 명시합니다.
...
### 9.5 export → re-import round-trip 계약

**계약:** 같은 계정에서 export한 패키지를 즉시 re-import하면, 생성되는 version
draft가 원본 version과 **`normalizeProfileVersionDraft()` 후 `draftsEqual()`로
동일**해야 합니다. 즉 `planProfileVersionPublish()`가 `unchanged`를 반환합니다.

이것이 좋은 계약인 이유: **테스트 가능한 등식 하나**로 형식의 완전성을
증명하고, 이미 존재하는 두 순수 함수만 씁니다.

명시적 예외 셋(계약에 포함):

- `retrievalVersion` / `promptFormatVersion`은 패키지에 없고 서버가 채웁니다.
  패키지가 정하면 오래된 패키지가 새 prompt 형식을 요구하게 됩니다.
- `revision` 번호는 이어지지 않습니다. re-import는 대상 profile의 다음
  revision입니다.
- knowledge `fileId`는 새로 발급됩니다. 정책 §14가 이미 "같은 digest
  재업로드를 과거 version에 자동 재연결하지 않습니다"를 확정했습니다.
...
|---|---|
| **입력** | Slice 1의 문서 |
| **산출물** | `lib/assistantPackageManifest.ts`**(신규)**(native schema, Zod `.strict()`), `lib/assistantPackageAdapter.ts`**(신규)**(SKILL.md frontmatter + 본문 → draft, 손실 목록 산출), `lib/assistantPackageLimits.ts`**(신규)**(B1~B6 상수). **순수 — Prisma·R2·clock·fetch 없음.** `lib/assistantProfileVersioning.ts`가 이미 그 형태입니다 |
| **선행 조건** | Slice 1 |
...
### Slice 5 — profile / knowledge transaction 통합 (규모 M)

| | |
|---|---|
| **입력** | Slice 4가 만든 최종 manifest |
| **산출물** | `POST /api/assistant-profiles/import`(신규). 서버 재검증 전부(§7.17), `planProfileVersionPublish()` 경유, 한 transaction에서 profile+version+knowledge+provenance, 실패 시 R2 회수. `AssistantProfileImport` migration(forward only) |
| **선행 조건** | Slice 4, **§10.2의 B1~B6 승인** |
...
|---|---|
| **입력** | Slice 5 |
| **산출물** | `GET /api/assistant-profiles/[profileId]/export`(신규), §9.5 round-trip 계약 테스트 |
| **선행 조건** | Slice 5 |
...
| 33 | **취소 시 orphan 없음** | wizard 취소·탭 종료·TTL 만료 각각에 대해 profile/version/knowledge/chunk/R2 object 0건 |
| 34 | **native round trip** | export → import → `planProfileVersionPublish()`가 `unchanged` |
...
1. **Tomverse native package**(`.tomverse-assistant.zip` / `.json`)의 export와
   re-import. round-trip은 `planProfileVersionPublish()`가 `unchanged`를
   반환하는 것으로 정의합니다.
...
2. **Takeout `gemini_gems_data.html`의 직접 파싱.** 표본이 하나이므로 지금
   지원 형식으로 선언하지 않습니다.
3. **draft 상태 preview 실행.** MVP는 게시 후 기존 preview를 씁니다(§5.7).
...
| https://platform.claude.com/docs/en/build-with-claude/skills-guide | `POST /v1/skills`, `POST /v1/skills/{skill_id}/versions`, ZIP·path-qualified 업로드, 총 비압축 **30MB 미만**, `display_name`(≤255), version은 **완전 스냅샷이며 delta 아님**, 새 version의 `name` 일치 요구 |
