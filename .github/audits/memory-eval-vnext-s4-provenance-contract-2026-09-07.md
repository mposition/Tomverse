...
implementationDigest, apiVersion, workflowPath, executionClosureDigest, trustPolicyDigest다.
...
fallback은 없다. 이후 변경은 새 protocol digest·검토/승인을 요구한다.
...
controller는 E의 실행 closure(모든 local transitive import, schema, lockfile, runtime image,
...
T의 workflow·controller 실행 closure는 승인된 E closure와 같아야 한다. ledger/receipt 같은
...
R은 E의 조상일 필요가 없지만 E와 R 모두 T의 조상이고, 모든 승인·freeze·seal은 각 사용
...
E, scoringContractDigest, protocolDigest, sealDigest, S5ApprovalDigest, pairApprovalDigest,
...
protocol/holdout/scorer/prompt digest, budget receipt, validFrom/validUntil(최대 30분),
...
예약 유효시간 만료 후 새 run은 거부한다. 재개가 필요하면 사람이 별도 판단하되 contact
불명이 남은 예약을 새 ID로 laundering하지 않는다.
...
subject result artifact는 unit당 정확히 하나다. no-contact unit에는 marker만 허용한다.
...
모든 관련 attempt는 completed와 non-null conclusion, artifact 업로드 완료, contact/usage
...
- NFR-5: 시간은 UTC 초 단위, controller/custodian clock 오차는 5초 이내 증거를 MUST 요구한다.
  오차 불명은 valid window/cutoff 검증을 막는다. 30초 관측 간격은 성능 SLA가 아니다.
...
|---|---|---|
| ProtocolDescriptor | schemaVersion:1,specifications,implementationDigest,apiVersion,workflowPath,executionClosureDigest,trustPolicyDigest | §1; S3/S4 두 snapshot 순; mem-protocol-descriptor-1 |
...
| GenesisRecord | repositoryId,protectedRef,protocolDigest,workflowId,registeredAt,trustPolicyDigest,journalId,bootstrapApproval:BlobRef | bootstrap은 이 tuple을 사전 승인, 미래 genesis/event/checkpoint hash 없음 |
...
| Programme | programmeId,holdoutId,manifestDigest,sealDigest,model:ModelIdentity,promptRawSha256,evaluatedCommit,scoringContractDigest,protocolDigest,S5ApprovalDigest,pairApprovalDigest,ordinals | 한 pair/holdout만; ordinals=[1] 또는 [1,2]; mem-programme-1 |
...
| DispatchApproval | reservationDigest,programmeDigest,unitWorkflowId,workflowPath,E,R,T,protocolDigest,manifestDigest,scoringContractDigest,promptRawSha256,budgetReceiptDigest,validFrom,validUntil,approvedBy,approvedAt,signatureReceiptDigest | max 30분; mem-dispatch-approval-1 |
...
| NoContactReceipt | unit:RunUnit,reservationId,gateFailure,credentialGrantCount:0,contactIntentCount:0,checkpointDigest,observedAt,signatureReceiptDigest | gateFailure는 Refusal enum 또는 expired_dispatch; journal과 일치; mem-no-contact-1 |
...
| SnapshotProjection | cutoff,units,artifacts,journalCheckpointDigest | units={unit,E,T,event,status,conclusion,latestAttempt,jobsDigest,markerDigest} unit 숫자순; artifacts={artifactId,name,sizeInBytes,archiveSha256,runId} 숫자순; null 결론은 pending만 |
...
| VerifiedHistory | s2Context:S2HistoryContext,checkpoint:Checkpoint,holdoutState,programmeDigest,reservationDigest,technicalClosureDigest,latestInvalidationDigest,verifiedAt | state=sealed/opened/development; digest null은 아직 event 없음만; 검증 결과이지 입력 자기 선언 아님 |
...
programmeId,E,T,manifestDigest,scoringContractDigest,protocolDigest,model,promptRawSha256,
...
DispatchObservation, contact_claimed→ContactClaim, contact_intent→ContactIntent,
...
dispatch_approval, dispatch_observation, contact_claim, contact_intent, no_contact,
upload_receipt, budget_receipt, usage_receipt, technical_closure, admission_receipt,
invalidation, consumption이다. 승인=사람 approver, 접촉/key=controller+custodian 관측,
closure/admission=importer다. 사람이 signer라는 이유만으로 다른 역할 권한이 생기지 않는다.
...
먼저 등록하고 ProtocolDescriptor를 만든다. GenesisRecord → 암호화 payload → genesis
event와 서명 → checkpoint → ApprovedTrustRoot의 외부 등록 순이다. 뒤에 생긴 root를
genesis payload에 다시 넣지 않는다. Programme의 pairApprovalDigest도 사전에 승인한
{programmeId,holdoutId,model,promptRawSha256,E,scoringContractDigest,protocolDigest,ordinals}
...
문서 hash와 함께 명시적으로 다뤄야 한다. 아직 독립 검토나 사람 수용이 있었다고 쓰지 않는다.

- R-S4-CONTROL-PLANE: E/R/T 분리와 dependency closure의 완전성은 importer 검토에 의존한다.
  T의 control-plane이 E와 다르게 실행되지 않음을 증명하지 못하면 실행을 포기한다.
