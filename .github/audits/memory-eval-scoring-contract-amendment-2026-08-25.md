...
## 9. 승인 후 보완 [운영자 결정 · 2026-08-25]

**§승인 기록의 승인 범위(§1–§6)는 그대로입니다.** 아래 셋은 batch-101 작성
중에 규칙이 좁혀 주지만 정해 주지는 않는 지점이 나와, 같은 날 같은 운영자가
**해석을 확정한 것**입니다. 새 기준을 만든 것이 아니라 §2·§3·§4가 이미 정한
방향을 어느 쪽으로 읽을지 고정했습니다.

### 9.1 `communication_style`은 style 영역의 residual generic kind

`communication_style`은 일반 `preference`와 같지 않지만, style 전용 kind
중에서는 **fallback**입니다. §2 규칙 1의 전용 목록에 **추가하지 않고**, 판정
순서를 이렇게 씁니다.

1. `tone` · `verbosity` · `structure` · `formatting` · `language` ·
   `explanation_depth` · `citation_preference` · `code_style`
2. 어느 전용 kind에도 정확히 들어가지 않는 **상호작용 방식**이면
   `communication_style`
3. 답변 방식이 아닌 일반 선호만 `preference`

| 발화 | kind |
|---|---|
| "결론 먼저, 이유는 뒤에" | `structure` |
| "표로 비교해 주세요" | `formatting` |
| "짧게 답변해 주세요" | `verbosity` |
| "질문을 던져 가며 같이 생각을 정리해 주세요" | `communication_style` |

마지막 줄이 residual이 필요한 이유입니다 — 특정 형식이 아니라 대화 방식 자체에
관한 것이라 전용 kind가 없습니다.

### 9.2 풍부한 대화는 라벨이 아니라 재작성으로 해결합니다

`exhaustive` 선언을 유지하려면 **세 조건을 모두** 충족해야 합니다.

1. 독립적으로 유효한 memory가 **3개 이하**
2. 합리적인 검수자가 추가 durable fact를 쉽게 지적할 수 없음
3. 모든 gold의 kind와 disposition이 명확함

하나라도 어긋나면 라벨을 조정하지 않고 **atomic case로 재작성**합니다.

> **작성 규칙.** 유효 후보가 3개를 넘거나 gold 포함 여부에 합리적인 이견이
> 생기는 대화는 라벨로 해결하지 않고 atomic case로 재작성한다.

§4.2가 범주 ①을 가능한 한 atomic하게 쓰라고 한 것과 §4.1의 `exhaustive` 요건이
만나는 지점이고, 둘을 동시에 지키는 방법은 대화를 줄이는 것뿐입니다.

### 9.3 제3자 건강 정보 — 최소화한 사용자 중심 statement, 언제나 sensitive

제3자의 건강·장애 정보도 **`bulk_safe`가 될 수 없습니다.** 동시에 **제3자의
의료 프로필 자체를 memory로 저장해서도 안 됩니다.** 추출 허용 조건은 다음
다섯입니다.

1. 사용자가 직접 말한 사실일 것
2. 향후 사용자 지원에 **지속적으로 필요한 맥락**일 것
3. 사용자 중심의 `constraint` 또는 `relationship`으로 **정규화**할 것
4. 필요한 **최소한의 제3자 정보만** 포함할 것
5. 언제나 `sensitive_review`

| | statement |
|---|---|
| 부적절 | 사용자의 어머니가 휠체어를 사용한다. |
| 적절 | 사용자는 휠체어 이용 가족과 이동할 때 계단 없는 경로가 필요하다. |

가족의 진단·치료 정보가 사용자 요청과 무관하게 언급됐을 뿐이라면 **추출하지
않습니다.**

**v3 프롬프트 계약에 한 줄이 추가됩니다** — *건강 정보에서 파생된 최소화된
statement 역시 sensitive입니다.* 정규화했다는 이유로 민감도가 내려가지
않습니다. §3.2의 deterministic pattern은 정규화된 `statement`를 검사하므로,
"휠체어"·"계단 없는 경로"처럼 파생형만 남은 문장도 상향 대상에 들어가야 합니다.
