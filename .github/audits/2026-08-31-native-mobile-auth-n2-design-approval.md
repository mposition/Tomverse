...
- **기준 HEAD (rev.2): `49c2624`** — rev.1은 `1ee7793`, 패킷 커밋은 `7a1dd05`
...
| **JWT를 권장하는 (수정된) 근거** | ① `proxy.ts`의 matcher는 `/api/*`만이 아니라 **거의 모든 요청**을 지납니다. 검증을 여기 두면 불투명 토큰은 **문서 요청까지 포함한 모든 요청의 임계 경로에 DB 왕복**을 얹습니다. ② DB가 흔들리면 인증이 곧 장애가 됩니다 — 지금은 `readSessionSecuritySnapshot`이 캐시와 백오프로 그 비용을 흡수하지만, proxy 계층에는 그런 완충이 없습니다. ③ 대안 B가 사는 것은 **즉시 취소**인데, 그것은 D12의 epoch + 짧은 캐시로 15초 이내에 근사됩니다 |
| **대안 B의 진짜 장점** | 취소가 **즉시**입니다. D12의 15초를 받아들일 수 없다면 대안 B가 정답이고, 그 경우 비용은 위 ①②입니다 |
| **비대칭 서명의 이점과 그 한계** | 검증자가 공개키만 보유하면 검증 계층 침해로 토큰을 찍어 낼 수 없습니다. **다만 proxy와 route가 같은 배포·같은 프로세스 그룹이면 실제 격리 이득은 작습니다.** 서명 비밀키를 실제로 분리 보관하는지는 **배포 결정**이고 이 패킷의 범위 밖입니다 — 분리하지 않을 것이면 HS256(대안 A)이 정직합니다 |
...
| **조회 실패 시 (rev.2 신설)** | **fail-closed.** `lib/sessionRevocationCore.ts`가 `lookup-error`·`user-not-found`를 별도 상태로 반환하고 거절하는 것과 **동일하게** 처리합니다. 캐시가 만료됐고 DB를 못 읽으면 **거절**입니다 — "못 읽었으니 통과"는 취소를 무력화합니다 |
| **대안** | 무캐시 조회(상한 0초, 부하 증가) / D1 대안 B의 불투명 토큰(상한 0초, 임계 경로에 왕복) |
| **승인이 확정하는 문장** | "취소는 최대 15초 안에 인가 경로에서 관측된다." 그 문장을 쓸 수 없다면 값을 바꿔야 합니다 |
| **최종 승인자** | Backend/AI + Security/Privacy |
| **상태** | `proposed` |
...
| 기전 | 방식 | 비용 |
|---|---|---|
| **B-1. 응답 보관** | 발급한 refresh token 원문을 창(권장 10초) 동안 봉투 암호화해 보관하고 재전달 | **원문을 저장합니다.** D5의 "어디에도 없다"가 창 동안 거짓이 됩니다. 키 관리와 확실한 삭제가 필요 |
| **B-2. 후속 재발급 (권장)** | 유효한 idempotent 재시도이면 **기존 후속을 폐기하고 같은 family에서 새 후속을 발급**합니다. 재사용 경보를 울리지 않고, family는 한 칸 더 전진합니다 | 원문 저장 **없음**. 클라이언트는 "같은 응답"이 아니라 "유효한 새 응답"을 받습니다 — 재시도의 목적(계속 쓸 수 있는 토큰)은 충족됩니다 |

**B-2를 권장합니다.** B-1은 이 설계의 가장 강한 불변식(원문 미저장)을 창 동안
포기하는데, 그 대가로 얻는 것이 "문자열이 같다"뿐입니다.

B를 고르면 함께 정할 것: 창 길이(권장 10초), idempotency key의 생성 주체(native)와
**digest만 저장**한다는 규칙, 그리고 **정책 문서 개정이 구현보다 먼저**라는 것.

| **최종 승인자** | Backend/AI + Mobile/Release |
|---|---|
| **상태** | `proposed` — A / B(+B-1 또는 B-2) / C 택일이 승인 행위 |
...
n1bReplacesMutationOriginCheck(request):
    ...
    c = parsed.claims
    for name in ["iss", "aud", "sub", "did", "fid", "jti", "tkn"]:
...
| **V24** | 정상 F, 미소비 RT | **유효한 `recordId` + 틀린 secret** | 401 `MOBILE_REFRESH_REJECTED` | **없음** | **F 무변경 — 폐기되지 않음** | `.refresh_rejected` | `recordId`, secret |
...
3. §7의 V17~V23, **V26·V26b·V27**이 통과한다.
