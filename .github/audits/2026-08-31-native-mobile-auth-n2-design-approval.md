...
- **기준 HEAD (rev.5): `d836680`** — rev.4는 `94e9e94`, rev.3은 `af06a99`, rev.2는 `49c2624`/`ffca693`, rev.1은 `1ee7793`/`7a1dd05`
- **A안 승인 검토의 기준본은 rev.4(`c0667ec`)이며, rev.5는 승인표 작성 안내 한 문단만 고쳤습니다** — 설계 내용은 rev.4와 동일합니다
...
| 6 | 취소 관측 상한 **N** — 그리고 D12의 1~3(기준 시점·경합·인가 금지·V29a/V29b)으로 그 N을 참으로 만드는 것 | D12 | 1·3 |
...
| **12** | **서명 비밀키를 실제로 격리 배포할지** — 하지 않기로 하면 **격리 이점을 주장하지 않으면 될 뿐**이고, 알고리즘 선택(D1)은 그와 별개입니다 | D1 | 2·3 |
...
| **17** | **B-2를 되살릴지** — 지연 요청·응답 유실·후속 사용까지 포함한 상태 전이의 정의와 검증이 먼저(§4.3) | §4.3 | 3 |
| **18** | `lib/sessionSecurity.ts`의 같은 staleness 성질을 **웹 경로에서도** 고칠지 | D12 | 3 |

**A안을 고를 때 승인표를 어떻게 채우는가 *(rev.5)*.**

| 항목 | A안에서의 처리 | 이유 |
|---|---|---|
| **5** | **해당 없음** | B 전용입니다 — 창 길이도 정책 개정도 A는 쓰지 않습니다 |
| ~~10~~ | **해소** | Node.js 런타임이므로 Ed25519 가용 여부를 물을 필요가 없습니다(D1) |
| **17 · 18** | **별도 후속** | 17은 A가 쓰지 않는 기전(B-2), 18은 이 저장소의 다른 코드 경로 |
| **6** | **남습니다** | N2 자신의 취소 관측 상한과 그것을 참으로 만드는 D12의 1~3은 **A안에서도 그대로 필요**합니다 |
| 1~4, 7~9, 11~16 | 승인 대상 | A안 여부와 무관 |

> rev.4는 이 문단을 "나머지(1~5, 7~16)는 A안 여부와 무관하게 승인 대상"이라고
> 적었습니다. **5번을 잘못 포함시켰습니다** — 5번은 B 전용이므로 A에서는 해당
> 없음입니다.
