...
허가됨으로, §8.2.1에 승인 범위를 적었습니다. **값은 어느 것도 이 문서를 쓴
에이전트가 정하거나 추정하지 않았습니다** — 관측·판정의 경계는 AGENTS.md의 규칙과
같습니다: 사람이 내린 판정을 옮겨 적는 것은 에이전트의 일이고, 판정 자체는 아닙니다.
정책 문서 Status와 release-gate registry는 **건드리지 않았습니다**(§8.3).

**rev.5 (2026-08-31, HEAD `d836680`)** — 설계 변경 없음. rev.4가 승인표 안내
문단에서 **5번을 A안 필수 항목에 잘못 포함**시킨 것 하나를 고쳤습니다. 5번은 창
길이와 정책 개정이므로 **B 전용**이고, A에서는 해당 없음입니다. 같은 문단을 표로
바꿔 5·10·17·18·6의 처리를 각각 적었습니다.

**rev.4 (2026-08-31, HEAD `94e9e94`)** — 검토가 D12의 남은 구멍 하나와 이전 문구
셋을 짚었고, 넷 다 맞았습니다.

| # | rev.3 | 무엇이 문제였나 | 고친 곳 |
|---|---|---|---|
| 1 | "낡은 결과를 **캐시에 쓰지 않는다**" | **캐시 저장 금지와 현재 요청의 인가 금지는 다릅니다.** 코드는 `snapshotCache.set(...)` 다음 줄이 `return snapshot`이므로, 저장을 막아도 **그 조회를 기다리던 요청은 낡은 값으로 인가**됩니다. 조회 0초·취소 1초·반환 20초면 캐시에 한 줄도 안 써도 그 요청은 19초 늦은 사실로 통과합니다 | **D12 — 계약 문장 추가, V29를 V29a/V29b로 분리** |
| 2 | V10에 `B-2는 새 후속` 잔존 | B-2는 rev.3에서 보류됐습니다 | **V10 — B-1 기준으로** |
| 3 | 미결 12번에 `아니면 HS256이 정직합니다` 잔존 | rev.3에서 철회한 문구 | **미결 12 — 정정** |
| 4 | D12 첫 행이 "15초 캐시이므로 상한 15초"로 읽힘 | 아래 정정 1~3을 충족해야 참이 되는 **목표**입니다 | **D12 첫 행 — 조건부 목표로** |

`aud`의 문자열·배열 원소별 정확 일치 계약은 검토자가 **RFC 7519 §4.1.3 원문으로
확인**했습니다. rev.3이 "원문 미확인"이라고 단 주는 그대로 두되, 이 줄이 그 확인을
기록합니다.
...
| | |
|---|---|
| **권장** | 무효화 확인은 **인가 경로(route)** 에서 합니다. `User.sessionsRevokedAt` + family/device epoch를 읽고 **15초 캐시**(권장값). **목표는 "취소 관측 상한 15초" 하나이지만, 캐시 TTL을 15초로 두는 것만으로는 그 목표가 성립하지 않습니다** — 아래 정정의 1~3을 함께 충족해야 참이 됩니다 |
...
2. **취소와 경합한 낡은 결과를 버립니다.** family/user별 write generation(또는
   무효화 시각)을 함께 읽고, 조회가 끝났을 때 **유효기간이 이미 지났거나 무효화
   generation이 바뀌었으면 그 결과를 캐시에 쓰지 않습니다.**

   **그리고 캐시 저장 금지와 현재 요청의 인가 금지는 다릅니다.** 저장만 막고 그
   결과를 **기다리던 요청에 그대로 돌려주면** 상한은 여전히 깨집니다. 지금 코드가
   정확히 그 모양입니다 — `snapshotCache.set(...)` 다음 줄이 `return snapshot`이고,
   조회를 기다린 요청은 **그 값으로 인가됩니다.** 조회 시작 0초, 취소 1초, 반환
   20초이면 캐시에 한 줄도 쓰지 않아도 그 요청은 19초 늦은 사실로 통과합니다.

   **계약:** *조회 완료 시 유효기간이 지났거나 무효화 generation이 변경된 결과는
   **캐시 저장과 현재 요청의 인가 모두에** 사용하지 않는다. 재조회하거나
   fail-closed로 거절한다.*

3. **지연 조회를 주입하는 테스트**로 약속한 상한을 고정합니다. 위 두 경계가 다르므로
   벡터도 둘입니다 — **V29a**(지연된 조회를 *기다리던 그 요청*)와 **V29b**(지연이
   끝난 *뒤에 오는 요청*). 숫자를 문서에 적는 것과 그 숫자가 지켜지는 것은 다릅니다.

> 위 반례는 설계상의 것이며, 구현이 없으므로 테스트로 재현한 것은 아닙니다.
...
| V10 | 미소비 RT | **동시 refresh 2건** | **§4 승인안에 따름.** A: 하나 200 / 하나 401 + F 폐기. B-1: 하나 200 / 같은 key면 **보관된 동일 응답** 200 | A: V9와 동일 | A: F 폐기 | A: `.reuse_detected` | RT, idempotency key |
...
| **V29a** | 무효화 조회를 **인위적으로 지연**(예: 20초)시키고, 조회 시작 후 취소 발생 | **그 지연된 조회를 기다리던 바로 그 요청** | **인가되지 않음** — 재조회 또는 fail-closed 거절 | 낡은 결과가 캐시에 **쓰이지 않음** | 취소가 관측됨 | — | — |
| **V29b** | V29a와 같은 상태 | 지연이 끝난 **뒤에 오는 요청** | **승인된 상한 N초를 넘겨 통과하지 않음** | 캐시에 낡은 값이 없음 | 취소가 관측됨 | — | — |
...
**rev.3이 추가한 벡터**: V24b·V24c·V24d, **V29a·V29b**, V30·V31.
...
> **rev.6: 승인됨.** 아래는 승인자가 내린 판정을 **그대로 옮겨 적은 것**입니다.
> 값은 어느 것도 이 문서를 쓴 에이전트가 정하거나 추정하지 않았습니다.

| 역할 | 판정 | 서명 | 날짜 |
|---|---|---|---|
| Backend/AI | **approve** | `mposition` | 2026-08-31 |
| Mobile/Release | **approve** | `mposition` | 2026-08-31 |

**§4의 택일:** **A — 엄격한 1회 사용.** native 계층 single-flight는 필수 구현
요건입니다. (B-1·C는 선택되지 않았고, B-2는 rev.3에서 보류.)

**단일 승인자에 대하여.** 승인자가 명시한 바에 따라, Tomverse는 1인 조직이고
동일인이 제품·보안·릴리스 책임자로서 두 역할의 공동 결정을 승인했습니다.
registry의 `approvalPolicy.soleApproverAllowed: true`가 이를 허용하며, 남아 있는
분리는 지켜졌습니다 — **문서와 증거를 작성한 주체(에이전트)와 승인자는 동일하지
않습니다.**

### 8.1.1 확정 결정 *(2026-08-31 승인, 승인자가 명시한 값 그대로)*

**미결정 목록은 이것으로 닫힙니다.** 아래가 N2 구현이 따르는 계약입니다.

| # | 항목 | 확정값 | 관련 |
|---|---|---|---|
| 1 | access token 수명 · clock skew | **10분** · **±60초** | D3 |
| 2 | refresh idle · absolute | **30일** · **180일** | D4 |
| 3 | 이전 **서명 키** 검증 기간 | **15분** | D6 |
| 3 | 이전 **pepper** | **30일 + skew** 동안 검증 가능하게 유지하고, **성공적인 refresh에서 현재 pepper 세대의 새 token으로 전환** | D6 |
| 4 | 동시 회전 정책 | **A — 엄격한 1회 사용.** native single-flight는 **필수 구현 요건** | §4 |
| 5 | B안의 idempotency window · 정책 개정 | **해당 없음** | §4.3 |
| 6 | 취소 관측 상한 | **15초.** D12의 **조회 시작 기준 TTL**, **generation 경합 검사**, **낡은 결과의 캐시 저장 및 현재 요청 인가 금지**, **V29a·V29b 통과**를 모두 **필수 계약**으로 함 | D12 |
| 7 | rate limit | `refresh` **기기당 분 20 / 일 500** · `exchange` **계정당 분 5 / 일 20** · `logout` **기기당 분 10** | D15 |
| 8 | 기기 목록의 IP | **표시하지도 저장하지도 않음** | D16 |
| 9 | `MobileAuthEvent` 보관 | **90일 상한.** 계정 삭제 시 **사용자·기기·family를 식별하는 행과 식별자를 삭제**하고, **비식별 집계만** 보존 가능 | §6.5 |
| 10 | Edge Ed25519 가용성 | **해소** — Node.js 런타임 확인 | D1 |
| 11 | access token 형식 | **JWT/JWS + EdDSA(Ed25519)** | D1 |
| 12 | 서명 키 배포 격리 | 최초 구현에서 **요구하지 않음.** 따라서 **배포 격리의 보안 이점을 주장하지 않음.** 서명 비밀키는 **서버 전용**이며 client·로그·응답·**proxy 전달 헤더**에 노출하지 않음 | D1 |
| 13 | `N1B_BEARER_ROUTES` 초기값 | **빈 목록.** 각 route는 bearer 신원·소유권 전환과 **테스트 증거가 갖춰진 뒤 하나씩** 등재 | D18 |
| 14 | `MobileLoginGrant` | 수명 **60초**, client 결속 **PKCE S256 verifier**, **단일 사용 · 원문 미저장 · 로그 비노출** | D14.1 |
| 15 | 재설치 | **새 기기 등록.** 기존 family를 자동으로 이어받지 않음. 잔존 token은 **자동 세션 복원에 쓰지 않고**, 가능한 경우 **기존 family를 폐기한 뒤 로컬 자격증명을 제거** | D17 |
| 16 | `AUTH-03` 증거 | refresh token이 **WebView JS 컨텍스트와 bridge 응답에 존재하지 않음**을 검증에 포함 | D19 |
| 17 | B-2 재검토 | **별도 후속.** A안 승인과 구현을 막지 않음 | §4.3 |
| 18 | 웹 `sessionSecurity` 캐시 조사 | **별도 후속.** 단 **N2 구현에는 D12의 강화된 계약을 처음부터 적용** | D12 |

**모든 결정표 항목의 상태는 `proposed` → `approved`(2026-08-31, `mposition`)로
읽습니다.** 각 표의 `상태` 칸을 개별로 고치는 대신 여기서 한 번에 선언합니다 —
같은 사실을 열아홉 곳에 복사하면 언젠가 그중 하나가 뒤처집니다.

**승인 SHA:** `190056fc2ee9ffc923a8f6e1331081e272762d2f`
**승인자:** `mposition`
**날짜:** `2026-08-31`

*(설계 기준본은 rev.4 `c0667ec`. 승인 대상 문서는 승인 안내를 포함한 rev.5
`190056f`이며, 위 SHA가 그 커밋입니다.)*

### 8.2 작업 경계

| 단계 | 허용 조건 |
|---|---|
| **N2 구현** | **허가됨** (2026-08-31). Prisma migration, 토큰 발급·검증, endpoint, native bridge, 테스트 — §8.1.1의 확정값을 계약으로 |
...
3. §7의 V17~V23, **V26·V26b·V27**, 그리고 **V24b·V24c·V29a·V29b·V30·V31**이 통과한다.
...
### 8.2.1 이 승인이 뜻하지 않는 것 *(승인자 명시)*

이 승인은 **N2 구현 착수를 허가하는 설계·정책 승인**입니다. 다음 어느 것도
아닙니다.
...
N1b는 §8.2의 **네 가지 선행 조건을 충족한 route에만 opt-in**으로 적용합니다.
production 활성화, `AUTH-01`·`AUTH-04` 실기기 판정, R2의 `capacitor://` CORS
확인은 **계속 별도 승인·검증 대상**입니다.
...
- **§4는 A로 확정됐으므로 정책 문서 개정이 필요 없습니다.** B·C가 요구했을 개정은
  해당 없음이 되었습니다(§8.1.1의 5번).
- D14가 `EXEMPT_MUTATION_PATHS`에 세 경로를 추가하자고 제안하지만, **이 패킷은
  `lib/requestOrigin.ts`를 수정하지 않았습니다.** N2 구현의 일부입니다.
