...
### 3.2 gold 수정 — 후보 5건, case 5건

기존 no-gold가 결함이며, 새 datasetVersion에서 아래 gold를 갖습니다.
`ko-23`은 **비공개 선호 gold만 추가**되고, 철회된 위치에 대한 부정형 gold는
계속 없습니다 — `ko-23`a는 §3.1의 위반으로 남습니다.
...
**이 표는 후보를 셉니다.** `succ-assistant-ko-23`은 후보를 둘 냈으므로 위반과
gold 수정에 한 번씩, 두 번 나옵니다. §4의 B+ 이동은 **case**를 세므로 같은
`ko-23`을 한 번만 셉니다 — 두 숫자가 맞지 않아 보이면 단위가 다른 것입니다.
...
regression에 보존되는 10 cases는 두 집합입니다. Gold 결함이 있었던 `ko-23`,
`en-311`, `en-92`, `en-10`, `en-27`은 승인된 **수정 expected**로 보존합니다.
이때 `ko-23`은 하나의 case이며, 비공개 선호 gold만 추가되고 철회된 위치의
부정형 gold는 여전히 없습니다. `ko-3`, `ko-19`, `ko-15`, `ko-53`, `ko-12`는
**기존 expected를 그대로** 유지합니다.

**단위를 섞지 않습니다.** §3의 17·5·9는 **후보** 판정이고 `ko-23`은 후보를 둘
냈으므로 그 표에 두 번 나옵니다. §4·§5.2의 10은 **case**이며 `ko-23`은 한 번
셉니다. 이동·provenance·`replacementId`는 정확히 **10개**이고, `ko-23`a와
`ko-23`b를 별도 이동 행으로 만들지 않습니다.
