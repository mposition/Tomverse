...
**승인:** `mposition`, 2026-09-02, N2 검토 11차. 여덟 회차를 미결정으로 들고 있던
항목이고, 제가 고를 수 있는 것이 아니었습니다(비밀을 어디에 두느냐).
...
| §3 | 0번(store에서 읽기)과 2번(store 먼저 갱신)이 생겼고, store와 Railway가 어긋나면 **배포를 중단**합니다. 6번이 배포 후 두 활성 id 대조입니다 |
| §5 | 들어오는 길이 둘이 됐습니다 — 키 유출, 그리고 **store 평문 상실**. 후자에서 정상 회전을 시도하지 않습니다. 대응이 끝나면 store가 새 링의 정본이 된다는 것도 적었습니다 |
| 릴리스 체크리스트 | 정본 일치 확인과 배포 후 두 id 대조가 항목이 됐습니다 |

**store를 먼저 갱신하는 순서에는 이유가 있습니다.** 정본이 사본보다 뒤에 있으면, 그
사이에 배포가 끝났을 때 **정본이 배포된 값을 모릅니다** — 정본이라는 말이 그 순간
거짓이 됩니다.

**결정란 셋은 비워 두었습니다** — secret store 제품명, vault 이름, 복구 권한. 저장소에
승인된 store 제품이 없고, **문서가 정하지 않은 것을 정한 것처럼 보이게 만드는 쪽이
비어 있는 칸보다 나쁩니다.** 그 셋이 채워지고 승인돼야 §3의 0번·2번이 실행할 수 있는
문장이 되며, 그때까지 production 활성화를 하지 않습니다.
...
>
> **rev.13 (2026-09-02).** §2.2가 **(a)로 확정**됐습니다(`mposition`, 2026-09-02).
> §5.10이 계약과 그 반영 위치를 적습니다. 남은 것은 **store 제품명·vault 이름·복구
> 권한** 세 결정란입니다.
>
...
| **§2.2의 세 결정란** | **활성화의 선결 조건.** 정본을 secret store에 두는 것은 2026-09-02에 승인됐고 §2.2·§3·§5·릴리스 체크리스트에 계약으로 들어갔습니다. 다만 **store 제품명·vault 이름·복구 권한이 비어 있는 동안 §3의 "store에서 읽는다"는 실행할 수 있는 문장이 아닙니다.** 저장소에 승인된 store 제품이 없어 임의로 적지 않았습니다 |
