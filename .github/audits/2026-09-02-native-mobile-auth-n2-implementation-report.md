...
2. **[높음] 서버의 refresh 성공을 앱의 자동 회복으로 적었습니다.** `authenticatedFetch`는
   401 뒤 bridge에 다시 묻고 **같은 token이면 그 401을 그대로 반환**하는데, 그 token은
   `unknown_kid`로 거절됐을 뿐 만료가 아니므로 bridge는 같은 것을 돌려줍니다 — 갱신을
   촉발하는 것은 만료이지 401이 아닙니다. **native bridge 구현도 아직 없습니다**
   (`unavailableBridge`). → **서버 쪽 회전 가능성**과 **앱의 401 회복**을 갈라 적고,
   뒤쪽을 **G11**(필수 증거로 요구할지 미검증 전제로 명시할지)과 **X10b**로 남겼습니다.
   "회복한다"를 관측처럼 적지 않습니다.
...
**승인 대상 열하나(G1~G11)**, **검증 벡터 열여섯**, **승인란은 비어 있습니다.**
