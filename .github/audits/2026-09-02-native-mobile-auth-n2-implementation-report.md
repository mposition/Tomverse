...
2. **[중간] V12가 콘솔 유출을 잡지 못했습니다.** 반환값과 예외만 봤으므로, 진입점에
   ...
   경로를 한 프로세스에서 돌리고 **출력이 아예 비어 있어야** 합니다. 유출 변이 셋
   (`console.error(key)` · `console.log(pepper)` · `process.stderr.write(signing)`)이
   모두 V12a에서 실패합니다. **§5.12·§5.23에서 두 번 배운 것을 세 번째 검사에 옮겨
