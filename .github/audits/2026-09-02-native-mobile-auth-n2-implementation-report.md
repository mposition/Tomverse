...
**mutation 다섯으로 확인했습니다** — 파서 안에 `Write-Warning`·`Write-Verbose`·
`Write-Debug`·`Write-Host`로 문제의 줄을 내보내면 **1d가 실패**하고,
`Write-Output`으로 내보내면 반환값이 오염돼 **run 자체가 exit 1**로 끝납니다(그 경우
합성값은 출력에 나타나지 않습니다). 어느 경로로도 통과하지 않습니다.
