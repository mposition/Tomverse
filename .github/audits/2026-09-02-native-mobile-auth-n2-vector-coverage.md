...
| V10 | 동시 refresh 2건 → 하나 200, 하나 401, **family 폐기**(A안) | 같은 파일. **실제 `Promise.all` 경쟁**이며, 다섯 회 반복해 어느 쪽이 먼저 읽었든 결과가 같음을 확인 |
...
### 1.1 정정 — V10은 2026-09-02 검토 전까지 통과가 아니었습니다

이 문서의 첫 판(2026-09-02, commit `60383b2`)은 V10을 통과로 적었습니다. **사실이
아니었습니다.**

당시 구현은 조건부 UPDATE가 0행이면 `lost_rotation_race`로 거절하고 **family를
폐기하지 않았습니다.** 승인된 V10은 A안에서 하나 200, 하나 401, **family 폐기**이고,
그 동작은 A안도 B-1도 아닌 네 번째 것이었습니다. 더 나쁜 것은 결정적이지 않았다는
점입니다 — 패자의 *조회*가 승자의 commit 앞에 있었는지 뒤에 있었는지에 따라 family가
살기도 하고 폐기되기도 했습니다. 같은 요청 둘이 마이크로초 차이로 다른 결과를 냈습니다.

통합 테스트도 그 잘못된 동작(“family 생존”)을 명시적으로 고정하고 있었으므로, 테스트가
있다는 사실이 통과를 뜻하지 않았습니다.

고친 내용: 0행일 때 같은 트랜잭션 안에서 행의 **실제 상태를 다시 읽고** 그것으로
판정합니다. 소비·폐기됐으면 재사용(D8)이고, 만료·부재면 거절입니다. 그러면 답이
순차적으로 온 두 번째 요청이 받았을 답과 같아지고, 인터리빙에 의존하지 않습니다.
반복 실행 테스트가 그 결정성을 고정합니다.
