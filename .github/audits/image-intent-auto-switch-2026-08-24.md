...
| # | v6의 문제 | v7 |
|---|---|---|
| 1 | §5.1에 `hidden` + artifact off 설명이 **두 번 연속** | 중복 문단 삭제 |
| 2 | §5.1 "블록이 말해야 하는 것"의 첫 줄이 여전히 **"이미지를 생성할 수 없다"**로 넓음 | 부록의 계약대로 **raster workflow·인라인 raster**로 좁힘 |
| 3 | C-② 비용 칸의 `0`이 아래의 "공짜가 아니다"·302토큰과 모순으로 읽힘 | **추가 UI 0 / L0 문안 302토큰 / 채팅 turn 1회**로 분리 표기 |
| 4 | 공유 분류기의 **런타임 계약이 미정** — 서버·클라이언트 첨부 모양이 다름 | §5.1에 정규화 DTO·양쪽 adapter·동기 순수 요건과 **parity 테스트 6줄** 추가, 부록 B-7 신설 |
...
- 이 대화는 **raster 이미지 생성 workflow를 실행하거나, 생성된 raster 이미지를
  메시지 안에 반환할 수 없다** — "이미지를 만들 수 없다"로 넓게 쓰면 같은 요청에
  실리는 SVG 안내와 모순됩니다(부록 A-1·A-3)
...
"같은 함수를 부른다"만으로는 부족합니다. 서버는 route가 만든
`turnAttachmentDescriptors`를 갖고 있고, 클라이언트는 아직 업로드되지 않은
composer의 첨부 상태를 갖고 있어 **모양이 다릅니다**. 그래서 계약을 이렇게
둡니다.
...
- 분류기는 **동기·순수**여야 합니다. 클라이언트는 타이핑 중(정확히는
  `compositionend` 이후)에 부르고, 서버는 요청 안에서 부릅니다.
...
| 안 | 동작 | 비용 | 사용자 경험 |
|---|---|---|---|
| **C-①** 제출 전 비동작 안내 칩 | 첨부 + 편집 의도 분류 시 "첨부 이미지 편집은 아직 지원하지 않습니다" 칩 | **L1과 별개의 UI·locale·테스트** + 분류기 | 크레딧 쓰기 전에 앎 |
| **C-②** 제출 허용, L0에서만 고지 | 추가 UI 없음 | **추가 UI 0 / L0 문안 302토큰 / 채팅 turn 1회** | 크레딧 1 turn 쓰고 나서 앎 |
