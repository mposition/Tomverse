# Independent design review, round 5: multi-provider routing — deployment identity
...
Revisions 1 through 4 were each reviewed and **rejected**. Revision 5 claims to close
round 4's blocker, its three majors and its two factual errors -- and one defect the
previous revision introduced itself (see the design's §13.1). Judge revision 5 on its
own merits; do not re-run the earlier rounds.

Four rounds of rejection is itself information. If the design is converging, say so
and say what is left. If it is circling -- fixing one rule by breaking another -- say
that instead, and say which decision has to be made by a person before another
revision is worth writing.

## Read first

1. `.github/audits/multi-provider-routing-deployment-identity-design-2026-09-22.md`
   — **the design under review** (revision 5, Korean). §13 lists what changed from
   revision 4, and §13.1 the self-inflicted one; §16 has the review history. Note that three of its rules were *reversed* in this revision, not merely tightened.
...
Round 4's blocker was: a `routingPolicyDigest` with no observation of actual
...
3. **round 4 findings 종결 여부** — blocker 1건·major 3건 각각
4. **새 발견 사항** — severity (`blocker` / `major` / `minor`), 근거, 확인 방법, 권장 수정
5. **U1–U5 각각에 대한 권고** — 기각한 쪽의 실패 양상을 함께
6. **권장 작업 순서**
