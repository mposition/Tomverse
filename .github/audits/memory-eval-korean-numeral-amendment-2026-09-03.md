...
이유는 문맥 자유이기 때문입니다 — **비교의 양쪽이 똑같이 바뀌므로 과잉 매치는
충돌은 만들 수 있어도 false negative는 만들 수 없습니다.** 서로 다른 두 사실을
같은 정규형으로 만들 수 있는 행은 여기 들어갈 수 없습니다.

`CANON_STEP_ORDER`의 단계 이름은 `numeral_words_to_digits` →
`reviewed_numeral_expressions_to_digits`입니다. 이름이 곧 변경 내용입니다.
