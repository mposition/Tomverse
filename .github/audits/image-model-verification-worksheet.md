...
**`generation_config.max_output_tokens`는 `output + thinking` 합계를 bound하지
않는다.** 우리가 과금당하는 양은 그 합계다.

thinking 단독에 대해서는 더 조심스럽게 써야 한다. 낮은 상한의 모든 표본에서
thinking이 `limit − 3`을 추적했으므로 **측정 범위에서는** thinking 단독 제한과
일치했다. 그러나 이는 공식·보편 상한으로 인정할 근거가 아니다 — 두 상한, 한
모델, 열 표본이 본 것이 전부다. 확정된 것은 부정 쪽뿐이다: 과금되는 합계는
명백히 제한되지 않았다.

**`worst_case_cost_unbounded`의 뜻도 한 번 못 박아 둔다.** 실제 비용이 수학적으로
무한하다는 뜻이 아니라, **정책이 요구하는 유한한 최악 원가를 입증하지 못했다**는
상태다. 유한한 값이 존재할 수는 있으나 우리가 그것을 증명하지 못했고, 증명되지
않은 값으로 과금 설계를 세우지 않는다는 것이 이 상태의 내용이다.
...
완성된 이미지를 받았고, **초과분이 billable usage로 보고됐다**.

정확히 쓴다: 우리가 확인한 것은 응답의 usage counter이지 provider 청구서가
아니다. 청구서를 대조하기 전까지 "청구됐다"가 아니라 "과금 대상 usage로
보고됐다"가 관측의 범위다. 결론은 바뀌지 않는다 — 과금은 그 counter를 근거로
이뤄진다.
...
256·512는 당시 이름으로 `consistent_with_limit_bounding_thinking`을 냈다
(지금은 `consistent_with_limit_bounding_billable_output`). 상한 − 3이라는
...
Flash Lite의 반례가 Flash Image·Pro Image의 동작까지 직접 반증한 것은 아니다.
측정은 한 모델에서만 했다.

| 모델 | 상태 |
|---|---|
| `gemini-3.1-flash-lite-image` | **합계 상한 주장이 실측으로 반증됨** |
| `gemini-3.1-flash-image` | 동일 파라미터를 비용 상한으로 인정할 **일반 근거가 사라졌고** 모델별 긍정 증거도 없음 → 보류 유지 |
| `gemini-3-pro-image` | 위와 같음 (+ 기존 제품 판단 보류) |

**4단계(Flash Image 실측) 취소는 과학적 결론이 아니라 운영 결정이다.** "Flash
Image도 반드시 같은 방식으로 초과한다"를 주장하는 것이 아니라, 활성화할 계획이
없어진 상황에서 긍정 가능성을 추가 비용을 들여 조사하지 않겠다는 것이다. 활성화
계획이 되살아나면 그 모델에 대한 측정부터 다시 계획한다.
...
- **이미지가 출력 토큰으로 계상된다.** `output_tokens_by_modality`가 매번
  `image: 1120`이고 `total_output_tokens`가 그것을 포함한다. 검증된 1K 가격
  $0.0336 ÷ 1,120 = **$30.00/1M**인데, 이는 "Google 이미지 전체의 단가"가 아니라
  **이번 Flash Lite 1K 표본과 현재 가격표에서 확인된 등가 단가**다. 다른 모델·
  해상도로 일반화하지 않는다.
- **원가는 토큰 합이 아니라 가중합이다.** 이미지·텍스트·thinking에 각각 다른
  단가를 적용해 더해야 한다. 이 문서의 금액은 모두 그렇게 계산했다.
