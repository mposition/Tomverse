...
2. ~~`docs/ops/staging-access-boundary.md` 갱신~~ — 완료. §0에 "다섯 번째
   ...
   **이 한 파일은 develop PR에만 실립니다.** main의 사본은 아직 이전
   revision이고, 거기에 main 모양으로 같은 내용을 또 쓰면 develop이 main으로
   갈 때 확실한 충돌이 됩니다. 나머지 파일은 두 브랜치에서 바이트가 같으므로
   그대로 합쳐집니다. 이것이 §3의 교훈을 적용한 결과입니다 — 브랜치마다
   내용이 다르다는 것을 알고 나서 그에 맞게 나눕니다.
