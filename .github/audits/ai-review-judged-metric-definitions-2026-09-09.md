...
**다만 verdict만 읽으면 위 표가 제외한 것이 다시 들어온다.** 인용 claim은 gold
밖이므로 verdict가 요구되고, 제외된 `support` claim은 **기입할 수는 있지만
필수가 아니다** — `unruled` 검사가 채점 대상만 읽으므로, verdict 없이도
`scored: true`·`supportClaims: 1`로 통과한다. 문제는 요구 여부가 아니라
**기록에 남은 값을 원시 집계가 다시 세었다**는 것이다. 초판의 실험이 실제로
그랬다 — 문서는 제외한다고 적고 계산은 포함했다.
...
| **옆에 독립 발견이 없는 단독 `support`** | 제외되지 않으므로 조합의 나머지 조건(`finding` 발화 · 확정 · `false_finding`)을 **충족할 때만** 분자에 들어간다 | `syn-10` |
| `submittedAs: "prose"` | 분자에 넣지 않는다 — 제출된 발견이 아니다 | — |
| 한 case에 `false_finding` 3건 | **case를 1로 센다.** case 비율이지 건수 비율이 아니다 | — |

**단독 `support`에 특례는 없다.** 계약은 옆에 독립 발견이 있을 때만 제외하므로
(`judgedScoredClaims()`) 단독 `support` claim은 그 제출물 자체이고 채점기가
채점한다. 그러므로 이 지표도 그것을 모집단에 넣고, **그 다음은 다른 claim과
똑같이** `finding` 발화·확정·`false_finding` 세 조건으로 판정한다. 단독이라는
사실이 분자에 넣는 이유가 아니라, 제외 사유가 없다는 사실이 모집단에 남기는
이유다.

대안은 "역할이 `support`면 전부 뺀다"이고, 그것을 고르면 채점기와 이 지표가 서로
다른 모집단을 읽게 된다 — 그 대가를 알고 고르는 것이면 된다.
...
| 단독 `support` claim | **모집단에 남기고 같은 세 조건으로 판정한다**(채점기와 같은 모집단) | 역할이 `support`면 전부 제외 |
