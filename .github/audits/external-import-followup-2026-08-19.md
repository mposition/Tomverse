...
## 4. 정리 의무 — 2026-08-19T13:48:00Z 완료

아래 목록은 이 회차가 만들어 낸 정리 대상이며, 실행자 보고로 같은 날 13:48Z에 모두 처리했습니다. 회차 기록의 정리 표에 시각과 확인자를 적었습니다. 목록을 지우지 않고 남기는 이유는, 다음 회차가 **무엇이 정리 대상이 되는지**를 미리 알고 시작하기 위해서입니다.

실제 개인 export를 썼으므로 대상이 원본 파일만이 아니었습니다. 실제 개인 export를 썼으므로 대상이 원본
파일만이 아닙니다.
...
다음 회차도 끝나면 회차 기록의 「staging 데이터 삭제 (UTC)」와 「로컬 파일 정리
(UTC) / 확인자」를 채웁니다. 이 두 칸이 비어 있으면 기록을 동결하지 않습니다.
