...
| **검토 권고** | **US$7.00** | **US$14.00** | **+6.75%** |

**"검토 권고"는 승인이 아닙니다.** US$7.00/US$14.00은 검토 과정에서 제시된
**기술 권고값**이며, 이 문서를 쓰는 시점에 `@mposition`이 승인한 값이 아닙니다.
§5의 블록에 그 숫자가 들어 있는 것은 승인 시 옮겨 적을 후보라는 뜻이지 승인
기록이 아니고, `approvedBy`와 `approvedAt`은 **사람이 승인하는 그 commit에서**
채워집니다. (첫 판은 이 값을 `@mposition`의 값이라고 적었습니다. 권고와 승인은
다른 행위이고, 승인 기록에 귀속할 수 있는 것은 뒤쪽뿐입니다.)

근거는 estimator 오차입니다. 모형 최악값보다 6.75% 높고, **추정 input 비용 기준
약 49%의 여유**입니다 — input이 회차당 US$0.905이므로, 이 estimator가 공급자
tokenizer보다 input을 그만큼 적게 세고 있어도 상한 안에 들어옵니다.

**그래도 공급자 청구의 절대 상한은 아닙니다.** output 쪽은 상한에 닿는 최악
경우를 이미 가정한 값이라 여유가 이만큼 없고, §2.4의 이유로 마지막 호출은
지출 전에 비교되지 않습니다.
...
```ts
evalBudget: {
    // 승인하는 사람이 자기 handle을 씁니다. 여기 미리 적어 두지 않는 이유는
    // §2.3과 같습니다 — 채워진 이름은 승인의 기록이지 승인의 제안이 아닙니다.
    approvedBy: "@____",
    // 검토 권고값이며 아직 승인된 값이 아닙니다. 모형 최악값 6.5574902 위에
    // estimator 오차 여유를 둔 것이고, 여유 0이면 6.56 / 13_120_000입니다 —
    // 두 선택지는 §2.3에 나란히 있습니다. 승인하는 사람이 어느 쪽이든 고르며,
    // 아래 두 필드는 그 commit에서 채워집니다.
    maxUsd: 7.0,
    programmeMaxMicroUsd: 14_000_000,
    ticket:
