...
작업입니다. — **완료 (2026-08-25), §49.**

## 49. §48.6 구현 기록 — 확장 원장의 사람 (2026-08-25 · 완료)

D10이 열어 준 작업입니다. 7차(§44)는 원장을 **개수로만** 읽었고, 그 이유를
화면에 적어 두었습니다 — 각 행이 주소를 들고 있고 관리자가 그것을 봐도 되는지가
미결이라, 목록을 만드는 것이 곧 그 질문에 답하는 일이었기 때문입니다. §48이
답했으므로 목록이 존재할 수 있습니다.

### 49.1 무엇을 만들었나

`GET /api/admin/email-campaigns/{id}/recipients?waveId=` 가 wave 하나의 원장
한 페이지를 **마스킹된 상태로** 돌려줍니다. 공개는 §48이 만든 그 경로
(`POST /api/admin/email-deliveries/reveal`)를 `kind: "campaign_recipient"`로
부르는 것이고, **두 번째 공개 경로를 만들지 않았습니다** — 두 번째 경로는
역할 검사와 감사 기록이 빠질 수 있는 두 번째 자리입니다.

`revealEmailAddresses()`는 `lib/adminEmailAddressReveal.ts`로 옮겼습니다. 표가
하나였을 때는 delivery 모듈 안이 맞았지만, 이제 두 기능에 걸친 표 셋이 답하므로
campaign 화면이 "deliveries"라는 이름의 모듈을 import 하는 모양은 결국 누군가
두 번째 공개를 새로 쓰게 만듭니다.

### 49.2 화면 하나가 무엇인지 정하기

D10은 **화면**을 공개의 단위로 정했습니다. campaign에는 wave가 여럿이고, 여러
원장을 한 번에 펼치면 "어느 화면"에 답이 없습니다 — 공개가 상한을 넘거나,
화면에 보이는 것의 일부만 조용히 덮게 됩니다. 그래서 **한 번에 wave 하나**만
열립니다.

**다음 페이지는 공개를 닫습니다.** 주소를 페이지 사이로 들고 가면 감사된 호출
하나가 한 페이지씩 원장 전체를 덮을 수 있고, 기록은 여전히 "100건"이라고
말합니다.

### 49.3 이 작업이 드러낸 §48의 결함

**`/admin/email-delivery`는 한 화면에 200행까지 담을 수 있는데 공개 상한은
100이었습니다.** `?limit=101` 이상에서 "Show addresses (200)"는 검증에 걸려
400을 받고, 화면은 "Could not show the addresses."만 말합니다. 평범해 보이는
페이지에서 버튼만 동작하지 않는 상태였습니다.

상한의 의미 전체가 "화면 하나"이므로 **상한이 곧 화면이어야 합니다.**
`DELIVERY_PAGE_SIZE_MAX`가 `ADDRESS_REVEAL_MAX_IDS`를 빌려 쓰도록 바꿔 숫자를
하나로 만들었습니다. 200 → 100은 좁히는 변경입니다. 원장 페이지도 같은 상수를
씁니다.

**감사 기록의 `targetType`도 하나로 박혀 있었습니다**(`"EmailDelivery"`).
그대로 두면 campaign 원장 공개가 `EmailDelivery` 아래 적히고, 잘못된 곳을
볼 줄 아는 사람만 찾을 수 있게 됩니다. `ADDRESS_REVEAL_TARGET_TYPES`가 kind에서
유도합니다.

### 49.4 응답에 없는 것

주소는 `listWaveRecipients()`를 나가기 전에 마스킹되므로 응답에도 브라우저에도
없습니다. `emailAddress`는 타입에서 **선택적이 아니라 부재**라, 검사를 빠뜨려서
렌더할 수 있는 필드 자체가 없습니다.

`deliveryId`도 나가지 않고 **존재 여부만** 나갑니다. 그 id는 이 행이 outbox로
이어지는 연결이고, 사람을 보여 주는 화면은 그것을 쓸 데가 없는 반면 응답에 실린
id는 누군가 다른 endpoint에 물어볼 수 있는 id입니다.

주소 없는 행은 `null`이지 점 세 개가 아닙니다. `no_email`로 제외된 사람에게
마스크를 그리면 있는 주소를 가리고 있다고 말하게 됩니다.

### 49.5 검증

- 신규 unit 5건(총 14건), DB 통합 8건 신규, admin E2E campaigns 17/17 ·
  read surfaces 36/36
- unit 5,627건 전수, 실패 0
- `check:api-cache-control` · `check:enum-constraints` ·
  `check:data-domain-registry` · `check:encoding:strict` · lint · production build
- 부정 assertion 셋은 변형 검사로 실제 실패함을 확인했습니다 — 공허하게 통과하는
  guard를 남기지 않기 위해서입니다.
