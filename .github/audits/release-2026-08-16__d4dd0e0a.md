...
```
Read 88 model(s); the application creates rows in 84 and deletes rows from 37.
Registered as bounded: 12; as deliberately retained: 16.
No unbounded, unswept table without a stated reason.
```

Nothing is unaccounted for, and one thing is accounted for in a way that needs
saying rather than ticking.

**`ChatAttemptUsageAdjustment` is new in this release** and appears in the
"removed only when their parent is" list -- the section the report prints its
own caveat next to: *a cascade answers whether the row outlives the account; it
does not answer whether the table stops growing while the account is active.*

Its chain is `ChatAttemptUsageAdjustment` → `ChatAttemptUsage` →
`ChatCreditReservation`, and that last one is **held**, pending the joint
finance-ops and privacy/legal decision due **2026-08-28 (AEST)**. So the new
table's only ceiling is a table that does not have one yet. It is append-only
...
| --- | --- | --- | --- |
| §1 automated gates, §6 build gates | Not yet run against this RC. They pass on the pull request's checks; this record needs them run at `82fa0bd2` itself and quoted | Release manager | `npm ci && npm run typecheck && npm run lint -- app components lib tests scripts && npm run test:unit && npm run test:server-contract && npm run security:regression`, plus the `check:*` list in §1 |
| §2 Nightly Visual Regression at the RC | The workflow cannot be dispatched from a development container | Release manager | `workflow_dispatch` on **Nightly Visual Regression** at `82fa0bd2`; record the run URL and who reviewed the diffs |
| §3 Staging verification of the RC | No staging deployment of `82fa0bd2` exists yet. **This is the item the release turns on**: five migrations and a provider-budget accounting fix that `/api/ready` cannot see | Release manager | Deploy `82fa0bd2` to staging, `curl https://<staging>/api/build-info` and confirm it names that SHA, then `/status` against `/api/models/status` in the same window |
| §3 One image generation per active provider on the RC | Requires the RC deployed with the flag on; production's flag is off and production does not run this build | Release manager | One generation each on `gpt-image-2`, `grok-imagine-image-quality-20260403` and `fal-ai/nano-banana-2`, then `GET /api/admin/image-generation` → each provider's **own** `providerBudgets[]` bucket moved. The defect this release fixes is invisible to readiness |
| §4 Accessibility rows 1–13 and 17–19 | Screen readers, mobile IMEs and a paired physical keyboard are not reachable from a container | @mposition | `.github/ACCESSIBILITY_QA_MATRIX.md` against this RC. Rows 14–16 carried from `b0cf10e` with a `git diff` justification: `.github/audits/accessibility-2026-08-15__5528317e19fb8f061f18a9fde98f68c9fecd6013.md` |
| §7.1 `prisma migrate status` at the RC, and the five pending migrations | No production or staging database is reachable from here | Release manager | `npx prisma migrate status` from a worktree at `82fa0bd2`; the five names are listed in [#651](https://github.com/mposition/Tomverse/pull/651) |
| §7.1 `report:credit-lot-invariants` re-run | `20260815012000_validate_credit_lot_non_negative` validates a CHECK deployed `NOT VALID`. The zero-violation survey behind it is from 2026-08-15 and is only as current as the day it ran | Release manager | `npm run report:credit-lot-invariants` against production, reporting zero, before the merge |
| §7.2 Production environment values | Environment variables are not readable from here, and are deliberately not read: `list-variables` returns every secret in plaintext | Infrastructure owner | The §7.2 rows checked in the Railway dashboard |
| §7.3 Cloudflare header spoof and the 421 direct-origin rejection | Requires a request from outside the edge against a real deployment. `CLOUDFLARE_ORIGIN_SECRET` was rotated 2026-08-15T14:13:54Z, so the previous run's evidence does not carry | Infrastructure owner | The two `curl` commands in §7.3 against the release deployment |
