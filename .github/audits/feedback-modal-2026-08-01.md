...
## 후속 (같은 날): 남은 위험 두 건 해소

### sanitizer가 패턴 기반이라는 한계

denylist는 알려준 형식만큼만 좋습니다. 두 축으로 보완했습니다.

**1. 형식이 아니라 모양으로 탐지하는 2차 패스.**
`redactUnknownSecrets()`가 credential 문자 집합의 최대 길이 run을 훑어
아래를 모두 만족하면 가립니다.

- 28자 이상
- 대문자·소문자·숫자가 모두 섞여 있음(또는 32자 이상 순수 hex)
- 서로 다른 문자 10종 이상
- Shannon 엔트로피 3.8 bits/char 이상

공백이 아니라 문자 클래스 기준으로 훑기 때문에 `key="..."`, `[...]`,
`(...)`처럼 따옴표·괄호에 싸인 값도 찾아냅니다. `=`는 run 내부에서 제외해
`label=SECRET`에서 라벨을 삼키지 않고 값만 가립니다.

반드시 살아남아야 하는 것은 명시적으로 통과시킵니다.

- **trace ID(UUID)** — 이 기능이 존재하는 이유입니다
- 모델 ID(`claude-haiku-4-5-20251001`), 파일 경로, 오류 코드
  (`PLAN_DAILY_CREDIT_LIMIT_REACHED`), ISO 타임스탬프, URL, 순수 숫자

**2. 사람이 마지막으로 확인할 기회.**
패턴이 무엇을 놓쳤는지는 패턴이 알 수 없습니다. 그래서 오류 신고 모달에
**실제로 전송될 sanitize된 본문을 그대로 보여주는** 접이식 영역을 넣었습니다.
"진단 정보 복사" 버튼도 원문이 아니라 이 텍스트를 복사합니다 — 미리보기와
전송 내용과 복사 내용이 항상 같습니다.

### 전송 중 닫기·Escape 잠금

없앴습니다. 닫기는 언제나 가능하고 요청은 계속 진행됩니다 — 사용자가 이미
결정한 제출을 중간에 버리는 편이 더 나쁩니다.

결과 보고 경로를 이원화했습니다. 대화 상자가 열려 있으면 기존대로 inline
`role="alert"`, 닫혀 있으면 toast입니다. 닫는 순간에도 "전송은 계속
진행됩니다"를 알립니다. 실패 이유는 상태로도 남아 다시 열면 초안과 함께
그대로 보입니다 — 대화 상자 상태 전체가 이어지는 것과 일관되며, 다음 제출이
시작되는 순간 지워집니다.

30초 abort 상한은 그대로 두었습니다. 이제 잠금이 아니라 요청 자체의 상한입니다.

### 검증

- `tests/feedbackPolicy.test.mjs` 39건(신규 9건) — 미지 형식 키 탐지,
  trace ID·모델 ID·경로·오류 코드 오탐 없음, 괄호·따옴표 우회 차단,
  미리보기와 전송 본문 일치, 닫기 잠금 부재
- `tests/e2e/feedback-modal.spec.ts` 49건(신규 6건) — 전송 중 닫기/Escape,
  닫은 뒤 도착한 성공·실패의 toast 전달과 초안 보존, 미리보기 내용과
  실제 전송 본문 일치
- `test:unit` 851, `test:server-contract` 100, feedback E2E 98(desktop+mobile)
- typecheck / eslint / build / security:regression 117 / encoding / accent 통과
