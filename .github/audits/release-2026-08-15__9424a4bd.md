...
- [x] `.github/ACCESSIBILITY_QA_MATRIX.md` filled in for this release SHA
- [x] No P0/P1 accessibility blocker outstanding
- [x] Any row still marked N/V is an accepted, named risk — not an oversight
...
- [x] `prisma migrate deploy` run against the target database
- [x] No pending migration remains (`prisma migrate status` is clean)
- [x] `20260730120000_add_user_sessions_revoked_at` is applied
- [x] `User.sessionsRevokedAt` is nullable, so the previous release still runs
      against this schema and a rollback does not need a down-migration
- [x] A backup or restore point exists from **before** the migration ran

```
Migrate output:     Database schema is up to date! (50 migrations found)
                    npx prisma migrate status, production DB, 2026-08-15
Backup / snapshot:  Prisma Postgres 자동 백업, AIHub / branch main,
                    2026-08-15 05:53 (현지) COMPLETED, 10.8 MiB
```

### 7.2 Environment

- [x] `CSP_MODE=enforce`
- [x] `REQUIRE_CLOUDFLARE_ORIGIN_SECRET=true`
- [x] `CLOUDFLARE_ORIGIN_SECRET` is at least 32 characters
- [x] `TRUSTED_PROXY_IP_HEADER=cf-connecting-ip`
- [x] `NEXTAUTH_URL` is exactly the public HTTPS origin. Session cookies lose
...
- [x] `E2E_AUTH_BYPASS` and `E2E_DISABLE_DATABASE` are unset, or at least not
      `"true"`, in production
- [x] `DATABASE_URL` and `DIRECT_DATABASE_URL` are private hosts, or carry
      `sslmode=verify-full` / `verify-ca`

### 7.3 Edge

- [x] Cloudflare strips or overwrites client-supplied `cf-*` headers.
...
- [x] A request that reaches the origin directly, without the Cloudflare origin
...
```
Deploy SHA:         851598eb8957342bc66d742596692961dbaec03f
Ran at (UTC):       14/08/2026 14:27
violationCount:     0
...
| --- | --- | --- | --- |
| §4 접근성 (스크린 리더·IME·외장 키보드·실제 확대) | 미실행 | @mposition |
  ACCESSIBILITY_QA_MATRIX.md를 63792fcd로 작성 |
| §1 e2e 시각 골든 | Windows 실행은 -win32 baseline이 없어 비교가 아닌 생성으로
  통과. 동작 단언은 유효. 시각 검증은 §2로 갈음 | @mposition | playwright.config에
  updateSnapshots:"none" |
| §7.8 AssistantKnowledgeCleanup, TokenEstimateShadowSample | 신규, 보존 결정 없음 |
  @mposition | retentionPolicyCore.ts 정책 또는 registry 등록 |
| 릴리스 체크리스트 template/기록 분리 강제 없음 | 검사 부재 | @mposition |
  staging-verification-features.mjs에 등록 |
| §7.3 Cloudflare 엣지 검증 | 미실행 | @mposition | §7.3의 curl 2건 |
| §7.7 credit lot invariants | 배포 후 항목 | @mposition | 배포 후 실행 |
| A11y #7–9 (VoiceOver / TalkBack / JAWS) | 해당 기기·라이선스 없음 | (이름) | 실기기 또는 JAWS 라이선스 확보 후 매트릭스 행 기입 |
| A11y #10–13 (한국어 IME, 외장 키보드) | 모바일 기기 없음 | (이름) | 폰에서 tomverse.app/chat 조합 중 Enter 확인 |
