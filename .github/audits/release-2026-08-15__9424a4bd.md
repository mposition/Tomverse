...
| --- | --- | --- | --- |
| §4 접근성 (스크린 리더·IME·외장 키보드·실제 확대) | 미실행 | @mposition | ACCESSIBILITY_QA_MATRIX.md를 851598eb로 작성 |
| §1 e2e 시각 골든 | Windows 실행은 -win32 baseline이 없어 비교가 아닌 생성으로 통과. 동작 단언은 유효. 시각 검증은 §2로 갈음 | @mposition | playwright.config에 updateSnapshots:"none" |
| §7.8 AssistantKnowledgeCleanup, TokenEstimateShadowSample | 신규, 보존 결정 없음 | @mposition | retentionPolicyCore.ts 정책 또는 registry 등록 |
| 릴리스 체크리스트 template/기록 분리 강제 없음 | 검사 부재 | @mposition | staging-verification-features.mjs에 등록 |
| A11y #7–9 (VoiceOver / TalkBack / JAWS) | 해당 기기·라이선스 없음 | @mposition | 실기기 또는 JAWS 라이선스 확보 후 매트릭스 행 기입 |
| A11y #10–13 (한국어 IME, 외장 키보드) | 모바일 기기 없음 | @mposition | 폰에서 tomverse.app/chat 조합 중 Enter 확인 |
