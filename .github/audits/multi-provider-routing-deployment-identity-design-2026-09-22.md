...
바꾸지 않습니다. 대체 식은 넣는 즉시 검증합니다. 기존 행이 전부 둘 다
null이라 그 스캔이 값을 바꾸지는 않습니다. `NOT VALID`는 이후 쓰기에
제약을 적용하고 기존 행 스캔만 건너뛰므로, 구멍을 연 수단이 아닙니다.

반환된 고정 객체는 덮어쓸 수 없습니다. pin의 `named()`는 다시 문자열만
받습니다.

남겨 둔 minor는 셋입니다. JavaScript `trim`이 보는 유니코드 공백과 SQL
문자 클래스가 보는 ASCII 여섯 자는 다릅니다. `named`와 `present`의 본문이
같습니다. `NOT VALID`는 이후 INSERT에도 제약을 적용하고 기존 행 스캔만
건너뜁니다. 구멍을 연 것은 검증을 미룬 것이 아니라 NULL을 통과시키던
식이었습니다. 즉시 검증을 고른 이유는 기존 행이 전부 둘 다 null이라는
것이고, 그 스캔이 텔레메트리 표에 잠금을 잡는다는 점은 그대로입니다.
