...
**상태: 승인됨 · 동결 (2026-08-27, @mposition).**

**이 문서는 `promptVersion`을 올리기 전에 문안을 동결하기 위한 것입니다.** §9의
네 조건이 모두 충족됐으므로 §3의 다섯 규칙은 **동결된 문안**이고,
`mem-extract-v5`는 여기 적힌 문장을 그대로 옮깁니다. 이 순서가 결과를 보고
규칙을 쓰는 일과 규칙을 정하고 나서 재는 일을 가르는 유일한 절차적 경계이며,
서명이 그 경계가 실제로 지켜졌음을 기록합니다.

**동결된 문안을 바꾸려면 새 개정과 새 서명이 필요합니다.** v5 실행 결과를 보고
이 문서의 규칙이나 §4~§6의 판정을 고치는 것은 이 개정이 막으려던 바로 그
일입니다.
...
| 4 | 사람의 승인과 서명 | **충족** — 승인자 @mposition, 승인일 2026-08-27 |
...
| 승인자 | @mposition |
| 승인일 | 2026-08-27 |
| 승인 범위 | §3 규칙 1~5의 문안, §4~§6의 케이스 판정, §7의 채점 계약, §8의 보류 결정 |
| 증거 | run1 — [run 32972243326](https://github.com/mposition/Tomverse/actions/runs/32972243326), commit `f6c60491`, `mem-eval-succ-2` digest `60aa43f1cf8ea23b…`, artifact `9609657913`, admissibility 6개 항목 통과 |

`docs/release-gates/tomverse-chat-v1.yaml`의
`approvalPolicy.soleApproverAllowed`가 1인 조직에서 한 사람이 두 역할을 겸하는
것을 허용합니다. 남은 분리는 **증거를 만든 주체와 승인자가 다르다**는 것이고,
여기서 증거는 CI가 만든 artifact이며 승인자는 사람입니다.

이 승인이 덮지 않는 것:

- **`mem-extract-v5`의 예산·pair 승인.** 새 pairing이므로 register 항목과 별도
  승인이 필요하며, `mem-extract-v4`에 승인된 US$15는 자동으로 이전되지
  않습니다.
- **`mem-eval-succ-3`의 동결.** 대체 케이스 작성과 재검수를 거쳐 별도로
  동결합니다.
- **release gate 전환.** registry의 `status`·`approvedBy`·`approvedAt`·
  `evidenceRefs`는 이 문서가 건드리지 않습니다.
