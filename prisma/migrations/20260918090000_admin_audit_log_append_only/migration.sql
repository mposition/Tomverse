...
--    entryHash (NULL when there is none), and its createdAt is not earlier
--    than that row's. The writer already does exactly this under the same
--    advisory lock, so it is unaffected; the lock is re-entrant within a
...
BEGIN
    ...
    IF head_created_at IS NOT NULL AND NEW."createdAt" < head_created_at THEN
        RAISE EXCEPTION 'AdminAuditLog entry % is dated before the chain head', NEW."id"
            USING ERRCODE = 'check_violation';
    END IF;
