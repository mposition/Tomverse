-- Assistant package import analytics events.
--
...
--
-- Adds the four events that measure importing a package: which step somebody
-- stopped at, what the wizard warned them about, and whether the import
-- finished. Their properties are three closed enums -- the step, a warning
-- kind, and what the parser read the package as.
--
-- A package carries somebody else's instructions, filenames and possibly
-- credentials, so the properties schema is strict and there is no key any of
-- those could travel in. No counts either: in a small population "five
-- documents" is closer to an identifier than to a measurement.
--
