-- Assistant profile discovery analytics events (Release C).
--
-- docs/policy/external-conversation-import-and-memory.md §14, §22.
--
...
--
-- Adds the three events that measure whether the composer's create entry point
-- earns its place: a create begun, a create finished, and a created profile
-- reaching the conversation it was started from. All three carry only
-- `assistant_profile_entry`, a two-value enum -- never a profile's name, its
-- description, its instructions or a knowledge filename. The properties schema
-- is strict, so an event carrying any of those is refused rather than trimmed.
--
