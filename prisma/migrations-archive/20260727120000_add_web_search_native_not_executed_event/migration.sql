-- Native web search settlement now distinguishes "requested on a native-
-- capable model, but the provider chose not to search this turn" from the
-- existing unsupported/failed/executed states -- the reserved surcharge is
-- always fully refunded for this case (see getSettledUsageCredits).
