-- STG-F008: the model picker now has a two-step funnel (recommended -> all
-- models), so the picker's own open/search/filter/confirm/abandon steps are
-- recorded. Search terms are never sent, only that a search happened.
