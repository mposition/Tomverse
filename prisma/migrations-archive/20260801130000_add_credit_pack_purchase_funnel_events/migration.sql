-- The credit-pack purchase funnel could not be measured end to end. Between
-- `pricing_view` and `checkout_started` the pricing page emitted nothing at
-- all, and its add-on CTA was a link to /chat, so every visitor who wanted
-- credits and gave up looked the same as a visitor who never wanted them.
--
-- These six events close that gap: the CTA being seen and clicked, the pack
-- being chosen, the sign-in detour being required and then resumed, and a
-- checkout the visitor cancelled at Stripe (which is not a failure and must
