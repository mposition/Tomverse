...
let endpoint;
try {
  endpoint = new URL("/api/internal/maintenance/notification-deliveries", baseUrl);
  const isLocal =
...
try {
  ...
  if (!response.ok) {
    console.error("Notification delivery drain failed:", response.status, result);
    process.exitCode = 1;
  } else {
    console.log("Notification delivery drain completed:", result?.result || {});
  }
} catch (error) {
  console.error("Notification delivery drain failed:", error);
  process.exitCode = 1;
} finally {
