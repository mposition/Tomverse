 ...
 * case counted fifty-four times, and §6.5 leaves that judgement to a person —
 * so the sheet has to put the evidence for it on the page.
 */
...
/**
 * The axes a same-boundary judgement actually turns on. Rendered side by side so
 * a difference is visible without the reviewer holding both cases in mind: a
 * replacement that quietly gained a turn, a gold, or a disposition is exactly
...
p("");
p(
  "> **자동 생성 파일입니다.** `npm run make:memory-eval-succ7-review-sheet`로",
);
...
p(
  "전제이며, 서명 전까지 `frozen`은 `false`이고 harness는 succ-6을 가리킵니다.",
);
p("");
...
for (const row of SUCC7_TRANSITION) {
  ...
  if (reg.correctionRecord.length > 0) {
    p("");
    p(
      "> 이 원본은 승인된 **수정 gold**로 보존됩니다. succ-6 행의 gold가 아닙니다.",
    );
  }
  p("");
  p("새 대화:");
  ...
  if (row.unresolvedPolicy) {
    ...
    p(
      "> 바꾸지 않습니다. 그래서 같은 경계 대체가 아니라 coverage repair입니다.",
    );
    p("");
  }
  p("새 gold:");
  ...
  for (const g of rep.expected ?? []) {
    ...
    p(
      `    - 근거: \`${g.evidence.evidenceMessageId}\` — "${g.evidence.evidenceQuote}"`,
    );
  }
  p("");
  p("경계 대조:");
...
p("");
p("`nearDuplicatePairs()`(§6.5)를 조립된 succ-7 전체에 돌린 결과입니다.");
...
p(
  "그래서 아래 세 표를 나눠 싣습니다 — 대체본끼리의 순위는 전체 순위에 묻힙니다.",
);
p("");
...
p("");
p(
  "각 cell의 대체본이 **서로 다른 케이스인지**를 봅니다. 개별 gold가 옳은 것과",
);
p("묶음이 다양한 것은 다른 질문이고, 뒤쪽은 위 54개 판정란 어디에도 없습니다.");
p(
  "한 줄에 하나씩, 대화 형태와 kind 분포를 함께 싣습니다 — 같은 형태가 한 cell을",
);
p("가득 채우면 그 cell은 표본 하나를 여러 번 센 것입니다.");
p("");
p(
  "| cell | 건수 | 대화 형태 | kind · polarity 분포 | 다양성 (충분 / 불충분) | 근거 |",
);
p("|---|---|---|---|---|---|");
...
p("");
p(
  "> 한 cell이라도 **불충분**이면 succ-7은 채택되지 않습니다. 개별 54건이 모두",
);
p("> 옳아도 마찬가지입니다.");
p("");
