...
// produce them are in report-marketing-reach.mjs, and the reason for the split
// is that the interesting part is the classification rather than the SQL.
//
...
/**
 * The three purposes consent gates. `security` and `billing` are locked on and
 * `service_status` is contract performance, so none of the three is a
 * marketing question (`lib/emailPreferenceCore.ts`).
 */
export const MARKETING_PURPOSES = ["product_updates", "newsletter", "promotions"];
 ...
 *     saying `granted` or `reconfirmed`. CASL and the Australian Spam Act put
 *     the burden of proving consent on the sender, and German case law is
 *     stricter still, so a switched-on preference with no record behind it is
 *     a send we could not defend.
 *
...
export const marketingReachRows = ({
  ...
  MARKETING_PURPOSES.map((purpose) => {
    ...
    return {
      ...
      enabledBySource: Object.fromEntries(
        rows
          .filter((row) => row.enabled)
          .map((row) => [row.source, row.count])
          .reduce((map, [source, value]) => {
 ...
 */
export const marketingReachFindings = (rows) => {
  const findings = [];
...
export const formatMarketingReachRow = (row) =>
