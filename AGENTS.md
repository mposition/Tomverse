...
- **eval 표본은 arm당 최소 300회(권장 500)이고, 판정은 점추정이 아니라 Wilson
  95% 구간 경계로 합니다.** arm당 60회는 smoke이며 은퇴 판정에 인용하지
  않습니다. `scripts/evalDefaultModel.mjs`가 미달 시 `SMOKE RUN`을 출력합니다.
