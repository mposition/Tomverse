...
test("the copy states the facts the policy requires", () => {
    // Locale-independent anchors: every translation must name the two
    // services, and none may omit the browser-only processing claim by
    // being suspiciously short.
    for (const [name, bundle] of Object.entries(LOCALES)) {
        const body = bundle.privacyPolicy.externalImport;
        assert.ok(body.includes("ChatGPT"), `${name} must name ChatGPT`);
        assert.ok(body.includes("Claude"), `${name} must name Claude`);
