...
test("both chat composers guard Enter against IME composition", () => {
  for (const file of [
    ...
    assert.match(
      source,
      /isComposing/,
      `${file} must not submit while an IME composition is active`
    );
    assert.match(
      source,
      /keyCode === 229/,
      `${file} must handle browsers that omit isComposing`
    );
    assert.match(source, /onCompositionStart/, `${file} must track composition start`);
    assert.match(source, /onCompositionEnd/, `${file} must track composition end`);
  }
});
...
test("the app ships branded not-found and error boundaries", () => {
  for (const file of [
    ...
    "app/(application)/chat/error.tsx",
  ]) {
...
test("blocking dialogs stack above the mobile drawer and the context menu", () => {
  const source = readRepoCode("app/(application)/chat/page.tsx");
  assert.doesNotMatch(
    source,
...
test("credit costs are exposed to assistive technology", () => {
  ...
  assert.match(
    source,
    /className="sr-only">\{accessibleLabel\}/,
    "aria-label on a roleless span is ignored, so the cost needs real text"
  );
});
