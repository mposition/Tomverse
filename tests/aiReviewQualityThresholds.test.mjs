...
const approvedEntry = (evaluationOverrides = {}) => ({
  ...
  evalBudget: null,
  evaluation: {
    // Two independent runs, each with its own artifact and its own numbers.
    // A single pooled figure would correspond to no artifact, so nothing
    // could check it.
    runs: [1, 2].map((runOrdinal) => ({
      artifactRef: `docs/ops/ai-review-evaluation-runs/run-${runOrdinal}.json`,
      runOrdinal,
