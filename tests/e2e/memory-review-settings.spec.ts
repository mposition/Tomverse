...
test.describe("memory review settings", () => {
  ...
  test("the AI personalization tab offers the memory entry point", async ({ page }) => {
    await prepareGuestPage(page, "ko");
    await mockAuthenticatedApi(page);
    await mockMemoryApi(page);
    ...
    await page.evaluate(() => {
      window.dispatchEvent(
        // Memory moved to the AI personalization tab: it shapes what a
        // model is told, which the data tab is not about.
