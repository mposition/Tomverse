...
export async function mockAuthenticatedApi(
  ...
  // binds it. In the real system this lives on the upload row and the client
  // is never believed about it; here the two mocks share one page-scoped
  // registry for the same reason -- the name on a restored card must come
  // from the save, not from whatever the composer happened to be holding.
  ...
  await page.route("**/api/conversations/qa-conversation/messages**", async (route) => {
    if (route.request().method() === "POST") {
      ...
      */
      const bound: Array<{ messageId: string } & NonNullable<
        QaConversationMessage["attachments"]
      >[number]> = [];
...
export async function mockAttachmentUpload(page: Page): Promise<AttachmentUploadQaState> {
  ...
  await page.route("**/api/chat", async (route) => {
    ...
    if (method === "PATCH") {
      ...
      // mock that still returned `key` would let a regression that reintroduced
      // key-passing keep passing its own tests.
      const body = route.request().postDataJSON() as {
        size?: number;
        name?: string;
        mediaType?: string;
 ...
 * control character reaches this file -- `tests/sourceControlCharacters` fails
 * the build on one, and a zip signature is two of them.
 */
