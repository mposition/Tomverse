...
// Server-decided from the media type, so there is no pair to disagree: a
// request calling a .docx "text" changes nothing about how it is read.
test("the kind follows the media type and nothing else", () => {
  assert.equal(messageAttachmentKindFor("text/plain"), "text");
  assert.equal(messageAttachmentKindFor("text/csv"), "text");
  assert.equal(messageAttachmentKindFor("application/json"), "text");
  assert.equal(messageAttachmentKindFor("application/pdf"), "file");
  assert.equal(messageAttachmentKindFor("image/png"), "file");
  assert.equal(
    messageAttachmentKindFor(
