...
- Advanced·Research 모델을 조합에 **새로** 넣어 저장하려면, 반복 기본 사용
  ...
  저장 요청을 보내지 않습니다.
- **현재 플랜에서 쓸 수 없는 모델은 선택 가능해 보이지 않습니다.** 잠금과
  필요 플랜을 표시하고 선택을 막으며, 업그레이드 경로를 함께 제공합니다.
  플랜을 아직 모르는 로딩 상태에서는 잠금 쪽으로 보수적으로 판정합니다 —
  잠깐이라도 선택 가능해 보였다가 저장에서 거부되는 흐름을 만들지 않습니다.
  서버의 400 검증은 이 표시와 무관하게 유지됩니다.
