...
**실행 가능한 카드를 앞에 두는 비용을 여기 적는다**: 살아남은 카드가 6개를
넘으면 잠긴 카드가 잘릴 수 있다. 첫 화면에서는 그것이 맞는 교환이고 — 새 대화를
여는 사람은 살 것을 고르는 중이 아니라 할 것을 찾는 중이다 — 잠금 노출 규칙을
약화시키지도 않는다. 그 규칙이 금지하는 것은 **클릭 뒤에 잠금을 숨기는 것**이고,
항상 있어야 하는 업그레이드 경로는 `image-generation-workspace` 계약이 정한
진입점들이지 이 갤러리가 지는 책임이 아니다.
