...
1. **C1이 바뀝니다.** "marketing은 전역 opt-in"에서 **"수신자 국가 rule + 호주
   발신자 authority, 둘 다 통과해야 발송"** 으로. 출발 값 표는 5.1.1에 새로 넣었고
   원본은 초안 4.3입니다 — 값이 갈리면 초안이 이깁니다. 표를 옮겨 적는 것은
   두 번째 진실을 만드는 일이라, **어느 쪽이 원본인지 표 옆에 적었습니다.**
...
8. **15.2와 21절 Q1이 Q1을 전역 차단으로 말하고 있었습니다.** 개정 이후 Q1이
   막는 것은 **EEA·영국의 soft opt-in뿐**입니다. 활성화 전에 닫아야 하는 것은
   R3(싱가포르 수신거부 주소)와 R2(발송 도메인 평판)이고, 22절 A5는 전제가
   사라져 개정 전 기록으로 표시했습니다.
...
함께 걸립니다 — 둘 다 통과해야 보냅니다. 원본은 제품 소식 재설계 초안 4.3이며,
값이 갈리면 그 문서가 이깁니다.
