...
- 법적 성격: **법률 자문이 아닙니다.** 21절의 질문 목록 중 **아직 열린 것이 막는
  ...
  **R2**(발송 도메인 평판)와 **R3**(싱가포르 수신거부 주소)이고, **EEA·영국은 그
...
2. **C8이 바뀝니다.** "묵시적 동의 미사용"에서 **"국가 rule이 허용하는 곳에서만"**
   ...
   **EEA·영국 soft opt-in은 열리지 않았습니다** — G(R1 외부 자문)가 회원국별로
...
8. **15.2와 21절 Q1이 Q1을 전역 차단으로 말하고 있었습니다.** 전역 차단은
   ...
   하는 것은 R3(싱가포르 수신거부 주소)와 R2(발송 도메인 평판)이고, **EEA·영국은
...
9. **A18을 아직 열린 결정으로 말하던 다섯 곳을 정정했습니다.** A18은 2026-09-14에
   **별도 Resend 계정**으로 결정됐고(22절), 1절 요약 8번·8.1·8.5·8.8·R23은 그
   ...
   15.2의 남은 선행 조건은 R2·R3(그리고 EEA·영국의 G와 Q1 잔여)입니다.

**이 개정은 S0의 전부가 아닙니다.** 초안 §12의 S0 행은 여섯 가지를 담고 있고,
여기서 한 것은 **권한 계약(C1·C8·5.1.1·5.6)과 분류표, 그리고 그 둘이 만든 층
구분**입니다. 나머지는 각자의 gate와 함께 남습니다.
...
v12로 처리 결과 답변이 주소만 있으면 나가게 되자, **보내지 않는 선택지가 사라졌습니다.**
중복 신고처럼 같은 사람이 이미 답을 받은 건에도 메일이 한 통 더 갑니다. 그 대안으로
운영자가 택하게 되는 것은 "신고를 닫지 않는 것"이고, 그것은 더 나쁩니다.

- Admin 완료 다이얼로그에 **"이번에는 메일을 보내지 않고 닫기"** 체크박스를 둡니다.
  기본은 해제이며(안내가 기본), 요청 body의 `notifyReporter: false`로 전달됩니다.
- 보류는 **그 호출 한 번**에만 적용됩니다. 신고자의 수신 설정이나 다른 신고에
  영향을 주지 않습니다.
- **lifecycle 이벤트를 만들지 않습니다.** 이벤트는 배달 성공의 증거가 아닙니다
  (주소가 없거나 suppression이면 이벤트가 있어도 발송은 거절됩니다). 이벤트가
  뜻하는 것은 **그 단계의 단 한 번뿐인 안내 시도가 이미 점유되었다**는 것이고,
  `(feedbackId, stage)`가 unique이므로 이벤트 생성만이 메일을 큐에 넣습니다.
  보내지도 않고 그 한 번을 써버리면 그 신고는 영원히 안내할 수 없게 됩니다
  (v12가 없앤 함정과 같은 것입니다). 종료 사실은 `Feedback` 행과 감사 로그에 남고,
  **같은 상태 버튼을 다시 누르면 그때 안내가 나갑니다.** (Admin 화면에서 이미 가진
  상태 버튼은 보통 비활성이지만, 안내하지 않은 종료 건에서는 활성입니다.)
- 보류 여부는 `AdminAuditLog`의 `notifyReporter`로 기록합니다. 화면 토스트도
  "보내지 않았다"를 명시합니다.

### v13 (2026-09-16) — Q4 해소: 이메일은 야간 전송 제한 대상이 아닙니다

- 제50조제3항 단서의 "대통령령으로 정하는 매체"를 **시행령 제61조제2항이
  전자우편으로 정합니다.** 이메일에는 21:00~08:00 별도 동의가 필요 없습니다.
  근거는 방송미디어통신위원회·KISA 「불법스팸 방지를 위한 정보통신망법
  안내서」(KISA-GD-2025-0037, 2025.12)이며, 이유까지 적혀 있습니다 — 전자우편은
  광고 수신 확인의 즉시성이 떨어져 이용자 통제가 비교적 용이하다는 것입니다.
- **KR profile의 `quietHours`를 비웠습니다.** seed 버전이
  `2026-09-16.jurisdictions.2`로 올라갔고, 실제 적용은 새 `EmailPolicyVersion`을
  만들어 활성화하는 사람의 행위입니다(§12.3).
- **지연 기계는 lane에 그대로 둡니다.** 창은 코드가 아니라 정책 버전 데이터이고,
  어떤 관할권이 창을 갖게 되면 seed 한 줄과 새 버전으로 되살아납니다. 기계를
  지우면 그때 다시 만들어야 하고, 캠페인 취소 시 미발송분을 건너뛰는 처리도 같은
  경로에 있습니다.
- **이 결론은 이메일에만 해당합니다.** 같은 내용을 SMS·푸시·알림톡으로 보내면
  그쪽은 여전히 야간 별도 동의 대상입니다.
- 남은 한계: 시행령 원문을 국가법령정보센터에서 직접 읽지 못했고(본문 미렌더링)
  안내서의 조문 재수록과 복수 2차 출처 일치로 확인했습니다. 관보 대조 미실시.
...
| # | 변경 | 근거 |
|---|---|---|
| 1 | **A18 결정** — marketing은 **별도 Resend 계정**. transactional 계정과 suppression 목록을 공유하지 않습니다 | 5.3.1, `docs/ops/a18-resend-suppression-boundary.md` |
...
- 제50조제3항: **오후 9시~다음날 오전 8시** 전송에는 별도 사전 동의. 다만
  **대통령령이 정하는 매체는 예외**이고, **시행령 제61조제2항이 그 매체를
  전자우편으로 정합니다 — 이메일은 야간 별도 동의 대상이 아닙니다**(Q4 해소
  2026-09-16). 안내서가 이유까지 적습니다: 전자우편은 다른 매체보다 광고 수신
  확인의 즉시성이 떨어져 이용자의 통제가 비교적 용이하다는 것입니다. **같은
  내용을 SMS·푸시·알림톡으로 보내면 그쪽은 여전히 야간 별도 동의 대상이며**,
  시각 판정은 한국 표준시 기준 **수신자 도달 시각**이고 국외 발신에도 적용됩니다.
  출처: 방송미디어통신위원회·한국인터넷진흥원 「불법스팸 방지를 위한
  정보통신망법 안내서」 KISA-GD-2025-0037(2025.12).
  **한계:** 시행령 원문을 국가법령정보센터에서 직접 읽지 못했고(본문 미렌더링),
  위 안내서의 조문 재수록과 복수 2차 출처의 일치로 확인했습니다. 관보 대조는
  하지 않았습니다.
...
| C8 | 묵시적 동의를 **국가 rule이 허용하는 곳에서만** 씁니다 — 호주의 추론 동의(초안 §4.4 관계 lifecycle, 승인 C의 조건 아래). EEA·영국 soft opt-in은 **G(R1 외부 자문)가 닫힐 때까지 쓰지 않습니다** | 아래 5.6, 5.1.1 |
...
규칙의 변경이 아닙니다. §12(구현 순서)는 그 뒤로 여러 번 바뀌었고, 이 계약이
규범적으로 참조하는 절이 아닙니다.
...
| **제공자 억제 목록** | **분리되지 않음 (확인됨)** | Resend의 suppression은 **team(계정) 전체**에 적용되며 도메인을 구분하지 않습니다. 같은 계정의 `news.*`에서 생긴 complaint가 `mail.*` 발송을 막습니다. 아래 5.3.1 |
...
1. 공식 문서 — [Resend, Email Suppressions](https://resend.com/docs/dashboard/emails/email-suppressions):
   suppression은 **team 전체**, 즉 그 계정의 **모든 도메인과 서브도메인**에 적용됩니다.
   확인일 2026-08-21, 표현 재확인 2026-09-14(v5) — region 단위 억제는 SES의 성질이고
   Resend 문서가 말하는 경계가 아닙니다.
...
| 2 | ~~결정한다~~ **결정됨 (2026-09-14): 별도 Resend 계정.** 같은 계정 안 team 추가는 생성에 유료 플랜이 요구되어 채택하지 않았고, 로그인이 다른 두 번째 계정을 만들었습니다. 결정 시점 transactional 계정의 suppression 등재는 0건이라 옮길 이력이 없었습니다 | 22절 A18, `docs/ops/a18-resend-suppression-boundary.md` |
...
그대로 유효하고**, 그래서 그 두 곳은 G(R1 외부 자문)가 회원국별로 확인하기 전까지
...
| suppression | Suppressions API + 대량 처리. **범위는 team(계정) 전체**(5.3.1) | Stream별 suppression + Subscription Change webhook | account/configuration-set/tenant 3계층 | 있음 | 있음 | 있음 | 있음 |
...
| Resend 계정 | 별도 (**A18 결정, 2026-09-14**) | 별도 |
| Resend API 키 | 별도 | 별도 |
...
5.3.1에서 확인했듯 Resend의 suppression은 **team(계정) 전체**에 적용됩니다.
...
그것입니다 — marketing 발송의 선행 조건은 **R2·R3**이며, **EEA·영국에는 두 개가
...
| 항목 | 내용 |
|---|---|
| marketing 발송 활성화 | ~~5.3.1의 결정 2를 먼저 확정~~ — **A18이 2026-09-14에 해소됐습니다**(별도 Resend 계정, 22절). 남은 전역 선행 조건은 **R3**(싱가포르 수신거부 이메일 주소)·**R2**(발송 도메인 평판)이고, 그 다음 flag 해제와 `news.` 도메인 warm-up(14.6)입니다. **EEA·영국은 그 위에 두 개가 더** 있습니다 — soft opt-in을 근거로 쓰는 것은 **G**, DE·FR·AT·CH 밖 국가로 보내는 것은 **21절 Q1의 잔여**(공통 baseline으로 아직 입증하지 못한 국가별 의무)가 각각 막고, 뒤의 것은 근거와 무관해 `express_consent` 발송에도 걸립니다 |
...
- [Resend — Email Suppressions](https://resend.com/docs/dashboard/emails/email-suppressions) — **suppression은 team(계정) 전체의 전 도메인에 적용.** 확인일 2026-08-21, 표현 재확인 2026-09-14(검토자 제시 + API 표면 + 공개 문서 문구 교차 확인)
- [Resend — Managing Webhooks](https://resend.com/docs/webhooks/introduction)
...
- **A14**(자동 opt-out 채택), **A2**(EU 리전), **A3**(일본), **A4**(중국),
  **A1**(B2C/B2B) — **다섯 중 넷**입니다. ~~**A5**~~(opt-in 비용)는 2026-09-16
