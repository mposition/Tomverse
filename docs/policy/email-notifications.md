...
- 개정: **v23 (2026-09-18).** 발송 진입점 allowlist와 로그인 방법 변경 안내를
  standard lane으로 옮겼습니다. 0절 참조.
...
둘로 나눈 앞쪽입니다. 뒤쪽(C-2)은 `SuppressionEntry` 쓰기 6곳과 A 단계의
trigger를 걷습니다.
...
2. **행 수 상한은 selector에 걸고 DB가 겁니다.** 원인을 `limit × n`개 읽어
   ...
   원인 일부만 가진 채 표시되고, 빠진 것이 하필 `privacy_request`면 해제할 수
   없는 줄이 해제 가능한 줄로 보입니다. 그래서 selector를 먼저 고르고, 그
...
5. **해제 요청은 화면이 본 cause id 목록을 함께 보냅니다**(`causeIds`, 필수).
   서버가 그 순간 스스로 읽은 집합은 **정의상 승인된 집합**이라 비교가 아무것도
   비교하지 않습니다. 목록 조회 뒤에 추가된 원인이 아무도 보지 못한 채 해제되는
   경로가 바로 그것입니다. 보낸 집합과 현재 집합이 다르면 `approval_stale`이며,
   비교는 **정렬이 아니라 집합 원소**로 합니다 — 한쪽은 PostgreSQL 순서로 오고
   다른 쪽은 호출자가 보낸 순서라, 서로 다른 collation으로 정렬하면 같은 집합이
   다르다고 판정됩니다.
   **개수 상한은 두지 않습니다.** `SuppressionCause`는 append-only이고 한
   selector가 가질 수 있는 활성 원인 수에 제한이 없는데(공급자가 soft bounce를
   재시도할 때마다 행이 늘어납니다) 목록은 그 전부를 돌려줍니다. 여기에 상한을
   두면 **보이지만 영원히 해제할 수 없는** selector가 생깁니다 — 전부 보내면
   스키마가 거절하고, 덜 보내면 집합이 다르며, 그 사이의 요청은 존재하지 않습니다.
   본문 크기는 `readLimitedJson`의 8 KiB가 이미 제한합니다.
6. **거절 코드는 네 상황을 구분합니다.** 보낸 집합이 현재와 다르면
   ...
   route에서 직접 고정하며, 거절이 아무것도 쓰지 않는다는 것까지 함께 봅니다.
