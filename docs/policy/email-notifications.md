...
5. **해제 요청은 화면이 본 cause id 목록을 함께 보냅니다**(`causeIds`, 필수).
   ...
   경로가 바로 그것입니다. 보낸 집합과 현재 집합이 다르면 `approval_stale`입니다.
6. **활성 원인이 없거나 손잡이가 그 집합에 없으면 `approval_stale`입니다**,
   `not_found`가 아니라. 손잡이는 해석됐고 **집합이 달라졌다**는 뜻입니다 — 다른
   사람이 해제했을 수도, 스스로 만료했을 수도, 못 본 원인으로 바뀌었을 수도
   있습니다.
...
9. **authority가 `entry`이면 해제를 거절합니다.** 이 build의 콘솔은 cause id를
   ...
   **이 배포의 rollback floor는 원인 전환(2026-09-17)입니다.** 이 조합은 문서와
   테스트로만 고정돼 있고 readiness가 거부하지는 않습니다 — 그 상태에서 GET은
