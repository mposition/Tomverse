...
1. **목록은 selector 단위입니다.** 한 줄은 주소·scope·purpose 하나이고, 그 위의
   **활성 원인 전부**를 함께 보여 줍니다. 차단 효과는 원인들의 합이므로 원인마다
   ...
   부추깁니다.
2. **만료된 원인은 목록에서 빠집니다.** `expiresAt`가 지난 soft bounce는 아무
   메일도 막지 않으며, 그것을 계속 보여 주는 화면은 없는 것을 해제하라고 시키는
   화면입니다. 판정은 읽는 쪽에 있습니다.
3. **손잡이는 cause id입니다.** entry id는 C-2가 쓰기를 멈추는 행을 가리키므로,
   그때 콘솔 전체가 눈먼 상태가 됩니다. cause는 들어가는 문일 뿐이고, 읽고 승인하고
   해제하는 대상은 그 selector의 활성 원인 전부입니다. 주소 공개(D10)도 같은 id를
   씁니다.
4. **해제 대상이 하나도 없으면 `approval_stale`입니다**, `not_found`가 아니라.
   손잡이는 해석됐고 그 selector에 활성 원인이 없다는 것은 **다른 누군가가 이미
   해제했다**는 뜻이며, 승인은 이제 없는 원인 집합에 묶여 있었습니다.
5. **entry는 selector로 지웁니다**, id가 아니라. C-2 이후에는 지울 행이 없고,
   0건 삭제는 실패가 아닙니다 — 이 경로가 그 변화를 넘어 계속 동작해야 합니다.
6. **authority가 `entry`이면 해제를 거절합니다.** 이 build의 콘솔은 cause id를
   ...
   **이 배포의 rollback floor는 원인 전환(2026-09-17)입니다.**
