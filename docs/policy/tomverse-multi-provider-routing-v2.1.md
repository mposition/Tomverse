...
> **상태: 제안(Proposed). 아직 채택되지 않았습니다.**
>
> 이 문서는 **그린필드 설계로 작성·검토되었습니다.** Tomverse에 이미
> `RoutingRun` / `RoutingAttempt` / `lib/router*.ts` / shadow 라우팅이
> 존재한다는 사실을 전제하지 않고 쓰인 문서입니다.
>
> 기존 구현과의 대조는
> [`.github/audits/multi-provider-routing-adr-gap-analysis-2026-09-22.md`](../../.github/audits/multi-provider-routing-adr-gap-analysis-2026-09-22.md)
> 에 있습니다. **이 문서를 구현 지시로 읽기 전에 갭 분석을 먼저 읽으십시오.**
> 정면 충돌 5건이 미해소 상태이며, 그중 목적함수 형태(§5)는 기존
> `docs/policy/tomverse-chat-router-score-policy.md`의 설계 원칙과 양립하지
> 않습니다.
>
> 기존 라우팅 정책: [`docs/policy/tomverse-chat-routing.md`](./tomverse-chat-routing.md)

- **작성일**: 2026-09-22
- **대체 관계**: v1.0, v2.0은 superseded.
