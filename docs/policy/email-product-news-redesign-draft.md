...
| **CH·KR·CA·AT·IE** | `express_consent` | CH는 FDPIC가 계정 개설만으로는 부족이라 명시. AT는 ECG-Liste, IE는 12개월 창 |
| **ZZ** | 발송 안 함 | — |
...
| 기존 · EEA | soft opt-in **불가**. DOI만 |
| 기존 · AU | 4.4의 관계 증거가 **실제로 있는** 주소만 |
| 기존 · express 관할 | 기존 DOI 기록이 있는 주소만 |
| 기존 · US·SG | opt-out 전환은 **별도 승인**(11절 F) — 현재 방침이 "신청한 사람에게만"이라 신뢰 문제가 함께 있습니다 |

**정책 변경 통지는 evidence도 consent도 만들지 않습니다.**
...
suppression 판정 시각**, **provider 제출 시각**.
...
| # | 무엇 |
|---|---|
| A | **C1·C8 개정** — 전역 opt-in과 soft opt-in 미사용을 국가 rule로 대체 |
| B | **가입 화면에 두 장치를 넣는 것** — 전환율 영향 |
| C | **호주에서 추론 동의에 기대는 것** — 입증책임이 우리에게 있고 ACMA 관할 안입니다 |
| D | **한국을 제품 내 안내로 돌리는 것** — 한국 사용자는 이메일로 기능 소식을 받지 못합니다 |
| E | **방침·약관 개정과 전체 변경 고지 발송** |
| F | **기존 US·SG 사용자를 opt-out으로 전환하는 것** — 현재 방침의 약속과 신뢰 문제 |
| G | **R1 외부 자문 의뢰** — EEA·영국 해제의 선행 게이트 |
