...
- **배포 중 공존은 권위 전환 세 배포(A/B/C)와 설정 정리 배포(D)로 없앱니다(C68, C75, C76, C90).** 한 배포 안에서 이전
  ...
  - **rollback 하한** — A 배포 중에는 지금 build까지. B 배포 중·설정 `entry`인 동안은
    ...
    중·후에는 B까지(설정 `causes` 유지). **D-1 배포 중·후에도 행이 남아 있는 한
    B까지입니다** — 행이 있으면 B도 `causes`를 읽기 때문입니다. 하한이 C로
    올라가는 것은 **행을 지운 뒤**이고, 그래서 v28은 지우기 전에 되돌릴 대상을
    먼저 확인하라고 합니다(이 줄의 원문은 행 삭제와 코드 삭제가 한 배포라는
    전제 위에 있었습니다). 설정을 다시 `entry`로 돌리는 것은 C 이후 금지입니다 —
    entry 쓰기가 없습니다.
