...
- Status: design, nothing built. **§8의 승인 패킷 여섯 항목이 하나로 확정되기
  전에는 구현을 시작하지 않습니다** — 일부만 받아 skeleton을 만드는 것도 포함해서.
...
## 8. 승인 패킷 — finance-ops가 정해야 하는 것

설계로 정할 수 없는 항목입니다. **여섯 개를 하나의 패킷으로 받습니다.** 일부만
답을 받아 skeleton부터 만들지 않습니다: 3번이 없으면 행을 쓸 수 없고, 4번이
없으면 그 행을 어떤 순서로 쓸지 모르며, 6번이 없으면 사용자에게 무엇을 보여줄지
모르는 채로 만든 화면을 나중에 다시 만들게 됩니다.

**비교 연산자를 각 항목마다 따로 정합니다.** "상한"과 "임계값"은 한국어로 같은
말처럼 들리지만 경계에서 반대로 동작하고, 그 차이는 경계값 하나의 지급이 승인
없이 통과하느냐로 나타납니다.

| # | 결정 | 답이 반드시 담아야 하는 것 | 경계 |
|---|---|---|---|
| 1 | 1회 지급 상한 / 기간 누적 상한 | 값, **기간 단위**(달력월 / 롤링 N일), **기간 경계를 걸치는 지급의 처리**, **시간대**(UTC / Asia/Seoul — 월 경계가 9시간 달라집니다) | 누적은 `newTotal > cap`이면 **거부** |
| 2 | 이중 승인 임계값 | 크레딧 수 | `amount >= threshold`이면 두 번째 승인 **요구** |
| 3 | 만료 | 기간, **그리고 기산점** | `now >= expiresAt`이면 만료. 기산점은 요청·승인 시각이 아니라 **실제 lot/ledger 생성 시각**을 권장합니다 — 승인이 지연돼도 사용자가 받는 유효 기간이 줄지 않습니다 |
| 4 | 소진 순서 | **lot 종류 간 순서**(goodwill이 구매보다 먼저/나중)와 **같은 종류 내부 순서** 둘 다 | 내부 순서는 `expiresAt` → `createdAt` → `id`. 부분 소진된 lot이 여럿일 때 결정적이어야 하고, `id`가 마지막 tie-break입니다 |
| 5 | 환불 상호작용 | "goodwill lot을 건드리지 않는다"는 **값 자체를 승인** | 기본값이라는 이유로 묵시적으로 채택하지 않습니다 |
| 6 | 사용자 가시성 | 공개 여부, 그리고 공개한다면 **안정적인 `reasonCode` allowlist**와 각 코드의 **번역 키** | 담당자가 쓴 내부 자유서술 사유는 **어떤 경로로도 노출하지 않습니다**. `metadata.reason`은 내부 전용이고, 사용자에게 가는 것은 `reasonCode` → `locales/*.ts` 뿐입니다 |

승인은 값만이 아니라 다음 네 가지와 함께 이 문서에 기록합니다. 그래야 나중에
"언제부터 이 규칙이었는가"에 답할 수 있습니다.

```
approvedBy:    ____________________   (finance-ops 담당자)
approvedAt:    ____________________   (승인 시각)
effectiveAt:   ____________________   (규칙이 적용되기 시작하는 시각)
policyVersion: ____________________   (이 패킷의 버전. 값이 바뀌면 올립니다)
```

`policyVersion`은 지급 시점의 규칙을 원장에서 되짚기 위한 것입니다 — 가격 정책의
`pricingVersion`과 같은 이유이고, 규칙 변경은 소급 적용하지 않습니다.
