...
### 3.4a 키워드로 유도되는 지표들은 **진단**입니다

**contradiction·omission의 precision·recall, 그리고 그것에서 유도되는
false-consensus rate는 의미 정확도가 아닙니다.** `scoreCase()`는 gold 항목의
`anyOf` 문구가 그 발견 종류의 **findings 필드**에 나오는지를 볼 뿐입니다. 그것이
답하는 질문은 하나 — **그 표현이 등장했는가.**

**필드 구분은 실제로 합니다.** 같은 문구가 발견 필드 밖 산문에만 있으면 TP 0이고,
그 범위 지정은 의도된 것입니다.

**의미 구분은 하지 못합니다.** 그 필드 **안**에 들어 있는 것이 독립적인 발견인지,
답변의 인용문인지, 곁들인 설명인지를 가르지 못합니다 — 같은 문구를 인용문으로
넣으면 TP 1입니다. 그리고 이것도 말하지 못합니다.

- **어느 답변을 지목했는가** — 다른 답변을 잘못 지목해도 같은 문구를 쓰면 통과합니다.
- **없다는 것인가 있다는 것인가** — gold와 정반대의 주장도 같은 문구를 담습니다.

**양방향으로 틀립니다.** 둘 다 답변 원문을 읽는 문제가 아닙니다 — 이 채점기는
답변을 보지 않습니다. 용어 목록에 없는 동의어로 옳게 지적한 발견은 **놓친 것으로**
세고, 문구를 짧게 하면 오지목과 반대 주장이 올바른 발견과 똑같이 **맞은 것으로**
셉니다. 측정은 `tests/aiReviewEvalScoringContract.test.mjs`에 고정돼 있습니다.

**`falseConsensusRate`도 같은 매칭에서 나옵니다.** 심은 항목을 키워드 pass가
하나도 못 찾으면 그 case는 false consensus로 셉니다. 그래서 **자기 말로 전부
보고한 검토자가 "모두 동의했다"로 기록**되고, 반대로 용어를 쓰면서 gold를 부정한
검토자는 그렇게 기록되지 않습니다. §6의 `falseConsensusRate` 상한이 이 값을
읽습니다.

**`inventedIssueRate`는 이 방식이 아닙니다.** 문제 없는 case에서 **제출된 발견
건수**를 세며 내용을 읽지 않습니다.

**그래서 이 지표들은 선별(screen)입니다.** 이름은 그대로 두는데, 이미 쓰인 artifact가
