...
| `lib/assistantPackageImportMetricsCore.ts` | 네 이벤트를 세는 순수 함수 |
| `lib/assistantPackageImportMetrics.ts` | 위 함수의 질의. server 전용 |
| `components/admin/AdminPackageImportPanel.tsx` | Admin Console의 단계·경고·완료 분포 |
...
- **6단계의 병합 대상 목록은 page render에서 서버가 읽어 prop으로 넘깁니다.**
  브라우저에서 목록 endpoint를 부르면 "요청이 한 번도 나가지 않았다"가 더는
  참이 아니게 되고, 그 문장이 곧 취소 계약의 근거입니다. prop에 담기는 것은
  표시용이며, 보내는 것은 profile id 하나뿐입니다 — publish가 의존하는 사실
  (대상의 revision과 identity digest)은 서버가 import 생성 시점에 다시 읽고
  publish에서 다시 확인합니다.
...
- **`merge`가 만드는 revision은 합집합이 아니라 검토한 내용 그대로입니다.**
  publish는 사용자가 확인한 draft를 그대로 게시하므로, 새 revision의 이름·설명·
  지시문·모델·manifest는 wizard가 보여 준 것과 같습니다. 대상이 이미 가진
  knowledge 파일은 **삭제되지 않고 profile에 그대로 남지만 새 revision의
  manifest에는 들어가지 않습니다** — 이전 revision은 이력에 남아 있으므로
  되돌릴 수 있습니다. 이 결과는 6단계 화면에서 선택 전에 알립니다. 되돌릴 수
  있는 일이라도, 사용자가 "합치기"라는 낱말에서 읽어 낼 수 없는 결과는 그
  선택지 옆에 적습니다.
