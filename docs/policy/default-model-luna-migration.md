...
3. **기존 사용자·대화 상태** — `scripts/run-default-model-reconciliation.mjs`
   ...
   **`--from`/`--to`는 실제 파라미터입니다**(ML-10, 2026-08-23). 이전에는 파일
   상수와 일치하는지만 검사했고, 그래서 다음 은퇴 때는 스크립트를 복사해
   고쳐야 했습니다 — 그 복사본이 위 승인 gate를 함께 가져온다는 보장은
   없습니다. dry run에도 두 값이 필요합니다(없으면 보고할 대상이 없습니다).

   **대신 타이밍 규칙이 검사가 됐습니다.** `--apply`는 쓰기 전에 registry를
   읽고, `--from`이 아직 `enabled`이거나 `publiclyListed`이면 거부합니다
   (`from_not_retired`). 행이 아예 없으면 은퇴를 증명할 수 없으므로 역시
   거부합니다(`from_unknown`) — 없는 행은 은퇴의 증거가 아닙니다. `--to`가
   비활성이거나 카탈로그에서 삭제됐으면 거부하고(`to_not_usable`), 공개
   목록에 없으면 경고합니다(동작하지만 옮겨진 계정이 picker에서 찾지
   못합니다). `--from`과 `--to`가 같으면 거부합니다(`same_target`) — 바뀌는
   것은 없는데 행마다 migration record가 남아 "옮겼다"고 말하게 됩니다.
   ...
   **이 스크립트는 은퇴 배포와 함께 실행합니다.** 대상 모델이 아직 정상
   동작하는 동안 사용자의 선택을 덮어쓰지 않기 위해서이고, 위의 registry
   precondition이 그것을 강제합니다.

   - `UserSettings.defaultModel`이 정확히 `--from` 값인 행만 갱신
   - `Conversation.selectedModels`는 문자열 치환이 아니라 **JSON 배열로 파싱**해
     항목 단위로 변환. 위치 유지, `--to` 중복 시 하나만 유지, 다른 ID 불변,
