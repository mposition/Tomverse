...
Status: active (2026-09-24 개정. 2인 승인 대기열은 폐지). 구현은 `lib/adminApproval.ts`의
...
1. **권한 있는 관리자를 셉니다.** 설정(`ADMIN_EMAILS`, `ADMIN_<ROLE>_EMAILS`,
   `ADMIN_ACCESS_EXPIRY_JSON`)에서 활성·미만료이고 **그 action이 요구하는 권한**
   (`approvalPermissionForAction()`)을 가진 관리자를 셉니다. 그 수는 실행을
   막지 않고 감사 metadata의 `eligibleApproverCount`에 남습니다.
   **`ADMIN_USER_IDS`로 허용된 관리자는 역할과 관계없이 셉니다.** ID 관리자는
   세션 이메일이 역할 목록에 있을 때 그 역할을 얻는데, 설정만으로는 그 이메일을
   알 수 없어 행이 `readonly`로 보입니다. 빼면 인원수가 감사 기록과 어긋나므로
   fail-closed로 셉니다. 요청자 자신의 ID는 요청자와 같은 사람으로 합칩니다.
2. **요청자가 그 집합에 있어야 합니다.** 권한이 없는 세션은 실행하지 않습니다.
3. **최근 재인증.** 기존과 같이 `assertRecentAdminAuthentication()`이 먼저
   통과해야 합니다.
4. **두 번째 관리자가 있어도 대기열로 돌아가지 않습니다.** 2026-09-24에
   2인 승인 대기열을 폐지했습니다. 권한 있는 관리자는 인원수와 관계없이 그
   작업을 실행하고, 감사 로그가 통제입니다. 자격 있는 관리자 수는 감사
   metadata의 `eligibleApproverCount`에 남습니다.
...
`action_has_bound_path`). 결속 없이 들어온 요청은 대기열로 가지 않고 거절됩니다.

## 5. 감사 기록에 남는 것

`admin_sole_approver.*` 행의 metadata:

- `action`, `rule: "general_sole_administrator"`, `eligibleApproverCount` — 그 작업을 실행할 수 있는 관리자 수. 1이 아닐 때도 대기열로 가지 않습니다.
...
**`AdminActionApproval` 행을 만들지 않습니다.** 새 작업은 승인 대기열에
들어가지 않습니다. 과거에 남은 `pending`·`approved` 행은 같은 요청자가 같은
작업을 다시 실행할 때 `expired`로 바뀝니다.

## 6. 바뀌지 않는 것

- 승인 API(`PATCH /api/admin/approvals`)는 여전히 자기 요청을 승인할 수 없습니다.
  새 작업은 그 API가 검토할 행을 만들지 않습니다. 과거에 남은 행의 승인은
  실행이 아닙니다.
...
- **실행은 같은 요청의 열린 승인 행을 닫습니다.** 요청자가 같고 action ·
  ...
  실행된 것이 없기 때문입니다. 결속된 두 경로(§4)도 같은 정리를 합니다.
  요청자·action·대상 단위 advisory lock을 잡은 뒤 읽습니다. 같은 변경의 행이
  이미 `executing`이고 아직 만료되지 않았으면 `approval_executing`으로 거절됩니다.
...
2인 승인 대기열은 설정으로 되돌아오지 않습니다. 되돌리려면 이 폐지를 도입한
변경을 되돌리고 이 문서의 Status를 바꿉니다. 결속된 확인(보존 정리의 dry run,
캠페인 카피 digest)은 그 대기열과 별개이며 그대로입니다.
