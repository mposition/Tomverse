...
10. **승인 증거는 권한 있는 사람의 required review다. 병합 주체가 아니다.**
    ...
    Commit statuses read)으로 직접 관측해 다음을 확인할 때만 승인이다.

    - base가 보호 branch이고, 기록된 base SHA와 같은가.
    - 병합 직전 head가 본 앱에 기록된 검증 통과 commit과 같은가.
    - 그 commit의 required check가 통과했는가.
    - **required review가 1건 이상 있고**, 그 review를 남긴 계정이 사람이며 승인
      권한이 있는가.
    - **그 review가 유효한가.** base나 diff가 바뀌면 이전 review는 승인이 아니다.
      보호 branch에 stale-approval dismissal(새 commit이 오면 기존 승인 무효)을
      **걸어야 하고**, 실제로 걸려 있는지 확인한 뒤에야 review를 승인으로 센다.
      `review.commit_id` 하나로는 부족하다 — base만 움직여 diff가 바뀌어도 그 값은
      그대로다. 설정을 걸 수 없는 동안에는 **결속 snapshot을 기록한 시각 이후 새로
      제출된 review만** 인정하고, 그 review의 id·`submitted_at`·`commit_id`를
      snapshot(base SHA·head SHA·diff digest·기록 시각)에 묶어 저장하며, 한 번
      무효가 된 review id는 다시 세지 않는다.

    하나라도 어긋나면 승인이 아니다. **review 없이 병합된 것은 승인이 아니고 정지
    대상이다.** 관측은 PR 번호 기준 멱등이고 확정 뒤 불변이다. runner의 자기 보고는
    승인 증거가 아니다 — 초안을 만든 쪽이 승인을 보고하는 것이고, 그 secret이 새면
    위조된다.
