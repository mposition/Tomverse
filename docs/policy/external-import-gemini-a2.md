...
## 6. HTML만 가진 사용자 — 거절하되 고칠 방법을 말합니다

My Activity는 JSON과 HTML 중 선택이며, 기본값이 항상 JSON이라고 가정할 수
없습니다. **JSON만 지원**하고 HTML은 거절합니다(2026-08-14 승인문 확정).
HTML 파서는 별도 결정이며, locale 의존 구조라는 원래의 우려가 **여기에만**
남습니다.

거절 방식이 계약의 일부입니다.

- 오류 코드는 `html_export_unsupported`이며, "읽을 수 없음"과 **구분**됩니다.
  사용자가 1분이면 스스로 고칠 수 있는 유일한 실패이므로, 일반 실패로 묶으면
  고칠 수 있는 것을 고객지원으로 보냅니다.
- 문구는 "내 활동을 JSON 형식으로 다시 내보낸 뒤 그 파일을 올려 주세요"입니다
  (`externalImport.parseFailedHtmlExport`, 7개 locale).
- **파일 선택기가 `.html`을 고를 수 있어야 합니다.** 고를 수조차 없으면 안내
  화면에 도달하지 못하고, 사용자는 자기 파일이 왜 안 되는지 알 수 없습니다.
- 판정은 **확장자**로 합니다. 경로 세그먼트와 파일명은 번역되므로(§3.1)
  `내활동.html`과 `MyActivity.html`이 같은 답에 도달해야 합니다.
- **다른 provider의 export에 섞인 HTML은 해당 없습니다.** ChatGPT export는
  `chat.html`을 늘 함께 담으므로, 이 판정은 *읽을 대화 데이터가 하나도 없을
  때*에만 적용합니다.
...
3. ~~HTML 전용 사용자를 첫 릴리스에서 거절하는 것이 맞는지~~(§6) —
   **2026-08-14 승인문이 "HTML은 범위 밖"이라고 정했습니다.** 거절하되 "JSON
   형식으로 다시 내보내 주세요"라고 안내합니다(`html_export_unsupported`).
4. ~~Gems를 A2에서 명시적 비목표로 유지할지~~(§4) — **승인문이 범위 밖으로
   확정.** scheduled actions와 첨부 복제도 같습니다.
5. ~~A2 scope 승인 기록~~ — **2026-08-14 기록됨**(§10, 상위 정책 §23).
6. ~~첨부로 들어온 중첩 아카이브를 거절이 아니라 skip으로 바꿀지~~ —
   **2026-08-14 결정됨.** 상위 정책 §5.2에 공통 규칙으로 반영했습니다(§3.2).

**남은 결정은 없습니다.** production 활성화만 §10의 별도 승인으로 남아 있으며,
그것은 결정이 아니라 **증거를 기다리는 항목**입니다.
