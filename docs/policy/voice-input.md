...
**남은 셋은 저장소가 답할 수 없습니다**: ZDR 자격·승인(영업 — 필수조건에서
빠졌을 뿐 신청은 열려 있습니다), DPA 편입과 처리 지역(법무), 그리고
§11.3.2-1의 `None` 대 30일(공급자 — 이제 차단이 아니라 출시 후 추적 항목).
...
| B-5 | provider 데이터 보존 (§11.3) | **차단** — 계정 관측 완료·ZDR 필수조건 결정 완료(2026-09-10, §11.3.2). 남은 것: **DPA·처리 지역 판정**과 위험 수용 **서명** |
...
결정됐고, 남은 것은 **DPA·처리 지역 판정과 위험 수용 서명** 둘입니다. 어느
것도 이 저장소에서 관측되지 않습니다.
