...
3. **provider 원가 검증 — audio 전용 register를 두고, `measured_clip`을 동기
   ...
   **유료 검증은 2026-09-08에 승인 아래 실행됐습니다(§6.1.3).** 기본 모델
   `gpt-4o-mini-transcribe`의 실제 청구가 관측됐고, 그 대조에서 **정가표에서
   읽은 input 요율이 transcription이 내는 요율이 아니라는 것**이 드러났습니다.
   `gpt-4o-transcribe`는 한 번도 호출된 적이 없어 미검증이며, register는 그
   모델의 audio 요율을 `null`로 듭니다 — 0도 text 요율도 아닌 **모름**입니다.
...
**1. transcription이 내는 input 요율은 정가표의 `Input` 열이 아닙니다.**

그 열은 text input이고, transcription의 유일한 입력인 audio input은
**$3.00 / 1M**이며 정가 페이지에 실려 있지 않습니다(2026-09-08 재확인 시에도
`$1.25 | $5.00 | $0.003 / minute`만 나옵니다). §6.1.1의 표는 정확하게 옮겨
적혔는데도 transcription의 원가를 내는 데 쓸 수 없습니다. output 요율은
맞습니다.

**이것이 `costObservation`이 존재하는 이유 그 자체입니다.** 공개된 페이지를 몇 번
다시 읽어도 나오지 않는 값이었고, 청구서만이 답했습니다. 정가를 잘못 읽은 것이
아니라, 정가가 답하지 않는 질문이었습니다.
...
한 번도 호출된 적이 없어 audio input 요율을 모릅니다. 정가표의 $2.50은 여기서
틀린 것으로 드러난 것과 같은 text 열이므로, 같은 혼동이 있다고 가정합니다.

**추론으로 채우지 않습니다.** mini의 $3.00에서 배수로 유도하는 것은 관측이
아니고, 이 절이 한 일의 정반대입니다. register는 이 모델의 audio 요율을
`null`로 들고, 검사는 그것을 **모름**으로 보고합니다.

기본 모델이 아니므로 이 보류가 지금 무엇을 막지는 않습니다. 다만
`VOICE_TRANSCRIPTION_MODEL`로 이 모델을 고르는 배포는 **원가를 모르는 채로
...
| B-4 | audio provider usage 예산 (§6.1-4) | **staging 완료 (2026-09-08, 서명됨)** — production 환경값 쓰기와 `main` 릴리스 후 검사 대기. 기록: `docs/ops/voice-provider-budget-records/` |
...
환경의 실제 한도 설정이 남았습니다. 표의 문구가 그 차이를 말합니다. B-5는 별개이고 법무 성격입니다.
B-6은 사람만 할 수 있습니다.

**production을 막고 있는 셋 전부에 사람의 손이 필요하고, 종류만 다릅니다.**
B-5는 공급자와 법무의 답을 기다리고(§11.3.2), B-6은 실기기와 사람의 서명을
기다립니다. B-4는 그 둘과 달리 외부의 답을 기다리지 않지만, **저장소가 스스로
끝낼 수 있는 것도 아닙니다** — production 환경변수 쓰기 한 번과, `main` 릴리스
뒤 그 배포의 shell에서 검사기를 돌리는 일 한 번이 남아 있고, 둘 다 저장소 밖의
권한입니다. staging 절반은 2026-09-08에 서명까지 끝났습니다
