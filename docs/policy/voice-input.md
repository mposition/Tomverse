...
**그래서 남는 위험은 무한 청구가 아니라 예산 고갈이었습니다.** cookie를
순환시키는 스크립트가 그날의 provider 예산을 소진시키면 실제 사용자가
거절됩니다. 이에 답하는 보호가 Turnstile이고, 게스트 개방 시점에는 없었습니다.

**2026-09-11에 추가했습니다** — `guest_voice`.

- **자기 action입니다. `guest_chat` 재사용이 아닙니다.** 메시지를 보내려고
  확인을 통과한 게스트가 그것으로 **제3자에서 하루치 초를 쓸 자격까지**
  보인 것은 아닙니다. `GRANT_COVERAGE`에서 `guest_voice`는 자기 자신만
  덮습니다.
- **게스트에게만 겁니다.** 로그인 사용자는 cookie를 지워도 살아남는 주체이고,
  확인을 요구하는 것은 이미 알려진 계정에게 스크립트가 아님을 증명하라는
  것입니다.
- **순서는 rate limit·media type 뒤, body 읽기 앞**입니다. 형식이 틀린 요청에
  퍼즐을 풀게 하지 않고, 퍼즐이 막으려는 비용(본문 읽기·컨테이너 검사·예약·
  provider 호출) 앞에 둡니다.
- **grant cookie는 모든 종료 경로에 싣습니다.** 확인을 통과한 게스트가 그
  뒤의 gate(예산·컨테이너·provider)에서 거절당했을 때 grant를 떨어뜨리면
  다음 시도에서 다시 확인을 요구받습니다. 사용자에게는 **끝나지 않는
  체크박스**로 보입니다.
- **클라이언트는 첫 시도에 토큰을 싣지 않습니다.** 유효한 grant가 있는
  게스트는 아예 확인받지 않으므로, 매번 토큰을 요청하면 필요 없는 사람 앞에
  위젯을 띄우게 됩니다. 서버의 `TURNSTILE_REQUIRED`가 위젯을 부르는 신호입니다.
- **`TURNSTILE_FAILED`는 재시도하지 않습니다.** 토큰이 제출됐고 거절됐으므로,
  곧바로 다시 요구하면 방금 거절한 확인을 사용자에게 반복시키는 것입니다.

**남은 것**: 이 보호는 자동화를 비싸게 만들 뿐 불가능하게 만들지 않습니다.
provider 전체 예산이 여전히 하루의 상한이며, 그것이 최종 방어입니다.
...
- §1(자동 전송 없음)과 §11.1(원본 미보존)을 완화하는 변경은 **릴리스 차단
  사유**입니다.
- **§4는 2026-09-10에 의도적으로 완화됐습니다**(게스트 개방). 이 줄은 그때까지
  "§4(로그인 전용)"를 차단 사유로 적고 있었고, 그대로 두면 문서가 자기 §4와
  모순됩니다. 지금 §4를 지키는 것은 "게스트를 막는다"가 아니라 **누가 쓸 수
  있는지를 `lib/voiceInputAccess.ts` 한 곳에서 정한다**는 것이며, composer와
  route에 각각 판정을 흩어 놓는 변경이 차단 사유입니다.
