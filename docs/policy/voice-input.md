...
| 모델 | 공표 audio input | 관측 실효 | 비 | output (공표=관측) |
|---|---|---|---|---|
| `gpt-4o-mini-transcribe` | $1.25 / 1M | **$3.00 / 1M** | 2.4 | $5.00 / 1M |
| `gpt-4o-transcribe` (§6.1.6) | $2.50 / 1M | **$6.00 / 1M** | 2.4 | $10.00 / 1M |

**output은 두 모델 모두 공표값과 정확히 일치합니다.** 어긋나는 것은 input 쪽뿐이고,
독립적으로 측정한 두 모델에서 **같은 배수**입니다.

**그 배수가 무엇인지는 확정할 수 없습니다.** Costs API는 금액을 돌려줄 뿐 과금
수량을 돌려주지 않으므로, **"요율이 다르다"와 "과금되는 단위 수가 다르다"가
동일한 관측을 만듭니다.** 계정·snapshot별 요율 차이, 별도의 billable quantity,
공급자 문서 오류, 과금 오류가 모두 열려 있습니다.

**그러므로 register는 두 값을 다 듭니다** — `audioInputPerMillionTokensUsd`(실제
청구를 재현하는 보수적 값)와 `publishedAudioInputPerMillionTokensUsd`(페이지가
말하는 값). 하나만 들면 "페이지가 맞다" 또는 "페이지는 무의미하다" 중 하나로
읽히는데, **둘 다 확립되지 않았습니다.**
...
두 거부가 모두 옳았다는 것이 §6.1.6에서 숫자로 확인됐습니다 — 실제 요율은
**$6.00/1M**입니다. 공표값을 썼다면 2.4배 낮게, mini에서 유도했다면 절반으로
기록했을 것입니다.
...
`line_item`은 Costs API가 금액을 쪼개는 이름이고, 토큰 수는 응답 자신이
보고한 usage입니다.

| line item | 토큰 | 금액 | 함의 요율 | 공표값 |
|---|---|---|---|---|
| audio, input | 95 | $0.00057 | **$6.00 / 1M** | $2.50 / 1M |
| text, input | 0 | $0.00000 | — | — |
| text, output | 26 | $0.00026 | $10.00 / 1M | $10.00 / 1M |
| 합계 | | **$0.00083** | | |
...
이제 숫자입니다 — 실제는 **$6.00/1M**, 유도값의 두 배입니다. 이 register가 추론을
거부하는 이유가 여기 있습니다.

**2. 공표된 $2.50은 이 청구를 재현하지 못합니다.**

공급자의 모델 페이지가 **"Audio tokens · Input"**으로 표시하는 값이 $2.50인데,
95 토큰에 대해 실제로 청구된 금액은 그것의 **2.4배**입니다. §6.1.3이 mini에서
본 것과 같은 배수이고, **output은 두 모델 모두 공표값과 정확히 일치**하므로
어긋나는 것은 input 쪽뿐입니다. 그 배수가 요율 차이인지 과금 단위 수 차이인지는
Costs API가 금액만 돌려주므로 **이 관측으로 판별할 수 없습니다**(§6.1.3-1).
