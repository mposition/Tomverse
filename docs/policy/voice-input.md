...
3. **provider 원가 검증 — audio 전용 register를 두고, `measured_clip`을 동기
   ...
   **티켓은 열 수 있는 참조여야 합니다.** `#<이슈 번호>`나 URL이며, 그럴듯한
   ...
   이 승인이 덮지 않는 것: 유료 검증(§6.1.1의 4개 값 대조)은 **별도 승인**이
   필요하고 아직 실행되지 않았습니다. 승인 없이 실행하지 않습니다.

   **`costObserved: false`인 채로 production을 허용하는 것도 별도 승인입니다.**
...
| B-3 | provider 원가 검증 계층 (§6.1-3) | **결정 완료, register 구축 완료** — 실제 비용 관측 대기(#1247) |
...
것이 같지 않은 항목이 둘 있습니다** — B-3은 register가 서 있고 실제 비용 관측이
남았으며, B-4는 코드가 서 있고 각 환경의 실제 한도 설정이 남았습니다. 표의
문구가 그 차이를 말합니다. B-5는 별개이고 법무 성격입니다.
B-6은 사람만 할 수 있습니다.

**남은 둘은 이 저장소가 답할 수 없는 것들입니다.** B-5는 공급자와 법무의 답을
기다리고(§11.3.2), B-6은 실기기와 사람의 서명을 기다립니다. 코드로 앞당길 수
있는 부분은 없습니다.
