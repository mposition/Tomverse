...
**나머지 둘도 같은 날 서명됐습니다** — 계약 전제 확인(DPA)과 처리 지역. 다만 두
서명이 각각 **열린 항목을 하나씩 남겼고, 기록은 그것을 서명과 분리해 적습니다.**

1. **법인 정식명칭이 미기입입니다.** 계약 전제 서명문의 `[호주 법인 정식명칭]`이
   대괄호인 채로 왔습니다. 권한과 계약 형태 확인은 유효하지만, **"어느 법인이
   당사자인가"에는 아직 답이 없습니다.** `Tomverse`는 제품명이지 등기 명칭이
   아닙니다.
2. **국외이전 고지가 미완입니다.** 처리 지역 서명문이 열거한 고지 항목을 현재
...
| B-5 | provider 데이터 보존 (§11.3) | **차단** — 판정과 서명 셋 모두 완료(2026-09-10, §11.3.2). 남은 것은 서명이 남긴 **열린 항목 둘**: 법인 정식명칭 미기입, **국외이전 고지 5개 항목 미완** |
...
들어왔습니다. **남은 것은 서명이 남긴 열린 항목 둘입니다** — 법인 정식명칭과
국외이전 고지 5개 항목. 뒤쪽은 저장소의 문구를 고치는 일이지만 **문구 작성이
아니라 법적 판단**이고, 이전받는 자의 국가는 서명문이 임의 기재를 금지했습니다.
