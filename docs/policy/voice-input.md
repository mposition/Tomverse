...
- **유료 검증**(§6.1.1의 네 값 대조). B-3은 register를 만들라고 했지
  ...
  하나에 대해서이고, `gpt-4o-transcribe`는 여전히 여기 남아 있습니다.
...
2. **개인정보 처리방침** — `privacyPolicy.voiceInput`, 7개 locale 전부. "외부 AI
   ...
   **그 승인은 현재 문구를 덮지 않습니다.** 승인은 문장이 아니라 SHA에 붙어
   있었고, 같은 날 US endpoint 고정이 철회되면서(§11.3.0 정정) 두 문자열이
   바뀌었습니다 — 산문의 처리 국가 문장과 2번 항목입니다. 현재 판본은
   **법률 승인 대기**입니다(같은 기록 §11-3).
