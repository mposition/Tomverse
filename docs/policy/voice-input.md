...
- production 활성화: **차단됨** — §14의 B-5(공급자 데이터 보존)와 B-6(실기기 검증)이 남아 있습니다. §6은 2026-09-02에 결정됐습니다
- 마지막 검토: 2026-09-08
...
| B-5 | provider 데이터 보존 (§11.3) | **차단** — 판정과 서명 셋 완료(2026-09-10, §11.3.2). US endpoint 고정은 공급자가 조직을 `incorrect_hostname` 401로 거절해 **철회**됐고 Global로 복귀했습니다(§11.3.0 정정). **2026-09-10 11:06:54Z에 첫 전사가 성공**해 deploy SHA·요청 성공이 관측됐습니다. 남은 것: 새 문구의 **법률 승인**(현재 판본은 미승인, §11.4), 수령 법인 **소재 국가의 계약 원문 대조**, 최종 판정·서명 |
...
**production을 막고 있는 둘 다 사람의 손이 필요합니다.** 법무의 답은
2026-09-10에 왔지만, 같은 날의 staging 관측이 새 차단을 만들었습니다 — 공급자가
이 조직에 대해 US 지역 endpoint를 거절합니다(§11.3.0 정정). B-5는 이제 관측이
아니라 **갈래 선택**을 기다립니다.
...
편입, 처리 지역이 모두 결정되고 전용 project·key가 만들어졌으며, 서명 셋이
들어왔습니다. 이어서 미국 지역 endpoint 고정이 병합되고(§11.3.0), 국외이전 고지
여덟 항목이 7개 locale에 들어갔으며(§11.4), **그 문구와 이전받는 자 명칭이
같은 날 승인·확인됐습니다**(같은 디렉터리의 `2026-09-10-us-regional-endpoint.md`
§8).

**그래서 남은 것은 문구가 아니라 관측입니다.** 이 코드가 실제로 도는 staging
deploy에서 Voice 요청 한 번이 성공하고 그것이 미국 endpoint를 썼다는 운영 증거가
생겨야 하며, 그 위에서 사람이 판정하고 서명합니다. **코드는 "요청이 미국
endpoint로 간다"를 보장하지만, "실제로 갔다"는 보장이 아니라 관측입니다.**
