...
- production 활성화: **B-1~B-6이 모두 닫혔습니다** (2026-09-10). 남은 것은 flag를 켜는 사람의 결정입니다. §6은 2026-09-02에 결정됐습니다
- 마지막 검토: 2026-09-10
...
## 4. 누가 쓸 수 있는가 — 2026-09-10에 게스트로 열렸습니다

**게스트도 로그인 사용자와 동일한 한도로 쓸 수 있습니다.** 제품 책임자 결정
(2026-09-10)이며, 아래 MVP 근거는 지우지 않고 남깁니다 — **무엇이 바뀌어
뒤집혔는지**가 이 절의 내용이기 때문입니다.

### 4.1 MVP의 근거 (2026-09-02, 지금은 뒤집힘)
...
오디오 1초는 사용자에게 무료이지만(§6.1-1) provider에게는 아닙니다. 게스트를
허용한다는 것은 **cookie 하나를 근거로 제3자에 대한 유료 호출을 제공한다**는
뜻이고, 게스트에게는 그 사용을 귀속시킬 계정도 없습니다.

**§6이 결정됐어도 이 판단은 바뀌지 않습니다.** 무료가 된 것은 사용자 쪽 값이고,
게스트를 막는 이유는 값이 아니라 **귀속과 abuse protection**입니다(§6.1.2).
열기로 한다면 `lib/voiceInputAccess.ts`의 정책 변경 한 곳이지, composer와 route에
흩어진 수정이 아닙니다.

### 4.2 무엇이 바뀌었는가 (2026-09-10)

위 문단의 마지막 문장이 그대로 실행됐습니다 — 변경 지점은
`lib/voiceInputAccess.ts` 한 곳이고, 나머지는 그 결정을 따라간 것입니다.

**바뀌지 않은 사실이 하나 있습니다.** MVP 근거가 지적한 것 — 게스트에게는
그 사용을 귀속시킬 계정이 없다 — 은 여전히 참입니다. 제품 책임자가 그 비용을
받아들이기로 한 것이지, 사실관계가 달라진 것이 아닙니다.

**주체는 `identifyChatCaller`의 `subjectKey`입니다.** 로그인 사용자는
`user:<hash>`, 게스트는 signed guest cookie에서 유도한 `guest:<hash>`입니다.
`ChatUsageBucket.bucketKey`는 임의의 문자열을 받으므로 schema 변경이 없고,
분당·일별 요청 한도와 초 예산이 세던 것을 그대로 셉니다.

**게스트 한도는 로그인 사용자와 같습니다**(§5, §7). 별도 숫자를 두지 않는 것이
결정의 내용입니다.

**한도가 실제로 무엇을 묶는지는 층마다 다릅니다.** 주체별 예산은 cookie를 지우면
새 주체가 되므로 하루 총량을 묶지 못합니다. 하루를 묶는 것은 그 아래의
**provider 전체 예산**(`VOICE_PROVIDER_SECONDS_PER_DAY`/`_PER_MONTH`)이고,
`reserveVoiceBudgets`가 모든 호출자에 대해 함께 예약합니다.

**그래서 남는 위험은 무한 청구가 아니라 예산 고갈입니다.** cookie를 순환시키는
스크립트가 그날의 provider 예산을 소진시키면 실제 사용자가 거절됩니다. 이에
답하는 보호는 Turnstile이며, **게스트 음성에는 아직 없습니다.** 새
`GuestTurnstileAction`과 클라이언트 토큰 획득이 필요하고, 이 변경에는 포함되지
않았습니다.

**cookie는 모든 종료 경로에서 발급합니다 — 거절 경로 포함.** 거절에서 cookie를
빠뜨리면 게스트는 매 시도마다 새 주체가 되고, 방금 거절한 한도가 다시 0부터
세기 시작해 두 번 다시 거절하지 못합니다. 증상은 청구가 아니라 **아무 한도도
작동하지 않는 침묵**입니다. route의 cookie 전달자를 `try` 바깥에 두는 이유가
이것이고(`catch`로 나가는 거절이 둘 있습니다),
`tests/server-contract/voice-transcription-route.test.ts`가 두 경로를 각각
고정합니다.

**`VOICE_AUTHENTICATION_REQUIRED`는 남습니다.** 처음부터 가리키던 조건 —
**해석 가능한 주체가 아예 없음** — 을 계속 가리키며, 이제 "이 호출자는
게스트다"와 겹치지 않습니다.

**고지 문구가 함께 바뀌었습니다.** 7개 locale의 `chat.voiceInput` 마지막 문장이
"로그인한 경우에만"이었고, 이 변경으로 **사실과 달라집니다.** 문구 변경은 법률
재승인 대상이며, 승인 전에 production에서 게스트 음성을 켜면 **고지가 틀린 채로
열립니다**(§11.4).

### 4.3 플랜은 gate가 아닙니다

**플랜은 gate가 아닙니다.** 모든 플랜(Free 포함)이 쓸 수 있습니다.
...
`isSignedIn`도 2026-09-10부터 같은 이유로 받되 읽지 않습니다.
...
**Voice 입력은 무료 편의 기능이며 크레딧을 차감하지 않습니다.** entitlement 층을
만들지 않습니다.

이전 판은 같은 코드를 두고 "정해지지 않았다는 사실의 정직한 구현"이라고 적었고,
그때는 그것이 맞았습니다. **지금은 같은 동작이 결정입니다.** 둘의 차이는 코드가
아니라 근거이고, 근거가 다르면 바뀌는 방식도 다릅니다 — 미결정은 결정되면
움직이지만, 결정은 §6.3의 조건을 밟아야 움직입니다.

### 6.1 결정

번호는 §14의 B-1~B-4와 같은 순서입니다.

1. **오디오 1초의 사용자 가격 — 없습니다.** Voice 입력은 무료이고 크레딧을
   차감하지 않으며, 플랜별 초 할당량도 두지 않습니다.

   **무료는 무제한이 아닙니다.** §7의 operational guardrail이 계속 유일한
   상한이고, 그 위에 B-4의 provider 예산이 얹힙니다. 사용자에게 값을 매기지
   않는다는 것과 지출을 통제하지 않는다는 것은 다른 말입니다.

   근거: 사용량 데이터가 없는 상태에서 크레딧 과금을 먼저 붙이면 그 숫자는
   관측이 아니라 **추측**입니다. 예산과 kill switch로 비용을 막으면서 사용량을
   모으는 쪽이 되돌리기 쉽습니다 — 나중에 값을 매기는 것은 할 수 있지만, 잘못
   매긴 과금은 이미 청구된 뒤이고 환급 대상이 됩니다.

2. **실패·빈 transcript에도 사용자 과금 없음.** 1의 따름정리이지만 **따로
   적습니다.** 1이 언젠가 유료로 바뀌어도 이 조항은 그때 독립적으로 다시
   결정되어야 하며, "유료가 됐으니 실패도 과금"이 자동으로 따라오지
   않습니다.

   **provider 예산은 다르게 답합니다.** 결과 불명(`indeterminate`)인 호출은
   provider가 과금했을 수 있으므로 예약을 유지합니다(§7.4). 사용자에게 0을
   청구하면서 예산에서는 초를 붙잡고 있는 것이 모순이 아닙니다 — 두 층이
   서로 다른 질문에 답하기 때문이고, 그 분리가 AGENTS.md의 계약입니다.

3. **provider 원가 검증 — audio 전용 register를 두고, `measured_clip`을 동기
   정산 기준으로 승인합니다.**

   register는 텍스트 모델의 `PENDING_VERIFIED_PRICE_REGISTER`와 **분리**합니다
   (`lib/voiceInputPricing.ts`). 담당자·검증 티켓·등록일·기한을 데이터로 들고
   있고, 기한이 지나면 검사가 경고에서 실패로 바뀝니다.

   **`measured_clip` 승인의 근거는 다른 선택지가 없다는 것이 아니라, 이것이
   맞는 것을 잰다는 것입니다.** 설정된 모델은 token-billed라 동기 응답에 초가
   없고(§6.1.1), 토큰을 초로 환산하는 것은 금지되어 있으며, 공급자의 집계
   `seconds`는 사후값이라 요청 하나를 정산할 수 없습니다. `measured_clip`은
   **우리가 실제로 보낸 바이트의 길이**이므로 공급자가 받은 것과 같은 것을
   잽니다.

   **티켓은 열 수 있는 참조여야 합니다.** `#<이슈 번호>`나 URL이며, 그럴듯한
   문자열은 추적성을 만들지 못합니다 — 아무도 열어서 무엇을 했는지 볼 수 없기
   때문입니다. 현재 항목은 **#1247**을 가리킵니다. 검사가 형태를 강제합니다.

   **유료 검증은 2026-09-08과 2026-09-09에 승인 아래 실행됐습니다(§6.1.3,
   §6.1.6).** 두 모델 모두 실제 청구가 관측됐고, 그 대조에서 **공표된 audio input
   요율과 실제 청구가 2.4배 어긋난다**는 것이 드러났습니다 — 두 모델에서 같은
   배수이고, 이유는 확정되지 않았습니다. register는 실제 청구를 재현하는 값과
   공표값을 **둘 다** 듭니다.

   **관측 없이 "관측됨"이라고 말할 수 없어야 합니다.** boolean은 일을
   하지 않고도 뒤집을 수 있습니다. 그래서 register는 boolean 대신 **증거
   객체**를 듭니다 — 청구 금액, 그 금액이 덮는 요청 수와 토큰 수, 그리고
   격리 근거입니다. 검사는 그 숫자들이 기록된 요율과 다시 계산해서 맞는지
   봅니다. **요율만 고치고 관측을 그대로 두면 검사가 실패합니다.**

   **`costObservation`이 `null`인 채로 production을 허용하는 것도 별도 승인입니다.**
   register의 owner를 지정한 것과 같은 행위가 아닙니다 — 앞은 "이 가격을 누가
   다시 읽는가"이고 뒤는 "관측되지 않은 원가로 운영해도 되는가"입니다. 최종
   활성화 전에 **사람의 승인과 그 근거를 기록**합니다.

4. **audio provider 전용 일·월 예산을 두고, production에서 `/api/ready`가 그
   부재를 실패로 봅니다.** 채팅의 `CHAT_PROVIDER_*_COST_*`와 **별개 층**이며
   이름을 빌리지 않습니다.

   **이것은 cost budget이 아니라 usage(exposure) budget입니다.** 이 표현을
   고정합니다 — 다른 provider 예산은 금액을 제한하지만 이것은 **이 배포에서
   나가는 오디오의 양**을 제한합니다. 금액을 정확히 제한한다고 주장하지
   않습니다. 초와 달러의 관계가 안정적인 만큼만 지출을 묶고, 이 층은 그
   관계를 안다고 말하지 않습니다.

   **USD 예산은 B-3 유료 검증 뒤에 별도 계층으로 더합니다.** 이 예산에서
   유도하지 않고, 검증된 요율 위에 새로 세웁니다.

   **단위는 초입니다. USD가 아닙니다.** 이것이 이 결정에서 가장 눈에 띄는
   선택이고, 이유는 하나입니다 — **승인된 USD 환산율이 없습니다.**
   토큰 단가는 정가지만 호출 전에 토큰 수를 알 수 없고, 분당 숫자는 공급자가
   "Estimated cost"라고 부르는 값입니다(§6.1.1). 예약 시점에 USD로 바꾸려면
   아무도 승인하지 않은 환율을 만들어야 하고, 그것이 AGENTS.md가 금지하는
   바로 그 일입니다. **잴 수 있는 단위로 예산을 잡습니다.**

   §7의 guardrail과 겹치지 않습니다: 그쪽은 **주체별**(사용자·게스트 하루)
   이고, 이쪽은 **배포 전체**의 하루와 한 달입니다. 채팅에서 plan guardrail과
   provider 예산이 별개인 것과 같은 구조입니다.

   B-3의 유료 검증으로 검증된 USD 요율이 생기면 그때 USD 형태를 **더할** 수
   있습니다. 지금 없는 것을 지금 만들지 않을 뿐입니다.

   **구현이 끝난 것과 운영 설정이 끝난 것은 다릅니다.** 코드가 예약·집계·
   정산을 하고 readiness가 부재를 막는다는 것은 B-4의 절반입니다. 나머지
   절반은 **staging과 production에 실제 일·월 초 한도가 사람이 골라
   설정되는 것**이고, 그 전까지 B-4를 "운영 설정 완료"로 보지 않습니다.
   `/api/ready`가 통과하는 것은 "쓸 수 있는 예산이 있다"이지 "그 숫자가
   맞다"가 아닙니다.

   구현이 갖춰야 하는 것은 넷입니다. 넷 다 있어야 예산이 예산입니다.

   - **provider 호출 전 예약**(`reserveVoiceProviderSeconds`). 호출 뒤에
     기록하면 동시 요청이 같은 잔여를 읽고 전부 통과합니다.
   - **인스턴스 간 원자적 집계.** 조건부 `UPDATE`가 잠금 없이 그것을
     보장하며, 두 프로세스가 같은 여유를 함께 가져갈 수 없습니다.
   - **일·월 두 bucket의 함께 움직임.** 한 문장으로 안 되므로 일을 먼저 잡고
     월이 거절하면 일을 **되돌린 뒤** 거절합니다.
   - **모든 종료 경로의 정산·해제.** 주체 예약과 provider 예약을 **한
     handle**(`VoiceBudgetReservation`)로 묶어, 종료 경로가 하나를 닫고
     다른 하나를 잊을 수 없게 합니다. 처음에는 route의 지역변수 둘이었고,
     그러면 나중에 추가되는 종료 경로가 둘 다 잊습니다.

#### 6.1.1 공급자에게서 확인된 것 (2026-09-02 읽음)

> **이 절은 2026-09-02에 읽은 것의 기록이고, 두 곳이 틀린 것으로 드러났습니다.**
> 아래 표의 `Input` 열은 **text input** 요율이며 transcription이 내는 요율이
> 아닙니다. 그리고 관측 경로 3은 이 모델에 대해 존재하지 않습니다. 정정은
> §6.1.3(2026-09-08 관측)에 있습니다. 읽은 것을 지우지 않고 남기는 이유는,
> 정가 페이지만 보면 지금도 같은 결론에 이르기 때문입니다.

공급자가 공개한 숫자와 관측 경로입니다. §6.1의 결정은 이것들을 **근거로
삼되 이것들에서 유도되지는 않습니다** — 특히 4의 예산 단위가 초인 이유가
여기에 있습니다.
...
> **정정 (2026-09-08).** 이 `Input` 열은 **text input**입니다. transcription의
> 유일한 입력인 audio input은 별개의 더 비싼 요율이고 이 페이지에 실려 있지
> 않습니다. `gpt-4o-mini-transcribe`의 audio input은 청구서에서 **$3.00 / 1M**
> 으로 관측됐습니다(§6.1.3). `Output` 열은 맞습니다.
...
| 3 | 공급자가 집계한 길이 | Admin Usage API `GET /organization/usage/audio_transcriptions` | 초 (`seconds`, `num_model_requests`) — **이 모델에는 없습니다, §6.1.3** |
...
#### 6.1.2 이 결정에 포함되지 않은 것

§6.1이 답한 것은 **사용자 가격·실패 처리·원가 register·provider 예산**
넷뿐입니다. 아래는 여전히 열려 있고, §6.1을 근거로 닫지 않습니다.

- **B-5 전부** — ZDR 신청, DPA 체결, 조직·프로젝트 분리, 그리고 공급자 문서
  안에서 조정되지 않은 진술(§11.3.2-1). 가격 결정은 데이터 보존 결정을
  대신하지 않습니다. **(2026-09-10에 B-5가 통과로 서명돼 닫혔습니다 — §14.
  이 줄은 §6.1이 무엇을 답하지 않았는지에 대한 서술이고, 그 사실은 그대로
  입니다.)**
- **유료 검증**(§6.1.1의 네 값 대조). B-3은 register를 만들라고 했지
  검증을 실행하라고 하지 않았습니다. 실행에는 별도 승인과 admin 자격증명이
  필요합니다. — **2026-09-08에 승인 아래 실행됐습니다(§6.1.3).** 기본 모델
  하나에 대해서였고, `gpt-4o-transcribe`는 **2026-09-09에 관측돼 함께
  닫혔습니다**(§6.1.6, §14 B-3).
- **duration-billed 모델로의 전환.** `measured_clip`을 승인했다는 것은
  `whisper-1`로 바꾸는 선택지를 닫았다는 뜻이 아니라, 그것을 **기다리지 않기로
  했다**는 뜻입니다. 품질·가격이 함께 걸린 별개 결정으로 남습니다.
- **게스트 허용**(§4). 무료가 됐다고 게스트가 열리지 않습니다 — 그쪽은
  abuse protection 판단이고 `lib/voiceInputAccess.ts`의 별도 변경입니다.

#### 6.1.3 유료 검증 결과 (2026-09-08 관측)

§6.1.1이 요구한 네 값의 대조가 승인 아래 실행됐습니다. 대조 대상은
2026-09-02의 3회 호출이고, **새 호출은 하지 않았습니다** — 그 3회가 집계에서
그대로 분리됐기 때문입니다. 질의와 원자료는 #1247에 있습니다.

**관측된 청구는 US$0.001616입니다.** 정가 산술이 낸 US$0.001000의 **1.616배**
이고, 차이는 전부 요율 하나에서 나옵니다.

| # | 값 | 결과 |
|---|---|---|
| 1 | 요청 토큰 `usage` | 352 input(audio) / 112 output |
| 2 | `measured_clip` | 35.249초 |
| 3 | 공급자 집계 초 | **없음** — 아래 2 |
| 4 | 실제 청구 | **US$0.001616** |

청구 내역입니다(`GET /organization/costs`, `1d` bucket `2026-09-02`).

| line item | USD | 역산 요율 |
|---|---|---|
| `gpt-4o-mini-transcribe-2025-12-15 audio, input` | 0.001056 | **$3.00 / 1M** |
| `gpt-4o-mini-transcribe-2025-12-15 text, input` | 0.0 | — |
| `gpt-4o-mini-transcribe-2025-12-15 text, output` | 0.00056 | $5.00 / 1M |

**1. 실제로 청구된 audio input 요율은 공표값의 2.4배입니다. 이유는 미상입니다.**

> **2026-09-10 정정.** 이 항목은 원래 **"그 열은 text input이고 audio 요율은
> 공표되지 않는다"**고 적었습니다. **그것은 틀렸습니다.** 공급자의 모델 페이지는
> 이 값을 명시적으로 **"Audio tokens · Input"**으로 표시하고, text 행은 아예
> 없습니다
...
> 2026-09-10 확인). 관측된 불일치는 실재하지만, 그것에 붙였던 설명이 근거 없는
> 추측이었습니다.
>
> **2026-09-10 두 번째 정정**(#1309 병합 이후). 정정 직후의 문구는 **"Costs API는
> 금액만 돌려주고 과금 수량은 돌려주지 않으므로 요율 차이와 수량 차이를 구분할
> 수 없다"**고 적었습니다. **그것도 틀렸습니다** — `group_by=line_item`의 각
> result는 `quantity`와 `quantity_unit`을 함께 돌려줍니다
> ([Usage/Costs API reference](https://developers.openai.com/api/reference/python/resources/admin/subresources/organization/subresources/usage)).
> 새 유료 호출 없이 기존 두 bucket의 원본 응답을 다시 읽어 아래 표를
> 채웠습니다.

확립된 사실은 이것입니다.

| 모델 | 공표 audio input | 응답 usage | 과금 수량 | 단위 | 금액 | 청구 요율 | 비 | output |
|---|---|---|---|---|---|---|---|---|
| `gpt-4o-mini-transcribe` (2026-09-02) | $1.25 / 1M | 352 | **352** | `tokens` | $0.001056 | **$3.00 / 1M** | 2.4 | $5.00 / 1M (일치) |
| `gpt-4o-transcribe` (2026-09-09, §6.1.6) | $2.50 / 1M | 95 | **95** | `tokens` | $0.00057 | **$6.00 / 1M** | 2.4 | $10.00 / 1M (일치) |

**output은 두 모델 모두 공표값과 정확히 일치합니다.** 어긋나는 것은 input 쪽뿐이고,
독립적으로 측정한 두 모델에서 **같은 배수**입니다.

**차이는 수량이 아니라 단가입니다.** 유력한 대안은 공급자가 응답이 보고한 것과
다른 수를 과금한다는 것이었고 — 228 토큰 × $2.50/1M도 $0.00057에 도달합니다 —
금액만으로는 두 가설이 구분되지 않습니다. `quantity`가 그것을 직접 답합니다:
**두 관측 모두 과금 수량이 응답 usage token과 정확히 같고 단위는 `tokens`**
입니다. 그러므로 같은 단위가 공표가의 2.4배로 청구됩니다.

**왜 그런지는 여전히 미상입니다.** 계정·snapshot별 요율 차이, 공급자 문서 오류,
과금 오류가 모두 열려 있습니다. 관측 하나의 line item 이름이 날짜 붙은 snapshot
(`gpt-4o-mini-transcribe-2025-12-15`)이고 다른 하나는 맨 모델명인 것도 아직
설명되지 않았습니다. **하지만 "무엇이 다른가"는 확정됐습니다** — 세는 방식이
아니라 값입니다.

**그러므로 register는 두 값을 다 듭니다** — `audioInputPerMillionTokensUsd`(이
계정이 실제로 청구받는, 청구를 재현하는 보수적 값)와
`publishedAudioInputPerMillionTokensUsd`(페이지가 말하는 값). 하나만 들면
...
`GET /organization/usage/audio_transcriptions`는 30일 전체에서 빈 응답입니다
(`1m`·`1h`·`1d` 모두). 집계 지연이 아닙니다 — 6일이 지났고, 같은 키로 다른
endpoint는 채워진 bucket을 돌려줍니다. 이 호출들은
`GET /organization/usage/completions`에 model `gpt-4o-mini-transcribe`로
들어가 있고, 그쪽은 **초가 아니라 토큰**을 보고합니다.

`audio_transcriptions`는 duration-billed 모델(`whisper-1`)의 경로로 보이고
token-billed 모델은 `completions`로 가는 것으로 보이지만, 공급자 문서가 그렇게
적고 있지는 않습니다. 말할 수 있는 것은 이것뿐입니다 —
**`gpt-4o-mini-transcribe`에 대해 공급자 측 초를 보고하는 경로를 찾지
못했습니다.**

이것은 §6.1-3의 `measured_clip` 승인을 흔들지 않고 **강화합니다.** 사후 집계
초조차 이 모델에는 없으므로, 잴 수 있는 유일한 초는 우리가 보낸 바이트입니다.

**3. 분당 US$0.003을 안전한 상한으로 쓰지 않습니다.**

관측은 **US$0.002751 / 분**으로 공급자 추정치보다 8% 낮습니다. 그 여유는
보이는 것보다 얇습니다.

| 성분 | tokens/분 | US$/분 | 무엇에 비례하는가 |
|---|---|---|---|
| audio input | 599.2 | 0.001797 | 길이 — 고정 |
| text output | 190.6 | 0.000953 | 발화 밀도 — 가변 |
| 합계 | | **0.002751** | |

input을 고정하면 **output 241 tokens/분**에서 공급자 추정치 US$0.003/분에
닿습니다. 이번 시료는 191이었고 그것도 희소한 합성음이었습니다. 보통의 사람
발화는 그 손익분기 근처입니다.

**그러므로 §6.1-4의 USD 층을 언젠가 더할 때 US$0.003/분에서 유도하지
않습니다.** 그 숫자는 이 워크로드에서 보수적이지 않습니다.

**4. 격리 근거.** 값 4는 집계 대 집계이므로 분리가 성립해야 합니다.

- 그날 그 모델의 `num_model_requests`가 **3** — 정확히 그 3회입니다.
- 집계된 토큰이 요청별로 기록된 값과 정확히 일치합니다(352 / 112). 다른
...
**5. `gpt-4o-transcribe`는 2026-09-09에 관측됐습니다**(§6.1.6).

이 항목은 원래 **보류**였습니다 — 한 번도 호출된 적이 없어 이 계정이 실제로
얼마를 청구받는지 몰랐고, register는 그것을 `null`로 들고 검사는 **모름**으로
보고했습니다. 후보는 둘이었고 **둘 다 거부**했습니다. 공표된 $2.50은 mini에서
이미 청구를 재현하지 못한 종류의 값이었고, **mini의 $3.00에서 배수로 유도하는
것은 관측이 아니라 추론**이었습니다.

두 거부가 모두 옳았다는 것이 §6.1.6에서 숫자로 확인됐습니다 — 이 계정이
청구받는 요율은 **$6.00/1M**입니다. 공표값을 썼다면 2.4배 낮게, mini에서
유도했다면 절반으로 기록했을 것입니다.

#### 6.1.6 `gpt-4o-transcribe` 유료 검증 결과 (2026-09-09 관측)

승인 조건은 **합성 음성 약 10초, 전사 API 정확히 1회, production 설정 무변경,
register는 별도 승인 전까지 무변경**이었고 그대로 지켰습니다.

**호출**

| | |
|---|---|
| 시각(UTC) | 2026-09-09T10:21:42Z → 10:21:44Z, HTTP 200 |
| `x-request-id` | `req_9101ef5cfb4544d2be264e08513ed92f` |
| 샘플 | espeak-ng 합성, 9.52초, WAV 22.05kHz mono 16-bit |
| 호출 수 | **1** — 재시도 없음 |

**공급자가 보고한 usage**: audio input 95 토큰, output 26 토큰(text input은 0).

**청구**(`GET /organization/costs`, 2026-09-10 읽음)

`line_item`은 Costs API가 금액을 쪼개는 이름입니다. **응답 usage**는 전사 응답
자신이 보고한 토큰 수이고, **과금 수량**은 같은 result의 `quantity`(단위
`quantity_unit`)입니다 — 두 열이 따로 있는 이유는 서로 다른 사실이기 때문이고,
이 관측에서는 값이 같습니다.

| line item | 응답 usage | 과금 수량 | 단위 | 금액 | 청구 요율 | 공표값 |
|---|---|---|---|---|---|---|
| audio, input | 95 | **95** | `tokens` | $0.00057 | **$6.00 / 1M** | $2.50 / 1M |
| text, input | 0 | 0 | `tokens` | $0.00000 | — | — |
| text, output | 26 | **26** | `tokens` | $0.00026 | $10.00 / 1M | $10.00 / 1M |
| 합계 | | | | **$0.00083** | | |

오차 0으로 맞아떨어집니다. Usage 쪽 `num_model_requests`도 1이라 호출이 하나뿐이었음이 집계로도 확인됩니다.

**1. 유도했다면 절반으로 틀렸습니다.**

§6.1.3-5가 "mini의 $3.00에서 배수로 유도하지 말라"고 적었을 때 그것은 원칙이었고,
이제 숫자입니다 — 청구는 **$6.00/1M**, 유도값의 두 배입니다. 이 register가 추론을
거부하는 이유가 여기 있습니다.

**2. 공표된 $2.50은 이 청구를 재현하지 못합니다.**

공급자의 모델 페이지가 **"Audio tokens · Input"**으로 표시하는 값이 $2.50인데,
청구된 금액은 그것의 **2.4배**입니다. §6.1.3이 mini에서 본 것과 같은 배수이고,
**output은 두 모델 모두 공표값과 정확히 일치**하므로 어긋나는 것은 input
쪽뿐입니다.

**그리고 그것은 수량 차이가 아닙니다.** 같은 bucket의 `quantity`가 **95**,
`quantity_unit`이 `tokens`로, 응답이 보고한 95와 정확히 같습니다 — 228 토큰을
공표가로 과금했다는 대안은 이 값으로 반증됩니다. 같은 단위가 2.4배 단가로
청구된 것이고, **왜인지는 여전히 미상입니다**(§6.1.3-1).

**3. 두 모델의 배수가 같다는 것은 규칙이 아니라 관측입니다.**

mini는 3.00/1.25, 이 모델은 6.00/2.50으로 관측 실효값과 공표값의 비가 둘 다
2.4배입니다. **이것으로 세 번째 모델을 유도하지 않습니다.** 데이터 두 점이고
공급자가 어디에도 공표하지 않은 관계이며, 원인조차 확립되지 않았습니다.
그렇게 하는 순간 방금 피한 실수를 되풀이하는 것입니다.

**4. 콘솔에 보이지 않는 것이 청구가 없다는 뜻은 아닙니다.**

$0.00083은 콘솔이 표시하는 소수 둘째 자리에서 **$0.00으로 반올림**됩니다. 화면에
없다고 판단하면 관측 자체를 놓칩니다. **판정은 API의 전체 정밀도로 합니다.**

**5. 전용 키는 여전히 없습니다.**

이 호출은 공용 `OPENAI_API_KEY`로 나갔습니다. `VOICE_TRANSCRIPTION_API_KEY`는
이 환경에 없고, 전용 provider 키는 **B-5의 주제**입니다. 이 관측은 그것을
앞당기지도 답하지도 않습니다.

**6. 이제 두 모델 다 관측된 원가를 갖습니다.**

그러므로 `VOICE_TRANSCRIPTION_MODEL`로 이 모델을 고른 배포도 §6.1.4의 세 조건을
만족하며, `/api/ready`가 더 이상 거부하지 않습니다. **이것은 이 모델을 쓰라는
권고가 아닙니다** — 기본 모델은 여전히 `gpt-4o-mini-transcribe`이고, 원가는 audio
기준 두 배입니다. 바뀐 것은 고를 경우 원가를 아느냐이지 무엇을 골라야 하느냐가
아닙니다.
...
| 층 | 묻는 것 |
|---|---|
| `/api/ready` | 지금 설정된 모델에 관측된 가격이 있는가 |
| `reverifyBy` | **앞으로의** 배포와 재검증을 막는가 |
| 운영 알림 | 만료 전에 담당자가 아는가 |

**만료를 잡는 것은 "CI가 계속"이 아니라 "CI가 실행될 때"입니다.** 오래 배포가
없으면 만료된 채로 지나갑니다. 그 구멍은 register 검사를 주기적으로 실행하는
scheduled workflow가 메우며, 이 절의 차단 조건이 아니라 별도 후속입니다(#1247).
...
관측됐습니다. 지금 우회로를 미리 뚫지 않는 이유가 이것입니다: 쓸 일이 없는
우회로는 쓸 일이 생겼을 때 검토 없이 쓰입니다.

필요해질 때 그것은 **boolean이 아니어야 합니다.** boolean은 누가 왜 켰는지
말하지 못하고, 그것이 `costObserved`를 `costObservation`으로 바꾼 것과 같은
이유입니다. 예외 장치는 다섯을 요구합니다.

| 무엇 | 왜 |
|---|---|
| 승인자 | 사람이어야 하고, 자동화는 자기 자신을 승인할 수 없습니다 |
| 승인 사유 | 나중에 읽고 아직 유효한지 판단할 수 있어야 합니다 |
| 대상 모델 | 한 모델의 예외가 다른 모델까지 열지 않습니다 |
| 만료일 | 예외는 영구 상태가 아닙니다 |
| 감사 로그 | 켜졌다는 사실 자체가 기록으로 남아야 합니다 |

#### 6.1.5 재검증 마감은 예고를 갖습니다

**마감 자체는 그대로 gate입니다.** `reverifyBy`가 지나면
`check:voice-price-register`가 실패하고, 그 검사는 PR Fast Gate에 있으므로 그
순간부터 모든 PR이 막힙니다. 이 규칙은 완화하지 않습니다 — 아무도 다시 읽지
않은 가격은 한때 참이었던 가격이고, 마감이 곧 그 층의 전부입니다.

**바뀐 것은 그 사실이 언제 처음 들리는가입니다.** 이전에는 마감 당일 아침에
모든 PR이 한꺼번에 빨개지는 것이 첫 신호였습니다. 검사는 옳게 동작했지만
**옳은 gate가 장애의 모습으로 도착**한 것입니다 — register는 전날에도 똑같이
참이었고, 누구도 조치하라는 말을 듣지 못했습니다.
...
1. **경고는 gate를 실패시킬 수 없습니다.** 경고 계산은
   `voicePriceReverificationNotices()`이고 차단 판정은
   `auditVoicePriceRegister()`이며, **다른 함수입니다.** 후자에는 임박한 마감을
   가리키는 code 자체가 없으므로, 나중에 누가 고치더라도 경고가 gate가 되지
   않습니다. 규칙이 아니라 구조입니다.
2. **알림은 한 곳으로만 갑니다.** register 항목이 이미 `ticket`을 이름 대고
   있고, 두 번째로 볼 곳은 놓칠 곳입니다. 같은 ticket·같은 마감을 공유하는
   모델들은 코멘트 하나로 묶입니다.
3. **같은 알림을 두 번 보내지 않고, 놓친 실행 때문에 알림을 잃지도 않습니다.**
   각 시점은 스레드에 남은 marker로 판정하며 marker는 마감일과 시점을 함께
   담습니다. 하루를 걸러도 다음 실행이 같은 시점으로 해석하고, 마감이 바뀌면
   marker가 달라져 다시 무장됩니다.
4. **어느 것도 register를 고치지 않습니다.** 마감을 옮기는 것은 가격을 다시
...
- **entitlement 층을 만들지 않습니다.** 이것은 이제 미결정의 결과가 아니라
  §6.1-1 그 자체입니다. Voice 경로가 크레딧을 예약·차감·환급하는 코드를 갖게
  되면 그것은 구현 실수가 아니라 **정책 위반**이고,
  `tests/voiceCreditNeutrality.test.mjs`가 그것을 실패로 만듭니다.
- §7의 operational guardrail과 §6.1-4의 provider 예산만 존재하며, 이름·오류
  코드·bucket·환경변수를 크레딧 층과 **절대 섞지 않습니다**.
- **무료를 "상한 없음"으로 읽지 않습니다.** guardrail이나 예산을 완화하는
  변경은 §6.1-1을 근거로 정당화될 수 없습니다 — 그 조항은 사용자에게 값을
  매기지 않는다고 말할 뿐, 지출을 열어 두라고 말하지 않습니다.

### 6.3 이 결정을 바꾸려면

무료에서 유료로 가는 것은 **되돌릴 수 없는 방향**입니다(잘못 청구된 크레딧은
이미 차감된 뒤입니다). 그래서 다음이 전부 있어야 합니다.

1. B-3의 유료 검증이 실행되어 **관측된 원가**가 있을 것 — 정가나 추정이
   아니라 대조된 값.
2. **관측된 사용량** — 무료 기간에 모은 초 분포. 가격은 이것 위에서 정해지고,
   이것이 §6.1-1이 지금 유료로 가지 않는 이유입니다.
3. 이 문서를 **먼저** 고치고 그 다음에 코드를 고칠 것(§16).
4. 기존 사용자에게 **예고**. 무료로 쓰던 기능이 말 없이 크레딧을 먹기
   시작하는 것은 되돌릴 수 없는 신뢰 문제입니다.
...
금액으로 환산하지 않습니다 — 승인된 환산율이 없고, 만드는 순간 §6.1-4가
**초를 단위로 고른 이유**를 코드가 뒤집는 것이 됩니다.
...
측정할 수 없습니다. 그 검증은 §14 B-3이었고 **2026-09-09에 두 모델 모두 관측돼
닫혔습니다**(§6.1.3, §6.1.6).
...
| 이름 | 필수 | 무엇 |
|---|---|---|
| `VOICE_TRANSCRIPTION_API_KEY` | key 둘 중 하나 | transcription 전용 키. 없으면 `OPENAI_API_KEY`로 내려갑니다(§11.3) |
| `VOICE_PROVIDER_SECONDS_PER_DAY` | **production에서 flag가 켜져 있으면 필수** | 배포 전체가 하루에 공급자에 보낼 수 있는 오디오 **초** |
| `VOICE_PROVIDER_SECONDS_PER_MONTH` | 같음 | 같은 것의 한 달 |
...
provider 예산(§6.1-4), 뒤는 주체별 guardrail(§7)입니다. 이름이 비슷해 보이지만
하나가 다른 하나의 상한이 아닙니다 — 주체별 한도로는 총액을 묶을 수 없고,
총액 예산으로는 한 사람의 남용을 막을 수 없습니다.

**환경변수를 먼저 배포하고 flag를 나중에 켭니다.** flag가 꺼져 있는 동안
...
- 기본 모델은 `gpt-4o-mini-transcribe`이고 `VOICE_TRANSCRIPTION_MODEL`로
  바꿉니다. 두 모델 중 싼 쪽이며, §6.1-3이 `measured_clip`을 정산 근거로
  승인한 것이 이 선택을 유지할 수 있게 합니다.
...
오디오는 transcription provider(현재 **OpenAI**)의 **Global endpoint**로
갑니다 — `https://api.openai.com/v1/audio/transcriptions`.

**2026-09-10에 이 문장은 하루 동안 미국 지역 endpoint를 가리켰습니다.** 그 고정과
철회는 §11.3.0에 있습니다 — 공급자가 이 조직에 대해 지역 host를
`401 incorrect_hostname`으로 거절했습니다.
...
> **정정 (2026-09-10, 관측 후).** 위 인용은 지금도 문서 원문 그대로입니다. 틀린
> 것은 그 다음에 이어져 있던 문장 — "그래서 새 프로젝트도 새 key도 이 변경의
> 전제가 아닙니다" — 을 **전제가 하나도 없다**는 뜻으로 쓴 것입니다.
>
> staging에서 실제로 요청해 보니 `us.api.openai.com`이 **두 key 모두**를
> `401 incorrect_hostname`으로 거절했고, 본문이 *"Please make your request to
> api.openai.com"*이었습니다. 프로젝트 `residency`는 `GLOBAL`이고 모델 권한도
> 정상이며 두 key 모두 Global host에서는 인증됩니다. 즉 **조직 자체가 data
> residency를 쓸 수 있는 상태여야 한다**는, 문서가 안내 대상으로 전제하고 있던
> 조건이 하나 더 있었습니다.
>
> **이 고정이 있는 한 Voice는 동작하지 않습니다.** 관측 전문과 세 갈래는
> `docs/ops/voice-provider-data-controls-records/2026-09-10-us-regional-endpoint.md`
> §10이며, 어느 갈래로 갈지는 사람의 결정입니다.
...
**미결정이었던 것**: OpenAI 계정의 audio 데이터 보존 설정, zero-data-retention
적용 여부, 별도 계정/조직/DPA 분리 여부. `VOICE_TRANSCRIPTION_API_KEY`를 별도로
둔 것은 그 결정이 내려질 때 **코드 변경 없이** 반영할 수 있게 하기 위해서입니다.

> **갱신 (2026-09-10).** 이 셋은 같은 날 결정·기록됐고
> (`docs/ops/voice-provider-data-controls-records/`의 ZDR 필수조건·DPA 편입·
> 처리 지역 기록과 전용 project·key 분리), **B-5가 통과로 서명되면서 더 이상
> §14의 차단 사유가 아닙니다.** 다만 ZDR 신청 자체는 여전히 열려 있으며
> 비차단입니다.
...
2. **이 계정의 실제 설정.** 2026-09-10에 **읽기를 시도했고 일부만 읽혔습니다**:
   `docs/ops/voice-provider-data-controls-records/2026-09-10-account-observation.md`.

   조직 조회는 **HTTP 403 `not_eligible`**("controls are not enabled for this
   organization")이고, 프로젝트 조회는 200에 `organization_default`입니다.
   **403이 증명하는 것은 관측 시점에 controls가 활성화·프로비저닝되지
   않았다는 것뿐이며**, 영구적인 자격 거절도 향후 승인 불가도 아닙니다.
   `organization_default`도 프로젝트가 조직 설정을 **상속하도록 구성됐다**는
   뜻이지 상속한 값을 말하지 않으므로, 조직 조회가 막혀 있는 동안 **실효
   retention 값은 미확정**입니다.

   그러므로 **ZDR 승인 여부와 활성 상태는 여전히 관측되지 않았습니다.** 403은
   "신청한 적 없음"과 "신청했으나 미승인"을 구분하지 않습니다.

   함께 관측된 것 둘: Voice 트래픽은 다른 모델 트래픽과 **같은 프로젝트**를
   쓰고, 이 환경에는 **Voice 전용 키가 없습니다**(공용 `OPENAI_API_KEY`).
...
**2026-09-10의 계정 관측도 B-5를 해제하지 않습니다.** 그 관측이 바꾼 것은
상태의 이름입니다 — "아직 안 봤다"에서 **"봤고, 읽을 수 없는 것이 무엇인지
확인됐다"**로.

**같은 날 넷 중 하나가 결정됐습니다: ZDR 승인은 초기 Production의 필수조건이
아닙니다**(`docs/ops/voice-provider-data-controls-records/2026-09-10-zdr-precondition-decision.md`).
근거는 둘이고 서로를 대신하지 않습니다 — endpoint 표의 `None`(공급자 쪽 보존)과
조직 API call logging `Disabled`(조직이 자기 대시보드에서 내용을 다시 볼 수
있게 보관하느냐, 운영자가 콘솔에서 확인). **`Disabled`는 뒤쪽을 껐다는 사실이고
앞쪽에 대해서는 아무 말도 하지 않습니다.** 그 결정은 `None` 대 30일의 답을 얻은
것이 아니라 답이 없는 상태를 감수한 것이며, 재검토 조건 다섯과 2026-12-01을
함께 답니다.
...
**원문은 Codex가 읽었고 이 저장소는 읽지 못합니다** — `openai.com/policies/*`가
egress 프록시에서 403이며 2026-09-10에 재확인했습니다. 기록에 누가 읽었는지를
적는 이유입니다. 그리고 그 판정은 **계정이 표준 온라인 계약을 쓰고 별도
Order Form·reseller 계약이 없다**는 전제 위에서만 성립하므로, 그 전제를
운영자가 확인하고 서명하는 칸이 기록에 있습니다.

**전용 Voice project와 key도 만들어졌습니다**(2026-09-10, 2회차 관측 기록).
비용 추적·rotation·사고 범위를 Voice에 한정하지만 **B-5를 닫지 않고 DPA 편입
요건도 아닙니다.** 그리고 `VOICE_TRANSCRIPTION_API_KEY`가 두 환경에 **설정돼
있다는 것까지가 관측**입니다 — Railway가 값을 돌려주지 않으므로 그 값이 새
...
**서명 셋 중 하나가 들어왔습니다.** 위험 수용(ZDR)은 2026-09-10에 Privacy
Owner가 서명했습니다. 서명문이 초안에 없던 것 둘을 못 박았으므로 여기 옮깁니다 —
이 결정은 **`/v1/audio/transcriptions`의 Voice 전사에만** 적용되고 다른 endpoint나
민감·의료·아동정보 처리를 포괄하지 않으며, **API call logging `Disabled`는 ZDR과
동일하거나 이를 대체하는 통제가 아닙니다.**

**나머지 둘도 같은 날 서명됐습니다** — 계약 전제 확인(DPA)과 처리 지역. 계약
전제의 법인 정식명칭은 자리표시자로 왔다가 **2026-09-10에 `Tomverse`로
확정됐습니다.** 그 이름은 Privacy Owner가 밝힌 것이고 이 저장소가 등기부와
대조한 것이 아닙니다.

**열린 항목이 하나 남습니다.** 처리 지역 서명문이 열거한 고지 항목을 현재
`privacyPolicy.voiceInput`과 대조하니 **다섯이 없습니다** — 이전 시점·방법,
이전받는 자의 국가, 수탁자의 보유·이용 기간, 거부 방법·절차, 거부 시 Voice
이용 제한. 서명문 자신이 "국가 정보를 확정할 수 없다면 해당 고지가 완료될
때까지 이 서명만으로 국외이전 요건이 충족됐다고 판단하지 않는다"고 적으므로,
**요약란의 `완료`보다 본문의 조건이 우선합니다.**

**§11.4의 "이미 고지돼 있다"는 이 다섯을 포함하지 않습니다.** 7개 locale이 담고
있는 것은 처리위탁 대상·국외이전 가능성·이전 항목·목적까지이며, 그것이 앞 절이
확인한 범위입니다. §11.3.2-1의 `None` 대 30일은 공급자 답변 대기이나 이제
...
2. **개인정보 처리방침** — `privacyPolicy.voiceInput`, 7개 locale 전부. "외부 AI
   ...
   **그 절 아래에 국외이전 고지 여덟 항목이 각각 한 줄로 붙습니다**
   (`voiceInputTransfer1`–`8`, 2026-09-10). 개인정보보호법 제28조의8 제2항이
   요구하는 항목이고, **한 문단에 여덟 답을 밀어 넣으면 어느 것도 찾을 수 없어
   감사되지 않습니다.** 그래서 `PrivacyPolicy.tsx`가 Voice 섹션만 목록으로
   렌더링합니다.

   **이전 국가를 이름 댈 수 있는 것은 §11.3.0이 국가를 코드로 고정했기
   때문입니다.** 그 고정이 풀리면 이 고지가 먼저 틀립니다.

   **이 여덟 항목의 문구는 2026-09-10에 한 번 승인됐고**, 승인된 revision은
   `6019e07afd598a1431862ec925189a04544e9c94`였습니다
   (`docs/ops/voice-provider-data-controls-records/2026-09-10-us-regional-endpoint.md`
   §8-2). 같은 날 이전받는 자 명칭 `OpenAI OpCo, LLC`가 계약 원문과 대조돼
   확인됐습니다(같은 기록 §8-1).

   **그 첫 승인은 현재 문구를 덮지 못했습니다.** 승인은 문장이 아니라 SHA에
   붙어 있었고, 같은 날 US endpoint 고정이 철회되면서(§11.3.0 정정) 두 문자열이
   바뀌었습니다 — 산문의 처리 국가 문장과 2번 항목입니다.

   **다시 쓴 문구는 같은 날 승인됐습니다**(같은 기록 §13-1). 승인된 revision은
   `e8613a554bfbb22676a1be6552932954d79a88c8`이고, 이번에는 8개 파일의 blob
   hash가 전부 적혀 있습니다 — 앞선 승인이 셋만 대표로 적어 두었다가 문구가
   바뀌었을 때 무엇이 승인 대상이었는지 따져야 했기 때문입니다.

   **2번 항목의 근거가 바뀌었습니다.** 이전에는 "미국 endpoint로 보내므로
   미국"이었고, 지금은 **"이전받는 법인 `OpenAI OpCo, LLC`의 소재 국가가
   미국"**입니다. 같은 항목 안에서 기술적 처리가 그 국가로 한정되지 않는다는
   것을 함께 적습니다 — Global API를 쓰기 때문입니다. **그 법인의 소재
   국가는 아직 계약 원문과 대조되지 않았습니다**(§11-4). 대조된 것은 명칭뿐입니다.

   **산문의 마지막 문장이 2026-09-10에 다시 바뀌었고, 이번 변경은 아직
   승인되지 않았습니다.** 게스트 개방(§4.2)으로 "음성 입력은 로그인한 경우에만
   사용할 수 있습니다"가 사실과 달라져, 로그인 여부와 무관하게 사용할 수 있다는
   문장과 미로그인 시 귀속되는 익명 식별자에 대한 문장으로 교체했습니다. 7개
   locale 전부입니다.

   **승인은 문장이 아니라 SHA에 붙는다는 규칙이 여기에도 적용됩니다.** 위의 두
   번째 승인이 덮는 revision은 `e8613a554bfbb22676a1be6552932954d79a88c8`이고,
   이 변경은 그 뒤입니다. 따라서 **production에서 게스트 음성을 켜기 전에 이
   문구에 대한 별도 승인이 필요합니다** — 승인 없이 켜면 고지가 틀린 채로
   열리고, 그것은 되돌릴 수 없는 종류의 실수입니다(AGENTS.md "검증 범위는
   되돌릴 수 없는 것에 비례합니다": 유출과 잘못된 고지는 회수가 성립하지
   않습니다).
...
| # | 사항 | 상태 |
|---|---|---|
| B-1 | 오디오 1초의 사용자 가격 (§6.1-1) | **해결 (2026-09-02)** — 무료, 크레딧 차감 없음 |
...
| B-5 | provider 데이터 보존 (§11.3) | **해결 (2026-09-10, 통과 서명됨)** — 여섯 조건 모두 충족. deploy `e8613a55…`에서 2026-09-10T11:06:54Z 첫 전사 성공. 서명: Tommy Han, Privacy Owner. **잔여 위험 다섯은 사라진 것이 아니라 수용됐습니다.** 그중 수령 법인 소재 국가는 같은 날 공식 계약 원문 대조로 **해소**됐고(`docs/ops/voice-provider-data-controls-records/2026-09-10-recipient-domicile-verification.md`), 나머지 넷은 열려 있습니다. 기록: `docs/ops/voice-provider-data-controls-records/2026-09-10-us-regional-endpoint.md` §14 |
| B-6 | 실기기 검증 (§15) | **해결 (2026-09-10, 통과 서명됨)** — 최소 경로(차단 항목만) 실행. deploy `e8613a55…`, 실행자·승인자 mposition. 기록: `docs/ops/voice-input-staging-verification-records/2026-09-10__e8613a554bfbb22676a1be6552932954d79a88c8.md` (frozen). **비차단 구획은 `미기록`이며 범위가 그 기록에 적혀 있습니다** |

B-1~B-4는 **§6이 열려 있다는 하나의 사실**의 네 얼굴이었고, 2026-09-02에 하나의
제품·finance 결정으로 함께 닫혔습니다(§6.1). **다만 결정이 닫힌 것과 일이 끝난
것이 같지 않은 항목이 있었습니다** — B-3은 register가 서 있고 실제 비용 관측이
남아 있었으며(기본 모델은 2026-09-08, `gpt-4o-transcribe`는 2026-09-09에
관측돼 닫혔습니다, §6.1.3·§6.1.6), B-4는 코드가 서 있고 각
환경의 실제 한도 설정이 남아 있었습니다(2026-09-08에 양쪽 환경에 설정되고 서명돼
닫혔습니다). B-5는 별개이고 법무 성격입니다. B-6은 사람만 할 수 있습니다.

**production을 막고 있는 것은 이제 B-6 하나입니다.** B-5는 2026-09-10에 통과로
서명됐습니다.

**B-5가 하루에 어떻게 닫혔는지는 남겨 둘 값이 있습니다.** 계정 설정 관측, ZDR
필수조건·DPA 편입·처리 지역 결정과 서명 셋, 전용 project·key 분리까지가 오전에
있었고(`docs/ops/voice-provider-data-controls-records/`), 이어서 국외이전 고지
여덟 항목이 7개 locale에 들어갔습니다(§11.4). **그 사이 미국 지역 endpoint 고정이
병합됐다가 같은 날 철회됐습니다** — 공급자가 이 조직에 대해 그 host를
`incorrect_hostname` 401로 거절했기 때문이고, 그 관측과 정정은 §11.3.0에
있습니다. Global로 되돌린 배포에서 첫 전사가 성공했고, 그 위에서 판정과 서명이
나왔습니다.

**그 하루가 남긴 규칙이 하나 있습니다** — 승인은 문장이 아니라 SHA에 붙입니다.
고지 문구가 US 고정과 함께 바뀌었을 때 앞선 승인이 자동으로 따라오지 않는다는
것이 그 고정 덕분에 드러났습니다(같은 기록 §8-2, §13-1).

**통과는 잔여 위험이 없다는 뜻이 아닙니다.** 같은 기록 §14의 7-1~7-5가 수용된
채로 열려 있었고, 그중 **수령 법인의 소재 국가가 계약 원문과 대조되지
않았다**는 것만이 사용자에게 보이는 고지 문장에 직접 걸렸습니다.

**그 하나는 같은 날 해소됐습니다.** 운영자가 공식 Services Agreement와 DPA
원문을 대조해 `OpenAI OpCo, LLC`가 미국 법인임을 확인했습니다
(`docs/ops/voice-provider-data-controls-records/2026-09-10-recipient-domicile-verification.md`).
**법인의 소재국을 확인한 것이지 Global API의 기술적 처리 지역을 미국으로
한정한다는 뜻이 아니며**, 고지 2번 항목이 이미 그 둘을 나눠 적고 있으므로
문구는 바뀌지 않습니다. 나머지 넷은 열려 있습니다.

**B-6도 같은 날 서명됐고, 같은 성질의 단서를 답니다.** 최소 경로로 실행돼
차단 항목만 통과했고, 비차단 구획(iOS·통화 경합·Safari·Firefox·유료 turn 다섯)은
`미기록`입니다. 그 기록이 무엇을 왜 건너뛰었는지 적고 있으므로 **비어 있는 것이
아니라 범위를 밝힌 것**이며, 다시 열어야 할 때 무엇부터인지도 거기 있습니다.

**그래서 여섯 차단 사유가 전부 닫혔고, production을 여는 것은 이제 저장소 밖의
결정입니다.** 위 두 단서를 보고 내리는 판단이며, 이 문서는 그것을 대신하지
않습니다.

B-4가 어떻게 닫혔는지는 남겨 둘 값이 있습니다. 외부의 답을 기다리지 않았지만
**저장소가 스스로 끝낼 수 있는 것도 아니었습니다** — production 환경변수 쓰기
한 번과, `main` 릴리스 뒤 그 배포의 shell에서 검사기를 돌리는 일 한 번이
필요했고, 둘 다 저장소 밖의 권한이었습니다. 두 회차 모두 서명까지 끝났습니다
(`docs/ops/voice-provider-budget-records/`).

**B-6은 마지막입니다.** B-1~B-4의 구현이 배포된 뒤의 deploy SHA에서 서명해야
하며, 구현 전에 서명하면 코드가 바뀐 뒤 다시 해야 합니다.
...
- 이 문서를 먼저 고치고, 그 다음에 코드를 고칩니다. §6.1의 결정도 그
  순서로 들어왔습니다.
- §6.1을 바꾸려면 §6.3의 네 조건을 밟습니다. 특히 무료에서 유료로 가는 것은
  되돌릴 수 없는 방향입니다.
