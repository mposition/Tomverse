...
말하는 연산자입니다. 확실한 확인은 Search Console의 URL 검사이고 그건 속성 인증을
요구하는데, 인증을 피하려고 확인하는 상황이라 순환입니다. 어느 쪽으로 판단해도
비용이 같은 건이라 `site:` 결과를 근거로 삼았습니다. 이 한계를 알고 내린 판단이라는
뜻이지, `site:`가 충분하다는 뜻은 아닙니다.
...
색인되는 다른 경로까지 덮습니다. Domain 속성 `tomverse.app`(DNS 인증)이면
서브도메인이 이미 포함되고, URL 접두어 속성 `https://tomverse.app/`만 있으면
포함되지 않습니다.
