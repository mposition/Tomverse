...
6. **서명 쪽이 미판정일 때의 종료 조건 — 비교안이 있고, 미승인입니다**(§3의 8번).
   ...
   - **"모른다"가 셋이고 답이 다릅니다** — ① 이전 서명 세대가 아직 받아들여지는가(시료
     만료), ② 현재 활성 키가 맞는가(6번 실패 = 배포 결함), ③ 이 증거가 이 배포에서
     나왔는가(§6의 5번). **이 항목은 ①뿐입니다.**
   - **①은 기다려도 판정되지 않습니다** — 이전 키로 새 access token을 만들 수 없기
     때문입니다. 그래서 **F1은 대기가 아니라 정지**이고, 그 상태는 다음 회전의
     preflight를 드리프트로 막습니다.
   - **비대칭이 핵심입니다** — 미판정의 대가는 불확실하고 작습니다(최대 10분치 access
     token, 클라이언트는 refresh로 회복). 롤백의 대가는 확실하고 큽니다(배포 후 발급된
     세션이 끊깁니다).
   ...
   승인 대상 열(G1~G10)과 검증 벡터 열둘은 그 문서에 있고, **승인란은 비어 있습니다.**
