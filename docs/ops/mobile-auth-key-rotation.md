...
6. **서명 쪽이 미판정일 때의 종료 조건 — 비교안이 있고, 미승인입니다**(§3의 8번).
   ...
   - **E7이 미결인 동안에는 F2로도 끝나지 않습니다.** §4가 F2의 종료 조건으로 적어 둔
     `-Mode preflight` 재확인은 **Active의 deployment ID를 기대값으로** 받으므로, 롤백 뒤
     그 칸을 정하지 못하면 재확인이 서지 않습니다 — **비우면 재료를 보기 전에 거절되고,
     이전 id를 두면 재료가 전부 맞는데도 결속 불일치로 실패**합니다(구조 검사도 빈
     `deploymentId`를 거절합니다). **롤백 실행과 F2 완료는 다른 상태**이고, 그 구분을
     기록에 적습니다.

   승인 대상 열하나(G1~G11)와 검증 벡터 스물하나는 그 문서에 있고, **승인란은 비어
