...
- **`createdAt`은 실제로 존재하는 날짜여야 합니다.** 모양만 보면
  ...
  파싱한 instant에서 연·월·일·시·분·초를 다시 꺼내 그대로 돌아오는지 봅니다.
...
- **링을 붙여 넣으면 거절합니다.** `signingKeys`·`refreshPeppers` 같은 field 이름과
  `id:재료` 모양·긴 base64 값을 **문서 안의 모든 문자열에 대해** 거절하며, **거절
  메시지는 그 값을 되풀이하지 않습니다** — 되풀이하는 순간 검사기가 막으려던 노출을
  검사기가 하게 됩니다. field 이름조차 짧은 식별자 모양일 때만 인용합니다.
...
> **`fingerprint.value`에는 링 탐지가 약합니다.** 64자 hex digest와 base64 개인키는
> 모양으로 구분되지 않으므로, 일반 규칙을 그대로 적용하면 **모든 진짜 fingerprint를
> 거절**하게 됩니다. 그래서 이 칸에는 `id:재료` 모양과 길이 상한만 겁니다. 이것도 §6의
