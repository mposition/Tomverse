...
5. **증거를 배포에 결속하는 방법 — 비교안이 있고, 미승인입니다.** 지금 verifier는
   ...
   **결속과 신선함은 서로를 대체하지 않습니다.** deployment ID는 배포 수명 내내 같으므로
   **같은 배포에서 만든 낡은 증거는 결속 축을 그대로 통과**하고, 그것은 `iat` 검사가
   막습니다.

   **미확인 사실 하나**: 롤백이 새 deployment ID를 만드는지 이전 것을 재사용하는지를
   확인하지 못했습니다(egress 정책). 그 답이 판정을 바꾸므로 승인 전에 운영자가
   확인합니다. 승인 대상 아홉과 검증 벡터 열둘은 그 문서에 있고, **승인란은 비어
   있습니다.**
