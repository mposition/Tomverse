...
- **은퇴 시각이 미래이면 그 키는 아무것도 검증하지 않습니다.** `sign-1@2099-01-01…`은
  ...
  승인된 15분이 아무것도 가리키지 않게 됩니다. 은퇴는 신뢰를 거둔 시각이므로 과거입니다.
  **허용 오차는 없습니다.** 그러므로 **은퇴 시각은 "지금"이 아니라 몇 분 전으로
  적습니다.** §2.1 검사가 배포 전에 실패시킵니다.

  > **rev.16 정정.** rev.15는 운영자 시계를 위해 5분을 허용했고, 유예는 여전히 **적힌
  > 시각부터** 셌습니다. 그래서 4분 앞선 은퇴 시각은 이전 서명 키를 **15분이 아니라
  > 19분** 동안 살려 두었습니다 — 허용 오차가 승인된 창에 조용히 더해진 것입니다.
  > 배포는 "이 값을 처음 본 시각"을 모르므로, 미래 시각을 존중하면서 신뢰를 창 안으로
  > 묶는 무상태 규칙은 없습니다. 그래서 관용을 코드가 아니라 **절차**로 옮겼습니다.

  > **rev.17 정정 — 그 비용은 0이 아닙니다.** rev.16은 "몇 분 전으로 적으면 비용이 0"
  > 이라고 적었고 틀렸습니다. **유예는 적힌 시각부터 흐르므로 준비와 배포가 그것을
  > 씁니다.** 2분 전으로 적고 준비·배포에 8분이 걸리면 전환 시점에 남는 것은 15분이 아니라
  > **5분**입니다. 검토자가 런타임으로 재현했습니다 — 전환 직전 발급 토큰이 만료까지
  > 298초 남았는데 `unknown_kid`로 거절됐습니다.
  >
  > 그래서 §2.1 검사가 **남은 시간을 초 단위로 출력**합니다. **거기서 멈춥니다** —
  > 아래 rev.18을 보십시오.

  > **rev.18 정정 — 재날짜를 권하는 것은 검사기의 일이 아닙니다.** rev.17은 남은 시간이
  > 절반 아래면 "은퇴 시각을 다시 적으라"는 `NOTE`를 냈습니다. **20일 전에 은퇴한
  > pepper에도 같은 안내가 나갔고**, 그대로 따르면 이미 지나간 신뢰가 **20일 연장**됩니다
  > — 검사 전후 모두 exit 0입니다.
  >
  > 검사기는 **변수 한 벌만 읽으므로 "이번 회전에서 방금 쓴 은퇴 줄"과 "이미 배포돼
  > 흘러가고 있는 은퇴 줄"을 구분하지 못합니다.** 그래서 안내는 §3의 5번으로 옮겼습니다 —
  > 거기서는 대상이 **2번에서 방금 쓴, 아직 배포되지 않은 줄** 하나뿐입니다.
  >
  > **이미 배포된 `retiredAt`은 다시 쓰지 않습니다.** 그것은 회전 절차가 아니라 그
  > 세대의 신뢰를 연장하는 일이고, 필요하다면 §5의 사고 대응이 답입니다.

- **서로 다른 id가 같은 재료를 담으면 실패합니다.** 이름을 바꾸는 것은 회전이
  아닙니다 — `sign-old`(은퇴) · `sign-new`(active)가 **같은 개인키**면 다른 모든 검사는
  끝난 회전으로 읽고, 유출 대응이었다면 **유출된 키가 계속 서명합니다.** 서명 키는 유도한
  공개키의 fingerprint로, pepper는 상수시간 비교로 판정합니다.
...
키마다 `ACTIVE` / `RETIRED, verifies until … (N s left of …)` /
`UNDECLARED -- verifies nothing`을 출력합니다. **은퇴한 키는 남은 시간이 함께 나오고**,
절반 아래면 `NOTE`가 붙습니다 — 유예는 적힌 시각부터 흐르므로 준비와 배포가 그것을
씁니다(§2의 rev.17 정정). 실패하는 것:

- `UNDECLARED`가 하나라도 있을 때
- **은퇴 시각이 미래일 때** (`RETIREMENT IN THE FUTURE`)
- **서로 다른 id가 같은 재료를 담을 때** (이름만 바꾼 "회전")
...
| 사례 (`Test-MobileAuthOpEnvTemplate.ps1`, 16건) | 무엇을 고정하나 |
|---|---|
| 1z · 1a–1d | template이 **링 둘만** 담고, 모든 값이 `op://` reference이며(평문 링이 커밋되면 여기서 걸립니다), vault·item·field가 **비어 있지 않고**, **같은 변수를 두 번 쓰면 거절**하며(`op run`은 마지막 것을 쓰므로 앞줄의 평문이 그대로 남습니다), **파서가 어느 stream으로도 문제의 줄을 내보내지 않습니다**(`*>&1`로 전부 수집합니다 — 예외 메시지만 보면 `Write-Warning` 한 줄로 새 나갑니다) |
...
> **rev.16 정정 — 신선함은 "이 배포의 것"을 증명하지 않습니다.** rev.15의 표는 "증거가
> 이 배포의 것인가 → 예"라고 적었고 **틀렸습니다.** `iat`·`exp`는 나이만 말합니다. 같은
> 후보 재료로 돌던 **직전 배포**에서 몇 분 전에 받은 token과 행도 그대로 통과하고,
> verifier는 증거를 Railway deployment ID·`rotationId`·target SHA 중 **아무것과도
> 결속하지 않습니다.**
>
...
**실패했을 때의 지시는 상황에 따라 다르고, 모드는 필수입니다.**

**증거 자체를 판정할 수 없으면 모드와 무관하게 "보류"이고, 같은 실행의 재료 불일치도
배포의 것으로 보지 않습니다.** token이 파싱되지 않거나 만료됐거나 너무 오래됐으면 그것은
**배포에 대한 판정이 아니라 증거에 대한 판정**입니다. **이전 키로 만든 8일 된 증거가 새
후보와 어긋나는 것은 당연합니다** — 다른 키로 만들었으니까요. 증거가 낡은 순간 "배포가
틀렸다"와 "증거가 낡았다"를 가를 방법이 없으므로, 안내는 "아무것도 승격·롤백·복구·
비활성화하지 말고 다시 모으라"입니다. 아래 표는 **증거가 신선한데 재료가 어긋났을 때**
적용됩니다.

| 모드 | 언제 | 재료가 어긋나면 |
|---|---|---|
| `preflight` | §3의 4번 — 배포 **전**, 지금 돌고 있는 배포에 Active를 대조 | **배포하지 마십시오.** Active를 기준으로 Railway를 복구하고 다시 |
| `rotation` | §3의 6번 — 배포 후, Pending 대조 | 승격하지 말고 Pending의 deployment ID로 Active 롤백 |
| `emergency` | §5.1의 8번 | **롤백하지 마십시오** — 되돌릴 링이 이 절차가 버린 것이고, 유출이었다면 유출된 링입니다. 비활성화 또는 roll-forward |

> **rev.16 정정 — 기본값을 없앴습니다.** 기본이 `rotation`이었으므로, emergency 실행에서
> 플래그를 빠뜨리면 **이 script가 낼 수 있는 가장 위험한 문장**이 나왔습니다. 모르는
> 문자열을 거절하는 것만으로는 부족했습니다 — 아무도 고르지 않은 모드는 모드가
> 아닙니다. verifier와 wrapper 양쪽에서 **필수**이고, 정상 회전도 `-Mode rotation`을
> 명시합니다.
...
1. 최종 값 셋을 정합니다.
   ...
   - `MOBILE_AUTH_RETIRED_SIGNING_KEYS` = `이전id@<몇 분 전 UTC instant>` (기존 줄에
     추가). **"지금"이 아니라 몇 분 전입니다** — 미래 시각은 허용되지 않고, 시계가
     조금만 앞서도 검사가 거절합니다.
...
4. **Active와 Railway가 일치하는지 확인합니다** — 배포 **전에**, 지금 돌고 있는 배포를
   상대로. 통제된 exchange를 한 번 하고 **Active 값으로** `-Mode preflight`를 돌립니다.
   ...
   **실패는 두 종류이고 하는 일이 다릅니다.** 출력의 안내가 어느 쪽인지 말해 줍니다.
   ...
   - **증거 불충분**(`Nothing was decided about the deployment`) — token 파싱 실패·만료·
     너무 오래됨. **아무것도 바꾸지 않습니다.** 배포도 복구도 하지 않고, exchange를 다시
     해서 증거를 모아 이 단계를 다시 돌립니다. 같은 실행에 재료 불일치가 함께 보이더라도
     그것은 **증거에 대한 사실**입니다.
   - **재료 불일치**(`Do NOT deploy`) — 증거는 신선한데 어긋났습니다. 이번 회전과 무관한
     드리프트이므로 **배포를 중단**하고, Active를 기준으로 Railway를 복구한 뒤 3번부터
     다시 합니다(계약 3번). 아직 배포한 것이 없으므로 "롤백"은 이 자리에서 말이 되지
     않습니다.

   어느 쪽이든 여기서 만든 exchange 세션은 폐기합니다.
4.5. **배포 전에 access token 하나와 refresh token 하나를 받아 둡니다.** 7번의
   이전 세대 확인이 이것을 씁니다 — 배포 뒤에는 이전 키로 만들 수 없습니다.

   **access token은 10분짜리이므로 배포 직전에 받습니다.** 5~6번(검사 재실행, 배포,
   증거 수집, 필요하면 재수집)이 그 10분을 넘기면 7번을 시작하기도 전에 만료됩니다.
   refresh token은 idle 30일이라 이 문제가 없습니다.
5. **배포 직전에 §2.1 검사를 다시 돌려 남은 유예를 봅니다.** 유예는 2번에서 적은
   은퇴 시각부터 흐르므로 3·4번에 쓴 시간이 그것을 씁니다. 검사가
   `N s left of 900s`를 출력합니다. **남은 시간이 전환에 필요한 만큼이 아니면 은퇴 시각을
   다시 적고 2번부터 다시 합니다** — 그 세대의 자격증명을 들고 있는 클라이언트가 받는
   것은 승인된 창이 아니라 **남은 시간**입니다.

   **다시 쓰는 대상은 2번에서 방금 쓴, 아직 배포되지 않은 Pending의 줄 하나뿐입니다.**
   Active에 이미 들어 있는 `retiredAt`은 건드리지 않습니다 — 그것을 다시 쓰는 것은
...
7. **이전 세대를 확인합니다**(유예 안): 4.5번에서 받아 둔 access token이 아직
   ...
   **먼저 시료가 아직 유효한지 봅니다. 두 시료의 "유효"가 서로 다릅니다.**

   **access token — `exp`만 보면 됩니다.** 6번이 오래 걸렸으면 10분을 넘겨 **스스로
   만료**됐을 수 있고, 그때의 거절은 배포에 대한 관측이 아닙니다 — 배포를 전혀 건드리지
   않은 대조군에서도 똑같이 거절됩니다.

   **refresh token — `exp` 이전이라고 유효한 것이 아닙니다.** 승인된 A안은 **엄격한
   1회용**이므로, 이미 소비됐거나 무효화된 token을 다시 내미는 것은 회전 실패가 아니라
   **재사용(D8)**이고 그 답은 **family 폐기**입니다. 판정 순서상
   `consumedAt`·`invalidatedAt`이 family·device 상태보다 **먼저** 걸리므로, 발급 2분
   뒤에 만료 시각이 한참 남아 있어도 `reuse_detected`입니다
   (`lib/mobileRefreshRotationCore.ts`).

   그러므로 refresh 시료는 **쓰기 전에 이것들을 확인합니다.**

   - 그 `MobileRefreshRotation` 행이 **미소비·미무효화**인지
   - family가 폐기되지 않았고 device도 살아 있고 계정이 active인지
   - **그 세션의 앱이 그 사이에 자동 refresh를 하지 않았는지** — 했다면 시료는 이미
     소비됐습니다

   **그래서 4.5번의 시료는 아무도 쓰고 있지 않은 통제된 세션에서 받습니다.** 앱이 계속
   들고 있는 세션의 refresh token을 시료로 쓰면, 이 확인이 그 세션을 **스스로
   폐기**합니다 — 그리고 그 폐기는 화면상 "새 배포가 이전 pepper를 못 받는다"처럼
   보입니다.

   | 시료 | 판정 전에 볼 것 | 결과 | 뜻 |
   |---|---|---|---|
   | access | `exp` 이전인가 | 받아들여짐 | 새 배포가 이전 서명 키를 받아 줍니다 |
   ...
   **서명 쪽이 미판정으로 남으면 이 회전에서 더 볼 방법은 없습니다.** 이전 키로 새
   ...
   서명 키가 실제로 빠졌다면 살아 있던 10분짜리 token이 거절되고 **클라이언트가 refresh로
   회복**합니다. **다만 그것이 "그러니 승격해도 된다"는 뜻은 아닙니다** — 8번을
   보십시오.

   > **rev.20 정정.** rev.19는 7번의 실패를 전부 롤백 갈래로 보냈습니다. **틀렸습니다** —
   > 6번의 수집·재수집이 access token의 10분을 넘기는 것은 흔한 일이고, 그때의 거절은
   > 배포가 아니라 시료에 대한 사실입니다. 증거 불충분을 조치 근거로 삼지 않는다는 규칙이
   > 여기에도 그대로 적용됩니다.

   > **rev.16 정정.** rev.15는 이 실패를 "롤백이 동작하지 않는 상태"라고 적었고
...
8. **6·7이 통과하면 Pending을 Active로 승격**하고 Pending을 비웁니다. 승격은 Pending
   ...
   - **증거 불충분**(`Nothing was decided about the deployment`) — **승격도 롤백도 하지
     ...
     없었으므로 배포 상태를 바꿀 근거도 없습니다.
   - **재료 불일치**(`Do NOT promote Pending to Active`), 또는 **유효한 시료에서
     확인된 7번의 실패** — 5번의 deployment ID를 기준으로 **Active로 롤백하고 Pending을
     폐기합니다.** 7번이 `미판정`인 것은 여기에 해당하지 않습니다.
   - **7번이 미판정으로 남은 경우**(6번 통과, 이전 refresh 통과, 이전 access 미판정) —
     **승격하지 않고 Pending을 그대로 둡니다. 미판정은 통과가 아닙니다.**

     > **이 상태는 그대로 두고 끝낼 수 없습니다.** 배포는 이미 나갔고 6번이 통과했으므로
     > Railway는 Pending을 돌리는데 Active는 이전 링을 가리킵니다 — 다음 preflight가
     > 드리프트로 잡을 상태입니다. 남은 선택지는 **롤백**(배포 후 발급된 세션을 잃습니다)
     > 이거나 **6번과 refresh 쪽만으로 승격하는 조건부 승격**인데, 뒤쪽은 승인된 계약이
     > 아닙니다 — 무엇을 허용 조건으로 삼고 그 판정을 어디에 기록할지가 정해져 있지
     > 않습니다. **§6의 6번**으로 올렸습니다. 정해지기 전까지 이 상태는 **사람에게
     > 올립니다.**
     >
     > 여기에 빠지지 않는 방법은 4.5번입니다 — access token을 **배포 직전에** 받으면
     > 7번까지 10분 안에 끝납니다.
...
**8번이 통과하지 못했을 때도 두 갈래이고, 급한 상황일수록 그 구분이 중요합니다.**

- **증거 불충분**(`Nothing was decided about the deployment`) — **아무것도 바꾸지
  않습니다.** 비활성화도 roll-forward도 하지 않고, exchange를 다시 해서 8번을 다시
  돌립니다. 판정이 없었으므로 배포 상태를 바꿀 근거가 없고, 여기서 성급히 비활성화하면
  **멀쩡한 배포를 503으로 만드는 것**입니다.
- **재료 불일치**, 또는 **이전 자격증명이 아직 거절되지 않음** — 이때는 **롤백할 곳이
  없습니다.** 이전 Active는 이미 신뢰를 잃었고, 그것이 이 절차로 온 이유입니다. 선택지는
  둘뿐입니다.
...
**"이전 값으로 되돌린다"는 어느 갈래에도 없습니다.** 그 값이 없다는 것이 전제입니다.
...
1. **실제 vault 연동 확인.** 명령과 template은 이제 있습니다 —
   ...
   - **항목의 id를 vault에서 읽어 template의 두 줄에 넣습니다.** vault 이름의 공백은
     따옴표로 처리되지만, 항목 이름의 **em dash는 reference가 지원하는 문자 집합에
     없습니다**. 그런 이름의 항목은 **id로 참조**하는 것이 문서화된 방법이므로 —
     `op://Tomverse Production Secrets/<item id>/signing-ring` — **항목 이름을 바꿀
     필요가 없습니다.** §2.2의 이름은 그대로 둡니다.
   - **해석 확인은 `op read`로 하지 않습니다.** `op read`는 비밀값을 stdout에 그대로
     출력하므로, 확인하려다 터미널과 그 기록에 링을 남기게 됩니다. 대신 **§2.1의
     `op run` 명령을 그대로 돌립니다** — 그것이 해석과 주입을 한 번에 확인하고, 출력에는
...
3. **상시 점검 — 도구는 있고, 운영은 미결입니다.**
   ...
   그리고 링에 없는 은퇴 id와 **일부만 설정된 상태**까지. **판정은 §2.1 검사와 같은
   module**(`scripts/mobile-auth-keyring-state.mjs`)에서 나오므로 둘이 갈라지지
   않습니다.

   **gate가 아니라 보고입니다.** 좋은 소식이든 나쁜 소식이든 0으로 끝나고, 유일한
   비정상 종료는 **링이 파싱되지 않는 경우**입니다 — 그때는 출력 전체가 배포와 무관한
   이야기가 되기 때문입니다. 실패하는 상시 점검은 누군가 끄는 상시 점검입니다.
...
4. **실행 기록 — 체계는 있습니다.**
   ...
   생성기·검사기·파일 이름 규칙은 다른 검증 기록들과 **같은 것**을 씁니다.

   ```
   npm run new:staging-verification-record -- --feature mobile-auth-key-rotation --sha <40자리 SHA>
   ```

   기준 SHA·실행 환경·명령·종료 코드·관측을 적고, **관측과 판정을 분리**하며, 실행하지
   않은 것은 `미기록`으로 남깁니다. **비밀값·토큰·원문 환경변수는 적지 않습니다** —
   기록 README의 6번.

   **이것으로 5번(증거 결속)이 해결되지 않습니다.** SHA와 deployment ID를 적는 것은
   *사람이 무엇을 대상으로 실행했다고 말했는지*를 남기는 일이지, 수집된 증거가 그
   배포에서 나왔다는 증명이 아닙니다.
5. **증거를 배포에 결속하는 방법.** 지금 verifier는 증거가 **오래되지 않았다**는 것만
   봅니다(§2.2의 정정). 15분 안에 두 배포가 겹치면 직전 배포의 증거도 통과합니다.
   후보는 둘이고 **둘 다 승인이 필요합니다** — 배포가 발급한 marker를 token이나 rotation
   행에 남겨 Pending의 deployment ID와 대조하는 방법(승인된 token 모양을 바꾸는
   일입니다), 또는 verifier가 그 배포를 직접 호출해 증거를 모으는 방법. 제가 임의로
   고르지 않았습니다.
6. **서명 쪽이 미판정일 때의 종료 조건**(§3의 8번). 지금 계약은 "미판정은 통과가
   아니므로 승격하지 않는다"이고, 그러면 배포는 나갔는데 Active가 옮겨가지 않은 상태로
   사람에게 올라갑니다. 조건부 승격을 허용할지, 허용한다면 **무엇을 조건으로 삼고 그
   판정을 어디에 기록할지**가 결정 사항입니다. 만료 오판을 고치는 일과는 별개이며,
   **미판정을 통과로 간주하는 형태는 안 됩니다.**
