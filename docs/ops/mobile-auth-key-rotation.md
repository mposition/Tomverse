...
**승인:** `mposition`, 2026-09-02, N2 검토 11차. 승인 범위는 **이 문서와 릴리스
...
3. **둘이 어긋나면 배포를 중단합니다.** secret store를 기준으로 Railway 값을 복구한 뒤,
   §2.1의 wrapper 검사와 **단일 staged 배포**를 다시 수행합니다. 어긋난 채로 진행하지
   않습니다.
...
5. **secret store의 평문까지 잃거나 믿을 수 없으면 정상 회전을 시도하지 않습니다.**
   그때는 §5 사고 대응입니다 — 새 링을 발급하고, 기존 세션 폐기와 사용자 재로그인을
   감수합니다. **잃은 평문을 추측해 회전을 흉내 내지 않습니다.**
...
### 아직 비어 있는 결정란 — 이것까지 정해져야 실행 가능한 최종본입니다

**저장소에 승인된 secret store 제품이 없습니다.** 그래서 여기에 업체를 임의로 적지
않았습니다 — 문서가 정하지 않은 것을 정한 것처럼 보이게 만드는 쪽이, 비어 있는 칸보다
나쁩니다.

| 결정란 | 값 | 왜 필요한가 |
|---|---|---|
| secret store 제품명 | ____________ | 절차의 명령이 여기서 나옵니다. §3의 "store에서 읽는다"가 무엇을 실행하는 말인지가 정해집니다 |
| vault(또는 project·folder) 이름 | ____________ | 링 둘이 **어느 경로에** 있는지. 이름이 없으면 복구 시점에 사람이 찾아야 합니다 |
| 복구 권한 | ____________ | 운영자 계정을 잃었을 때 정본에 도달하는 경로. **이것이 없으면 계약 5번(사고 대응)이 유일한 복구가 됩니다** |

세 칸이 채워지고 승인되면 §3·§5의 "store에서 읽는다"를 실제 명령으로 바꿉니다.
**그 전까지 production 활성화를 하지 않습니다.**
...
0. **secret store에서 현재 링을 읽습니다** — §2.2 계약 1번. `이전키`의 평문은 거기서
   오고, Railway에서 오지 않습니다. **읽히지 않거나 값을 믿을 수 없으면 여기서
   멈춥니다**: 그것은 회전이 아니라 §5입니다(계약 5번).
1. 최종 값 셋을 정합니다.
...
2. **secret store를 먼저 갱신합니다** — 새 키를 링에 넣습니다. 정본이 사본보다 뒤에
   있으면, 그 사이에 배포가 끝났을 때 정본이 배포된 값을 모릅니다.
3. §2.1의 검사를 그 세 값으로 실행합니다. 이전 키가
   `RETIRED, verifies until …`로, 새 키가 `ACTIVE (signs)`로 나와야 합니다.
4. **Railway 값이 store와 어긋나 있으면 배포를 중단합니다**(계약 3번). store를 기준으로
   Railway를 복구하고 3번부터 다시 합니다.
5. 셋을 **한 번에** 저장하고 배포합니다.
6. **배포 뒤 두 활성 id를 대조합니다**(계약 4번, §2.2의 (c)). 통제된 exchange를 한 번
   성공시켜 access token의 `kid`와, 그때 생긴 `MobileRefreshRotation` 행의 `pepperKid`를
   읽어 의도한 id와 같은지 봅니다.
7. **15분 뒤부터** 이전 키는 검증에 쓰이지 않습니다.
...
**여기로 오는 길은 둘입니다** — 키가 유출됐을 때, 그리고 **secret store의 평문을
잃거나 믿을 수 없을 때**(§2.2 계약 5번). 두 번째 경우 정상 회전은 시도하지 않습니다:
이전 링을 재현할 방법이 없으므로 아래 절차로 **새 링을 발급하고 그 세대 사용자의
재로그인을 감수**합니다. 잃은 평문을 추측하지 않습니다.
...
**그리고 사고 대응이 끝나면 secret store가 새 링의 정본이 됩니다** — 급한 순서상
Railway를 먼저 고쳤더라도, 그 값을 store에 옮겨 적기 전까지는 정본이 없는 상태입니다.
그 상태로 다음 회전을 시작하면 같은 사고를 다시 겪습니다.
...
- **§2.2의 세 결정란**(secret store 제품명 · vault 이름 · 복구 권한) — 정본을 secret
  store에 두는 것은 2026-09-02에 승인됐지만, **어느 store인지가 비어 있는 동안 §3의
  0번과 2번은 실행할 수 있는 문장이 아닙니다.** 활성화의 선결 조건입니다.
