...
7. **이전 세대를 확인합니다**(유예 안): 4.5번에서 받아 둔 access token이 아직
   ...
   **먼저 시료가 아직 유효한지 봅니다.** 6번이 오래 걸렸으면 그 access token은 10분을
   넘겨 **스스로 만료**됐을 수 있고, 그때의 거절은 배포에 대한 관측이 아닙니다 — 배포를
   전혀 건드리지 않은 대조군에서도 똑같이 거절됩니다. **만료된 시료의 거절은 실패가
   아니라 `미판정`입니다.**

   | 시료 | 결과 | 뜻 |
   |---|---|---|
   | 유효(`exp` 이전) | 받아들여짐 / 회전 성공 | 새 배포가 이전 세대를 받아 줍니다 |
   | 유효 | 거절 / 회전 실패 | **배포 결함.** 그 세대 사용자가 지금 로그아웃되고 있습니다 |
   | 만료 | 거절 | **미판정.** 승격 근거도 롤백 근거도 아닙니다 |

   **서명 쪽이 미판정으로 남으면 그대로 둡니다.** 이전 키로 새 access token을 만들 수는
   없으므로 이 회전에서는 더 볼 방법이 없습니다. 그 미확인의 크기는 제한적입니다 —
   이전 서명 키가 실제로 빠졌다면 살아 있던 10분짜리 token이 거절되고 **클라이언트가
   refresh로 회복**합니다. pepper 쪽(refresh token)은 idle 30일이라 거의 언제나 유효하고,
   그쪽이 실패하면 그것은 **사용자가 다시 로그인해야 하는** 훨씬 큰 관측입니다.
...
8. **6·7이 통과하면 Pending을 Active로 승격**하고 Pending을 비웁니다. 승격이 검증 뒤에
   ...
   - **재료 불일치**(`Do NOT promote Pending to Active`), 또는 **유효한 시료에서
     ...
     폐기합니다.** 7번이 만료로 `미판정`인 것은 여기에 해당하지 않습니다.
