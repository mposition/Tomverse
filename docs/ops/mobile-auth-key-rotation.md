...
2. **Active·Pending 두 항목 — 모양은 정해졌고, 두 가지가 남았습니다.**
   ...
   **승인용 비교안이 있습니다 — 아직 승인되지 않았습니다.**
   `.github/audits/2026-09-08-mobile-auth-fingerprint-rule-approval.md`가 후보 셋(A·B·C),
   권장안의 명세와 고정 합성 벡터, 세 절차와의 충돌 점검, 규칙 변경 시 이행, 승인 대상
   열일곱, 합성 테스트 벡터 스물넷을 담습니다. **그 문서는 규칙을 확정하지 않으며 이 절의
   상태를 바꾸지 않습니다.**
