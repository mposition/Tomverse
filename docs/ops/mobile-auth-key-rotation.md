...
|---|---|
| 1z · 1a–1d | template이 **링 둘만** 담고, 모든 값이 `op://` reference이며(평문 링이 커밋되면 여기서 걸립니다), vault·item·field가 **비어 있지 않고**, **같은 변수를 두 번 쓰면 거절**하며(`op run`은 마지막 것을 쓰므로 앞줄의 평문이 그대로 남습니다), **거절 메시지가 문제의 줄을 되풀이하지 않습니다** |
