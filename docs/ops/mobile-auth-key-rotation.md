...
- **은퇴 시각이 미래이면 그 키는 아무것도 검증하지 않습니다.** `sign-1@2099-01-01…`은
  ...
  **허용 오차는 없습니다.** 그러므로 **은퇴 시각은 "지금"이 아니라 몇 분 전으로
  ...
  > 몇 분 전으로 적으면 비용이 0이고, 일찍 적는 것은 조용한 연장이 아니라 검사가 보고하는
  > 거절이 됩니다.
...
4. **Active와 Railway가 일치하는지 확인합니다** — 배포 **전에**, 지금 돌고 있는 배포를
   ...
   실패하면 이번 회전과 무관한 드리프트이므로 **배포를 중단**하고, Active를 기준으로
   Railway를 복구한 뒤 3번부터 다시 합니다(계약 3번). `preflight` 모드의 실패 안내가
   그렇게 말합니다 — 아직 배포한 것이 없으므로 "롤백"은 이 자리에서 말이 되지 않습니다.
   여기서 만든 exchange 세션도 폐기합니다.
...
7. **이전 세대를 확인합니다**(유예 안): 4.5번에서 받아 둔 access token이 아직
   받아들여지는지, refresh token이 회전에 성공하는지. 실패하면 **새 배포가 이전 세대를
   받아 주지 못하는 상태** — 즉 그 세대 사용자가 지금 로그아웃되고 있다는 뜻이므로 8번의
   승격을 하지 않습니다.
