...
8.1. **서명 쪽이 미판정일 때 — 2026-09-10 결재된 절차.** 근거와 대가는
   ...
   **먼저 무엇을 모르는지 가릅니다**(§1의 셋). 이 갈래는 ①뿐입니다.
   ...
   | 아직 유효한 이전 access token이 거절됨 | **②** | **롤백** |
   ...
   **F2(롤백)로 끝내려면 완료 조건이 따로 있습니다.** §4가 적어 둔 `-Mode preflight`
   재확인 통과이고, 그 재확인은 **Active의 deployment ID를 기대값으로** 받습니다.
   **E7이 미확인인 동안에는 그 값을 확인할 수 없으므로 완료를 선언하지 않습니다** —
