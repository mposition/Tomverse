...
8.1. **서명 쪽이 미판정일 때 — 2026-09-10 결재된 절차.** 근거와 대가는
   ...
   **먼저 무엇을 모르는지 가릅니다**(§1의 셋). 이 갈래는 ①뿐입니다.
   ...
   | 이전 access token이 거절됐고, **그 시점에 유예 창이 남아 있었음** | **②** | **롤백** |
   | 이전 access token이 거절됐고, **그 시점에 유예 창이 이미 닫혔음** | **①** | **롤백이 아닙니다** — 계약대로 거절한 것입니다. 답이 된 것도 아니므로 아래로 가되 ①은 **해당 없음**입니다(G7) |
   | 거절인데 **그 시점의 유예 상태를 기록하지 않았음** | **판정 없음** | 위 둘 중 무엇인지 모릅니다. **먼저 그 값을 확인**하고 다시 분류합니다 |
   ...
   > 입니다(`tests/mobileAuthRollbackMaterial.test.mjs`). 뒤쪽을 결함으로 읽으면
   > **제대로 동작한 배포를 되돌리게 됩니다.** 그래서 기록해야 하는 것은 "거절됐다"가
   > 아니라 **"거절된 그 시각에 유예가 얼마나 남아 있었는가"**이고, 이는 **판정 시점의
   > 남은 창과 다른 관측**입니다(둘 다 적습니다).
   >
