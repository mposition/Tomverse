...
- **`createdAt`은 실제로 존재하는 날짜여야 합니다.** 모양만 보면
  `2026-13-40T99:99:99Z`가 통과하고, `Date.parse`만 쓰면 `2026-02-31`이 3월로 굴러갑니다.
  둘 다 그대로 아래 순서 대조까지 흘러가서 **그 대조가 아무 말도 못 하게** 만들므로,
  파싱한 instant에서 연·월·일·시·분·초·**밀리초**를 다시 꺼내 그대로 돌아오는지 봅니다.
  **소수점 이하 초는 순서 대조에도 그대로 들어갑니다** — 허용해 놓고 초까지만 비교하면
  `10:00:00.900Z`와 `10:00:00.100Z`가 같은 순간이 되고, 먼저 쓴 Pending이 나중에 쓴
  것으로 판정됩니다.
...
- **재료가 같은지는 `algorithm`이 같을 때만 묻습니다.** 계산 규칙이 다른 두 값은 같은
  측정이 아니므로, 문자열이 같다고 "같은 재료"라고 말하면 정상 회전을 "이름만 바꿨다"고
  거절하게 됩니다. 규칙이 다르면 **판정 불가**라고 말하고, **판정 불가는 통과가
  아닙니다** — 한 규칙으로 둘 다 다시 계산해야 이 대조가 성립합니다.
- §5.1의 전면 교체는 같은 모양을 `kind: "emergency-pending"`으로 씁니다. 회전이 아닌
  것이 회전으로 기록되지 않게 하기 위해서입니다.
- **모양이 닫혀 있습니다.** 위 field 말고 다른 이름은 거절하고, `fingerprint`와
  `$comment` 밖에는 중첩 구조를 두지 않습니다. **검사가 닿지 못하는 자리를 남기지 않기
  위해서입니다** — 링은 최상위에만 붙여 넣어지지 않습니다.
- **링을 붙여 넣으면 거절합니다.** `signingKeys`·`refreshPeppers` 같은 field 이름과
  `id:재료` 모양·긴 base64 값을 **문서 안의 모든 문자열에 대해** 거절합니다. 배열
  안이든 `$comment` 안이든 마찬가지이며, `$comment`는 문자열이나 문자열 배열이어야
  합니다 — 검사가 닿지 못하는 자리를 남기지 않기 위해서입니다.
- **거절 메시지는 그 값을 되풀이하지 않습니다** — 되풀이하는 순간 검사기가 막으려던
  노출을 검사기가 하게 됩니다. field **이름**도 **이 모양이 선언한 이름일 때만**
  인용하고, 그 밖의 key는 내용과 무관하게 위치로만 가리킵니다. 모양이 안전해 보이는
  것과 내용이 비밀이 아닌 것은 다릅니다 — 32자 영숫자 pepper는 흔한 식별자처럼 생겼고,
  값이 아니라 key로도 들어올 수 있습니다.

**fingerprint는 구조만 봅니다.** `algorithm`이 비어 있지 않은지, `value`가 8~128자의
불투명한 문자열인지만 확인하고 값이 옳은지는 보지 않습니다 — **fingerprint를 무엇으로
계산할지가 아직 정해지지 않았기 때문입니다**(§6의 2번). template의 그 두 칸은
자리표시자이고, 검사기는 **저장소의 그 두 파일을 읽을 때만, 정확히 그 문자열에
한해** 통과시킵니다. 실제 항목에 자리표시자가 남아 있으면 실패합니다.

> **`fingerprint.value` 한 칸에만 링 탐지가 약합니다.** 64자 hex digest와 base64 개인키는
> 모양으로 구분되지 않으므로, 일반 규칙을 그대로 적용하면 **모든 진짜 fingerprint를
> 거절**하게 됩니다. 그래서 이 칸에는 `id:재료` 모양과 길이 상한만 겁니다. **`algorithm`과
> `$comment`는 산문이므로 일반 규칙을 그대로 받습니다** — 약한 규칙을 fingerprint 전체에
>걸면 그 둘이 링을 담을 수 있는 자리가 됩니다. 이것도 §6의
> 2번이 답해야 하는 것 중 하나입니다 — **정해질 계산 규칙의 결과는 비밀이 아닌 절반에
> 두어도 안전한 값이어야 합니다.**
...
6a. **세 시료로 한 회차를 판정합니다**(E5, 2026-09-09 승인). 6번을 **`iat` 기준 2분
   ...
   - **판정은 세 시료를 다 모은 뒤 한 번에** 하고, 그 시점에 셋이 **모두 유효**해야
     ...
     > **여기서 60초 skew는 붙지 않습니다.** verifier와 회차 판정기는 `exp`에서
     > **바로** 거절합니다(`exp <= now`). 7번의 런타임 검사만 `exp + 60초`를 쓰므로,
     > **발급 후 630초인 같은 token이 런타임에서는 받아들여지고 6·6a에서는 재수집**
     > 입니다. 두 경계는 서로 다른 검사의 것이고, **문구를 맞추려고 검사기를 완화하지
     > 않습니다** — 증거는 런타임보다 엄격한 쪽이 맞습니다.
...
7. **이전 세대를 확인합니다**(유예 안): 4.5번에서 받아 둔 access token이 아직
   ...
   **access token — 두 가지를 봅니다: 시료의 `exp`와 이전 서명 키의 유예.** 6번이
   오래 걸렸으면 시료가 **스스로 만료**됐을 수 있고, 그때의 거절은 배포에 대한 관측이
   아닙니다 — 배포를 전혀 건드리지 않은 대조군에서도 똑같이 거절됩니다. **그리고
   이전 서명 키의 900초 유예가 끝났으면 거절이 정상입니다** — 그때는 만료 여부와
   상관없이 거절이며, 그것을 결함으로 읽으면 제대로 동작한 배포를 되돌립니다.

   **두 경계를 실제 함수로 확인했습니다**(`iat` = 은퇴 시각인 시료,
   `tests/mobileAuthRollbackMaterial.test.mjs`). **아래는 런타임 검사의 숫자입니다** —
   6·6a의 증거 검사는 skew 없이 `exp`에서 바로 거절하므로 경계가 다릅니다(6a의 상자).

   | 제시 시각 | 응답 | 왜 |
   |---|---|---|
   | +659초 | 받아들여짐 | 만료 검사는 `exp + 60초 skew` 기준입니다 — TTL 600초에 skew를 더한 **660초**가 경계이지 600초가 아닙니다 |
   | +660초 ~ +899초 | `expired` | 시료가 만료됐습니다. **배포에 대한 관측이 아닙니다** |
   | +900초 이후 | `unknown_kid` | **키 조회가 만료 검사보다 먼저** 돕니다. 유예가 끝나면 만료 여부와 무관하게 이 답입니다 |

   > **두 경계는 시각의 출처가 다릅니다.** 만료는 시료의 `iat`에서, 유예는
   > `MOBILE_AUTH_RETIRED_SIGNING_KEYS`에 적은 **은퇴 시각**에서 셉니다. §3은 은퇴
   > 시각을 몇 분 **과거로** 적으라고 하므로 둘은 대개 어긋나 있고, 위 표의 숫자는
   > 둘이 같은 시각일 때의 것입니다. **그러므로 TTL과 유예의 길이만 비교해 응답을
   > 단정하지 말고, 그 시료의 `iat`와 실제로 적어 둔 은퇴 시각으로 각각 셉니다.**
   ...
   **그래서 4.5번의 시료는 아무도 쓰고 있지 않은 통제된 세션에서 받습니다.** 앱이 계속
   ...
   | 시료 | 판정 전에 볼 것 | 결과 | 뜻 |
   |---|---|---|---|
   | access | 시료 유효(`exp + 60초` 이전) **그리고** 키 유예 안 | 받아들여짐 | 새 배포가 이전 서명 키를 받아 줍니다 |
   | access | 〃 | 거절 | **배포 결함.** 그 세대 token이 지금 거절되고 있습니다 — 이 한 줄만 롤백입니다 |
   | access | 시료 유효인데 **키 유예가 이미 끝남** | 거절(`unknown_kid`) | **결함이 아닙니다.** 계약대로 거절한 것이고 답이 된 것도 아니므로 **8.1의 ①**로 갑니다(유예 종료이므로 **해당 없음**) |
   | access | 시료 만료(`expired`) | 거절 | **미판정** |
   | access | **거절 시각의 시료 유효성·키 유예를 기록하지 않음** | 거절 | **미판정.** 위 셋 중 무엇인지 모릅니다 — **롤백하지 않고** 그 값부터 확인합니다. **두 값은 여기서 관측해 적습니다**(8.1의 모듈은 받아 적을 뿐 확인하지 못합니다) |
...
8. **6·7이 통과하면 Pending을 Active로 승격**하고 Pending을 비웁니다. 승격은 Pending
   ...
   - **재료 불일치**(`Do NOT promote Pending to Active`), 또는 **7번의 확인된 실패**
     — 5번의 deployment ID를 기준으로 **Active로 롤백하고 Pending을 폐기합니다.**

     > **"확인된 실패"는 7번 표의 두 줄입니다 — 축마다 하나씩.**
     >
     > - **access**: 거절된 그 시각에 **시료가 유효했고**(`exp + 60초` 이전)
     >   **이전 서명 키의 유예도 남아 있던** 경우. **유예가 끝난 뒤의 거절은 실패가
     >   아니라 계약**이고 8.1의 ①로 갑니다. 만료된 시료의 거절, 그리고 **거절 시점의
     >   두 값을 기록하지 않은 거절**은 `미판정`이며 여기에 해당하지 않습니다.
     > - **refresh**: 정상·미소비 시료가 감사 기록에서 `secret_mismatch`로 실패한
     >   경우. **이쪽은 좁히지 않았습니다** — 현재 활성 재료는 맞는데 **이전 pepper가
     >   빠진** 배포가 여기 걸리고, 그 세대는 재로그인하게 됩니다. 8.1은 **refresh
     >   통과**를 전제하므로 이 실패는 그쪽으로 갈 수도 없습니다.
     >
     > access 쪽 판정 규칙은 `scripts/mobile-auth-undetermined-signing-core.mjs`가
     > 계산합니다. **그 모듈은 거절 시각의 시료 유효성과 남은 유예를 입력으로 받고,
     > 둘 중 하나라도 없으면 분류를 거부합니다** — 다만 **그 값이 맞는지는 확인하지
     > 못합니다.** 시료가 실제로 유효했는지 확인하는 것은 7번의 일이고, 모듈은 그
     > 관측을 받아 적을 뿐입니다.
   - **7번이 미판정으로 남은 경우**(6번 통과, 이전 refresh 통과, 이전 access 미판정) —
     **아래 8.1을 따릅니다.** 미판정은 통과가 아닙니다.
     ...
     > 7번까지 10분 안에 끝납니다.
8.1. **서명 쪽이 미판정일 때 — 2026-09-10 결재된 절차.** 근거와 대가는
   `.github/audits/2026-09-09-mobile-auth-undetermined-signing-half-approval.md`
   (G1~G11)에 있습니다. 판정 규칙은
   `scripts/mobile-auth-undetermined-signing-core.mjs`가 계산합니다 — **그 모듈은 규칙만
   계산하고 승인하지 않습니다.**

   **먼저 무엇을 모르는지 가릅니다**(§1의 셋). 이 갈래는 ①뿐입니다.

   | 관측 | 갈래 | 답 |
   |---|---|---|
   | 6번의 증거를 판정할 수 없음 | **②'** | **재수집.** 롤백이 아닙니다 |
   | 6번이 **판정 가능한 증거로** 재료 불일치 확인 | **②** | **롤백.** ①은 묻지 않습니다 |
   | 거절 — 그 시점에 **시료가 유효했고**(7번 기준) **유예 창도 남아 있었음** | **②** | **롤백.** 두 조건이 함께일 때만입니다 |
   | 거절 — 그 시점에 **시료는 유효했는데 유예 창이 이미 닫혔음** | **①** | **롤백이 아닙니다** — 계약대로 거절한 것입니다. 답이 된 것도 아니므로 아래로 가되 ①은 **해당 없음**입니다(G7) |
   | 거절 — 그 시점에 **시료가 이미 유효하지 않았음**(유예가 남아 있어도) | **①** | **롤백이 아닙니다** — 그 거절은 시료에 대한 사실이지 배포에 대한 사실이 아닙니다. ①의 이름은 **판정 시점의 창**이 정합니다(거절 시점의 유예도 닫혀 있었다면 **해당 없음**) |
   | 거절 — **두 관측 중 하나라도 기록이 없음**(시료 유효성 · 남은 유예) | **판정 없음** | 위 셋 중 무엇인지 모릅니다. 모듈이 **없는 관측의 이름을 대며 분류를 거부**합니다 — **먼저 그 값을 확인**하고 다시 분류합니다 |
   | 6번 통과 · refresh 축 통과 · access 시료가 **만료** | **①** | 아래 |

   > **거절을 봤다고 바로 롤백이 아닙니다.** 이전 서명 키의 유예는 **900초이고 경계는
   > 배타**입니다 — 실제 경로로 재현하면 **+899초는 통과, +900초는 `unknown_kid`**
   > 입니다(`tests/mobileAuthRollbackMaterial.test.mjs`, 시료가 유예보다 오래 사는
   > 전제). 뒤쪽을 결함으로 읽으면 **제대로 동작한 배포를 되돌리게 됩니다.**
   >
   > **그래서 기록하는 것은 "거절됐다"가 아니라 그 시각의 두 값입니다** —
   > **시료가 유효했는가**와 **유예가 얼마나 남아 있었는가**. 한쪽으로 다른 쪽을
   > 유추하지 않습니다: 유예가 남았다는 것이 시료가 살아 있었다는 뜻은 아니고, 그
   > 유추가 **7번에서 미판정으로 넘어온 시료를 여기서 다시 롤백으로** 만듭니다.
   > 남은 유예는 **판정 시점의 남은 창과도 다른 관측**입니다(셋 다 적습니다).
   >
   > 덧붙여, **두 실패를 같은 칸에 적지 않습니다.** 어느 쪽이 나오는지는 TTL과 유예의
   > 길이 비교로 단정할 수 없습니다 — 만료 검사에는 60초 skew가 붙고(경계는 `exp +
   > 60초`), **키 조회가 만료 검사보다 먼저** 돌며, 은퇴 시각은 발급 시각과 다릅니다.
   > 7번의 두 경계 표에 실제 함수로 확인한 숫자를 적어 두었습니다.

   **① 이면 F1(보류)로 들어가고, 다음을 기록합니다.**

   - **기산점** — 7번의 access 축을 미판정으로 판정한 **그 시각**. 배포 완료 시각이
     아닙니다.
   - **기한 24시간**(G9). **넘겨도 자동으로 아무 일도 일어나지 않습니다** — (a)
     운영자의 처리기한이므로 **초과를 기록하고 보류는 이어집니다.**
   - **유예 창의 남은 시간**. 닫혔으면 ①은 **해당 없음**이지 통과가 아닙니다(G7).
   - **보류 중임을 Active 항목에 적습니다.** 그 상태에서 무조건 산문 셋이 틀린 복구를
     지시하기 때문입니다(§6의 6번). **산문만 보고 복구하지 말고 verifier를 돌립니다.**

   **F3(조건부 승격)는 G3의 여섯을 모두 갖춘 회차에서만 가능합니다.**

   1. 6번 통과 · 2. **두 결속 축이 모두** 기대 deployment ID를 이름 댐 · 3. 회복 경로
   검사를 **실제로 돌려** 통과 · 4. ①의 원인이 시료 만료임을 **관측으로** 기록 ·
   5. 유예 창의 남은 시간을 기록 · 6. 그 회차에 다른 미판정·불일치 없음.

   > **여섯을 갖춘 것은 승격이 아닙니다.** 승격의 근거는 "미판정이 통과여서"가 아니라
   > **"미판정이 감수 가능한 위험이어서"**이고, 그 **위험 수용 한 줄**(G4)과 **승인자**
   > (G6, `mposition` — 실행자와 승인자가 같다는 사실도 함께 적습니다)가 기록에
   > 있어야 합니다. **판정 모듈이 요건 충족을 반환해도 그것을 대신하지 않습니다.**

   **판정란은 축별로 적습니다**(G5) — 재료 / 이전 refresh / 이전 access / 결속, 그리고
   **G11의 전제**(앱이 401에서 회복하는지는 **미검증**이며 관측처럼 적지 않습니다).
   한 칸에 "통과"만 남으면 다음 사람은 ①이 있었다는 것을 모릅니다.

   **F2(롤백)로 끝내려면 완료 조건이 따로 있고, 그것은 둘입니다** — **롤백 완료**,
   **그리고 그 뒤에** §4의 `-Mode preflight` 재확인 통과. 두 시각을 다 적습니다:
   **재확인이 롤백보다 먼저 돌았다면 그 통과는 롤백된 배포에 대해 아무 말도 하지
   않습니다.** 그 재확인은 **Active의 deployment ID를 기대값으로** 받으므로,
   **E7이 미확인인 동안에는 그 값을 확인할 수 없어 완료를 선언하지 않습니다** —
   G9에서 **예외를 열지 않기로** 했습니다. **롤백 실행과 F2 완료는 다른 상태**이고,
   기록에 그렇게 구분해 적습니다.

   **그래서 이 갈래가 실제로 끝나는 경로는 둘뿐이고 둘 다 조건부입니다** — 결속을 갖춘
   회차의 **F3**, 또는 **E7 실측 뒤의 F2**. 기한은 종료를 강제하지 않고 **초과를 눈에
   보이게** 만듭니다.
...
**30일을 기다리지 못할 이유가 있다면 그것은 사고 대응이고, 이 문서가 아니라 §5입니다.**
pepper를 일찍 지우는 것은 같은 결과를 설명 없이 만드는 일입니다.

> **정정.** 이 자리에 "전체 로그아웃(`revokeAllUserSessions`)"이라고 적혀 있었고
> **틀렸습니다.** 그 함수는 **`userId` 하나**를 받아 그 계정의 웹·모바일 세션을
> 폐기합니다(`lib/sessionSecurity.ts`) — 서비스 전원 로그아웃 함수가 아닙니다. **재료
> 제거와 세션 폐기는 다른 층**이고, 그 세대 전체에 걸친 효과는 **재료 쪽**(링 항목
> 제거, §5.1의 전면 교체)에서 나옵니다.
...
어느 쪽이든 **그 계정들의 세션을 함께 끊을지**를 따로 판단합니다 —
`revokeAllUserSessions(userId)`는 **계정 하나**의 웹·모바일 세션을 폐기하므로, 대상이
여럿이면 **누구를 대상으로 삼을지와 무엇으로 실행할지를 먼저 정해야 합니다.** 지금
저장소에 있는 실행 경로는 계정 단위 admin route뿐입니다.

**그리고 은퇴 줄만 지우지 않습니다.** 링에 남은 항목은 `UNDECLARED`가 되어 §2.1 검사가
거절합니다 — 지울 때는 **링 항목과 은퇴 선언을 같은 배포에서 함께**(§3.1).
...
살아 있던 refresh token은 전부 검증에 실패합니다 — **이것이 그 세대 전체에 걸친
효과이고, 재료 쪽에서 나옵니다.** 세션 정리는 별개 층이며
`revokeAllUserSessions(userId)`는 **계정 하나**의 웹·모바일 세션을 폐기합니다. 여럿을
대상으로 해야 한다면 대상 선정과 실행 경로를 먼저 정합니다.
...
2. **Active·Pending 두 항목 — 모양은 정해졌고, 두 가지가 남았습니다.**
   ...
   **(a) fingerprint 계산 규칙 — 2026-09-08 승인됐습니다(1차 11건).** 승인자
   ...
   `tv-mobile-keyring-fp-1/<K 세대>`입니다. **pepper를 salt 없이 해싱하지 않는 이유**가
   그 keyed 구성입니다 — pepper는 운영자가 고른 값이라 엔트로피가 보장되지 않고, 평범한
   digest는 비밀 아닌 절반에 **오프라인 대조기**를 두는 일이 됩니다.

   **template의 두 칸은 아직 자리표시자입니다.** 실제 값은 §6의 2번 (b)에서 항목을 처음
   만들 때 들어가고, 검사기는 그때까지 template에 한해 자리표시자를 통과시킵니다.

   **근거 문서는 `.github/audits/2026-09-08-mobile-auth-fingerprint-rule-approval.md`**
   이고, 후보 셋(A·B·C), 확정된 명세와 고정 합성 벡터, 세 절차와의 충돌 점검, 규칙 변경
   시 이행, 승인 대상 열일곱, 합성 테스트 벡터 스물넷을 담습니다. **1차 11건은 승인됐고
   2차·3차는 미승인입니다**(그 문서 9.4절).

   **승인은 세 묶음이고 1차만 끝났습니다** — **1차 계산 규칙 11건 승인(2026-09-08)**,
   **2차 운영 절차 5건 미승인**(D9는 첫 긴급 교체 전에, D6·D6a는 첫 노출 판단 전에
   필요합니다), **3차 1건**(다중 계정 세션 폐기)은 fingerprint와 독립한 문제로 분리했고
   미승인입니다. 승인란은 그 문서의 9.4절입니다.

   **그 초안이 찾아낸 것 하나는 이 절 밖입니다**: `K` 세대가 다른 긴급 교체
   (§5.1)에서는 지금의 쌍 검사가 "판정 불가"로 거절합니다. 계약을 그대로 두고 절차로
   답하는 것이 권장이며(초안 D9), 검사기를 바꾸는 쪽을 고른다면 **별도 승인**입니다.
   ...
   **구조 검사는 배포 결속이 아닙니다.** 모든 field를 사람이 손으로 적으므로 —
   ...
   5번은 이것으로 해결되지 않습니다.
3. **상시 점검 — 방식이 결재됐고, 두 값이 남았습니다**(2026-09-10, `mposition`).
   근거와 조건은
   `.github/audits/2026-09-10-mobile-auth-keyring-standing-check-approval.md`
   (S1~S8)에 있습니다.

   **저장소에 들어간 것**: 앱 안에서 살아 있는 링을 읽는 내부 endpoint
   (`POST /api/internal/mobile-auth/keyring-health`, bearer 인증, 읽기 전용),
   CLI와 공유하는 조립(`lib/mobileAuthKeyringHealth.ts`), `ScheduledJobRun` 기록,
   구조화 로그 `mobile_auth_keyring_health`. 응답·로그·행에는 **key id와 코드만**
   들어가고 재료는 들어가지 않습니다.

   **아직 돌지 않습니다.** cron 서비스도 스케줄러 등록도 없고, job key는
   `PENDING_SCHEDULED_JOB_KEYS`에 있어 **행은 쌓이되 지연 판정은 꺼져 있습니다.**
   켜려면 둘이 필요합니다 — **S2의 정확한 UTC 실행 시각**과 **S6의 선결 조건**
   (job별 마지막 실행을 확보하는 공용 조회 보완 + 별도 회귀 검증). `undeclared`의
   등급(S7)과 보관 수단 확인(U3)도 미결입니다.

   아래는 그 결재 전의 상태 설명이며, 도구 자체는 그대로입니다.
   ...
   **결재된 값**(S1~S8, 2026-09-10): 앱 내부 endpoint + Railway cron · 하루 1회 ·
   `mposition` 단독이며 자동 발송 없음 · 보관 요구 90일
   (`MOBILE_AUTH_EVENT_RETENTION_DAYS`와 **나란히 놓은** 값이며 유도된 값이 아닙니다) ·
   실패 처리 네 갈래 · 26시간 침묵 예산 · 구조화 로그. 이 script 자체는 그대로
   **스케줄러에 자기를 등록하지 않고, 아무것도 보내지 않고, 키를 지우거나 회전시키지
   않습니다** — 그 일을 하는 것은 endpoint이고, 그것도 읽기만 합니다.

   > **왜 CLI가 아니라 endpoint인가**(S1). 점검이 보려는 것은 *지금 살아 있는* 링인데,
   > 이 script는 `--import tsx`로 돌고 `.ts`를 import 하며 **배포되는 script들은 전부
   > 순수 `.mjs`입니다**(§2의 rev.5 정정). 저장소가 있는 곳에서 값을 손으로 넣어 돌리면
   > 검증 대상이 *설정하려는 값*이어서 §2.1과 같아지고, 링을 CI secret으로 복사하면
   > **production 재료의 두 번째 사본**이 생깁니다. 그래서 판정은 링이 이미 있는 앱
   > 프로세스 안에서 돌고, cron 컨테이너는 URL과 bearer secret만 듭니다.
   >
   > **아직 남은 값 둘**: S2의 정확한 UTC 실행 시각, S6의 공용 조회 보완. 그 둘 전에는
   > job key가 `PENDING_SCHEDULED_JOB_KEYS`에 있어 **행은 쌓이고 지연 판정은 꺼져
   > 있습니다.**
4. **실행 기록 — 체계는 있습니다.**
...
5. **증거를 배포에 결속하는 방법 — 확정 범위가 승인됐고, 두 항목이 남았습니다.**
   ...
   (verifier가 배포를 직접 호출). **"직접 호출했다"는 사실만으로는 결속이 아닙니다**:
   `/api/build-info`를 따로 읽고 exchange를 따로 하면 두 요청이 다른 인스턴스에 갈 수
   있고, 그것이 정확히 막으려던 상태입니다. 결속은 **증거를 나른 그 응답 안에서** 또는
   **증거 자체 안에서** 만들어집니다.

   **위협 모델은 위조가 아니라 혼동입니다** — 이전 배포의 신선한 증거, 동시 서비스 중인
   구버전 인스턴스, 재사용, 롤백 넷. 배포를 장악한 자는 어느 안에서든 자기 식별자를
   속일 수 있고, 그것을 배제한다고 적지 않습니다.

   **결속과 신선함은 서로를 대체하지 않고, 둘을 합쳐도 재사용을 막지 못합니다.**
   deployment ID는 배포 수명 내내 같으므로 **같은 배포의 낡은 증거는 결속 축을 그대로
   통과**하고 그것은 `iat`가 막지만, **신선한 증거를 두 번 넣는 것은 지금 어느 축도
   막지 않습니다** — verifier가 상태를 갖지 않기 때문입니다. 일회성을 요구할지 배제
   범위를 좁힐지가 결정 사항입니다.

   **미확인 사실 하나**: 롤백이 새 deployment ID를 만드는지 재사용하는지. 공식 문서는
...
6. **서명 쪽이 미판정일 때의 종료 조건 — 2026-09-10에 결재됐습니다**(§3의 8번).
   **아직 구현하지 않았습니다** — 아래 결재 결과를 절차·기록에 반영하는 것이 다음
   일입니다.
   지금 계약은 "미판정은 통과가 아니므로 승격하지 않는다"이고, 그러면 배포는 나갔는데
   Active가 옮겨가지 않은 상태로 사람에게 올라갑니다.

   `.github/audits/2026-09-09-mobile-auth-undetermined-signing-half-approval.md`가 **F1 보류 유지 · F2 롤백 · F3 조건부 승격**을 나란히 놓습니다.

   - **"모른다"가 넷이고 답이 다릅니다** — ① 이전 서명 세대가 아직 받아들여지는가(시료
     만료), ② **판정 가능한 증거로 확인된** 활성 재료 불일치(= 배포 결함, 롤백),
     ②' 6번의 증거 자체를 판정할 수 없음(= **재수집**, 롤백 아님), ③ 이 증거가 이
     배포에서 나왔는가(§6의 5번). **이 항목은 ①뿐입니다.**
   - **①은 기다려도 판정되지 않습니다** — 이전 키로 새 access token을 만들 수 없기
     때문입니다. 그래서 **F1은 대기가 아니라 정지**이고, 그 상태는 다음 회전의
     preflight를 드리프트로 막습니다.
   - **비대칭이 핵심이되 크기는 조건부입니다** — 미판정의 대가는 불확실하고, 최악이라도
     그 세대 access token이 남은 수명 동안 `unknown_kid`로 거절되는 것입니다. **서버가
     refresh를 받아 준다는 것은 확인할 수 있지만, 앱이 그 401에서 스스로 회복하는지는
     아직 확인되지 않았습니다** — `authenticatedFetch`의 계약은 조건부이고(같은 token이면
     재시도 없음, 다른 쓸 수 있는 token이면 재시도), **어느 쪽이 일어나는지를 정하는
     bridge의 갱신 정책이 아직 없습니다**(native 미구현). 롤백의 대가는 확실하되
     **회전 대상에 따라 다릅니다**: 서명만 회전했으면 access는 `unknown_kid`이고 refresh는
     `rotate`, pepper까지 회전했으면 refresh가 `secret_mismatch`가 되어 **재로그인**입니다.
   - **F3는 미판정을 통과로 바꾸지 않습니다.** 승격의 근거는 "미판정이 통과여서"가
     아니라 **"미판정이 감수 가능한 위험이어서"**이고, 그 위험 수용과 축별 판정이 기록에
     남아야 합니다.
   - **유예 창(900초)이 닫히면 질문은 사라집니다** — 그것은 **해당 없음**이지 통과가
     아닙니다.

   - **보류(F1) 중에는 무조건 산문 셋이 틀린 복구를 지시합니다.** 보류 상태는 정의상
     `Active ≠ Railway`이고, **§2.2의 계약 3번** · `.github/RELEASE_CHECKLIST.md`의
     **preflight 항목**과 **vault 일치 항목**은 실패의 종류를 가르지 않은 채
     **"Active를 기준으로 복구하라"**고 말합니다. 보류 중에 그것을 따르면 **이전 링을
     Pending이 돌고 있는 배포에 복원**하는 일, 즉 **아무도 결정하지 않은 롤백**입니다.
     **보류는 "아무것도 하지 않는 상태"가 아닙니다.**

     **다만 verifier는 이 위험의 원인이 아니라 방어입니다.** Active 값으로
     `-Mode preflight`를 실제로 돌리면 기대 deployment ID는 Active의 것이고 증거는
     Pending이 돌고 있는 배포에서 나오므로 **결속 축이 먼저 걸려**
     `Nothing was decided about the deployment` — **복구 금지·재수집**이 나옵니다.
     `Do NOT deploy`에 닿으려면 **Active 항목의 deployment ID가 이미 새 배포를 가리켜야**
     하고, 그것은 보류가 아니라 **반쯤 승격된 store**입니다. §2.2의 안내표와 위 4번도
     출력으로 갈라지므로 이 위험에 해당하지 않습니다.

     **그러므로 보류 중에는 산문만 보고 복구하지 말고 verifier를 돌립니다.** 그리고
     **보류 중임을 Active 항목에 적고**(관측) 보류를 짧게 끝냅니다(통제). 기한 값과
     기산점·초과 시 처리는 미결입니다(G9).

   - **E7이 미결인 동안에는 F2의 완료를 확인할 수 없습니다.** §4가 F2의 종료 조건으로
     적어 둔 `-Mode preflight` 재확인은 **Active의 deployment ID를 기대값으로** 받으므로,
     롤백 뒤 그 칸에 무엇을 적을지가 결과를 가릅니다. **비우면 재료를 보기 전에
     거절**되고(구조 검사도 빈 `deploymentId`를 거절합니다), 나머지는 **롤백이 id를
     새로 발급하는지 재사용하는지에 달려 있습니다** — 새 id면 이전 id를 둔 채로는 재료가
     전부 맞아도 결속 불일치로 실패하고, 재사용이면 이전 id 그대로 통과합니다.

     **E7이 미확인이라는 것은 그 둘 중 무엇인지 모른다는 뜻이지 이전 id가 반드시
     틀렸다는 뜻이 아닙니다.** 그래서 규칙은 하나입니다 — **실제 기대 ID를 확인하지
     않은 채 F2 완료를 선언하지 않습니다.** **롤백 실행과 F2 완료는 다른 상태**이고,
     그 구분을 기록에 적습니다.

   승인 대상 열하나(G1~G11)와 검증 벡터 스물둘은 그 문서에 있고, **승인란은 그
   문서의 9절**입니다.

   **2026-09-10 결재 결과** — G1~G8·G10·G11은 **권장대로**이고, **G9는 기한 24시간 ·
   기산점 ①로 분류된 시각 · 초과 시 (a) 운영자의 처리기한 · 완료 불가 시 §4의 완료
   조건에 예외를 열지 않음**입니다.

   **그래서 F1에 기한은 생기지만 보장된 종점은 생기지 않습니다.** (a)이므로 24시간을
   넘기면 **초과가 기록될 뿐** 보류는 이어지고, 예외를 열지 않으므로 **E7이 미확인인
   동안 F2도 완료되지 않습니다.** 실제 종점은 **결속을 갖춘 회차의 F3** 또는 **E7 실측
   뒤의 F2** 둘뿐입니다 — 확인하지 않은 것을 끝났다고 적지 않기로 한 결과입니다.
   **G10**(결속 미확정 상태에서 F3 허용 여부)이 이 항목을 §6의 5번에 묶습니다 —
   그쪽은 승인·구현됐고 **합성 검증까지**이므로, F3의 필수 증거 2번은 **실제 배포가
   `dep`을 실은 증거를 만든 뒤부터 생길 수 있습니다.** 다만 그것은 **시점이지 사건이
   아닙니다** — **회차마다 두 결속 축이 모두** 기대값을 이름 대야 하고(`dep`이 맞아도
   행의 `mintedByDeploymentId`가 비면 verifier는 실패합니다), E5의 회차 조건과 G3의
   나머지도 그 회차에서 충족돼야 합니다.
