...
| 사례 | 무엇을 고정하나 |
|---|---|
| 1 | 두 링 어느 쪽도 **parameter가 아닙니다.** parameter 값은 명령줄에 있고, 명령줄이 저장되는 것입니다 |
| 2a–2d | 프롬프트가 두 번 뜨고, `-RequireConfigured`가 검사기까지 가고, 검사기의 **종료 코드가 그대로 반환**됩니다 |
| 3a–3c | 성공·실패·**중단** 뒤 `MOBILE_AUTH_*` 여덟 개가 전부 사라집니다 |
| 4a–4b | 두 비밀이 출력에 **없고**, 길이는 있습니다 |
...
- **10 사례 전부 통과했습니다** — PowerShell 7.4.6(linux-x64)을 작업 컨테이너에 받아
  실행했습니다. rev.7까지 "`pwsh`가 없어 실행하지 못했다"고 적은 것은 **받아 보지
  않았기 때문**이고, 없다는 말이 맞지 않았습니다.
- **테스트가 실제로 무언가를 잡는지도 확인했습니다.** wrapper를 네 가지로 일부러
  망가뜨렸고 전부 걸렸습니다 — `finally` 제거(3a·3b·3c 실패), 길이 대신 값 출력
  (4a·4b 실패), 무조건 `exit 0`(2d 실패), 링을 parameter로 추가(1 실패). 그 뒤 트리를
  되돌렸습니다.
- **운영자의 셸에서 돌려 본 것은 아닙니다.** 여기서 쓴 것은 Linux의 PowerShell 7.4.6이고
  운영자 환경은 Windows PowerShell입니다. 두 script는 5.1에도 있는 기능만 쓰지만
  (`$PSNativeCommandUseErrorActionPreference`는 있을 때만 건드립니다) **거기서 돌려 본
  증거는 아직 없습니다.** 릴리스 체크리스트가 release SHA에 묶인 실행 기록을 요구하는
  이유가 이것입니다.
...
없으므로(§6), 지금 관측 가능한 것은 하나뿐입니다.

| 대조하고 싶은 것 | 지금 관측 가능한가 |
|---|---|
| 활성 **서명** 키 id | **가능.** 발급된 access token의 JWS 헤더 `kid`가 그 id입니다. 그 환경에서 exchange를 한 번 성공시키면 읽힙니다 |
| 서명 링의 나머지 항목(은퇴 포함) | **불가.** 어떤 키가 링에 있을 뿐인지를 알리는 것이 없습니다 |
| 활성 **pepper** id, pepper 링 | **불가.** `MobileRefreshRotation`을 포함해 어떤 행도 digest를 계산한 pepper의 id를 남기지 않습니다 |

그러므로 (c)를 지금 채택하면 **대조 범위는 "활성 서명 키 id 하나"입니다.** 그 이상을
원하면 §6의 "살아 있는 설정 감사"가 먼저 필요하고, 그것은 별개의 작업입니다. **(c)를
전면 대조로 적어 두고 실제로는 한 필드만 보는 것이 가장 나쁜 결과입니다** — 대조했다는
기록이 대조되지 않은 값까지 덮습니다.
...
- **§2.2의 (a) 또는 (b)** — 이것은 활성화의 **선결 조건**입니다. 나머지와 달리
  ...
  위에 얹는 선택이고, 채택한다면 무엇을 관측할지도 함께 정합니다.
