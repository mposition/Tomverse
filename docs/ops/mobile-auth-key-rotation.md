...
- **은퇴 시각이 미래이면 그 키는 아무것도 검증하지 않습니다.** `sign-1@2099-01-01…`은
  ...
  **허용 오차는 없습니다.** 그러므로 **은퇴 시각은 "지금"이 아니라 몇 분 전으로
  ...
  >
  > 그래서 §2.1 검사가 **남은 시간을 초 단위로 출력**하고, 절반 아래로 떨어지면
  > `NOTE`로 알립니다. 실패가 아닌 이유는 그것이 판단이기 때문입니다 — 그 세대의
  > 자격증명을 아직 누가 들고 있는지는 검사가 알 수 없습니다. **부족하면 은퇴 시각을
  > 다시 적고 검사를 다시 돌립니다**(§3의 5번).
...
**증거 자체를 판정할 수 없으면 모드와 무관하게 "보류"입니다.** token이 파싱되지 않거나
만료됐거나 너무 오래됐으면 그것은 **배포에 대한 판정이 아니라 증거에 대한 판정**이므로,
안내는 "아무것도 승격·롤백·복구·비활성화하지 말고 증거를 다시 모으라"입니다. 아래 표는
**재료가 어긋났을 때** 적용됩니다.
