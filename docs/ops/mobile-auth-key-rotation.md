...
6. **서명 쪽이 미판정일 때의 종료 조건 — 비교안이 있고, 미승인입니다**(§3의 8번).
   ...
   - **보류(F1) 중에는 이 문서의 복구 지시가 틀립니다.** 보류 상태는 정의상
     `Active ≠ Railway`이고, 그 조건에서 §2.2의 계약 3번과 같은 절의 preflight 안내표 ·
     §3의 4번 · `.github/RELEASE_CHECKLIST.md`의 두 항목 ·
     `scripts/verify-mobile-auth-deployment.mjs`의 preflight remedy가 **여섯 곳에서** 모두
     **"Active를 기준으로 Railway를 복구하라"**고 말합니다. 보류 중에 그것을 따르면
     **이전 링을 Pending이 돌고 있는 배포에 복원**하는 일, 즉 **아무도 결정하지 않은
     롤백**입니다. **보류는 "아무것도 하지 않는 상태"가 아닙니다** — 무관한 점검으로 그
     지시에 닿는 것으로 충분하므로, **보류 중임을 Active 항목에 적고** 보류를 짧게
     끝냅니다. 상한 값은 미결(G9)입니다.

   승인 대상 열하나(G1~G11)와 검증 벡터 열여덟은 그 문서에 있고, **승인란은 비어
