...
6a. **세 시료로 한 회차를 판정합니다**(E5, 2026-09-09 승인). 6번을 **`iat` 기준 2분
   ...
   - **판정은 세 시료를 다 모은 뒤 한 번에** 하고, 그 시점에 셋이 **모두 유효**해야
     ...
     값은 verifier의 900초가 아니라 **access token의 600초**입니다(만료 검사에 60초
     skew가 붙으므로 실제 경계는 `iat + 660초`이고, 그래도 900초보다 먼저입니다).
...
7. **이전 세대를 확인합니다**(유예 안): 4.5번에서 받아 둔 access token이 아직
   ...
   **두 경계를 실제 함수로 확인했습니다**(`iat` = 은퇴 시각인 시료,
   `tests/mobileAuthRollbackMaterial.test.mjs`).
   ...
   **그래서 4.5번의 시료는 아무도 쓰고 있지 않은 통제된 세션에서 받습니다.** 앱이 계속
   ...
   | access | **거절 시각의 시료 유효성·키 유예를 기록하지 않음** | 거절 | **미판정.** 위 셋 중 무엇인지 모릅니다 — **롤백하지 않고** 그 값부터 확인합니다 |
...
8. **6·7이 통과하면 Pending을 Active로 승격**하고 Pending을 비웁니다. 승격은 Pending
   ...
   - **재료 불일치**(`Do NOT promote Pending to Active`), 또는 **7번의 확인된 실패**
     ...
     > **"확인된 실패"는 7번 표의 한 줄뿐입니다** — 거절된 그 시각에 **시료가 유효
     > 했고**(`exp + 60초` 이전) **이전 서명 키의 유예도 남아 있던** 경우. **유예가
     > 끝난 뒤의 거절은 실패가 아니라 계약**이고 8.1의 ①로 갑니다. 만료된 시료의
     > 거절, 그리고 **거절 시점의 두 값을 기록하지 않은 거절**은 `미판정`이며 여기에
     > 해당하지 않습니다. 판정 규칙은
     > `scripts/mobile-auth-undetermined-signing-core.mjs`가 계산합니다 — 기록이
     > 없으면 그 모듈도 **분류를 거부**합니다.
