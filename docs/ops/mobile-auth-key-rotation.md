...
합니다. **배포하려는 변수 값을 그 창의 환경변수로 넣고** 실행합니다. production
자격증명이 필요한 것이 아니라, **배포할 값 자체**가 필요합니다. 읽기 전용이고
아무것도 바꾸지 않습니다.
...
출력하고, 마지막 것이 하나라도 있으면 실패합니다. 오타 난 은퇴 id도 실패로
보고합니다. 출력에 비밀값은 없으므로 결과는 그대로 붙여도 안전합니다.

변수는 **한 배포에서 함께** 바꿉니다. Railway는 변수 변경을 한 배포로 적용하므로,
아래 절차의 각 단계를 나눠 배포하지 않으면 중간 상태가 존재하지 않습니다.
