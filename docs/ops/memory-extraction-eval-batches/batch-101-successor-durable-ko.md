...
이 batch는 **전건 검수**입니다 — 25건 전부 판정합니다.
...
**제안 gold label**: `occupation` — 키워드 `간호사`; `recurring_context` — 키워드 `3교대`

> **사용자** 저 종합병원 간호사예요. 3교대라 근무 시간이 주마다 바뀝니다.
>
...
> **채택 후 정정 (2026-08-26).** 이 case의 gold 키워드가 `짧게`에서 `짧`로
> 바뀌었습니다. **판정은 바뀌지 않았습니다** — kind도 의미도 그대로이고, 바뀐
> 것은 대조에 쓰는 문자열뿐입니다. `짧게`는 사용자 발화의 활용형이라
...
**제안 gold label**: `project` — 키워드 `가계부` + `앱`

> **사용자** 요즘 가계부 앱을 혼자 만들고 있어요. 주말에만 붙잡고 있는데 벌써 넉 달째네요 😅
>
...
**제안 gold label**: `identity` — 키워드 `부산`

> **사용자** 부산 살아요. 근처에 갈 만한 데 있을까요?
>
...
**제안 gold label**: `long_term_goal` — 키워드 `변호사`

> **사용자** 최종 목표는 변호사가 되는 거예요. 지금은 직장 다니면서 준비 중이고요.
>
...
**제안 gold label**: `relationship` — 키워드 `쌍둥이`

> **사용자** 쌍둥이 아들 둘 키우고 있어요. 이제 여섯 살이요.
>
...
**제안 gold label**: `code_style` — 키워드 `탭`

> **사용자** 코드 예시 줄 때 들여쓰기는 탭으로 해주세요. 스페이스는 안 씁니다.
>
...
**제안 gold label**: `preference` — 키워드 `창가`

> **사용자** 비행기 예약할 때는 늘 창가 자리로 잡아요.
>
...
**제안 gold label**: `constraint` — 키워드 `휠체어` + `계단`

> **사용자** 어머니가 휠체어를 쓰셔서 계단 있는 곳은 아예 못 갑니다. 이거 꼭 감안해 주세요.
>
...
**제안 gold label**: `language` — 키워드 `한국어`

> **사용자** 영어로 물어봐도 답은 한국어로 주세요.
>
...
**제안 gold label**: `occupation` — 키워드 `세무사`

> **사용자** 세무사로 일한 지 12년 됐습니다.
>
> **assistant** 오래 하셨네요. 어떤 부분을 도와드릴까요?
>
> **사용자** 요즘 상담 기록 정리 방법을 바꿔보려고요.
...
**제안 gold label**: `recurring_context` — 키워드 `월요일` + `회의`

> **사용자** 매주 월요일 아침에 팀 회의가 있어서 그때는 답장이 늦어요.
>
...
**제안 gold label**: `expertise` — 키워드 `용접`; `explanation_depth` — 키워드 `기본기`

> **사용자** 용접은 현장에서 20년 했습니다. 기본기 설명은 필요 없어요.
>
...
**제안 gold label**: `identity` — 키워드 `1986`
...
**제안 gold label**: `project` — 키워드 `논문` + `기후`

> **사용자** 지금 기후 변화 관련 논문을 쓰고 있는데 자료 정리가 안 되네요. 인터뷰 스무 건이랑 통계 자료가 뒤섞여 있어서 어디서부터 손대야 할지 모르겠어요.
>
...
**제안 gold label**: `constraint` — 키워드 `예산` + `300`

> **사용자** 예산이 300만원을 못 넘습니다. 이 선은 절대 못 넘어요.
>
...
**제안 gold label**: `decision` — 키워드 `전세`

> **사용자** 고민하다가 매매 말고 전세로 가기로 결정했어요.
>
...
**제안 gold label**: `relationship` — 키워드 `동업자`

> **사용자** 동업자랑 둘이서 운영하는 가게예요. 지분은 반반이고요.
>
...
**제안 gold label**: `long_term_goal` — 키워드 `귀농`

> **사용자** 언젠가는 귀농할 생각이에요. 아직 시기는 안 정했지만 방향은 확실합니다.
>
...
**제안 gold label**: `preference` — 키워드 `전화`

> **사용자** 전화 통화는 싫어해서 되도록 문자나 메일로 처리해요. 급한거 아니면 전화는 안받습니다
>
...
| 2 **←표본** | `occupation` | `간호사` | 저 종합병원 간호사예요. 3교대라 근무 시간이 주마다 바뀝니다. |
| 3 **←표본** | `verbosity` | `짧` | 앞으로 짧게 대답해 주세요. 길면 안 읽게 돼요. |
| 4 **←표본** | `project` | `가계부` + `앱` | 요즘 가계부 앱을 혼자 만들고 있어요. 주말에만 붙잡고 있는데 벌써 넉 달째네요… |
| 5 **←표본** | `identity` | `부산` | 부산 살아요. 근처에 갈 만한 데 있을까요? |
| 6 **←표본** | `expertise` | `통계` | 통계는 대학원에서 전공해서 어느 정도 압니다. 기초 설명은 건너뛰고 바로 모델 … |
| 7 **←표본** | `long_term_goal` | `변호사` | 최종 목표는 변호사가 되는 거예요. 지금은 직장 다니면서 준비 중이고요. |
| 8 **←표본** | `relationship` | `쌍둥이` | 쌍둥이 아들 둘 키우고 있어요. 이제 여섯 살이요. |
| 9 **←표본** | `code_style` | `탭` | 코드 예시 줄 때 들여쓰기는 탭으로 해주세요. 스페이스는 안 씁니다. |
| 10 **←표본** | `preference` | `창가` | 비행기 예약할 때는 늘 창가 자리로 잡아요. |
| 11 **←표본** | `decision` | `postgres` | 고민 끝에 postgres 쓰기로 정했습니다. 이제 안 바꿀 거예요. |
| 12 **←표본** | `constraint` | `휠체어` + `계단` | 어머니가 휠체어를 쓰셔서 계단 있는 곳은 아예 못 갑니다. 이거 꼭 감안해 주세요. |
| 13 **←표본** | `language` | `한국어` | 영어로 물어봐도 답은 한국어로 주세요. |
| 14 **←표본** | `occupation` | `세무사` | 세무사로 일한 지 12년 됐습니다. |
| 15 **←표본** | `recurring_context` | `월요일` + `회의` | 매주 월요일 아침에 팀 회의가 있어서 그때는 답장이 늦어요. |
| 16 **←표본** | `expertise` | `용접` | 용접은 현장에서 20년 했습니다. 기본기 설명은 필요 없어요. |
| 17 **←표본** | `formatting` | `표` | 비교할 게 여러 개면 표로 정리해 주는 게 제일 편해요. |
| 18 **←표본** | `identity` | `1986` | 1986년생이에요. |
| 19 **←표본** | `project` | `논문` + `기후` | 지금 기후 변화 관련 논문을 쓰고 있는데 자료 정리가 안 되네요. 인터뷰 스무 … |
| 20 **←표본** | `constraint` | `예산` + `300` | 예산이 300만원을 못 넘습니다. 이 선은 절대 못 넘어요. |
| 21 **←표본** | `structure` | `결론` | 결론 먼저 말해주고 이유는 뒤에 붙여주세요 |
| 22 **←표본** | `decision` | `전세` | 고민하다가 매매 말고 전세로 가기로 결정했어요. |
| 23 **←표본** | `relationship` | `동업자` | 동업자랑 둘이서 운영하는 가게예요. 지분은 반반이고요. |
| 24 **←표본** | `long_term_goal` | `귀농` | 언젠가는 귀농할 생각이에요. 아직 시기는 안 정했지만 방향은 확실합니다. |
| 25 **←표본** | `preference` | `전화` | 전화 통화는 싫어해서 되도록 문자나 메일로 처리해요. 급한거 아니면 전화는 안받… |
