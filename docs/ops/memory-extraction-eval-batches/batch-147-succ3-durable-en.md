# batch-147 — `durable_facts:en` (`mem-eval-succ-3`)
...
`batch-106`의 후속입니다. 케이스를 새로 쓴 것이 아니라 **2건을 뺀 것**이고,
남은 23건은 `batch-106`가 들고 있던 바로 그 객체입니다 —
...
`batch-106`도 그 안에 그대로 있습니다.

| 항목 | 값 |
|---|---|
| 원본 batch | `batch-106` |
| 원본 기록 | `docs/ops/memory-extraction-eval-batches/batch-106-successor-durable-en.md` |
| 원본 digest (작성 시점) | `11966002cbddaa10ec85c564899ebac37f24bd03c5b73a2edc2996a2035e7133` |
| 원본 케이스 수 | 25 |
| 제외 | 2 |
| 남은 케이스 수 | 23 |
...
|---|---|---|---|
| `succ-durable-en-56` | rule-4 | 규칙 4의 재사용 명제 판정 근거 | `succ-durable-en-315` |
| `succ-durable-en-57` | rule-4 | 규칙 4의 재사용 명제 판정 근거 | `succ-durable-en-316` |

## 남은 케이스의 판정

**다시 검수한 것이 아닙니다.** 아래 판정은 전부 `batch-106`의 기록에서
읽어 온 것이고, 이 파일을 만드는 script는 원본 기록에 `채택`이 없는 케이스에
대해서는 아무것도 쓰지 않고 실패합니다.

### succ-durable-en-51

batch-106에서 2026-08-26에 @mposition가 내린 판정입니다. 케이스는 바뀌지 않았습니다.

| 판정 | 사유 (반려일 때만) |
|---|---|
| 채택 | docs/ops/memory-extraction-eval-batches/batch-106-successor-durable-en.md 에서 이관 |
...
케이스를 새로 뽑지 않았으므로 초안 생성자는 `batch-106`의 것을 그대로
