# batch-149 — `durable_facts:ko` (`mem-eval-succ-3`)
...
`batch-108`의 후속입니다. 케이스를 새로 쓴 것이 아니라 **4건을 뺀 것**이고,
남은 21건은 `batch-108`가 들고 있던 바로 그 객체입니다 —
...
`batch-108`도 그 안에 그대로 있습니다.

| 항목 | 값 |
|---|---|
| 원본 batch | `batch-108` |
| 원본 기록 | `docs/ops/memory-extraction-eval-batches/batch-108-successor-durable-ko.md` |
| 원본 digest (작성 시점) | `c9595623da72924a72ec7aaf74de3430696d28d3e2aec16f0a0636b339a8b946` |
| 원본 케이스 수 | 25 |
| 제외 | 4 |
| 남은 케이스 수 | 21 |
...
|---|---|---|---|
| `succ-durable-ko-105` | rule-4 | 규칙 4의 재사용 명제 판정 근거 | `succ-durable-ko-301` |
| `succ-durable-ko-106` | rule-4 | 규칙 4의 재사용 명제 판정 근거 | `succ-durable-ko-302` |
| `succ-durable-ko-107` | rule-4 | 규칙 4의 재사용 명제 판정 근거 | `succ-durable-ko-303` |
| `succ-durable-ko-116` | rule-3 | 규칙 3의 kind 경계 판정 근거 | `succ-durable-ko-304` |

## 남은 케이스의 판정

**다시 검수한 것이 아닙니다.** 아래 판정은 전부 `batch-108`의 기록에서
읽어 온 것이고, 이 파일을 만드는 script는 원본 기록에 `채택`이 없는 케이스에
대해서는 아무것도 쓰지 않고 실패합니다.

### succ-durable-ko-101

batch-108에서 2026-08-26에 @mposition가 내린 판정입니다. 케이스는 바뀌지 않았습니다.

| 판정 | 사유 (반려일 때만) |
|---|---|
| 채택 | docs/ops/memory-extraction-eval-batches/batch-108-successor-durable-ko.md 에서 이관 |
...
케이스를 새로 뽑지 않았으므로 초안 생성자는 `batch-108`의 것을 그대로
