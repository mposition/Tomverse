# batch-103 — `durable_facts:ko` 검수 시트

> **자동 생성 파일입니다.** `npm run make:memory-eval-review-sheet -- --batch=batch-103`
...
| token | shape | 쌍 |
|---|---|---|
| 0.50 | 0.50 | cand-durable-ko2-18 ~ succ-durable-ko-43 |
...
| 0.30 | 0.37 | cand-durable-ko4-18 ~ succ-durable-ko-39 |
...
**제안 gold label**: `occupation` — 키워드 `제빵`; `recurring_context` — 키워드 `새벽 세 시`
...
| 18 | `long_term_goal` | `박사` | 최종적으로는 박사까지 갈 생각입니다. 오래전부터 정해 둔 목표예요. |
...
| 25 | `constraint` | `야간` + `운전` | 야간 운전은 못 합니다. 밤에 시야가 너무 흐려서요. 일정 짤 때 꼭 빼주세요. |
