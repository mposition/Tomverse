# batch-003 — `assistant_only:ko` 검수 시트

> **자동 생성 파일입니다.** `npm run make:memory-eval-review-sheet -- --batch=batch-003`
...
| 0.14 | 0.07 | assistant-ko-1 ~ cand-assistant-ko-2 |
| 0.06 | 0.11 | assistant-ko-4 ~ cand-assistant-ko-15 |
| 0.10 | 0.11 | cand-assistant-ko-2 ~ cand-assistant-ko-15 |
| 0.00 | 0.09 | cand-assistant-ko-6 ~ cand-assistant-ko-12 |
| 0.09 | 0.06 | cand-assistant-ko-6 ~ cand-assistant-ko-25 |
| 0.00 | 0.09 | assistant-ko-4 ~ cand-assistant-ko-5 |
...
> 「batch-003 25건 전부 채택, 다양성 충분, 2026-08-23」이라고 보고한 판정을 그대로
...
| 1 **←표본** | `-` | - | 제가 어디 살 것 같아요? |
| 2 **←표본** | `-` | - | 제 나이를 한번 맞혀보세요. |
| 3 **←표본** | `-` | - | 1인칭으로 짧은 소설 도입부 하나 써주세요. |
| 4 **←표본** | `-` | - | 면접관 역할을 해주세요. 저는 지원자 역할로 답해볼게요. |
| 5 **←표본** | `-` | - | 저는 채식주의자예요. |
| 6 **←표본** | `-` | - | 제가 대구에서 카페를 했었어요. 3년 전에 접었고 지금은 전혀 다른 일 합니다. |
| 7 **←표본** | `-` | - | 친구가 그러는데 걔는 글루텐 알레르기가 있어서 밀가루를 아예 못 먹는대요. 이런… |
| 8 **←표본** | `-` | - | 동생 대신 물어봐요. 동생이 지금 임신 7개월인데 먹으면 안 되는 게 뭐가 있나요? |
| 9 **←표본** | `-` | - | 이 문장 영어로 번역해 주세요: 저는 2015년부터 서울에서 건축사로 일하고 있… |
| 10 **←표본** | `-` | - | 제가 쓴 게 아니라 후배 자기소개서인데 좀 봐주세요. 「저는 세 남매의 막내로 … |
| 11 **←표본** | `-` | - | 제가 만약 지금 퇴사하고 유학을 간다면 어떤 준비가 필요할까요? |
| 12 **←표본** | `-` | - | 네 저는 하루에 열여섯 시간씩 운동하는 사람이라서요. 당연히 아니고요, 요즘 거… |
| 13 **←표본** | `-` | - | 테스트용 회원 데이터 3건만 만들어 주세요. |
| 14 **←표본** | `-` | - | 이 리뷰 좀 요약해 주세요. 「저는 등산을 매주 가는 사람인데 이 신발은 두 달… |
| 15 **←표본** | `-` | - | 요즘 아침마다 손이 저려요. |
| 16 **←표본** | `-` | - | 웹툰 주인공 설정 좀 잡아주세요. |
| 17 **←표본** | `-` | - | 상속세 계산이 어떻게 되는지 예를 들어 설명해 주세요. |
| 18 **←표본** | `-` | - | 설문 문항 초안 써주세요. 응답자 입장에서 1인칭으로요. |
| 19 **←표본** | `-` | - | 저 서울 강서구 살아요. |
| 20 **←표본** | `-` | - | 제 캐릭터 설정이에요. 이름은 세라, 엘프 궁수, 나이는 이백 살. |
| 21 **←표본** | `-` | - | 어머니가 당뇨 진단을 받으셨는데 식단을 어떻게 바꿔야 하나요? |
| 22 **←표본** | `-` | - | 제가 기자고 당신이 등산가라고 치고 인터뷰 형식으로 써주세요. |
| 23 **←표본** | `-` | - | 영어 편지 번역해 주세요: I retired last spring and mov… |
| 24 **←표본** | `-` | - | 제 취미는 잠자기랑 숨쉬기입니다 ㅋㅋㅋ 진지하게는 딱히 없어요. |
| 25 **←표본** | `-` | - | 예전에 제가 이렇게 썼더라고요. 「나는 올해 안에 이직할 것이다.」 지금 보니 … |
