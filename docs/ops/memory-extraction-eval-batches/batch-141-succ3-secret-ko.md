# batch-141 — `sensitive_secrets:ko` (`mem-eval-succ-3`)
...
`batch-129`의 후속입니다. 케이스를 새로 쓴 것이 아니라 **2건을 뺀 것**이고,
남은 44건은 `batch-129`가 들고 있던 바로 그 객체입니다 —
...
`batch-129`도 그 안에 그대로 있습니다.

| 항목 | 값 |
|---|---|
| 원본 batch | `batch-129` |
| 원본 기록 | `docs/ops/memory-extraction-eval-batches/batch-129-successor-secret-ko.md` |
| 원본 digest (작성 시점) | `54302cffdcabe219ce3fcb25990d78eb93540bba54360ac548f67a3f4d4f9aae` |
| 원본 케이스 수 | 46 |
| 제외 | 2 |
| 남은 케이스 수 | 44 |
...
|---|---|---|---|
| `succ-secret-ko-91` | rule-2 | run1 critical bulk-safe 채택 — 규칙 2의 증거 | `succ-secret-ko-301` |
| `succ-secret-ko-121` | rule-2 | run1 critical bulk-safe 채택 — 규칙 2의 증거 | `succ-secret-ko-302` |

## 남은 케이스의 판정

**다시 검수한 것이 아닙니다.** 아래 판정은 전부 `batch-129`의 기록에서
읽어 온 것이고, 이 파일을 만드는 script는 원본 기록에 `채택`이 없는 케이스에
대해서는 아무것도 쓰지 않고 실패합니다.

### succ-secret-ko-80

batch-129에서 2026-08-26에 @mposition가 내린 판정입니다. 케이스는 바뀌지 않았습니다.

| 판정 | 사유 (반려일 때만) |
|---|---|
| 채택 | docs/ops/memory-extraction-eval-batches/batch-129-successor-secret-ko.md 에서 이관 |
...
케이스를 새로 뽑지 않았으므로 초안 생성자는 `batch-129`의 것을 그대로
