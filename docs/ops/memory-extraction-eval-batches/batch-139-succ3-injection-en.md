# batch-139 — `injection_directives:en` (`mem-eval-succ-3`)
...
`batch-118`의 후속입니다. 케이스를 새로 쓴 것이 아니라 **2건을 뺀 것**이고,
남은 27건은 `batch-118`가 들고 있던 바로 그 객체입니다 —
...
`batch-118`도 그 안에 그대로 있습니다.

| 항목 | 값 |
|---|---|
| 원본 batch | `batch-118` |
| 원본 기록 | `docs/ops/memory-extraction-eval-batches/batch-118-successor-injection-en.md` |
| 원본 digest (작성 시점) | `4eeb9d656679678f218814ebbdcd4cd52672038194c2efc9799ee8577a7b9322` |
| 원본 케이스 수 | 29 |
| 제외 | 2 |
| 남은 케이스 수 | 27 |
...
|---|---|---|---|
| `succ-injection-en-23` | rule-1 | run1 critical bulk-safe 채택 — 규칙 1의 증거 | `succ-injection-en-301` |
| `succ-injection-en-26` | rule-1 | run1 critical bulk-safe 채택 — 규칙 1의 증거 | `succ-injection-en-302` |

## 남은 케이스의 판정

**다시 검수한 것이 아닙니다.** 아래 판정은 전부 `batch-118`의 기록에서
읽어 온 것이고, 이 파일을 만드는 script는 원본 기록에 `채택`이 없는 케이스에
대해서는 아무것도 쓰지 않고 실패합니다.

### succ-injection-en-1

batch-118에서 2026-08-26에 @mposition가 내린 판정입니다. 케이스는 바뀌지 않았습니다.

| 판정 | 사유 (반려일 때만) |
|---|---|
| 채택 | docs/ops/memory-extraction-eval-batches/batch-118-successor-injection-en.md 에서 이관 |
...
케이스를 새로 뽑지 않았으므로 초안 생성자는 `batch-118`의 것을 그대로
