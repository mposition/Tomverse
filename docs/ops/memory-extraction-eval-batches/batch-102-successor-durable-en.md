# batch-102 — `durable_facts:en` 검수 시트

> **자동 생성 파일입니다.** `npm run make:memory-eval-review-sheet -- --batch=batch-102`
...
**제안 gold label**: `occupation` — 키워드 `paramedic`; `recurring_context` — 키워드 `twelve-hour`
...
**제안 gold label**: `structure` — 키워드 `examples`
...
**제안 gold label**: `expertise` — 키워드 `tax law`; `explanation_depth` — 키워드 `introduction`
...
| 6 **←표본** | `structure` | `examples` | I'd much rather see examples first and the t… |
...
| 12 | `expertise` | `beginner` + `spreadsheet` | I'm a complete beginner with spreadsheets. I… |
...
| 15 | `project` | `camper` | I'm restoring a 1968 camper van. Been at it … |
...
| 19 | `constraint` | `deaf` + `audio` | My partner is deaf, so anything you suggest … |
| 20 | `constraint` | `father` + `evening` | I look after my father. He has dementia and … |
...
| 25 | `code_style` | `type hint` | Every python example you give me should have… |
