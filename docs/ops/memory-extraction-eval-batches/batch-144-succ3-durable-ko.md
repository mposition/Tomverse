# batch-144 — `durable_facts:ko` (`mem-eval-succ-3`)
...
`batch-103`의 후속입니다. 케이스를 새로 쓴 것이 아니라 **3건을 뺀 것**이고,
남은 22건은 `batch-103`가 들고 있던 바로 그 객체입니다 —
...
`batch-103`도 그 안에 그대로 있습니다.

| 항목 | 값 |
|---|---|
| 원본 batch | `batch-103` |
| 원본 기록 | `docs/ops/memory-extraction-eval-batches/batch-103-successor-durable-ko.md` |
| 원본 digest (작성 시점) | `d5d041a1307b4519d326b94e48f69b2706ca8569a6a7c8eb49bad3851eed1730` |
| 원본 케이스 수 | 25 |
| 제외 | 3 |
| 남은 케이스 수 | 22 |
...
|---|---|---|---|
| `succ-durable-ko-28` | rule-3 | 규칙 3의 kind 경계 판정 근거 | `succ-durable-ko-319` |
| `succ-durable-ko-29` | rule-3 | 규칙 3의 kind 경계 판정 근거 | `succ-durable-ko-320` |
| `succ-durable-ko-47` | rule-4 | 규칙 4·5의 경계 판정 근거 | `succ-durable-ko-321` |

## 남은 케이스의 판정

**다시 검수한 것이 아닙니다.** 아래 판정은 전부 `batch-103`의 기록에서
읽어 온 것이고, 이 파일을 만드는 script는 원본 기록에 `채택`이 없는 케이스에
대해서는 아무것도 쓰지 않고 실패합니다.

### succ-durable-ko-26

batch-103에서 2026-08-26에 @mposition가 내린 판정입니다. 케이스는 바뀌지 않았습니다.

| 판정 | 사유 (반려일 때만) |
|---|---|
| 채택 | docs/ops/memory-extraction-eval-batches/batch-103-successor-durable-ko.md 에서 이관 |
...
케이스를 새로 뽑지 않았으므로 초안 생성자는 `batch-103`의 것을 그대로
