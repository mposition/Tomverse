# batch-151 — `durable_facts:en` (`mem-eval-succ-3`)
...
`batch-110`의 후속입니다. 케이스를 새로 쓴 것이 아니라 **2건을 뺀 것**이고,
남은 23건은 `batch-110`가 들고 있던 바로 그 객체입니다 —
...
`batch-110`도 그 안에 그대로 있습니다.

| 항목 | 값 |
|---|---|
| 원본 batch | `batch-110` |
| 원본 기록 | `docs/ops/memory-extraction-eval-batches/batch-110-successor-durable-en.md` |
| 원본 digest (작성 시점) | `7fe70bb4ceb5d7a7b572fb46d76c99e5c5c29ca2778aa7219a60ba547a154f30` |
| 원본 케이스 수 | 25 |
| 제외 | 2 |
| 남은 케이스 수 | 23 |
...
|---|---|---|---|
| `succ-durable-en-105` | rule-4 | 규칙 4의 재사용 명제 판정 근거 | `succ-durable-en-301` |
| `succ-durable-en-106` | rule-4 | 규칙 4의 재사용 명제 판정 근거 | `succ-durable-en-302` |

## 남은 케이스의 판정

**다시 검수한 것이 아닙니다.** 아래 판정은 전부 `batch-110`의 기록에서
읽어 온 것이고, 이 파일을 만드는 script는 원본 기록에 `채택`이 없는 케이스에
대해서는 아무것도 쓰지 않고 실패합니다.

### succ-durable-en-101

batch-110에서 2026-08-26에 @mposition가 내린 판정입니다. 케이스는 바뀌지 않았습니다.

| 판정 | 사유 (반려일 때만) |
|---|---|
| 채택 | docs/ops/memory-extraction-eval-batches/batch-110-successor-durable-en.md 에서 이관 |
...
케이스를 새로 뽑지 않았으므로 초안 생성자는 `batch-110`의 것을 그대로
