...
**이 절차는 2026-09-08에 양쪽 환경에서 완료됐고 §14의 B-4는 해결입니다**(§7).
문서는 남깁니다 — 한 번 쓰고 버리는 절차가 아니라, 한도를 조정하거나 새 환경을
세울 때 다시 밟는 절차이고, §5.2가 세 번 고쳐진 자리라는 것 자체가 다음 사람에게
필요한 정보이기 때문입니다.
...
### 다섯 조건은 2026-09-08에 모두 충족됐습니다

| 조건 | 무엇이 증명하는가 |
|---|---|
| 1. 양쪽에 값이 설정됨 | staging 1,800/27,000 · production 3,600/54,000 (A안) |
| 2. 양쪽에서 검사기가 `usable` | 두 기록에 출력과 적용된 규칙 첫 줄이 함께 있음 |
| 3. staging flag-on `/api/ready` 둘 다 healthy | staging 기록, traceId 포함 |
| 4. 선택한 값과 근거 | 두 기록의 머리말 |
| 5. 사람의 서명 | @mposition, 두 기록 각각 |

기록 둘:

- `docs/ops/voice-provider-budget-records/2026-09-08-staging-d79d3c8673db4a9f5f4cd109bc99c50c21e182e3.md`
- `docs/ops/voice-provider-budget-records/2026-09-08-production-4b2a0cfd66f63e8907ba35ff410655628608a176.md`

**production 절반은 두 조건이 서로 다른 시점에 충족됐습니다**(§5.2). 값 설정은
릴리스와 무관하게 먼저 가능했고, 검사기는 `main`이 script를 받은 뒤에야 돌 수
있었습니다. 이 순서를 지키지 않았다면 코드와 변수가 한 배포에 함께 들어왔을
것이고, env-first가 막으려는 것이 그 동시 변경입니다.

**릴리스는 저장소의 결정이었지만 환경값 쓰기와 검사 실행은 아니었습니다.**
이전 판이 한때 "저장소가 스스로 끝낼 수 있다"고 적었고 그것이 틀렸습니다 —
실제로 production 환경 쓰기 한 번과 배포된 shell 실행 한 번이 필요했고, 둘 다
저장소 밖의 권한이었습니다.

**값의 증거와 이름의 증거는 서로 다른 곳에서 나옵니다.** 검사기는 숫자를
출력하지 않고(예산이 존재하고 정합한지만 판정합니다), 대시보드는 긴 변수명을
잘라서 보여 줍니다. 그래서 production 기록은 둘을 나눠 적습니다 — 한쪽만으로
1번과 2번을 동시에 만족시켰다고 적으면 그 기록은 틀린 것입니다.

**B-4는 해결입니다. Production Voice 활성화는 여전히 막혀 있습니다** — B-5와
B-6이 각각 별도 차단 사유이며, B-4의 해결이 그중 무엇도 건드리지 않습니다.
