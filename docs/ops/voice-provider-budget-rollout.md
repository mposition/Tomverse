...
거절 응답의 `resetAt`은 UTC 자정까지의 초로 계산되므로 이 경계와 일치합니다.
...
(운영 예산, 공개 범위, 감시 여력)에 달려 있습니다. 다만 **A보다 낮은 값은
권하지 않습니다** — B-6 검증만으로도 10회 남짓의 유료 호출이 필요하고, 예산이
그보다 빠듯하면 검증 자체가 예산에 막힙니다.
