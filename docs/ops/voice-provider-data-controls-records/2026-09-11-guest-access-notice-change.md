...
**Turnstile 부재는 별개로 열려 있었습니다**(docs/policy/voice-input.md §4.2).
cookie를 순환시키는 스크립트가 그날의 provider 예산을 소진시키면 실제 사용자가
거절됩니다 — 청구 위험이 아니라 가용성 위험입니다.

> **추가 (2026-09-11).** `guest_voice` Turnstile이 develop에 병합됐습니다(PR
> #1349). **위 문단을 고치지 않고 여기에 적습니다** — 이 기록이 서술하는 것은
> 고지 문구가 노출되기 시작한 시점의 상태이고, 그 시점에 보호가 없었다는 것은
> 계속 참입니다. 나중에 본문을 손봐 더 나아 보이게 만드는 것이 이 저장소가
> 막는 일입니다.
>
> **그리고 위험이 사라진 것도 아닙니다.** 확인은 자동화를 비싸게 만들 뿐
> 불가능하게 만들지 않으며, 하루의 실제 상한은 여전히 provider 전체 예산입니다.
>
> **§5의 승인 칸은 이것과 무관하게 비어 있습니다.** Turnstile은 고지 문구의
> 승인을 대신하지 않습니다.
