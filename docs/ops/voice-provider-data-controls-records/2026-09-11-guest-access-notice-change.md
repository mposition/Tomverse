...
**승인 대상은 §5.1의 revision `d2ebd52a`와 그 7개 blob hash입니다.** 문장이
아니라 그 파일들의 그 상태에 붙습니다 — 이 저장소가 2026-09-10에 한 번 배운
규칙이고(§11.4가 기록), 그래서 여기도 hash를 먼저 계산해 두고 그 위에
서명합니다.

**이 승인이 덮는 것과 덮지 않는 것.**

- **덮습니다** — 게스트 개방으로 바뀐 `chat.voiceInput`의 마지막 문장, 7개
  locale 전부. §3이 이전·이후를 인용하고 있습니다.
- **덮지 않습니다** — 국외이전 여덟 항목(`voiceInputTransfer1`–`8`). 이 변경이
  건드리지 않았고, 그쪽 승인은 `e8613a55`에 붙은 채로 유효합니다.
- **덮지 않습니다** — §4의 "대조되지 않음" 두 항목. 승인은 문구에 대한 판정이지
  §4가 하지 않았다고 적은 대조를 수행한 것이 아닙니다.

**이 승인 시점에 문구는 이미 사용자에게 노출 중이었습니다**(2026-09-11T00:55:31Z
배포). 승인이 그 사실을 소급해 바꾸지 않으며, §1이 그것을 이 문서의 존재 이유로
적고 있습니다. 다음 문구 변경 때 같은 순서가 반복되지 않도록 하는 것이 이
기록의 쓸모입니다.
