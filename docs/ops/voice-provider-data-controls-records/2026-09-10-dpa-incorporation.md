...
- **기록일(UTC)**: 2026-09-10
- **상태**: **검증·서명 완료 (2026-09-10) — 법인 정식명칭 미기입**
...
**아래는 Privacy Owner가 쓴 것을 그대로 옮긴 것입니다.** 대괄호 부분도 원문
그대로입니다 — 아래 "미기입" 절을 보십시오.

> 본인은 Tomverse를 운영하는 **[호주 법인 정식명칭]**이 OpenAI API의 직접
...
**서명문의 `[호주 법인 정식명칭]`은 대괄호가 채워지지 않은 채로 왔고, 그대로
옮겼습니다.** 지어내지 않습니다 — §5가 "Tomverse 계정의 실제 계약 당사자 명칭"을
**외부에서 증명되지 않는 것**으로 이미 지목했고, 그것이 바로 이 칸입니다.

`적용 법인: Tomverse`는 제품명이며 호주 법인의 등기 명칭이 아닙니다. 계약 당사자를
지목하는 기록에서 그 둘이 같지 않다는 것이 요점입니다.

**이것이 서명을 무효로 만들지는 않습니다** — 권한과 계약 형태에 대한 확인은
그대로 유효합니다. 다만 **"어느 법인이 당사자인가"라는 질문에 이 기록은 아직
답하지 못합니다.** 규제기관이나 고객이 그것을 물으면 이 파일로는 답이 나오지
않습니다.

- **법인 정식명칭**: _(미기입 — 등기 명칭으로 채워야 합니다)_
