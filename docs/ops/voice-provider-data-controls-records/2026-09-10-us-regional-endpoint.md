...
- **기록일(UTC)**: 2026-09-10
- **상태**: **코드 구현 완료 — staging 미관측, 고지 문구 미승인**
...
**locale 문구를 이번 변경에 넣지 않았습니다.** 8개 중 둘이 막혀 있고, 여섯만
적으면 **완성된 법정 고지처럼 보이는 미완성 고지**가 배포됩니다. 브리프도
placeholder를 제품 문구로 commit하지 말라고 적었습니다.

**Blocker 4 — 법인명과 연락처.** 이 저장소는 계약 원문을 읽지 못합니다
(`openai.com/policies/*`가 egress 프록시에서 403, `2026-09-10-dpa-incorporation.md`
§1). OpenAI의 계약 법인은 고객 소재지에 따라 갈리므로 **"OpenAI"라고 적는 것으로
제28조의8이 요구하는 "정확한 명칭"이 충족되지 않습니다.** 계약·DPA에서 확인해야
합니다.

**Blocker 6 — 보유·이용 기간.** endpoint 표의 `None`과 같은 문서의 "최대 30일"
abuse-monitoring 문장이 공급자에 의해 조정되지 않았습니다(§11.3.2-1). 둘 중
하나를 임의로 참으로 고르지 않습니다. **보수적 초안**: "이전받는 자는 전사 목적
달성 후 음성을 보관하지 않는다고 밝히고 있으며, 남용 모니터링 목적의 기록이
일정 기간 보관될 수 있습니다." — **최종 문구는 법률·정책 승인 대상입니다.**

**구조 제안**: 현재 `PrivacyPolicy.tsx`는 섹션당 문자열 하나를 문단으로
렌더링합니다. 여덟 항목을 한 문장에 숨기면 감사할 수 없으므로, **Voice 섹션만
항목 목록 또는 표로 분리**하는 것을 권합니다. 그것은 component 변경이므로 문구
승인과 함께 다뤄야 합니다.
