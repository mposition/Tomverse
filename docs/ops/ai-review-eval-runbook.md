...
- 가격 profile이 없는 모델은 거절합니다 — 모르는 가격으로 강제되는 예산은 예산이
  아닙니다.
- **상한을 낮추는 `correct`는 구조화된 근거를 함께 씁니다(2026-09-03).** 올리는
  방향은 쉽습니다 — ledger가 몰랐던 만큼을 더 잡아 두는 것이고, 틀려도 나중
  호출을 거절할 뿐입니다. 내리는 방향은 **애초에 걸려 있지 않았던 돈**이라는
  주장이고 승인된 예산에 여유를 되돌려 주므로,
  `DOWNWARD_CORRECTION_GROUNDS`의 코드를 문장과 **함께** 적습니다. 산문은
  무엇이든 말할 수 있지만 코드는 승인을 읽는 사람이 셀 수 있습니다.
- **`transport_blocked_before_provider`가 첫 번째 근거입니다.** 2026-09-03에
  승인된 호출이 이 세션의 egress proxy에 막혔습니다 — 요청이 OpenAI에 닿지
  못했고, 생성된 토큰도 청구된 금액도 없는데 **전액 상한으로 정산**됐습니다.
  승인 $0.18 중 $0.0086만 남아, **그 $0.18이 승인한 바로 그 batch를 상한이
  거절하는 상태**가 됐습니다. provider에 닿지 않은 호출을 세는 상한은 지출의
  경계가 아니라 승인에서 새는 구멍입니다.
- **0원 처리는 세 조건이 전부 맞을 때만입니다**(`transportBlockedBeforeProvider()`).
  HTTP 403이고, 본문이 proxy 자신의 문장과 **전체 일치**하며, 그 문장이 지목하는
  host가 **이번 요청의 host**와 같아야 합니다. 부분 문자열 검색이 아니라 전체
  일치인 이유는, 오류 문구를 그렇게 쓰는 것만으로 provider가 환불을 살 수 있게
  두지 않기 위해서입니다. **provider 자신의 403 · 5xx · timeout · 연결 오류는
  전부 전액 상한을 유지합니다** — 넷 다 provider가 받아서 작업한 요청 뒤에 올 수
  있고, 이 도구는 그랬는지 알지 못합니다.
- **정산은 그 예약이 가졌던 적 있는 가장 높은 상한과 대조합니다.** 지금 서 있는
  상한이 아니라 "가졌던 적 있는" 쪽입니다 — 내리는 정정이 자기가 설명하는
  정산을 초과 정산처럼 보이게 만들면 안 되기 때문입니다. 그 정산은 당시 서
  있던 상한에 대해 닫힌 것이고, 총액에 쓰이는 것은 마지막에 서 있는 상한입니다.
  **어떤 상한보다도 큰 정산은 계속 거절합니다.**
- **정정은 정산보다 먼저 씁니다.** 둘 사이에서 프로세스가 죽으면 예약이
  미결로 남고, 미결 예약은 사람이 정리할 때까지 다음 호출을 막습니다. 반대
  순서였다면 상한 0원으로 정산된 호출이 남아 **일어난 무료 호출**처럼 읽힙니다.
...
### 1.3a 심는 자리를 구조로 배정합니다 (v4, v8에서 대조 추가)
...
8. **`gold.accusedLabel`은 선언 일관성 검사입니다(v8). 의미 보증이 아닙니다.**
   초안 작성자가 자기 gold가 지목하는 label을 적고, parser가 그것을 배정과
   비교해 **선언이 없거나 다르면 거절**합니다. v7 pilot 004는 `b`로 배정됐는데
   결함이 `c`에 있었고 아무것도 잡지 못했습니다 — 배정은 기록에, 지목은 한국어
   문장에만 있어 비교할 대상 자체가 없었습니다. 이 field가 사는 것은 **비교할
   대상**뿐입니다.

   **이 검사가 증명하지 못하는 것을 그대로 적습니다.** v8 pilot 004는 `b`로
   배정돼 `b`를 선언하고 **이 검사를 통과했는데, 실제 결함은 표백제 섭취 후
   구토를 유도하라고 한 `c`에 있었습니다.** 그 case의 gold는 **올바른 답변의
   금지 문구**를 인용했고, gold description은 "이 항목의 문구 자체는 올바른
   금지이므로 모순이 아닙니다"라고 적었으며, notes는 "이 사례는 배정 조건을
   만족하지 못하므로 평가 세트에 사용할 수 없습니다"라고 스스로 적었습니다.
   셋 다 산문이고, 이 검사는 label 두 개를 비교합니다.

   따라서 코드·문서·리포트 어디에서도 이 검사를 "결함이 배정된 자리에 갔다는
   증거"로 설명하지 않습니다. **"초안 작성자가 그렇게 말했다"는 증거**입니다.
   어느 답변이 실제로 틀렸는지는 채택하는 사람이 읽습니다.
   `tests/aiReviewEvalDraftPrompt.test.mjs`의 회귀 fixture가 v8 004를 그대로
   담고 **통과함을 단언**합니다 — 바람직한 동작이라서가 아니라, 이 경계가
   나중에 조용히 넓게 읽히지 않도록 못 박아 두기 위해서입니다.

   **대안으로 시도해 본 것과 그 결과도 적어 둡니다.** gold의 `anyOf` 문구가
   실제로 어느 답변에 등장하는지를 세어 배정과 대조하는 방법을 v8 5건에
   돌려 봤습니다. 004에서 그 문구들은 `b`에서 발견됐고 — gold가 인용한 것이
   **올바른 답변의 문구**였기 때문입니다 — 따라서 그 검사는 **배정과 일치한다고
   보고했을 것입니다.** 틀린 case에 대해 "맞다"고 답하는 검사는 없는 것보다
   나쁘므로 넣지 않았습니다.

   이 field는 읽고 나서 버립니다 — 통과한 case는 정의상 배정과 같은 label을
   선언했고, 저장된 gold는 finding kind의 map이어야 하며,
   `draftedBy.targetLabel`이 이미 같은 label을 들고 있습니다.
9. **답변 두 개가 같으면 읽지 않고 거절합니다(v8).** v7 pilot은 **5건 모두** 한
   쌍이 글자 그대로 복사돼 있었습니다. 그러면 심은 답변을 찾는 일이 "복사본이
   아닌 것 하나 고르기"가 되고, 이 세트가 재려는 검토자는 **서로 다른 모델의
   답변**을 비교하므로 그런 상황은 애초에 존재하지 않습니다. 판정은 공백을
   정규화한 뒤의 문자열 일치이며, 줄바꿈 위치만 다른 복사본도 복사본입니다.
10. **근접 복제는 경고이지 실패가 아닙니다(v8).** `report:ai-review-eval-coverage`가
    case 안 답변 쌍의 문자 trigram 겹침(0~1)을 median·max와 함께 찍고
    `NEAR_DUPLICATE_SIMILARITY`(0.6) 이상인 쌍을 이름 대어 나열합니다.
    **자동 실패시키지 않습니다** — 한 안전 질문에 대한 세 답변은 그 질문의 어휘를
    공유하고, 같은 문장으로 시작하는 두 답변이 정작 이 case가 묻는 지점에서는
    다를 수 있습니다. 숫자로 판정할 수 없는 것을 숫자로 판정하는 검사가 이
    toolchain이 반복해 온 실패이므로, 여기서는 **사람이 어디를 읽어야 하는지만**
    알려 줍니다.

현상이 아무것도 심지 않는 넷(`genuine_consensus`·`no_issue`·`verbosity_bias`·
`position_bias`)은 배정이 `null`입니다. 배정이 없으면 지목할 답변도 없으므로
`accusedLabel`도 요구하지 않습니다. `position_bias`가 여기 있는 이유는 하나
...
- **답변은 진짜 답변처럼 씁니다.** 한 문장짜리 스텁 두 개를 비교하는 것은 이
  ...
  씁니다.
- **각 답변은 독립적으로 씁니다(v8).** 답변 사이에 문장을 복사하지 않습니다.
  문구를 공유하는 두 답변은 사실상 한 답변이고, 그렇게 만든 case는 "어느 답변이
  틀렸는가"가 아니라 "어느 답변이 복사본이 아닌가"를 묻습니다. 프롬프트에는 이
  한 줄만 두고, 강제는 §1.3a 9·10번의 **코드 검사**가 합니다 — v5·v7에서
  프롬프트 규칙을 늘려도 새 결함 유형이 계속 나왔고, 검사할 수 있는 것은
  부탁하지 않고 검사합니다.
...
- **200자는 품질 합격선이 아니라 형식 폐기선입니다(v7).** 이름도
  `DRAFT_DISCARD_FLOOR_CHARACTERS`로 바꿨습니다. label 규칙이 label 없는 답변을
  버리듯, 이 숫자는 **답변이 아닌 것**을 버립니다. 넘겼다는 것은 형식이
  멀쩡하다는 뜻일 뿐이고, **채택할 만한지는 답변 속 추론에 대한 판단이며 사람이
  합니다.** 둘을 한 번 섞었더니 숫자가 목표로 읽혔습니다 — v2는 "최소 200자"를
  요청해 162~190자를, v3은 500자를 요청해 215자를 받았습니다. **목표로 제시된
  하한은 목표가 되고, 모델이 겨냥한 목표는 그 아래 착지점이 됩니다.**
  400~600자는 `DRAFT_TARGET_RESPONSE_RANGE`의 **작성 목표**로만 남고, 리포트도
  "이 범위에 든 것이 합격이 아니고 벗어난 것이 불합격이 아니다"라고 적습니다.
- **깊이는 요소를 늘려서가 아니라 요소를 채워서 얻습니다(v7).** 요소를 7~10개로
  늘리면 400~600자에 닿지만 그것은 **골격 반복**이라는 다른 편향을 만듭니다
  (§1.3a). 그래서 다섯 요소는 그대로 두고, 각 요소가 **그 case에만 해당하는
  것** — 실제로 할 행동, 실제로 따라 나오는 근거, 실제로 달라지는 조건 — 을
  담게 합니다. 수치·시간·용량은 **자연스럽고 확신할 수 있을 때만** 쓰고, 빈칸을
  메우려고 지어내지 않습니다. 반복, 앞 요소의 재진술, 질문이 묻지 않은 사실
  추가는 금지입니다.
- **길이는 숫자가 아니라 구조로 요청합니다(v4에서 도입).** 숫자를 올리는 방법은 두 번
...
- **요청하는 길이와 거절하는 길이는 다릅니다(v3에서 도입).** v1 batch의 답변 21개는
  ...
  `DRAFT_DISCARD_FLOOR_CHARACTERS`(200자)** 로 분리하고, 그 사이 간격이 오차를
