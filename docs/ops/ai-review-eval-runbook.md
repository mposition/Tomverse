...
### 1.3a 심는 자리를 구조로 배정합니다 (v3)

`ai-review-eval-draft-v3`의 계약입니다. **프롬프트 문구만 바꾸는 것으로는
...
- **요청하는 길이와 거절하는 길이는 다릅니다(v3).** v1 batch의 답변 21개는
...
- **출력 상한은 batch 크기에 맞춥니다(v3).** 고정 12,000은 두 방향으로 틀렸습니다.
  ...
  `draftingOutputTokenCap(count)` = 500 + 3,000 × count이며, 예약 상한도 그에
