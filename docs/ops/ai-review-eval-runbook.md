...
### 1.3a 심는 자리를 구조로 배정합니다 (v2)

`ai-review-eval-draft-v2`의 계약입니다. **프롬프트 문구만 바꾸는 것으로는
...
- **답변 길이는 하한이 있습니다(v2).** 첫 batch의 답변 21개는 평균 108자,
  최단 81자, 최장 133자였습니다. 이 문서가 "수백~수천 자"를 요구하는 이유는
  분량 취향이 아니라, **두 문장짜리 답변에는 누락이 숨을 곳도 모순이 묻힐 곳도
  없기 때문**입니다. `DRAFT_MIN_RESPONSE_CHARACTERS`(200자)를 밑도는 답변이
  하나라도 있으면 그 case는 거절됩니다. 하한은 분포를 말해 주지 않으므로
  `report:ai-review-eval-coverage`가 min·median·mean·max를 함께 찍습니다.
