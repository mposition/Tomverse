...
8. **gold가 지목하는 답변을 기계가 읽을 수 있는 field로 받아 배정과
   대조합니다(v8).** v7 pilot 004는 `b`로 배정됐는데 실제 결함과 gold는 `c`에
   있었고, **아무것도 그것을 잡지 못했습니다** — 배정은 기록에 있고 지목은 한국어
   문장에만 있어서 둘을 비교할 검사가 존재하지 않았습니다. 7번의 heuristic은
   보고용이고 gold가 문구를 인용하기 시작하면 아무것도 읽지 못합니다. 그래서
   v8부터 초안 작성자가 `gold.accusedLabel`에 label을 적고, **배정과 다르면 그
   case를 거절**합니다. 이 field는 읽고 나서 버립니다 — 통과한 case는 정의상
   배정과 같고, 저장된 gold는 finding kind의 map이어야 하며,
