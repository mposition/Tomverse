...
- **심는 자리를 답변마다 돌립니다.** 2026-09-02 첫 batch 7건은 **7건 모두 `c`**
  가 어긋난 답변이었습니다. 각 case 자체는 흠이 없지만, 그렇게만 채운 세트는
  의도한 것을 재지 못합니다 — **마지막 답변을 무조건 지목하는 검토자가 읽는
  검토자와 같은 recall을 받습니다.** 이것은 `position_bias` 현상 cell과는 다른
  문제입니다. 그쪽은 순서에 속는지를 **측정하려고** 만든 case이고, 이쪽은 다른
  현상을 재는 case에 순서가 **섞여 들어온** 것입니다.
  `report:ai-review-eval-coverage`가 gold 항목이 **처음 지목하는 label**을 세어
  줍니다(heuristic이며 gate가 아닙니다). 쏠림이 보이면 사람이 몇 건 읽고
  판단합니다.
