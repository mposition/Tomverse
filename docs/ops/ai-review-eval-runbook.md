...
### 1.3b 심는 규칙은 현상마다 다릅니다 (v9)

8번(한 지점만 다르다)과 9번(심은 답변은 자기를 믿는다)은 **말해진 결함**을 위해
쓰였습니다. `direct_contradiction`에서 결함은 뒤집힌 문장 하나이고, 9번은 그
문장을 말하는 동안 답변이 자기를 반박하지 않게 붙듭니다.

`omission`이 그것을 그대로 읽으면 **엉뚱한 것을 요구받습니다.** 8번은 결함이
"빠진 행동"인데 "잘못된 행동"을 요구하고, 9번은 정당화할 권고가 없는데 권고를
정당화하라고 합니다.

이 오독은 가정이 아닙니다. 존재하는 유일한 omission 자료 — `development-v0`의
손으로 쓴 4건 — 이 정확히 여기서 어긋납니다. **4건 모두 지목된 답변이 다른
답변들이 가진 것 중 둘을 빠뜨렸습니다.** 요소 하나를 빼면 옆 요소가 딸려 나가기
때문이고, 그중 하나만 적은 gold는 나머지를 찾아낸 검토자를 오답 처리합니다.

그래서 v9부터 심는 규칙을 현상으로 고릅니다(`PLANTING_RULES`). omission의 8·9번은
이 뜻입니다.

> 질문과 제공된 원문이 **필수로 요구하는**, 독립적으로 보고 가능한 정보나 행동
> **한 가지**가 배정된 답변에서만 빠져 있어야 한다. 나머지 필수 요소는 모든
> 답변이 **의미상** 충족해야 한다. 선택적인 설명 차이는 누락으로 세지 않는다.

풀어 쓰면 넷입니다.

1. **필수성은 질문이 정합니다.** "다른 답변들이 마침 언급했다"는 근거가 아닙니다.
   어떤 해석에서도 필수여야 하며, 아니면 질문에서 못 박습니다.
2. **단일성은 보고 단위입니다.** 원칙 하나가 세 행동으로 나타나는 것도, 함께
   딸려 나가는 두 요소도 안 됩니다.
3. **나머지 요소는 의미상 충족이면 됩니다** — 같은 문구일 필요가 없습니다.
   자기 말로 다룬 답변은 다룬 것이고, 아니라고 하는 gold는 **어휘를 채점**합니다.
   강조·순서·표현·선택적 부연 차이는 누락이 아니며 gold에 넣지 않습니다.
4. **9번은 "남은 내용이 정확하면 된다"로 바뀝니다.** 심은 답변은 무엇도 잘못
   주장하지 않습니다. 누락을 눈에 띄게 하려고 실수를 더하면, gold에 없는 진짜
   결함을 만들어 그것을 찾은 검토자를 오답 처리합니다. 반대로 다른 문장에서
   **슬쩍 보완해도 안 됩니다** — 보완했으면 찾을 누락이 없습니다.

**요소 개수 비교는 검토 보조 자료일 뿐 검사가 아닙니다.** "배정된 답변에 없고
나머지 둘에 있는 요소를 센다"는 방법은 필수성도, 동의어로 충족한 경우도, 간접
보완도 문자열 비교로 증명하지 못합니다. 그래서 **만들지 않았습니다** —
§1.3a 8번의 `anyOf` 실험과 같은 이유입니다. 필요하면 보조 자료로 붙이되,
gate로는 쓰지 않습니다.

reply shape의 gold kind도 현상을 따릅니다 — omission은 `missingPoints`,
`meaningful_difference`는 `differences`, 나머지는 `contradictions`입니다.

### 1.3c template을 올리면 예산이 따라와야 합니다 (v9)

혼합 template guard 때문에 template을 올리면 **새 `--set`이 필요**한데, ledger
경로는 set에서 유도됩니다. 그대로 두면 새 set이 **자기 ledger를 0에서 시작**하고,
"총 $0.18" 승인이 조용히 $0.18을 더 사게 됩니다.

`--ledger=<경로>`가 새 set을 기존 이력이 있는 ledger에 붙입니다. 잠금도 여기서
유도되므로 같은 ledger를 쓰는 모든 실행이 서로 직렬화됩니다 — 같은 승인 총액을
쓰고 있으니 그래야 합니다. `tests/aiReviewDraftLedgerCli.test.mjs`가 거절·허용
양쪽과 "새 set 옆에 ledger를 만들지 않는다"까지 고정합니다.

**명시한 ledger는 이미 존재해야 하고, 없으면 예약 전에 거절합니다.** 이 script의
다른 모든 곳에서 "파일 없음"은 "잔액 0"이고, **유도된** 경로에서는 그것이
맞습니다 — 새 set의 첫 실행에는 이력이 없고 파일은 그때 만들어집니다. 하지만
`--ledger`는 **이력을 이어받으려고만** 쓰므로, 없는 경로는 오타이거나 작업
디렉터리가 다르거나 파일이 옮겨진 것입니다. 그것을 빈 이력으로 읽으면 committed
총액이 $0으로 보고되고 **승인 예산 전체를 다시 쓰게 됩니다.** 게다가 눈에 띄지
않습니다 — 실행은 그대로 진행되고, 작은 총액은 첫 실행의 정상적인 모습이기
때문입니다. 2026-09-08에 `.spend.json`(끝의 `l` 누락)으로 재현됐습니다.

### 1.3d requirement 등록과 채택 심사 — 사람이 지키는 규칙 (v3 계약)

**2026-09-09 승인(mposition).** 근거와 계산은
`.github/audits/ai-review-decomposition-atomicity-2026-09-09.md`,
계약 본문은 `docs/ops/ai-review-eval-scoring-contract.md`다. **검사는 만들지
않았습니다** — 여기가 그 이유와 대신 지키는 방법입니다.

**하나의 requirement id는 독립적으로 판정 가능한 요구·명제 하나를 나타냅니다.**
"행동"이라고 쓰면 빠지는 것이 있습니다 — `송달일부터 2주`는 기한이고
`얼음을 대지 않는다`는 금지입니다. 가르는 질문은 하나입니다: **이것 하나만
따로 판정할 수 있는가.** 묶을 때와 쪼갤 때 같은 질문을 씁니다.

**왜 이것이 비싼가.** 같은 검토자의 같은 문장이 **같은 충분성 판정 아래에서**
등록 방식만으로 recall 1.00과 0.50으로 갈립니다. `contact-bank-payment-hold`가
은행 연락·지급정지·추가 출금 차단 셋을 한 항목에 담고 있고, 그 아래에서
지급정지만 지적한 검토자가 **만점**을 받는 것은 **그 반쪽을 충분하다고
판정했을 때**입니다 — `sufficiency`를 적지 않은 기록이 곧 그 판정입니다(기본값
`sufficient`). 쪼개면 같은 판정에서 TP 1 / FN 1이고, 불충분이라 판정하면
묶인 채로도 TP 0 / FN 1 / FP 1입니다.

**그래도 `sufficiency`가 D를 대신하지는 못합니다.** 불충분 판정은 검토자를 벌할
뿐 놓친 두 번째 요구를 FN으로 세지 않습니다 — 묶인 gold에서는 FN이 1이고,
쪼갠 뒤에야 2가 됩니다.

**자동 검사를 만들지 않은 이유.** 요구 설명 안의 접속 표현(`와`·`과`·`및`·
`그리고`·`,`)을 신호로 삼아 후보 gold 9건에 대 보았고 **9건 전부가 걸렸습니다.**
여러 요구를 묶은 것으로 확인된 것은 하나뿐입니다. 그 신호는 설명이 한국어로
쓰였다는 사실을 재고 있습니다. `anyOf` 키워드 채점기와 notes label 검사에서 이미
두 번 배운 것과 같은 결론입니다 — **자동으로 판정할 수 없으면 검사로 만들지
않습니다.**

case를 **등록할 때** 확인합니다.

1. gold 항목마다 "이것 하나만 따로 판정할 수 있는가"를 묻습니다.
2. 아니라면 쪼갭니다. 쪼갠 뒤 **각 항목의 완전성 주장을 다시 판정합니다** —
   항목이 둘이 되면 "이것이 전부인가"가 새 목록에 대해 다시 열립니다.
3. 지나치게 쪼개지 않습니다. 하나로 판정되는 요구를 셋으로 나누면 통째로 지적한
   검토자가 TP 3을 받고 case 간 비교가 항목 수에 좌우됩니다. 기준은 같은
   한 줄입니다.

**채택 심사에서 다시 확인합니다.**

- **기존 후보를 자동으로 고치지 않습니다.** 어떤 script도 후보의 gold를 다시
  쓰지 않습니다.
- **그러나 채택할 때 면제하지도 않습니다.** 굵은 gold를 가진 후보가 그대로
  채택되면 위 비용이 그대로 들어옵니다. 고치는 것은 사람이고, 고치는 시점은
  채택 심사입니다.
- **gold를 고쳤다면 그 case를 쓴 모든 reviewer 결과를 새 gold로 다시
  채점합니다.** gap을 발견한 reviewer만 제외하면 나머지는 자기가 측정된 것과
  다른 목록 위에서 비교됩니다. artifact가 `caseDigest`를 들고 있으므로 낡은
  점수는 검증에서 어긋납니다 — 절차를 건너뛰면 조용히 통과하는 것이 아니라
  거절됩니다.

**판정 기록 쪽의 짝은 C1입니다**(계약 본문). 같은 규칙을 기록 쪽에서 지키는
것이고, 한쪽만 지키면 다른 쪽이 굵어 임의가 남습니다.
