...
**왜 이것이 비싼가.** 같은 검토자의 같은 문장이 등록 방식만으로 recall 1.00과
0.50으로 갈립니다. `contact-bank-payment-hold`가 은행 연락·지급정지·추가 출금
차단 셋을 한 항목에 담고 있고, 그 아래에서는 지급정지만 지적한 검토자가
**만점**을 받습니다. 쪼개면 TP 1 / FN 1입니다.
