...
유료 turn: 항목당 1회, 총 **약 10회**. 각 클립은 5초 이하로, 기본 모델
`gpt-4o-mini-transcribe`로 검증합니다.

**단가는 이제 관측돼 있습니다**(§14 B-3 해결, `docs/policy/voice-input.md`
§6.1.3). 이 계정이 청구받는 요율은 audio input **$3.00/1M 토큰**, output
**$5.00/1M**입니다. 기준점은 2026-09-02의 관측 — 3회 호출, 352 audio input +
112 output 토큰, **US$0.001616**.

그 요율과 시료를 기준으로 **예상 비용은 US$0.01 미만**입니다. **보장된 상한이
아닙니다** — 관측 시료는 합성 음성 3건이고, output 토큰 수는 발화 밀도에 따라
달라지므로 말이 촘촘한 클립은 같은 길이에 더 많이 씁니다.

**분당 예상 비용을 상한으로 쓰지 않습니다.** 가격표의 "Estimated cost"
$0.003/min은 §6.1.3이 이미 안전한 상한이 아니라고 기록했습니다 — 그 관측은
분당 191 output 토큰이었고 추정치는 241에서 도달합니다.

**공표 가격표로 계산하지도 않습니다.** 페이지의 audio input 가격은 $1.25/1M이고
청구는 그 2.4배입니다. 원인은 미상이며, 그것이 register가 두 숫자를 다 드는
이유입니다(§6.1.3).
...
브라우저가 실제로 만들 수 있는 전환은 넷이고, 넷 다 자동화돼 있습니다.
**아래는 실행할 항목이 아니라 그 spec이 무엇을 덮는지에 대한 서술입니다** —
체크박스를 쓰지 않는 이유가 그것이고, template에 체크된 상자가 있으면 사람이
손으로 확인한 것처럼 보입니다.

- `A → A` (같은 계정으로 refetch) → **취소되지 않는다**
  (session을 다시 읽었다는 사실까지 함께 확인합니다 — refetch가 일어나지
  않았다면 이 항목은 아무것도 지키지 않는 채 통과합니다)
- `A → B` (다른 계정이 같은 탭을 차지) → `VOICE_SCOPE_CHANGED`, 클립
  미전송, 마이크 해제, **계정 A의 음성 transcript가 B의 입력창에 추가되지
  않음**
- `A → 로그아웃` → `VOICE_SCOPE_CHANGED`, 마이크 해제
- 변환 중 전환 → **진행 중인 업로드를 버리고** 어느 초안에도 쓰지 않음
  (요청이 실제로 route에 도달한 뒤 전환합니다)

**"B의 입력창이 빈 상태"는 여전히 이 항목의 주장이 아닙니다 — 다만 이유가
바뀌었습니다.**

이 문단은 원래 "초안은 대화 id로만 key가 잡히므로 B는 A가 **타이핑한** 글을
그대로 본다"고 적었고, 그것은 2026-09-02까지 사실이었습니다. **그날 고쳐졌습니다**
— `draftKeyFor()`가 신원과 대화를 함께 지목하므로 B는 A의 초안을 보지 않습니다
(`lib/conversationDraftStore.ts`,
`docs/policy/conversation-draft-identity-scope.md`,
`docs/policy/voice-input.md` §8.4의 "초안의 신원 격리는 별개 계약입니다").

그래도 이 항목이 그것을 주장하지 않는 이유는 **계약이 다르기 때문**입니다.
초안 격리는 초안 저장소의 계약이고 자체 테스트를 가지며, 여기서 통과·실패로
읽히면 Voice가 보장하지 않는 것을 Voice의 서명이 덮게 됩니다. Voice가 보장하는
것은 **A의 음성 transcript가 B의 입력창에 추가되지 않는다**까지이며, 테스트도 그
범위만 단언합니다.
