...
한국어·영어·중국어 구조화 콘텐츠만 편집합니다. 포스터, 포스터 대체 텍스트와 라벨도
...
작성 화면은 `나의 AI 어시스턴트 + Knowledge` 한국어·영어·중국어 문안으로 시작합니다.
다음 제품 경계만 설명합니다.
...
- AI 어시스턴트와 Knowledge는 계정 전용이며 공개 목록이나 다른 사용자에게
  공유되지 않는다.
- package import flag가 켜진 환경에서만 검토 후 가져올 수 있는 Agent Skill ZIP과
  Tomverse 패키지를 소개한다. 스크립트 실행이나 외부 도구 자동 연결을 약속하지
  않고, flag가 꺼져 있으면 이메일 카드 자체를 만들지 않는다.
...
메일은 클라이언트 호환성을 위해 영상이나 iframe을 직접 삽입하지 않습니다. 정적
포스터와 사용 단계 텍스트를 보여주고, 단일 CTA로 Tomverse의
`/guides/assistant-knowledge` 인터랙티브 안내에 연결합니다. 이 안내는 로그인 전에도
볼 수 있고, 사용자 설정 언어에 맞는 음성 영상과 Supademo 클릭형 안내를 제공합니다.
한국어·중국어는 해당 언어 UI를, 나머지 locale은 영어 UI를 사용합니다. Supademo
embed URL이 있으면 외부 클릭형 안내를 추가로 표시하고, 같은 페이지의 키보드 접근
가능한 native 안내는 URL 유무와 관계없이 항상 함께 제공합니다. 로그인 후에는 실제
생성 화면으로 이어집니다. 생성 성공,
Knowledge가 포함된 버전 저장, 대화 이동을 실제 저장 결과로 판정하므로 URL만 다시
열어도 완료된 단계 다음부터 이어집니다.
...
1. `Email > Campaigns`에서 한국어·영어·중국어의 제목, preheader, headline, 본문,
