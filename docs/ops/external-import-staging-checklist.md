...
실행·판정·서명은 사람이 합니다. 에이전트는 이 문서의 검증 항목을 갱신하고,
실행자가 보고한 관측을 기록 초안에 옮겨 적을 수 있습니다. 쓸 수 없는 것은
**판정과 서명**뿐입니다 — 정책 frontmatter의 승인 필드와 같은 규칙입니다.
경계는 관측과 판정이며, 기록 README의 5번에 자세히 있습니다.
...
- **template revision**: `2026-08-18b` — 항목이 바뀌면 이 값을 올리고, 실행
...
- [ ] **정리 담당자와 완료 기한을 실행 전에 정해 기록에 적었다.** "검증 후
      정리한다"는 담당자도 기한도 없어 지켜졌는지 알 수 없다. 기록 template의
      "검증용 export와 정리" 표에 전용 칸이 있다. 정리 확인 전에는 기록을
      동결하지 않는다.
