...
> **이 표는 2026-08-24 대조 시점의 상태입니다.** 표는 *무엇이 어떻게 어긋나 있었는가*의
> 기록으로 남기고 고치지 않습니다 — 지금 상태를 물으려면 코드를 보십시오.
>
> 그 뒤 무엇이 어떻게 됐는지는 **판정별로 다릅니다.** 두 경우를 섞어 적었던 것을
> 2026-08-26에 갈랐습니다.
>
> - **범위 불일치** 행: #943 일괄 전환에서 `Tomverse`로 정리됐습니다.
> - **미결정/보류** 행(앱 타이틀): 같은 전환의 승인 범위에 포함돼
>   **상위 브랜드 `Tomverse`로 확정**됐습니다. 보류가 저절로 풀린 것이 아니라
>   별도로 결정된 것이므로, 그 한 행만 결정 결과를 담고 스스로 그렇게 밝힙니다.
>
...
| 앱 타이틀(locale) | `Tomverse` | 플랫폼/사이트 | 상위 브랜드 `Tomverse` | 정합 — 후속 일괄 전환 결정 반영 | `locales/en.ts:14`, `locales/ko.ts:18` |
...
**앱 타이틀(locale)은 이 절을 쓸 때는 `미결정`이었고, 후속 결정에서 확정됐습니다.**
사이트와 앱 셸의 이름이 상위 브랜드인지 제품인지는 §7.3이 정한 이메일 분류와 별개
질문이고, `lib/seo.ts`의 `SITE_NAME`·OG·마케팅 페이지가 함께 걸리는 문제였습니다.

**확정 내용 (후속 All-in-one 활성 표면 전환 결정, #943으로 구현).** 앱
타이틀(locale), 공용 `SITE_NAME`, 루트 메타데이터는 **상위 브랜드 `Tomverse`** 를
씁니다.

**이 결정이 포함하지 않는 것** — 아래는 명시적으로 제외됐습니다.

- `/chat`을 `Tomverse Chat`으로 전환하는 것
- 루트 랜딩에 Chat·Studio·Code 제품군을 노출하는 것
- Studio·Code 제품 표면을 신설하는 것

**제품 전용 표면은 실제 제품 상태에 따라 별도로 명명합니다.** 상위 브랜드 표시를
`Tomverse`로 확정한 것이지, 존재하지 않는 제품을 공개하기로 한 것이 아닙니다.
`/chat`이 `Tomverse Review`로 남은 것도 같은 이유입니다 — 실제 동작이 Review
워크스페이스이기 때문입니다.
