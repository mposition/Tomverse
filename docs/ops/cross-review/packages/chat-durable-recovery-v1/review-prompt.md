# Independent review — task chat-durable-recovery-v1, round 2
...
Tomverse Chat의 인증 계정 대화에 영속 초안과 중단 후 응답 복구를 구현한다. 초안은 계정·대화 scope와 revision CAS로 격리하고, 순서가 있는 opaque attachment reference만 저장하며, 최초 Message 저장과 draft consume을 원자적으로 결속한다. 응답 attempt는 이미 소유권이 확인된 source Message에 결속하고 deterministic request identity, fingerprint, lease owner CAS, bounded checkpoint, terminal 상태와 비용 settlement를 유지한다. reload와 재진입은 GET-only로 canonical Message 또는 공개 attempt 상태를 복구하며 provider 재전송·phantom assistant·다른 대화/계정 데이터 노출을 만들지 않는다. 삭제·export·account lifecycle·attachment provenance와 7개 locale 및 desktop/mobile 실패 UI를 포함한다. 기존 Review 제품과 manual/Auto 모델 동작은 보존하고 production flag, 실제 provider/R2 호출, 가격·score·quality band, 유료 평가, 배포는 변경하지 않는다. 이번 새 digest-bound exchange는 author=codex, reviewer=claude이며 사용자가 허용한 읽기 전용 --skip-preflight 예외만 사용한다.

## Completion criteria

- Composer draft는 account identity와 new/conversation scope에 결속되고 revision CAS, 계정별 advisory lock, conversation/account 용량 상한, private no-store 응답을 사용한다. server response의 scope·revision·snapshot·ordered attachment identity/ordinal을 엄격히 검증하며 malformed/ambiguous 응답은 로컬 입력을 버리거나 성공으로 추정하지 않는다.
- 첫 user Message 저장과 draft consume은 한 트랜잭션에서 수행되고 deterministic request identity replay는 exact text, requested model과 ordered attachment provenance가 모두 같을 때만 성공한다. 저장 응답이 불명확하면 receipt GET으로 확인하며 재요청이 중복 Message·attachment bind·provider dispatch를 만들지 않는다.
- 응답 attempt는 owner-scoped source Message와 conversation에 결속된다. claim·lease renew·checkpoint·terminal 전이는 CAS이고 stale worker가 최신 prefix 또는 terminal 결과를 덮을 수 없다. GET polling은 먼저 소유권을 확인하고 reconciliation UPDATE와 SELECT를 같은 authorized conversation에 제한하며 reload가 provider POST를 만들지 않는다.
- 응답 본문은 최대 100,000자로 제한되고 작은 첫 chunk도 timer로 checkpoint된다. 빈 pre-dispatch refusal은 assistant bubble로 표시하지 않고, partial·failure·terminal canonical Message의 우선순위와 모델 provenance를 보존한다. poll timeout·429·terminal lookup과 draft sync failure는 7개 locale의 사용자 복구 UI와 bounded retry를 제공한다.
- 대화·메시지·모델 이력·계정 삭제는 active/expired attempt와 recovery epoch를 안전하게 처리하고 draft/attempt를 export 및 data-domain registry에 포함한다. 공개 응답은 worker owner, fingerprint, lease, attachment object key를 노출하지 않는다. 비용 settlement는 durable terminal 저장 실패와 분리되어 정산 우회나 fallback 중복 정산이 없어야 한다.
- 검토 diff에는 exact base c91a7c74a8e00e54fb8aeb73901116604f950cc5 이후의 전체 기능·migration·test·운영 문서와 이 task/승인 기록만 포함한다. 검토 산출물 디렉터리만 제외하며 production source, migration 또는 test는 제외하지 않는다. Windows test portability 수정은 의미를 완화하거나 test를 skip하지 않아야 한다.
- 패키지의 집중 unit/server-contract test는 실패·skip 없이 통과하고 typecheck, lint, Prisma validate, API cache-control, unconsumed response body, enum constraint, data-domain registry, conversation writer, encoding, document/policy reference와 diff whitespace guard가 모두 통과해야 한다. 내부 전체 검증 기록은 server unit files 677개+client 49/49, server-contract 591/591, PostgreSQL 102 migrations·drift 0·DB 38/38, 관련 Playwright 235 pass·9 intentional project skip·0 fail이지만 Claude는 이를 자체 실행 관측과 구분해야 한다.
- Claude는 원 요구와 diff를 먼저 읽고 test 기록과 작성자 설명을 그 뒤에 읽는다. verdict는 package digest를 정확히 명시하고 finding마다 location, severity, basis와 재현 절차를 제공한다. 최초 검토와 최대 2회 수정 검토만 허용하며 actionable finding이 상한에서 남으면 on_hold다. review-despite-check-failures, API key fallback, 실제 provider 호출, 과금, push·merge·deploy는 이 task가 승인하지 않는다.

## Change under review — digest sha256:6b7f8382f7e39f14c39d0f6804abe3e8d1bc6d12b6a0f2e4d9cbb2ce04bc9a0a, commit 3defa1a9f6972a193bef5a44ccff1572a45b6230

```diff
diff --git a/app/(site)/(application)/chat/ChatPageClient.tsx b/app/(site)/(application)/chat/ChatPageClient.tsx
index 5b396cfa..d1908a85 100644
...
@@ -240,6 +245,49 @@ const uniqueStrings = (values: string[]) => Array.from(new Set(values));
...
@@ -3975,6 +4127,24 @@ export function ChatPageClient({
...
index df4074d0..39e97b43 100644
...
diff --git a/app/api/conversations/[conversationId]/messages/receipt/route.ts b/app/api/conversations/[conversationId]/messages/receipt/route.ts
new file mode 100644
index 00000000..f0906a8a
--- /dev/null
+++ b/app/api/conversations/[conversationId]/messages/receipt/route.ts
@@ -0,0 +1,129 @@
...
+  }
+}
diff --git a/app/api/conversations/[conversationId]/messages/route.ts b/app/api/conversations/[conversationId]/messages/route.ts
index b370514a..0ce91f51 100644
--- a/app/api/conversations/[conversationId]/messages/route.ts
+++ b/app/api/conversations/[conversationId]/messages/route.ts
@@ -15,6 +15,17 @@ import {
   readLimitedJson,
 } from "@/lib/apiSecurity";
...
@@ -47,7 +59,11 @@ const attachmentUploadIdSchema = z.string().trim().min(1).max(64);
...
@@ -66,6 +82,10 @@ const userMessageSchema = z
...
+  )
   ...
   .refine(
@@ -74,14 +94,26 @@ const userMessageSchema = z
...
@@ -125,21 +157,49 @@ export async function POST(
       }
...
@@ -160,31 +220,74 @@ export async function POST(
...
+      return { count };
     ...
     /*
@@ -208,7 +311,7 @@ export async function POST(
...
@@ -220,6 +323,8 @@ export async function POST(
     return NextResponse.json({
       success: true,
...
@@ -238,6 +343,12 @@ export async function POST(
         { status: error.status }
       );
     }
...
@@ -249,7 +360,15 @@ export async function POST(
         })
       );
       return NextResponse.json(
...
@@ -320,6 +439,11 @@ export async function DELETE(
...
+            });
             const deletedSources = await tx.message.deleteMany({
                 where: {
                     conversationId,
@@ -339,10 +463,17 @@ export async function DELETE(
         });
...
index 2f813f4b..ae2b446e 100644
...
diff --git a/app/api/products/chat/attempts/[assistantMessageId]/route.ts b/app/api/products/chat/attempts/[assistantMessageId]/route.ts
new file mode 100644
index 00000000..3f111a1f
--- /dev/null
+++ b/app/api/products/chat/attempts/[assistantMessageId]/route.ts
@@ -0,0 +1,89 @@
...
+  }
+}
diff --git a/app/api/products/chat/drafts/[scopeKey]/route.ts b/app/api/products/chat/drafts/[scopeKey]/route.ts
new file mode 100644
index 00000000..2348ab15
--- /dev/null
+++ b/app/api/products/chat/drafts/[scopeKey]/route.ts
@@ -0,0 +1,254 @@
+export const dynamic = "force-dynamic";
+
+import { getServerSession } from "next-auth/next";
...
index 4957f293..39815da8 100644
...
@@ -1271,6 +1573,12 @@ function ChatAppComponent({
...
@@ -1280,7 +1588,10 @@ function ChatAppComponent({
...
@@ -1327,6 +1638,53 @@ function ChatAppComponent({
...
@@ -1356,6 +1714,58 @@ function ChatAppComponent({
         return;
       }
...
@@ -1769,7 +2179,10 @@ function ChatAppComponent({
...
@@ -1777,27 +2190,32 @@ function ChatAppComponent({
...
@@ -1812,54 +2230,111 @@ function ChatAppComponent({
...
@@ -1931,35 +2406,48 @@ function ChatAppComponent({
...
+        }
   ...
   return (
@@ -2007,7 +2495,11 @@ function ChatAppComponent({
...
@@ -2044,7 +2536,7 @@ function ChatAppComponent({
...
@@ -2061,7 +2553,7 @@ function ChatAppComponent({
                           type="submit"
...
index ba18f583..5df5b81d 100644
...
@@ -556,7 +557,21 @@ type ChatInputProps = {
   onSubmit: () => void;
   onCancel: () => void;
   disabled?: boolean;
+  /**
...
@@ -818,7 +833,10 @@ export function ChatInput({
   onSubmit,
   onCancel,
   disabled = false,
+  draftLocked = false,
...
+  allowEditingWhileSending = false,
   ...
   currentChatId = null,
@@ -1265,7 +1283,16 @@ export function ChatInput({
...
@@ -1319,6 +1346,8 @@ export function ChatInput({
     */
...
@@ -2244,7 +2273,10 @@ export function ChatInput({
...
+    if (!isDraftMutationDisabled && !isVoiceSubmissionBlocked && !isSending) {
       ...
       onSubmit();
@@ -2516,7 +2548,7 @@ export function ChatInput({
   );
...
@@ -2538,6 +2570,7 @@ export function ChatInput({
...
+      if (draftLocked) return;
       ...
       void runUploadBatch(
@@ -2545,12 +2578,13 @@ export function ChatInput({
         failed.scopeId
       );
     },
-    [runUploadBatch]
...
@@ -2579,6 +2613,7 @@ export function ChatInput({
...
@@ -2601,7 +2636,7 @@ export function ChatInput({
...
@@ -2619,6 +2654,8 @@ export function ChatInput({
     event.stopPropagation();
...
+
     if (!canAttach) {
       ...
       return;
@@ -2630,6 +2667,10 @@ export function ChatInput({
     const handlePaste = (
...
@@ -2651,7 +2692,7 @@ export function ChatInput({
     };
...
+    if (draftLocked || !canConnectGoogleDrive || isUploading) return;
     ...
     if (availableSlots <= 0) {
@@ -2786,6 +2827,7 @@ export function ChatInput({
   };
...
@@ -3303,6 +3345,7 @@ export function ChatInput({
                     <button
                       type="button"
...
@@ -3314,6 +3357,7 @@ export function ChatInput({
                     <button
                       type="button"
...
+                      disabled={draftLocked}
                       ...
                       className={`relative flex items-center justify-center rounded-full text-red-800 transition hover:bg-red-200 dark:text-red-100 dark:hover:bg-red-900/60 ${
@@ -3390,6 +3434,7 @@ export function ChatInput({
                   <button
                     type="button"
...
@@ -3489,6 +3534,7 @@ export function ChatInput({
...
@@ -3505,6 +3551,7 @@ export function ChatInput({
...
@@ -3664,10 +3711,10 @@ export function ChatInput({
               state={voice.state}
               copy={voiceCopy}
...
+              disabled={isDraftMutationDisabled}
               ...
               onStop={voice.stop}
             />
@@ -3687,12 +3734,14 @@ export function ChatInput({
               type="button"
...
@@ -3819,7 +3868,7 @@ export function ChatInput({
                   <button
                     type="button"
...
+                    disabled={draftLocked || !canAttach || attachments.length >= maxAttachments}
                   ...
                   >
@@ -4154,7 +4203,7 @@ export function ChatInput({
                   <button
                     type="button"
...
@@ -4181,8 +4230,8 @@ export function ChatInput({
...
+                      (canConnectGoogleDrive && attachments.length >= maxAttachments)
                     ...
                     onClick={() => {
                       closeMenu(false);
@@ -4721,6 +4770,7 @@ export function ChatInput({
           type="file"
           multiple
...
+          disabled={draftLocked}
           ...
           className="hidden"
         />
diff --git a/components/chat/ChatMessageList.tsx b/components/chat/ChatMessageList.tsx
index 452b16b8..fa101718 100644
...
@@ -350,6 +356,9 @@ export function DesktopChatShell({
...
+  allowEditingWhileSending,
   ...
   onNewImage,
   imageLock,
@@ -1319,7 +1328,10 @@ export function DesktopChatShell({
...
+  allowEditingWhileSending: boolean;
   ...
   onNewImage?: (() => void) | null;
   /** Set when image generation is visible to this viewer but not usable. */
@@ -395,6 +401,9 @@ export function MobileChatShell({
   isModelSelectionReady,
   isConversationSelectionResolved,
   pendingSubmission,
+  durableDraftLocked,
...
+  allowEditingWhileSending,
   ...
   onNewImage,
   imageLock,
@@ -1685,7 +1694,10 @@ export function MobileChatShell({
             personalizedPrompt={personalizedPrompt}
             onSubmit={onSubmit}
             onCancel={() => setStopSignal((current) => current + 1)}
+            draftLocked={durableDraftLocked}
...
+            allowEditingWhileSending={allowEditingWhileSending}
             ...
             currentChatId={currentChatId}
diff --git a/components/chat/chatDurableRecoveryClient.ts b/components/chat/chatDurableRecoveryClient.ts
new file mode 100644
index 00000000..416e48aa
--- /dev/null
+++ b/components/chat/chatDurableRecoveryClient.ts
@@ -0,0 +1,536 @@
...
+    );
+  });
diff --git a/components/chat/useConversationDrafts.ts b/components/chat/useConversationDrafts.ts
index 4858d22d..3fbc0df6 100644
--- a/components/chat/useConversationDrafts.ts
+++ b/components/chat/useConversationDrafts.ts
@@ -1,6 +1,6 @@
 "use client";
...
+import { discardResponseBody } from "@/lib/discardResponseBody";
 ...
 export {
   draftKeyFor,
@@ -55,7 +67,107 @@ export type ConversationDraftsApi = {
...
@@ -66,7 +178,9 @@ const releasePreviews = (items: ChatAttachment[], keep: ChatAttachment[]) => {
 };
...
@@ -74,12 +188,6 @@ const releasePreviews = (items: ChatAttachment[], keep: ChatAttachment[]) => {
...
@@ -96,10 +204,20 @@ const releasePreviews = (items: ChatAttachment[], keep: ChatAttachment[]) => {
  */
...
@@ -110,22 +228,620 @@ export function useConversationDrafts(
   useEffect(() => {
...
+
   const resolveKey = useCallback(
     (scopeId?: string | null) =>
       scopeId === undefined
@@ -137,27 +853,43 @@ export function useConversationDrafts(
...
@@ -176,15 +908,29 @@ export function useConversationDrafts(
       const key = resolveKey(scopeId);
...
+      const toKey = draftKeyFor(toScopeId, expectedIdentityKey);
       ...
       if (!moving) return;
@@ -194,11 +940,365 @@ export function useConversationDrafts(
...
@@ -209,5 +1309,14 @@ export function useConversationDrafts(
     hasDraft,
     discardDraft,
     migrateDraft,
...
diff --git a/docs/ops/cross-review/packages/chat-durable-recovery-v1.authorization.md b/docs/ops/cross-review/packages/chat-durable-recovery-v1.authorization.md
new file mode 100644
index 00000000..7814e4bc
--- /dev/null
+++ b/docs/ops/cross-review/packages/chat-durable-recovery-v1.authorization.md
@@ -0,0 +1,47 @@
...
+  대신하지 않는다.
diff --git a/docs/ops/cross-review/packages/chat-durable-recovery-v1.task.json b/docs/ops/cross-review/packages/chat-durable-recovery-v1.task.json
new file mode 100644
index 00000000..a2571c9a
--- /dev/null
+++ b/docs/ops/cross-review/packages/chat-durable-recovery-v1.task.json
@@ -0,0 +1,99 @@
+{
+  "taskId": "chat-durable-recovery-v1",
...
+    "Claude는 원 요구와 diff를 먼저 읽고 test 기록과 작성자 설명을 그 뒤에 읽는다. verdict는 package digest를 정확히 명시하고 finding마다 location, severity, basis와 재현 절차를 제공한다. 최초 검토와 최대 2회 수정 검토만 허용하며 actionable finding이 상한에서 남으면 on_hold다. review-despite-check-failures, API key fallback, 실제 provider 호출, 과금, push·merge·deploy는 이 task가 승인하지 않는다."
+  ],
+  "baseCommit": "c91a7c74a8e00e54fb8aeb73901116604f950cc5",
+  "writableScope": [
+    "app/(site)/(application)/chat/ChatPageClient.tsx",
+    "app/api/chat/route.ts",
+    "app/api/conversations/[conversationId]/messages/receipt/route.ts",
+    "app/api/conversations/[conversationId]/messages/route.ts",
+    "app/api/conversations/[conversationId]/route.ts",
+    "app/api/products/chat/attempts/[assistantMessageId]/route.ts",
+    "app/api/products/chat/drafts/[scopeKey]/route.ts",
+    "components/chat/ChatApp.tsx",
+    "components/chat/chatDurableRecoveryClient.ts",
+    "components/chat/ChatInput.tsx",
+    "components/chat/ChatMessageList.tsx",
+    "components/chat/DesktopChatShell.tsx",
+    "components/chat/MobileChatShell.tsx",
+    "components/chat/useConversationDrafts.ts",
+    "docs/ops/cross-review/packages/chat-durable-recovery-v1.authorization.md",
+    "docs/ops/cross-review/packages/chat-durable-recovery-v1.task.json",
+    "docs/ops/tomverse-chat-progress.md",
+    "docs/policy/chat-durable-draft-and-attempt-recovery.md",
+    "docs/policy/tomverse-chat-data-domain-registry.yaml",
+    "lib/accountDataExport.ts",
+    "lib/accountDataExportDomains.ts",
+    "lib/chatComposerDraftCore.ts",
+    "lib/chatComposerDraftPersistence.ts",
+    "lib/chatDraftMessageConsume.ts",
+    "lib/chatDurableAttemptStream.ts",
+    "lib/chatDurableRecoveryAccess.ts",
+    "lib/chatDurableSourceMessage.ts",
+    "lib/chatResponseAttemptCore.ts",
+    "lib/chatResponseAttemptDeletion.ts",
+    "lib/chatResponseAttemptPersistence.ts",
+    "lib/chatSecurity.ts",
+    "lib/chatStreamRuntime.ts",
+    "lib/messageAttachmentResend.ts",
+    "lib/messageAttachmentStorage.ts",
+    "lib/messageRequestIdentity.ts",
+    "locales/de.ts",
+    "locales/en.ts",
+    "locales/es.ts",
+    "locales/fr.ts",
...
+    "prisma/migrations/20260913130000_chat_recovery_epoch/migration.sql",
+    "prisma/migrations/20260913131000_message_attachment_source_provenance/migration.sql",
+    "prisma/schema.prisma",
+    "scripts/check-api-cache-control.mjs",
+    "scripts/check-enum-constraints-core.mjs",
+    "scripts/check-enum-constraints.mjs",
+    "scripts/cross-review.mjs",
+    "scripts/run-db-integration-tests.mjs",
+    "tests/anthropicPromptCachingWiring.test.mjs",
+    "tests/bodyScrollLock.test.mjs",
+    "tests/chatDurableAttemptStream.test.mjs",
+    "tests/chatDurableRecoveryClient.test.mjs",
+    "tests/chatDurableRecoveryCore.test.mjs",
+    "tests/chatDurableRecoverySchema.test.mjs",
+    "tests/chatDurableRecoveryServerOrder.test.mjs",
+    "tests/chatResponseAttemptDeletion.test.mjs",
+    "tests/ciPlaywrightInstall.test.mjs",
+    "tests/e2e/attachment-persistence.spec.ts",
+    "tests/e2e/attachment-unavailable-recovery.spec.ts",
+    "tests/e2e/chat-failure-recovery.spec.ts",
+    "tests/e2e/chat-unified-workspace.spec.ts",
+    "tests/e2e/conversation-draft-isolation.spec.ts",
+    "tests/e2e/external-conversation-continuation.spec.ts",
+    "tests/e2e/model-change-send-barrier.spec.ts",
+    "tests/e2e/support/app-fixtures.ts",
+    "tests/e2e/support/chat-state-fixtures.ts",
+    "tests/e2e/upgrade-discovery.spec.ts",
+    "tests/e2e/voice-input-composer.spec.ts",
+    "tests/enumConstraints.test.mjs",
+    "tests/integration/chat-durable-recovery.db.test.ts",
+    "tests/integration/message-attachment-resend.db.test.ts",
+    "tests/memoryEvalSucc4Dataset.test.mjs",
+    "tests/providerApiKeyNames.test.mjs",
+    "tests/providerErrorClassification.test.mjs",
+    "tests/server-contract/chat-durable-recovery-route.test.ts",
+    "tests/server-contract/chat-route-fallback-liveness.test.ts",
+    "tests/server-contract/message-attachment-resend.test.mjs",
+    "tests/server-contract/message-request-identity.test.mjs"
+  ],
+  "generatedPaths": []
+}
diff --git a/docs/ops/tomverse-chat-progress.md b/docs/ops/tomverse-chat-progress.md
index cac2ed9b..8496fb23 100644
...
@@ -330,18 +440,19 @@ candidate=baseline 투영을 거부하며, runtime 소스 변조 후 전체 19
...
diff --git a/docs/policy/chat-durable-draft-and-attempt-recovery.md b/docs/policy/chat-durable-draft-and-attempt-recovery.md
new file mode 100644
index 00000000..8c38f7bd
--- /dev/null
+++ b/docs/policy/chat-durable-draft-and-attempt-recovery.md
@@ -0,0 +1,263 @@
...
index 46d1573b..673ae672 100644
--- a/docs/policy/tomverse-chat-data-domain-registry.yaml
+++ b/docs/policy/tomverse-chat-data-domain-registry.yaml
@@ -208,6 +208,42 @@ domains:
     implementationStatus: implemented
     inUnifiedExport: included
 
+  - domain: chatComposerDraft
+    prismaModel: ChatComposerDraft
+    userLinkageRole: subject
+    owner: backend-ai
+    deletionAction: delete
+    deletionMechanism: cascade_from_user
+    retentionPolicy: immediate
+    implementationStatus: implemented
+    inUnifiedExport: included_filtered
+    note: >-
+      Authenticated Chat's unsent composer text and ordered opaque attachment
+      handles. The row cascades both from the account and, for a stored scope,
+      from its conversation. Unified export includes the user's text, scopeKey
+      (which is the conversation id for a stored scope), revision and timestamps
+      while withholding the duplicate conversation foreign key, internal row id
+      and attachment identifiers. Deleting this row never deletes the referenced
+      upload or attachment; their existing lifecycle remains authoritative.
+
...
+    inUnifiedExport: included_filtered
+    note: >-
+      Durable recovery state for one authenticated stored-Chat assistant
+      response. The user-visible committed response prefix, status, attribution
+      and terminal metadata are exported. Fingerprint, worker owner and lease
+      are withheld concurrency internals. User, conversation and source-message
+      cascades all remove the row; the assistant id is deliberately not a
+      foreign key because the attempt must be claimed before an assistant
+      Message exists.
+
   - domain: comparisonReview
     prismaModel: ComparisonReview
     userLinkageRole: subject
diff --git a/lib/accountDataExport.ts b/lib/accountDataExport.ts
index bfdb4824..6b39c9b7 100644
--- a/lib/accountDataExport.ts
+++ b/lib/accountDataExport.ts
...
index 82774ed2..323b34ae 100644
--- a/lib/accountDataExportDomains.ts
+++ b/lib/accountDataExportDomains.ts
@@ -190,6 +190,22 @@ export const EXPORT_DOMAIN_DECLARATIONS: ExportDomainDeclaration[] = [
     withheldReason:
...
diff --git a/lib/chatComposerDraftCore.ts b/lib/chatComposerDraftCore.ts
new file mode 100644
index 00000000..da0678fa
--- /dev/null
+++ b/lib/chatComposerDraftCore.ts
@@ -0,0 +1,125 @@
+import { z } from "zod";
+
...
+  };
+}
diff --git a/lib/chatComposerDraftPersistence.ts b/lib/chatComposerDraftPersistence.ts
new file mode 100644
index 00000000..f530e938
--- /dev/null
+++ b/lib/chatComposerDraftPersistence.ts
@@ -0,0 +1,213 @@
+import "server-only";
+
...
+}
diff --git a/lib/chatDraftMessageConsume.ts b/lib/chatDraftMessageConsume.ts
new file mode 100644
index 00000000..0b5d6f1a
--- /dev/null
+++ b/lib/chatDraftMessageConsume.ts
@@ -0,0 +1,345 @@
+import "server-only";
+
+import type { Prisma } from "@prisma/client";
...
+  });
+}
diff --git a/lib/chatDurableAttemptStream.ts b/lib/chatDurableAttemptStream.ts
new file mode 100644
index 00000000..b614caf4
--- /dev/null
+++ b/lib/chatDurableAttemptStream.ts
@@ -0,0 +1,150 @@
+import "server-only";
+
+import {
...
diff --git a/lib/chatDurableRecoveryAccess.ts b/lib/chatDurableRecoveryAccess.ts
new file mode 100644
index 00000000..5e86749e
--- /dev/null
+++ b/lib/chatDurableRecoveryAccess.ts
@@ -0,0 +1,81 @@
+import "server-only";
+
...
+}
diff --git a/lib/chatDurableSourceMessage.ts b/lib/chatDurableSourceMessage.ts
new file mode 100644
index 00000000..8f150c5c
--- /dev/null
+++ b/lib/chatDurableSourceMessage.ts
@@ -0,0 +1,117 @@
+import "server-only";
+
+import { prisma } from "@/lib/prisma";
...
+}
diff --git a/lib/chatResponseAttemptCore.ts b/lib/chatResponseAttemptCore.ts
new file mode 100644
index 00000000..bc6165fe
--- /dev/null
+++ b/lib/chatResponseAttemptCore.ts
@@ -0,0 +1,305 @@
...
+  };
+}
diff --git a/lib/chatResponseAttemptDeletion.ts b/lib/chatResponseAttemptDeletion.ts
new file mode 100644
index 00000000..e7d5c2ab
--- /dev/null
+++ b/lib/chatResponseAttemptDeletion.ts
@@ -0,0 +1,121 @@
+import "server-only";
+
...
+  return result.count;
+}
diff --git a/lib/chatResponseAttemptPersistence.ts b/lib/chatResponseAttemptPersistence.ts
new file mode 100644
index 00000000..d6f8ab33
--- /dev/null
+++ b/lib/chatResponseAttemptPersistence.ts
@@ -0,0 +1,495 @@
+import "server-only";
+
+import { Prisma } from "@prisma/client";
...
+  return readChatResponseAttemptFrom(db, input.userId, input.assistantMessageId);
+}
diff --git a/lib/chatSecurity.ts b/lib/chatSecurity.ts
index 7a97894c..8895bb93 100644
--- a/lib/chatSecurity.ts
+++ b/lib/chatSecurity.ts
...
@@ -5373,6 +5374,29 @@ export const validateChatPayload = (body: unknown) => {
             "Invalid message ID."
         );
     }
+    if (
...
index 73ea83e5..cf47802c 100644
--- a/lib/chatStreamRuntime.ts
+++ b/lib/chatStreamRuntime.ts
@@ -107,6 +107,15 @@ export type ChatRuntimeLastPrompt = {
   text: string;
   targetChatId: string;
   attachments: ChatAttachment[];
+  /**
...
+  sourceUserMessageId: string;
    ...
    *
diff --git a/lib/messageAttachmentResend.ts b/lib/messageAttachmentResend.ts
index dad1656d..1a0325c1 100644
--- a/lib/messageAttachmentResend.ts
+++ b/lib/messageAttachmentResend.ts
@@ -45,14 +45,49 @@ type PreparedRef = { uploadId: string } | { sourceId: string; source: Metadata;
...
diff --git a/lib/messageAttachmentStorage.ts b/lib/messageAttachmentStorage.ts
index 26140b78..0214b118 100644
--- a/lib/messageAttachmentStorage.ts
+++ b/lib/messageAttachmentStorage.ts
...
diff --git a/lib/messageRequestIdentity.ts b/lib/messageRequestIdentity.ts
new file mode 100644
index 00000000..5a17b115
--- /dev/null
+++ b/lib/messageRequestIdentity.ts
@@ -0,0 +1,35 @@
+import "server-only";
+
+import { createHash } from "node:crypto";
...
index 13b0de7a..1fcfe885 100644
--- a/locales/de.ts
+++ b/locales/de.ts
@@ -111,9 +111,20 @@ export const de = {
     },
     chat: {
        ...en.chat,
+        draftConflictTitle: "Dieser Entwurf wurde in einem anderen Tab geändert",
+        draftConflictBody: "Wähle den Serverentwurf oder überschreibe ihn ausdrücklich mit dem in diesem Tab behaltenen Entwurf.",
+        draftConflictUseServer: "Serverentwurf verwenden",
+        draftConflictOverwriteLocal: "Mit meinem Entwurf überschreiben",
+        draftSyncFailedTitle: "Entwurf konnte nicht synchronisiert werden",
+        draftSyncFailedBody: "Der Entwurf bleibt in diesem Tab, wurde aber möglicherweise nicht auf dem Server gespeichert. Prüfe die Verbindung und versuche es erneut.",
+        draftSyncRetry: "Entwurf erneut synchronisieren",
+        messageReceiptRecoveryTitle: "Gespeicherte Frage bestätigen",
...
index 6fd3065a..6a2cad7c 100644
--- a/locales/en.ts
+++ b/locales/en.ts
...
+        draftConflictTitle: "This draft changed in another tab",
+        draftConflictBody: "Choose the server draft, or explicitly replace it with the draft kept in this tab.",
+        draftConflictUseServer: "Use server draft",
+        draftConflictOverwriteLocal: "Overwrite with my draft",
+        draftSyncFailedTitle: "Draft sync failed",
+        draftSyncFailedBody: "Your draft is still in this tab, but it may not be saved on the server. Check your connection and try again.",
+        draftSyncRetry: "Retry draft sync",
+        messageReceiptRecoveryTitle: "Confirm this saved question",
+        messageReceiptRecoveryBody: "The save response was interrupted, so editing is paused to prevent a duplicate or lost question. Reload this conversation to verify the saved state safely.",
+        messageReceiptRecoveryReload: "Reload conversation",
...
index 016c4fd2..cbe945ce 100644
--- a/locales/es.ts
+++ b/locales/es.ts
@@ -111,9 +111,20 @@ export const es = {
     },
     chat: {
        ...en.chat,
+        draftConflictTitle: "Este borrador cambió en otra pestaña",
+        draftConflictBody: "Elige el borrador del servidor o reemplázalo explícitamente por el que se conserva en esta pestaña.",
+        draftConflictUseServer: "Usar borrador del servidor",
+        draftConflictOverwriteLocal: "Sobrescribir con mi borrador",
+        draftSyncFailedTitle: "No se pudo sincronizar el borrador",
+        draftSyncFailedBody: "El borrador sigue en esta pestaña, pero puede que no se haya guardado en el servidor. Comprueba la conexión e inténtalo de nuevo.",
+        draftSyncRetry: "Reintentar sincronización",
+        messageReceiptRecoveryTitle: "Confirma la pregunta guardada",
...
index ee9ee204..278abb6e 100644
--- a/locales/fr.ts
+++ b/locales/fr.ts
@@ -111,9 +111,20 @@ export const fr = {
     },
     chat: {
        ...en.chat,
+        draftConflictTitle: "Ce brouillon a été modifié dans un autre onglet",
+        draftConflictBody: "Choisissez le brouillon du serveur ou remplacez-le explicitement par celui conservé dans cet onglet.",
+        draftConflictUseServer: "Utiliser le brouillon du serveur",
+        draftConflictOverwriteLocal: "Remplacer par mon brouillon",
+        draftSyncFailedTitle: "Échec de la synchronisation du brouillon",
+        draftSyncFailedBody: "Le brouillon reste dans cet onglet, mais il n’est peut-être pas enregistré sur le serveur. Vérifiez la connexion et réessayez.",
+        draftSyncRetry: "Réessayer la synchronisation",
+        messageReceiptRecoveryTitle: "Confirmer la question enregistrée",
...
index 29b5a95f..3f708f0b 100644
--- a/locales/ko.ts
+++ b/locales/ko.ts
@@ -115,6 +115,17 @@ export const ko = {
...
+        draftConflictTitle: "다른 탭에서 이 초안이 변경되었습니다",
+        draftConflictBody: "서버 초안을 사용하거나, 이 탭에 유지된 내 초안으로 명시적으로 덮어쓸 수 있습니다.",
+        draftConflictUseServer: "서버 초안 사용",
+        draftConflictOverwriteLocal: "내 초안으로 덮어쓰기",
+        draftSyncFailedTitle: "초안을 동기화하지 못했습니다",
+        draftSyncFailedBody: "이 탭의 초안은 유지되어 있지만 서버에 저장되지 않았을 수 있습니다. 연결을 확인한 뒤 다시 시도해 주세요.",
+        draftSyncRetry: "초안 동기화 다시 시도",
+        messageReceiptRecoveryTitle: "저장된 질문을 확인해 주세요",
+        messageReceiptRecoveryBody: "저장 응답이 중단되어 질문의 중복 전송이나 유실을 막기 위해 편집을 일시 중지했습니다. 대화를 새로고침하면 저장 상태를 안전하게 확인할 수 있습니다.",
+        messageReceiptRecoveryReload: "대화 새로고침",
...
index aeeac855..cda93f89 100644
--- a/locales/pt.ts
+++ b/locales/pt.ts
@@ -111,9 +111,20 @@ export const pt = {
     },
     chat: {
        ...en.chat,
+        draftConflictTitle: "Este rascunho foi alterado em outra aba",
+        draftConflictBody: "Escolha o rascunho do servidor ou substitua-o explicitamente pelo rascunho mantido nesta aba.",
+        draftConflictUseServer: "Usar rascunho do servidor",
+        draftConflictOverwriteLocal: "Substituir pelo meu rascunho",
+        draftSyncFailedTitle: "Falha ao sincronizar o rascunho",
+        draftSyncFailedBody: "O rascunho continua nesta aba, mas pode não ter sido guardado no servidor. Verifique a ligação e tente novamente.",
+        draftSyncRetry: "Tentar sincronizar novamente",
+        messageReceiptRecoveryTitle: "Confirmar a pergunta guardada",
...
+        retryQuestionSaveFailed: "Não foi possível salvar a pergunta para esta nova tentativa. Nenhuma solicitação de resposta foi enviada.",
         restoreQuestion: "Restaurar pergunta",
         restoreQuestionHint: "Restaura a pergunta no campo de texto. Envie separadamente para solicitar uma nova resposta.",
         singleModelRequired: "Chat usa um modelo de resposta. Selecione um modelo para substituir a seleção atual.",
diff --git a/locales/zh.ts b/locales/zh.ts
index 3fc5f15d..56d8b2c4 100644
--- a/locales/zh.ts
+++ b/locales/zh.ts
@@ -128,9 +128,20 @@ export const zh = {
     },
     chat: {
        ...en.chat,
+        draftConflictTitle: "此草稿已在另一个标签页中更改",
+        draftConflictBody: "请选择服务器草稿，或明确使用此标签页保留的草稿覆盖它。",
+        draftConflictUseServer: "使用服务器草稿",
+        draftConflictOverwriteLocal: "用我的草稿覆盖",
+        draftSyncFailedTitle: "草稿同步失败",
+        draftSyncFailedBody: "草稿仍保留在此标签页中，但可能尚未保存到服务器。请检查网络连接后重试。",
+        draftSyncRetry: "重试草稿同步",
+        messageReceiptRecoveryTitle: "确认已保存的问题",
...
diff --git a/prisma/migrations/20260913100000_chat_durable_recovery_foundation/migration.sql b/prisma/migrations/20260913100000_chat_durable_recovery_foundation/migration.sql
new file mode 100644
index 00000000..fd8f6637
--- /dev/null
+++ b/prisma/migrations/20260913100000_chat_durable_recovery_foundation/migration.sql
@@ -0,0 +1,143 @@
...
diff --git a/prisma/migrations/20260913130000_chat_recovery_epoch/migration.sql b/prisma/migrations/20260913130000_chat_recovery_epoch/migration.sql
new file mode 100644
index 00000000..b8c4d334
--- /dev/null
+++ b/prisma/migrations/20260913130000_chat_recovery_epoch/migration.sql
@@ -0,0 +1,9 @@
...
+  );
diff --git a/prisma/migrations/20260913131000_message_attachment_source_provenance/migration.sql b/prisma/migrations/20260913131000_message_attachment_source_provenance/migration.sql
new file mode 100644
index 00000000..cbae7cbb
--- /dev/null
+++ b/prisma/migrations/20260913131000_message_attachment_source_provenance/migration.sql
@@ -0,0 +1,9 @@
...
index 802d1d05..7a99c508 100644
--- a/prisma/schema.prisma
+++ b/prisma/schema.prisma
@@ -72,6 +72,8 @@ model User {
   messageArtifacts                     MessageArtifact[]
   messageAttachments                   MessageAttachment[]
   messageAttachmentUploads             MessageAttachmentUpload[]
+  chatComposerDrafts                   ChatComposerDraft[]
+  chatResponseAttempts                 ChatResponseAttempt[]
...
@@ -317,6 +319,11 @@ model Conversation {
   routerSwitchReason    String?
   routerRecoveryModelId String?
 
+  /// Monotonic fence for Chat recovery claims. A per-model history clear
+  /// increments it under the same advisory lock used by claim, so a request
+  /// prepared before that clear cannot commit an attempt after it.
+  chatRecoveryEpoch Int @default(0)
+
   /// Policy: docs/policy/external-conversation-import-and-memory.md.
...
@@ -362,6 +369,8 @@ model Conversation {
   routingRuns           RoutingRun[]
   messageArtifacts      MessageArtifact[]
   messageAttachments    MessageAttachment[]
+  composerDrafts        ChatComposerDraft[]
+  responseAttempts      ChatResponseAttempt[]
   comparisonReviews     ComparisonReview[]
   imageGenerations      ImageGeneration[]
   imageGenerationGroups ImageGenerationGroup[]
@@ -446,6 +455,11 @@ model Message {
   /// re-read server-side to build a later prompt.
   attachments MessageAttachment[]
 
+  /// Durable Chat response attempts whose persisted user turn is this row.
+  /// The assistant id is deliberately not a Message relation: the attempt is
+  /// claimed before an assistant Message exists.
+  responseAttempts ChatResponseAttempt[] @relation("ChatResponseAttemptSource")
+
   conversationId String
   conversation   Conversation @relation(fields: [conversationId], references: [id], onDelete: Cascade)
   createdAt      DateTime     @default(now())
@@ -601,6 +615,12 @@ model MessageAttachment {
   /// request that accepted it.
   uploadId String?
 
+  /// Exact source attachment identity when this row was created by carrying
+  /// a prior conversation attachment forward. Kept as opaque provenance (not
+  /// a relation) so deleting the source turn cannot rewrite an idempotency
+  /// receipt for the saved message.
+  sourceAttachmentId String?
+
   /// When storage confirmed, with a 404, that this object is gone.
   ///
   /// Nullable, and set only from a confirmed 404 -- never from a read that
@@ -1006,6 +1026,61 @@ model ProviderProbeResult {
   @@index([runId])
 }
 
+// One durable authenticated Chat composer draft per account and logical scope.
+// `scopeKey` is either "new" or the conversation id. Attachment references
+// are ordered opaque ids only; route validation owns their exact JSON shape.
+model ChatComposerDraft {
...
+}
+
+// One durable execution identity for an authenticated Chat answer. The
+// primary key is the assistant id selected before dispatch. It is not an FK to
+// Message because claim must precede provider and reservation work, while the
+// assistant Message is committed only later.
+model ChatResponseAttempt {
...
+  user              User         @relation(fields: [userId], references: [id], onDelete: Cascade)
+  conversation      Conversation @relation(fields: [conversationId], references: [id], onDelete: Cascade)
+  sourceUserMessage Message      @relation("ChatResponseAttemptSource", fields: [sourceUserMessageId], references: [id], onDelete: Cascade)
+
...
diff --git a/scripts/check-api-cache-control.mjs b/scripts/check-api-cache-control.mjs
index 6278cee4..0c58e645 100644
--- a/scripts/check-api-cache-control.mjs
+++ b/scripts/check-api-cache-control.mjs
...
+  .filter((file) => file.endsWith("/route.ts"));
 ...
 const routes = files.map((file) => ({
   file,
diff --git a/scripts/check-enum-constraints-core.mjs b/scripts/check-enum-constraints-core.mjs
index 9d8e8244..bff720da 100644
--- a/scripts/check-enum-constraints-core.mjs
+++ b/scripts/check-enum-constraints-core.mjs
...
index 743ef067..55637242 100644
...
index 5ec6d50a..7cc77682 100644
...
index 93e4b87c..0cd84316 100644
...
diff --git a/tests/anthropicPromptCachingWiring.test.mjs b/tests/anthropicPromptCachingWiring.test.mjs
index e2c189c5..8fc9fa54 100644
--- a/tests/anthropicPromptCachingWiring.test.mjs
+++ b/tests/anthropicPromptCachingWiring.test.mjs
...
diff --git a/tests/bodyScrollLock.test.mjs b/tests/bodyScrollLock.test.mjs
index da0ceef6..0d13de3a 100644
--- a/tests/bodyScrollLock.test.mjs
+++ b/tests/bodyScrollLock.test.mjs
...
diff --git a/tests/chatDurableAttemptStream.test.mjs b/tests/chatDurableAttemptStream.test.mjs
new file mode 100644
index 00000000..315f2582
--- /dev/null
+++ b/tests/chatDurableAttemptStream.test.mjs
@@ -0,0 +1,96 @@
...
+});
diff --git a/tests/chatDurableRecoveryClient.test.mjs b/tests/chatDurableRecoveryClient.test.mjs
new file mode 100644
index 00000000..2dddc807
--- /dev/null
+++ b/tests/chatDurableRecoveryClient.test.mjs
@@ -0,0 +1,420 @@
+import assert from "node:assert/strict";
+import test from "node:test";
+
...
+  );
+});
diff --git a/tests/chatDurableRecoveryCore.test.mjs b/tests/chatDurableRecoveryCore.test.mjs
new file mode 100644
index 00000000..772ffa64
--- /dev/null
+++ b/tests/chatDurableRecoveryCore.test.mjs
@@ -0,0 +1,254 @@
+import assert from "node:assert/strict";
+import test from "node:test";
+
...
+});
diff --git a/tests/chatDurableRecoverySchema.test.mjs b/tests/chatDurableRecoverySchema.test.mjs
new file mode 100644
index 00000000..aa40f226
--- /dev/null
+++ b/tests/chatDurableRecoverySchema.test.mjs
@@ -0,0 +1,111 @@
+import assert from "node:assert/strict";
+import test from "node:test";
...
+});
diff --git a/tests/chatDurableRecoveryServerOrder.test.mjs b/tests/chatDurableRecoveryServerOrder.test.mjs
new file mode 100644
index 00000000..da835a65
--- /dev/null
+++ b/tests/chatDurableRecoveryServerOrder.test.mjs
@@ -0,0 +1,67 @@
+import assert from "node:assert/strict";
+import { readFileSync } from "node:fs";
+import { resolve } from "node:path";
...
+});
diff --git a/tests/chatResponseAttemptDeletion.test.mjs b/tests/chatResponseAttemptDeletion.test.mjs
new file mode 100644
index 00000000..2b3ee76e
--- /dev/null
+++ b/tests/chatResponseAttemptDeletion.test.mjs
@@ -0,0 +1,160 @@
+import assert from "node:assert/strict";
+import test from "node:test";
+
...
+  });
+});
diff --git a/tests/ciPlaywrightInstall.test.mjs b/tests/ciPlaywrightInstall.test.mjs
index f96ab9da..3fc2329a 100644
--- a/tests/ciPlaywrightInstall.test.mjs
+++ b/tests/ciPlaywrightInstall.test.mjs
@@ -1,5 +1,6 @@
 import assert from "node:assert/strict";
...
diff --git a/tests/e2e/attachment-persistence.spec.ts b/tests/e2e/attachment-persistence.spec.ts
index 37b0610f..1db3878d 100644
--- a/tests/e2e/attachment-persistence.spec.ts
+++ b/tests/e2e/attachment-persistence.spec.ts
@@ -8,6 +8,7 @@ import {
   mockChatStream,
...
diff --git a/tests/e2e/attachment-unavailable-recovery.spec.ts b/tests/e2e/attachment-unavailable-recovery.spec.ts
index e97b5524..b74c4867 100644
--- a/tests/e2e/attachment-unavailable-recovery.spec.ts
+++ b/tests/e2e/attachment-unavailable-recovery.spec.ts
...
diff --git a/tests/e2e/chat-failure-recovery.spec.ts b/tests/e2e/chat-failure-recovery.spec.ts
index faa86590..c7e21459 100644
--- a/tests/e2e/chat-failure-recovery.spec.ts
+++ b/tests/e2e/chat-failure-recovery.spec.ts
...
index 599ac861..f258189d 100644
...
+  mockAttachmentUpload,
   ...
   prepareGuestPage,
@@ -39,6 +42,17 @@ const UNCLASSIFIED_ANSWER = "A late owned answer from the unclassified conversat
...
+
 type RecordedRequest = {
   modelId: string;
   conversationId?: string;
@@ -55,7 +69,25 @@ type ConversationFixture = {
...
@@ -65,13 +97,40 @@ function installControlledChatFetch(options: { routedModelId?: string; responseE
     aborted: boolean;
   }> = [];
...
+    window.sessionStorage.setItem(countKey, String(priorCount + 1));
     ...
     const control = {
@@ -81,15 +140,59 @@ function installControlledChatFetch(options: { routedModelId?: string; responseE
       fail: () => {},
       aborted: false,
     };
...
+    }
     const stream = new ReadableStream<Uint8Array>({
       start(controller) {
         control.push = (text) => {
@@ -109,6 +212,9 @@ function installControlledChatFetch(options: { routedModelId?: string; responseE
         else signal?.addEventListener("abort", abort, { once: true });
       },
     });
...
+    window.sessionStorage.setItem(streamCountKey, String(priorStreamCount + 1));
     ...
     return new Response(stream, {
       status: 200,
@@ -127,6 +233,18 @@ async function requests(page: Page) {
...
@@ -138,6 +256,22 @@ async function drive(page: Page, index: number, action: "push" | "finish" | "fai
   }, { index, action, text });
 }
...
@@ -149,6 +283,31 @@ async function openChat(page: Page, options: {
...
@@ -173,7 +332,8 @@ async function openChat(page: Page, options: {
   }]);
...
@@ -195,11 +355,167 @@ async function openChat(page: Page, options: {
   });
...
+    return route.fulfill({ json: { draft: next } });
   ...
   await page.route(/\/api\/(?:products\/chat\/conversations|conversations)(?:\/[^?]*)?(?:\?.*)?$/, async (route) => {
     const url = new URL(route.request().url());
@@ -214,36 +530,157 @@ async function openChat(page: Page, options: {
     }
...
@@ -254,19 +691,75 @@ async function openChat(page: Page, options: {
...
@@ -279,9 +772,1102 @@ async function chooseModel(page: Page, modelId: string) {
...
+
   test("new Chat is gated while legacy Review keeps its existing route", async ({ page }) => {
     await prepareGuestPage(page, "en");
     await mockAuthenticatedApi(page);
@@ -292,6 +1878,54 @@ test.describe("Chat unified workspace", { tag: "@ui-risk" }, () => {
     await expect(page.getByTestId("chat-textarea")).toBeVisible();
   });
...
@@ -308,6 +1942,51 @@ test.describe("Chat unified workspace", { tag: "@ui-risk" }, () => {
...
+
   test("a late Chat create response cannot adopt itself after navigation to another conversation", async ({ page }) => {
     ...
     let release = () => {};
@@ -323,9 +2002,11 @@ test.describe("Chat unified workspace", { tag: "@ui-risk" }, () => {
...
@@ -346,7 +2027,7 @@ test.describe("Chat unified workspace", { tag: "@ui-risk" }, () => {
...
+    await expect(page.getByTestId("chat-textarea")).toHaveAttribute("readonly", "");
     ...
     release();
@@ -355,6 +2036,8 @@ test.describe("Chat unified workspace", { tag: "@ui-risk" }, () => {
...
@@ -415,12 +2098,12 @@ test.describe("Chat unified workspace", { tag: "@ui-risk" }, () => {
     await openChat(page);
...
@@ -428,7 +2111,10 @@ test.describe("Chat unified workspace", { tag: "@ui-risk" }, () => {
   });
...
@@ -451,18 +2137,29 @@ test.describe("Chat unified workspace", { tag: "@ui-risk" }, () => {
       await chooseModel(page, MODEL_B);
       release();
       await expect(page.getByTestId("chat-textarea")).toBeEnabled();
-      await expect(page.getByTestId("chat-textarea")).toHaveValue(prompt);
...
+      const state = await openChat(page);
       ...
       const gate = new Promise<void>((resolve) => { release = resolve; });
       let held = false;
@@ -482,16 +2179,38 @@ test.describe("Chat unified workspace", { tag: "@ui-risk" }, () => {
...
@@ -659,7 +2378,7 @@ test.describe("Chat unified workspace", { tag: "@ui-risk" }, () => {
...
+    await expect(page.getByTestId("chat-textarea")).toBeEnabled();
     ...
     await expect(page.getByTestId("chat-textarea")).toBeEnabled();
     expect(await requests(page)).toHaveLength(1);
@@ -685,6 +2404,250 @@ test.describe("Chat unified workspace", { tag: "@ui-risk" }, () => {
     expect(await requests(page)).toHaveLength(1);
   });
...
@@ -711,10 +2674,12 @@ test.describe("Chat unified workspace", { tag: "@ui-risk" }, () => {
...
@@ -746,21 +2711,36 @@ test.describe("Chat unified workspace", { tag: "@ui-risk" }, () => {
         if (route.request().method() !== "POST") return route.fallback();
...
index bb57a368..d74acdce 100644
...
+      }, 201));
     ...
     await route.fulfill(json({}));
diff --git a/tests/e2e/external-conversation-continuation.spec.ts b/tests/e2e/external-conversation-continuation.spec.ts
index 520c4ace..b71a9ef3 100644
--- a/tests/e2e/external-conversation-continuation.spec.ts
+++ b/tests/e2e/external-conversation-continuation.spec.ts
...
diff --git a/tests/e2e/model-change-send-barrier.spec.ts b/tests/e2e/model-change-send-barrier.spec.ts
index 4e562ba4..f264fdc9 100644
--- a/tests/e2e/model-change-send-barrier.spec.ts
+++ b/tests/e2e/model-change-send-barrier.spec.ts
@@ -5,6 +5,7 @@ import {
   openModelPickerCatalogue,
   openRecentConversation,
   prepareGuestPage,
...
+    messageRequestIds: [],
     ...
     patchResponses: [],
     holdPatch: false,
@@ -82,6 +86,23 @@ async function recordModelSyncTraffic(page: Page): Promise<Recorder> {
...
+      sourceUserMessageId: body?.sourceUserMessageId ?? null,
     ...
     await route.fallback();
@@ -280,11 +303,130 @@ test.describe("model change send barrier (desktop)", () => {
...
index 90183c02..4412d207 100644
...
+          }, 201));
         ...
         await route.fulfill(json({}));
diff --git a/tests/e2e/support/chat-state-fixtures.ts b/tests/e2e/support/chat-state-fixtures.ts
index b3ff8c98..c3b4a9b9 100644
--- a/tests/e2e/support/chat-state-fixtures.ts
+++ b/tests/e2e/support/chat-state-fixtures.ts
...
+          (requestLog.__chatModelStubRequests ??= []).push(parsed ?? {});
           ...
           modelId = undefined;
diff --git a/tests/e2e/upgrade-discovery.spec.ts b/tests/e2e/upgrade-discovery.spec.ts
index 7284ede8..835fefdc 100644
--- a/tests/e2e/upgrade-discovery.spec.ts
+++ b/tests/e2e/upgrade-discovery.spec.ts
@@ -5,8 +5,40 @@ import {
   openModelPickerCatalogue,
   openRecentConversation,
   prepareGuestPage,
...
diff --git a/tests/e2e/voice-input-composer.spec.ts b/tests/e2e/voice-input-composer.spec.ts
index 6d975da6..a7d93bcb 100644
--- a/tests/e2e/voice-input-composer.spec.ts
+++ b/tests/e2e/voice-input-composer.spec.ts
...
diff --git a/tests/enumConstraints.test.mjs b/tests/enumConstraints.test.mjs
index 8b335001..21938bb4 100644
--- a/tests/enumConstraints.test.mjs
+++ b/tests/enumConstraints.test.mjs
...
+
 ...
 test("a non-enum CHECK is ignored", () => {
   const constraints = readEnumConstraints([
diff --git a/tests/integration/chat-durable-recovery.db.test.ts b/tests/integration/chat-durable-recovery.db.test.ts
new file mode 100644
index 00000000..a9f907d0
--- /dev/null
+++ b/tests/integration/chat-durable-recovery.db.test.ts
@@ -0,0 +1,1440 @@
+import assert from "node:assert/strict";
...
+  }
+});
diff --git a/tests/integration/message-attachment-resend.db.test.ts b/tests/integration/message-attachment-resend.db.test.ts
index d59323ff..c23531df 100644
--- a/tests/integration/message-attachment-resend.db.test.ts
+++ b/tests/integration/message-attachment-resend.db.test.ts
...
+  await prisma.$executeRawUnsafe('TRUNCATE TABLE "ChatComposerDraft", "MessageAttachmentCleanup", "MessageAttachment", "MessageAttachmentUpload", "Message", "Conversation", "User" RESTART IDENTITY CASCADE');
 ...
 async function seed() {
@@ -163,6 +166,61 @@ test("capacity refusal happens before storage writes; authoritative transaction
...
+});
 test("source availability is checked again inside the binding transaction", async () => {
   const { input, source, messageId } = await seed();
   let checks = 0;
diff --git a/tests/memoryEvalSucc4Dataset.test.mjs b/tests/memoryEvalSucc4Dataset.test.mjs
index 7ac823f0..0597bbb2 100644
--- a/tests/memoryEvalSucc4Dataset.test.mjs
+++ b/tests/memoryEvalSucc4Dataset.test.mjs
...
diff --git a/tests/providerApiKeyNames.test.mjs b/tests/providerApiKeyNames.test.mjs
index ccada5b6..c22c7b49 100644
--- a/tests/providerApiKeyNames.test.mjs
+++ b/tests/providerApiKeyNames.test.mjs
@@ -1,6 +1,6 @@
 import assert from "node:assert/strict";
 import test from "node:test";
...
+    .filter(Boolean)
   ...
   const names = new Set(
diff --git a/tests/providerErrorClassification.test.mjs b/tests/providerErrorClassification.test.mjs
index 81c4682c..23095da5 100644
--- a/tests/providerErrorClassification.test.mjs
+++ b/tests/providerErrorClassification.test.mjs
...
diff --git a/tests/server-contract/chat-durable-recovery-route.test.ts b/tests/server-contract/chat-durable-recovery-route.test.ts
new file mode 100644
index 00000000..96594f0e
--- /dev/null
+++ b/tests/server-contract/chat-durable-recovery-route.test.ts
@@ -0,0 +1,569 @@
+import assert from "node:assert/strict";
...
+});
diff --git a/tests/server-contract/chat-route-fallback-liveness.test.ts b/tests/server-contract/chat-route-fallback-liveness.test.ts
index eede9352..3e7ebc20 100644
--- a/tests/server-contract/chat-route-fallback-liveness.test.ts
+++ b/tests/server-contract/chat-route-fallback-liveness.test.ts
@@ -1,4 +1,5 @@
 import assert from "node:assert/strict";
+import { createHash } from "node:crypto";
 import test, { mock } from "node:test";
...
@@ -101,6 +103,89 @@ mock.module("next-auth/next", {
   },
 });
...
+
 ...
 /* -------------------------------------------------------------------------- */
@@ -122,6 +207,7 @@ const attempts: Attempt[] = [];
  * spanning the swap.
  */
...
+let failAssistantPersistence = false;
 ...
 const DEFAULTS: Record<string, () => unknown> = {
   findUnique: () => null,
@@ -204,6 +292,28 @@ const DEFAULTS: Record<string, () => unknown> = {
   ever consulted.
 */
...
+let lastDurableClaimInput: Record<string, unknown> | null = null;
 ...
 const conversationRow = () => ({
   id: CONVERSATION_ID,
@@ -211,8 +321,9 @@ const conversationRow = () => ({
   password: null,
...
@@ -221,10 +332,21 @@ const OVERRIDES: Record<string, Record<string, (args: never) => unknown>> = {
...
@@ -233,6 +355,9 @@ const OVERRIDES: Record<string, Record<string, (args: never) => unknown>> = {
       return args.data;
     },
   },
+  messageAttachment: {
...
@@ -276,6 +401,65 @@ const prismaProxy: unknown = new Proxy(prismaFake, {
...
@@ -395,7 +583,11 @@ const realBillingEntitlements = require(
...
@@ -460,15 +652,34 @@ const whileWaiting = async <T,>(read: () => Promise<T>): Promise<T> => {
   }
 };
...
@@ -478,17 +689,23 @@ const ask = async (behaviour: "answers" | "silent") => {
         "Content-Type": "application/json",
...
+  if (response.status !== expectedStatus) {
   ...
   /*
@@ -545,6 +762,200 @@ test("a routed turn whose primary dies pre-token is finished by a second model",
...
@@ -583,6 +994,34 @@ test("a fallback settles once, releases once, and holds once more", async () =>
...
diff --git a/tests/server-contract/message-attachment-resend.test.mjs b/tests/server-contract/message-attachment-resend.test.mjs
index 79438f2c..e7374e40 100644
--- a/tests/server-contract/message-attachment-resend.test.mjs
+++ b/tests/server-contract/message-attachment-resend.test.mjs
@@ -7,12 +7,22 @@ import { ZodError } from "zod";
...
@@ -63,12 +90,13 @@ mock.module(mod("lib/apiSecurity.ts"), { namedExports: {
...
@@ -129,17 +181,349 @@ test("database failure queues only new copies while capacity failure copies noth
...
+});
diff --git a/tests/server-contract/message-request-identity.test.mjs b/tests/server-contract/message-request-identity.test.mjs
new file mode 100644
index 00000000..10e0f68c
--- /dev/null
+++ b/tests/server-contract/message-request-identity.test.mjs
@@ -0,0 +1,31 @@
+import assert from "node:assert/strict";
+import test from "node:test";
+
...
- PASS `node --conditions=react-server --experimental-test-module-mocks --no-warnings=ExperimentalWarning --import tsx --test --test-concurrency=1 tests/chatDurableRecoveryCore.test.mjs tests/chatDurableAttemptStream.test.mjs tests/chatDurableRecoveryClient.test.mjs tests/chatDurableRecoveryServerOrder.test.mjs tests/chatResponseAttemptDeletion.test.mjs tests/server-contract/chat-durable-recovery-route.test.ts tests/server-contract/chat-route-fallback-liveness.test.ts tests/server-contract/message-attachment-resend.test.mjs tests/server-contract/message-request-identity.test.mjs` (11472ms)
  # fail 0
  # cancelled 0
  # skipped 0
  # todo 0
  # duration_ms 11394.1897

## Guard results (run by the control program)

- PASS `npm run typecheck` (33829ms)
  > ai-chat-hub@0.1.0 typecheck
  > next typegen && tsc --noEmit --incremental false
  
  Generating route types...
  ✓ Types generated successfully
- PASS `npm run lint -- --max-warnings=0` (48698ms)
  > ai-chat-hub@0.1.0 lint
  > eslint --max-warnings=0
- PASS `npx prisma validate` (1891ms)
  The schema at prisma\schema.prisma is valid 🚀
- PASS `npm run check:api-cache-control` (658ms)
  > ai-chat-hub@0.1.0 check:api-cache-control
  > node --conditions=react-server --import tsx scripts/check-api-cache-control.mjs
  
  API cache-control check passed: 202 route(s), 5 choosing their own caching, every one of them listed.
- PASS `npm run check:unconsumed-response-bodies` (1381ms)
  Unconsumed response body check over 1660 source file(s): 314 fetch call site(s).
  Blocking scope: runtime in [browser, either], target api_default_no_store, plus any unparsed file in that scope.
  
  No browser-capable call site leaves an `/api/*` body unread.
- PASS `npm run check:enum-constraints` (940ms)
  > ai-chat-hub@0.1.0 check:enum-constraints
  > node --conditions=react-server --import tsx scripts/check-enum-constraints.mjs
  
  Enum constraint check passed: 87 closed list(s) in the schema — 38 compared against an application list, 16 held only as a TypeScript union, 33 written down only in the database.
- PASS `npm run check:data-domain-registry` (708ms)
...
- PASS `npm run check:conversation-writers` (889ms)
  > ai-chat-hub@0.1.0 check:conversation-writers
  > node scripts/check-conversation-writers.mjs
  
  Conversation writer check passed: 2705 file(s) scanned, every production write goes through lib/conversationCreation.ts.
- PASS `npm run check:encoding` (1158ms)
  > ai-chat-hub@0.1.0 check:encoding
  > node scripts/check-text-encoding.mjs
  
  Text encoding check passed. No mojibake markers found.
- PASS `npm run check:doc-references` (1251ms)
  > ai-chat-hub@0.1.0 check:doc-references
  > node scripts/check-doc-references.mjs
  
  Document reference check passed: 776 referenced path(s) across 95 instruction document(s), and 898 path(s) named by comments across 2709 source file(s), all present.
- PASS `npm run check:policy-section-references` (981ms)
  > ai-chat-hub@0.1.0 check:policy-section-references
  > node scripts/check-policy-section-references.mjs
  
  Policy section reference check passed: 4211 citation(s) against 31 policy document(s). 2561 resolve to a named document and none point at a section that does not exist. No added line introduces an unscoped or ambiguous one (1421 and 229 predate this change).
- PASS `git diff --check c91a7c74a8e00e54fb8aeb73901116604f950cc5` (81ms)

## Findings from the previous round (check each was addressed)

- [warning/evidence] app/api/chat/route.ts:5425-5432 (assistant persistence transaction, `id: sourceUserMessageId!`): The newest-user-message `orderBy` fallback was deleted and replaced by a non-null-asserted client field, so for any persisted turn that omits `sourceUserMessageId` Prisma drops the `id` filter entirely (undefined == 'not provided') and the now-unordered `findFirst` stales an arbitrary prompt's ComparisonReview instead of the current one.
- [warning/evidence] components/chat/ChatApp.tsx:384 and app/(site)/(application)/chat/ChatPageClient.tsx:3987-3992 (submit guard vs. `durableAttemptIds`): During post-reload durable recovery polling the panel reports `responding` (isSending includes `durableAttemptIds`) but `handleGlobalSubmit`'s only barrier is `isChatRuntimeStreaming`, which is false for a recovered attempt, so an enabled Send dispatches a second concurrent turn that persists a Message and consumes the durable draft before /api/chat can refuse it on concurrency.
- [warning/evidence] components/chat/ChatInput.tsx:1290-1295 and :3735-3745 (send button) with ChatPageClient.tsx:3987-3992: On the authenticated chat surface the Send button is now enabled for the whole duration of a live stream (isSending no longer disables it and `draftLocked` is false once `pendingSubmission` clears), but `handleGlobalSubmit` returns immediately on the streaming guard, so the click is silently dropped with no toast and no `sendDisabledReason`/aria-describedby explanation — the one case this composer has an established contract for.
- [warning/evidence] app/api/chat/route.ts:2190-2197 (`CHAT_CONTEXT_BUNDLE_ALREADY_CONSUMED`) vs. docs/policy/external-conversation-import-and-memory.md:1484 and :1859-1861: The bundle-replay refusal code changed from `CHAT_CONTEXT_BUNDLE_STALE` to a new `CHAT_CONTEXT_BUNDLE_ALREADY_CONSUMED` and lost `details.requiresPreflight`, which contradicts the §18 code table the policy marks 확정본 and the §22 sentence '둘 다 CHAT_CONTEXT_BUNDLE_STALE로 거절되지만', neither of which is in this task's writable scope, and leaves lib/memoryMetricsCore.ts:358 asserting the opposite of the code.
- [nit/judgement] app/api/chat/route.ts:2100-2107 (`INVALID_PERSISTENCE_TARGET` for chat-product turns): Making `sourceUserMessageId` mandatory for every non-deep-research chat-product turn means a tab still running the previous JS bundle across a deploy gets a hard 400 on every send with no localized copy or recovery affordance until the page is reloaded; degrading to the pre-durable path (skip the attempt) would fail open for that window without weakening the owner-verified binding for new clients.
...
```json
{
  "taskId": "chat-durable-recovery-v1",
  "round": 2,
  "reviewedDigest": "sha256:6b7f8382f7e39f14c39d0f6804abe3e8d1bc6d12b6a0f2e4d9cbb2ce04bc9a0a",
