# Independent review — task prompt-refiner-execution-pricing-drift-v1, round 0
...
## Change under review — digest sha256:6d16ffd111ad9082db7138957a83e0000c7401d7069ed0176677d71f95282c9b, commit 25f2f50de7b943105a06e19fbc8ffb9569ca2017

```diff
diff --git a/AGENTS.md b/AGENTS.md
index fafe4330..f9a9b9c3 100644
--- a/AGENTS.md
+++ b/AGENTS.md
...
index 0fc988bb..4d1bd591 100644
--- a/docs/ops/tomverse-chat-progress.md
+++ b/docs/ops/tomverse-chat-progress.md
@@ -938,7 +938,11 @@ AppSetting writer, flag 활성화와 품질·rollout 승인을 추가하지 않
...
@@ -957,14 +961,14 @@ provider adapter/API/model 호출, product mode, Router 배선, AppSetting write
...
index 3c7f7b85..1742f90b 100644
...
index 9272e3e3..147ce8d2 100644
...
index 477cd0e1..33541ec0 100644
...
+    | "cachedInputPriceMultiplier"
 ...
 /**
@@ -186,6 +193,27 @@ export const promptRefinerExecutionContractProblems = (input?: {
...
index 8ebf8f78..9c1b9309 100644
...
@@ -47,6 +50,46 @@ const candidate = (overrides = {}) => ({
     ...overrides,
 });
...
@@ -140,6 +183,86 @@ test("the checked-in catalogue and pricing must match the exact pin", () => {
     );
 });
...
- PASS `node --conditions=react-server --import tsx --test tests/promptRefinerExecutionContract.test.mjs tests/promptRefinerReceiptCore.test.mjs tests/promptRefinerSuggestion.test.mjs` (1127ms)
  # fail 0
  # cancelled 0
  # skipped 0
  # todo 0
  # duration_ms 1051.9668

## Guard results (run by the control program)

- PASS `npm run typecheck` (35368ms)
  > ai-chat-hub@0.1.0 typecheck
  > next typegen && tsc --noEmit --incremental false
  
  Generating route types...
  ✓ Types generated successfully
- PASS `npm run lint -- --quiet` (46910ms)
  > ai-chat-hub@0.1.0 lint
  > eslint --quiet
- PASS `npm run check:model-pricing` (677ms)
...
- PASS `npm run check:doc-references` (1277ms)
...
- PASS `npm run check:policy-section-references` (959ms)
...
- PASS `npm run check:encoding:strict` (1200ms)
...
- PASS `git diff --check 96e20b351335a802f53a33e9f2af96572c989705 HEAD -- . ':(exclude)docs/ops/cross-review/packages/prompt-refiner-execution-contract-v1' ':(exclude)docs/ops/cross-review/packages/prompt-refiner-execution-pricing-drift-v1'` (47ms)

## Findings from the superseded exchange prompt-refiner-execution-contract-v1, left open there (check each was addressed)

- [nit/evidence] lib/promptRefinerExecutionContract.ts:145-257 (promptRefinerExecutionContractProblems): The drift check reads only the static checked-in profile via getModelPricingProfile, so a per-model env price override (the same override that lib/modelPricing.ts resolveModelPricing honours at cost-resolution time) can raise the effective input/output rate far above the pinned 0.2/1.2 while the preregistration still reports no drift and keeps the frozen 24,916 microUSD per-request ceiling.

## Author's account (read last; a claim, not a finding)

Summary: 종결 exchange의 effective pricing drift finding을 후속 코드로 닫는다. 비용·예약 경로와 같은 resolveModelPricing()으로 input/output effective rate를 exact pin과 비교하고 환경 override 또는 runtime row drift를 dispatch 전 execution_contract_mismatch로 거절한다. 기존 on_hold 기록, provider/API/model 호출, runtime authority/adapter와 flag 상태는 변경하지 않는다.
...
```json
{
  ...
  "round": 0,
  "reviewedDigest": "sha256:6d16ffd111ad9082db7138957a83e0000c7401d7069ed0176677d71f95282c9b",
