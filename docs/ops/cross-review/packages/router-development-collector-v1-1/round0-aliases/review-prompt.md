# Independent review — task router-development-collector-v1-1, round 0
...
## Change under review — digest sha256:55cb20818f04feabb2ea0af86cb641da5caed13d329ea34d424558ac2b5b3420, commit 40ce5fc2179f040d3d8bf4a3e4c147f33af93133
...
+const run = (args, env = {}) => spawnSync(process.execPath, ["--conditions=react-server", "--import", "tsx", "--import", pathToFileURL(trap).href, "scripts/router-development-collect.mjs", ...args], { cwd: root, encoding: "utf8", maxBuffer: 20 * 1024 * 1024, env: { ...environment, ...env } });
+let preview;
+let proposal;
...
- PASS `npm run check:policy-section-references -- --since 88dc1d6bfd6cbac983d6d0c2609b7eb37df08910` (971ms)
...
- PASS `npm run check:model-pricing` (712ms)
...
- PASS `npm run check:router-context-window` (623ms)
...
- PASS `npm run check:context-window-register` (692ms)
...
const authored=paths("diff-tree","--no-commit-id","--name-only","-r",source);assert.equal(authored.length,11);for(const p of authored.filter(p=>p!=="package.json"))check(source,p);
...
```json
{
  "taskId": "router-development-collector-v1-1",
  "round": 0,
  "reviewedDigest": "sha256:55cb20818f04feabb2ea0af86cb641da5caed13d329ea34d424558ac2b5b3420",
