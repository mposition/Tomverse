# Independent review — task prompt-refiner-confirmatory-shadow-v4-integration-v5, round 0
...
최신 origin/develop 3df6f3d44002b5666a6a5f04095fa95309db0ddc 위에서 Prompt Refiner confirmatory shadow v4 PR의 최종 diff와 새 email suppression D-1 통합을 독립 검토한다. 이전 Claude Code Max review는 digest sha256:a0b83ea542f34d33722e0c6e4e31ae355e9928f221331df7cef781ae79617175에 approve/0 findings였지만 새 merge의 bytes를 자동 승인하지 않는다. 특히 tests/promptRefinerRuntimeSourceClosure.test.mjs 충돌 해소가 D-1과 v4 양쪽 의미를 보존하는지, 228개 dynamic access의 position-free inventory가 실제로 같고 position-bound digest만 정확히 repin되었는지, 188-file runtime closure와 stage/run source binding이 안전한지 검토한다. 기존 email suppression D-1 코드는 최신 develop에서 이미 검증된 base 변경이다. provider 호출, 유료 실행, stage/run 승인, flag 변경, 제품 노출, Router 결합 및 rollout은 하지 않는다. author=Codex, reviewer=Claude Code Max subscription CLI.

## Completion criteria

- 최신 develop D-1과 Prompt Refiner v4 변경을 모두 보존하고 충돌 마커가 없다.
- 동적 element access 228개가 그대로이며 path+expression text의 position-free digest와 새 position-bound digest가 각각 일치한다.
- v4 runtime source closure는 188개 고정 경로와 실제 import closure가 일치하고 stage/run admission이 exact bytes를 결속한다.
- v4 content-free terminal evidence, atomic audit, aggregate 재구성 및 제품/Router 권한 false가 유지된다.
- focused tests, TypeScript, ESLint와 CI가 통과하고 Claude 검토의 finding이 0건이다.

## Change under review — digest sha256:290f57a7e59518c8926c8beae97ec96ba13615436208ea0686f0bcec0de5d6c1
...
+  ]
+}
diff --git a/docs/ops/cross-review/packages/prompt-refiner-confirmatory-shadow-v4-integration-v5/authorization.md b/docs/ops/cross-review/packages/prompt-refiner-confirmatory-shadow-v4-integration-v5/authorization.md
new file mode 100644
index 00000000..df2258a5
--- /dev/null
+++ b/docs/ops/cross-review/packages/prompt-refiner-confirmatory-shadow-v4-integration-v5/authorization.md
@@ -0,0 +1,19 @@
...
+이 승인에 포함되지 않는다.
diff --git a/docs/ops/cross-review/packages/prompt-refiner-confirmatory-shadow-v4-integration-v5/task.json b/docs/ops/cross-review/packages/prompt-refiner-confirmatory-shadow-v4-integration-v5/task.json
new file mode 100644
index 00000000..ff8ceb31
--- /dev/null
+++ b/docs/ops/cross-review/packages/prompt-refiner-confirmatory-shadow-v4-integration-v5/task.json
@@ -0,0 +1,60 @@
+{
+  "taskId": "prompt-refiner-confirmatory-shadow-v4-integration-v5",
+  "requirement": "최신 origin/develop 3df6f3d44002b5666a6a5f04095fa95309db0ddc 위에서 Prompt Refiner confirmatory shadow v4 PR의 최종 diff와 새 email suppression D-1 통합을 독립 검토한다. 이전 Claude Code Max review는 digest sha256:a0b83ea542f34d33722e0c6e4e31ae355e9928f221331df7cef781ae79617175에 approve/0 findings였지만 새 merge의 bytes를 자동 승인하지 않는다. 특히 tests/promptRefinerRuntimeSourceClosure.test.mjs 충돌 해소가 D-1과 v4 양쪽 의미를 보존하는지, 228개 dynamic access의 position-free inventory가 실제로 같고 position-bound digest만 정확히 repin되었는지, 188-file runtime closure와 stage/run source binding이 안전한지 검토한다. 기존 email suppression D-1 코드는 최신 develop에서 이미 검증된 base 변경이다. provider 호출, 유료 실행, stage/run 승인, flag 변경, 제품 노출, Router 결합 및 rollout은 하지 않는다. author=Codex, reviewer=Claude Code Max subscription CLI.",
+  "completionCriteria": [
+    "최신 develop D-1과 Prompt Refiner v4 변경을 모두 보존하고 충돌 마커가 없다.",
+    "동적 element access 228개가 그대로이며 path+expression text의 position-free digest와 새 position-bound digest가 각각 일치한다.",
+    "v4 runtime source closure는 188개 고정 경로와 실제 import closure가 일치하고 stage/run admission이 exact bytes를 결속한다.",
...
+    "focused tests, TypeScript, ESLint와 CI가 통과하고 Claude 검토의 finding이 0건이다."
+  ],
+  "baseCommit": "3df6f3d44002b5666a6a5f04095fa95309db0ddc",
+  "writableScope": [
+    ".gitleaksignore",
+    "app/api/admin/prompt-refiner/shadow-run/execute/route.ts",
+    "app/api/admin/prompt-refiner/shadow-run/route.ts",
...
+    "docs/ops/cross-review/packages/prompt-refiner-confirmatory-shadow-v4-followup-v2/records",
+    "docs/ops/cross-review/packages/prompt-refiner-confirmatory-shadow-v4-integration-v3/records",
+    "docs/ops/cross-review/packages/prompt-refiner-confirmatory-shadow-v4-integration-v4/records"
+  ]
+}
...
@@ -202,6 +207,7 @@ const fixedNonImportPaths = Object.freeze([
   "tsconfig.json",
   "prisma/schema.prisma",
   "prisma/migrations/20260918130000_prompt_refiner_stage_admission/migration.sql",
+  "prisma/migrations/20260921100000_prompt_refiner_confirmatory_shadow_v4/migration.sql",
...
@@ -1064,6 +1070,13 @@ test("runtime source allowlist is exactly the deterministic local runtime import
...
@@ -1229,17 +1242,26 @@ test("TypeScript options and workspace metadata control local resolution", () =>
 });
...
+    /"schemaVersion":"prompt-refiner-shadow-execution-manifest-v2"[\s\S]*?"runtimeSource":\{"fileCount":(\d+),/
   ...
   assert.equal(
@@ -1247,6 +1269,11 @@ test("TypeScript and PostgreSQL enforce the identical ordered runtime source pat
...
@@ -1256,8 +1283,9 @@ test("operator-facing contracts name the enforced runtime source closure size",
...
@@ -1266,18 +1294,27 @@ test("operator-facing contracts name the enforced runtime source closure size",
   }
...
- PASS `node --conditions=react-server --import tsx --test --test-concurrency=1 tests/marketingAutomationAccess.test.mjs tests/promptRefinerRuntimeSourceClosure.test.mjs tests/gitleaksAllowlist.test.mjs` (4858ms)
  # fail 0
  # cancelled 0
  # skipped 0
  # todo 0
  # duration_ms 4762.2191

## Guard results (run by the control program)

- PASS `npm run typecheck` (100816ms)
  > ai-chat-hub@0.1.0 typecheck
  > next typegen && tsc --noEmit --incremental false
  
  Generating route types...
  ✓ Types generated successfully
- PASS `npm run lint` (78348ms)
  > ai-chat-hub@0.1.0 lint
  > eslint
- PASS `npm run check:enum-constraints` (1229ms)
...
- PASS `npm run check:protected-table-writers` (3777ms)
...
- PASS `npm run check:doc-references` (1802ms)
  > ai-chat-hub@0.1.0 check:doc-references
  > node scripts/check-doc-references.mjs
  
  Document reference check passed: 909 referenced path(s) across 121 instruction document(s), and 1019 path(s) named by comments across 3083 source file(s), all present.
...
```json
{
  "taskId": "prompt-refiner-confirmatory-shadow-v4-integration-v5",
  "round": 0,
  "reviewedDigest": "sha256:290f57a7e59518c8926c8beae97ec96ba13615436208ea0686f0bcec0de5d6c1",
