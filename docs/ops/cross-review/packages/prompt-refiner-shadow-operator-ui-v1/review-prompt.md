# Independent review — task prompt-refiner-shadow-operator-ui-v1, round 2
...
CHAT-01 Prompt Refiner의 동결된 staging shadow 실행을 owner가 제품의 기존 관리자 인증 세션 안에서 안전하게 조작할 최소 운영 화면을 추가한다. 화면은 stage preview/승인, 정확히 16건·retry 0·US$0.398656 run preview/승인, execution preview/명시적 실행을 순서대로 제공한다. 서버가 준 deployment·commit·digest·confirmation과 고정 model/cost/runtime 사실을 client에서도 fail-closed 검증하고 승인 body는 검증된 서버 값만 재사용한다. mount와 refresh는 GET-only이며 POST는 각각 명시적 버튼 동작에서만 발생한다. 실행 POST는 전송 전에 잠그고, transport unknown·서버 거부·malformed response에서는 자동 재시도나 같은 화면의 재전송을 금지한다. 오직 명시적 paused 응답만 남은 case의 수동 continuation을 다시 연다. page와 기존 API는 owner 및 recent-auth 경계를 유지하고 재인증 복귀 링크를 제공한다. prompt·refined prompt·모델 출력·provider error·credential은 화면에 표시하지 않으며 제품 Chat 트래픽과 연결하지 않는다. author는 Codex, reviewer는 Claude Code Max이고 승인된 --skip-preflight 예외 아래 Read/Grep/Glob only 및 strict MCP로 검토한다.

## Completion criteria

- 관리자 detail route는 owner가 아니면 notFound하고 최근 인증이 만료된 API 응답에는 기존 admin 재인증 복귀 경로를 렌더링한다.
- stage/run/execution preview parser가 정확한 환경·배포·commit·digest·model·tokenizer·timeout·retry·case 수·비용 상한·confirmation을 검증하고 drift나 malformed 응답을 fail-closed한다.
- mount와 status refresh는 GET-only이며 세 POST는 각각 사용자의 버튼 동작에서만 시작되고 검증된 preview의 exact binding body만 전송한다.
- execution POST는 전송 전에 잠기며 unknown/refusal/malformed 결과는 재시도를 열지 않고, retryCount=0·redispatched=0인 valid paused 응답만 수동 continuation을 다시 허용한다.
- 운영 화면과 응답 처리는 prompt·refined prompt·모델 출력·원시 provider 오류를 노출하지 않고 content-free status·count·고정 비용 사실만 표시한다.
- focused UI/core/navigation tests, 기존 shadow live-adapter tests, owner-only API server-contract tests, TypeScript, ESLint, encoding, production build와 diff whitespace 검사가 통과한다.
- Claude verdict는 package digest와 결속하고 finding마다 location, severity, basis와 reproduction을 제공한다. 최초 검토와 최대 두 번의 수정 검토 후 actionable finding이 남으면 on_hold다.

## Change under review — digest sha256:21ed48d0da73a51014390e3c86d8722e9b39189665078cd95ce08a13fdfa8eca

```diff
diff --git a/app/(site)/(application)/admin/prompt-refiner-shadow/page.tsx b/app/(site)/(application)/admin/prompt-refiner-shadow/page.tsx
new file mode 100644
index 00000000..9c49f237
--- /dev/null
+++ b/app/(site)/(application)/admin/prompt-refiner-shadow/page.tsx
@@ -0,0 +1,15 @@
...
+  );
+}
diff --git a/docs/ops/cross-review/packages/prompt-refiner-shadow-operator-ui-v1.authorization.md b/docs/ops/cross-review/packages/prompt-refiner-shadow-operator-ui-v1.authorization.md
new file mode 100644
index 00000000..0e779595
--- /dev/null
+++ b/docs/ops/cross-review/packages/prompt-refiner-shadow-operator-ui-v1.authorization.md
@@ -0,0 +1,28 @@
...
diff --git a/docs/ops/cross-review/packages/prompt-refiner-shadow-operator-ui-v1.task.json b/docs/ops/cross-review/packages/prompt-refiner-shadow-operator-ui-v1.task.json
new file mode 100644
index 00000000..1de634a4
--- /dev/null
+++ b/docs/ops/cross-review/packages/prompt-refiner-shadow-operator-ui-v1.task.json
@@ -0,0 +1,28 @@
+{
+  "taskId": "prompt-refiner-shadow-operator-ui-v1",
+  "requirement": "CHAT-01 Prompt Refiner의 동결된 staging shadow 실행을 owner가 제품의 기존 관리자 인증 세션 안에서 안전하게 조작할 최소 운영 화면을 추가한다. 화면은 stage preview/승인, 정확히 16건·retry 0·US$0.398656 run preview/승인, execution preview/명시적 실행을 순서대로 제공한다. 서버가 준 deployment·commit·digest·confirmation과 고정 model/cost/runtime 사실을 client에서도 fail-closed 검증하고 승인 body는 검증된 서버 값만 재사용한다. mount와 refresh는 GET-only이며 POST는 각각 명시적 버튼 동작에서만 발생한다. 실행 POST는 전송 전에 잠그고, transport unknown·서버 거부·malformed response에서는 자동 재시도나 같은 화면의 재전송을 금지한다. 오직 명시적 paused 응답만 남은 case의 수동 continuation을 다시 연다. page와 기존 API는 owner 및 recent-auth 경계를 유지하고 재인증 복귀 링크를 제공한다. prompt·refined prompt·모델 출력·provider error·credential은 화면에 표시하지 않으며 제품 Chat 트래픽과 연결하지 않는다. author는 Codex, reviewer는 Claude Code Max이고 승인된 --skip-preflight 예외 아래 Read/Grep/Glob only 및 strict MCP로 검토한다.",
+  "completionCriteria": [
+    "관리자 detail route는 owner가 아니면 notFound하고 최근 인증이 만료된 API 응답에는 기존 admin 재인증 복귀 경로를 렌더링한다.",
+    "stage/run/execution preview parser가 정확한 환경·배포·commit·digest·model·tokenizer·timeout·retry·case 수·비용 상한·confirmation을 검증하고 drift나 malformed 응답을 fail-closed한다.",
+    "mount와 status refresh는 GET-only이며 세 POST는 각각 사용자의 버튼 동작에서만 시작되고 검증된 preview의 exact binding body만 전송한다.",
...
+    "Claude verdict는 package digest와 결속하고 finding마다 location, severity, basis와 reproduction을 제공한다. 최초 검토와 최대 두 번의 수정 검토 후 actionable finding이 남으면 on_hold다."
+  ],
+  "baseCommit": "51c1648f0d721d975617260342c2b7fe1ae77aa7",
+  "writableScope": [
+    "app/(site)/(application)/admin/prompt-refiner-shadow/page.tsx",
+    "components/admin/AdminPromptRefinerShadowPanel.tsx",
+    "docs/ops/cross-review/packages/prompt-refiner-shadow-operator-ui-v1.authorization.md",
+    "docs/ops/cross-review/packages/prompt-refiner-shadow-operator-ui-v1.task.json",
+    "lib/adminMessages/promptRefinerShadow.ts",
+    "lib/adminNavigation.ts",
+    "lib/adminNavigationLocale.ts",
+    "lib/promptRefinerShadowOperatorCore.ts",
+    "tests/adminNavigation.test.mjs",
...
+  },
+});
diff --git a/lib/adminNavigation.ts b/lib/adminNavigation.ts
index ec7cb077..cb6a06bd 100644
--- a/lib/adminNavigation.ts
+++ b/lib/adminNavigation.ts
@@ -600,6 +600,20 @@ export const findAdminNavItem = (pathname: string): AdminNavItem | null =>
...
+  },
   {
     id: "user-detail",
     pattern: /^\/admin\/users\/[^/]+$/,
diff --git a/lib/adminNavigationLocale.ts b/lib/adminNavigationLocale.ts
index 90e2e560..8c2867b4 100644
--- a/lib/adminNavigationLocale.ts
+++ b/lib/adminNavigationLocale.ts
...
+  confirmation: preview.confirmation,
+});
diff --git a/tests/adminNavigation.test.mjs b/tests/adminNavigation.test.mjs
index 8001a866..f35dc392 100644
--- a/tests/adminNavigation.test.mjs
+++ b/tests/adminNavigation.test.mjs
...
- PASS `node --conditions=react-server --experimental-test-module-mocks --no-warnings=ExperimentalWarning --import tsx --test --test-concurrency=1 tests/promptRefinerShadowOperatorCore.test.mjs tests/adminPromptRefinerShadowOperator.test.mjs tests/adminNavigation.test.mjs tests/promptRefinerShadowRunContract.test.mjs tests/promptRefinerShadowTokenizer.test.mjs tests/promptRefinerShadowLiveAdapter.test.mjs tests/promptRefinerShadowRunner.test.mjs tests/server-contract/admin-prompt-refiner-shadow-stage-route.test.ts tests/server-contract/admin-prompt-refiner-shadow-run-route.test.ts tests/server-contract/admin-prompt-refiner-shadow-execution-route.test.ts` (8763ms)
  # fail 0
  # cancelled 0
  # skipped 0
  # todo 0
  # duration_ms 8675.6089

## Guard results (run by the control program)

- PASS `npm run typecheck` (51094ms)
  > ai-chat-hub@0.1.0 typecheck
  > next typegen && tsc --noEmit --incremental false
  
  Generating route types...
  ✓ Types generated successfully
- PASS `npx eslint lib/promptRefinerShadowOperatorCore.ts components/admin/AdminPromptRefinerShadowPanel.tsx lib/adminMessages/promptRefinerShadow.ts tests/promptRefinerShadowOperatorCore.test.mjs tests/adminPromptRefinerShadowOperator.test.mjs tests/adminNavigation.test.mjs lib/adminNavigation.ts lib/adminNavigationLocale.ts --max-warnings=0` (3723ms)
- PASS `npm run check:encoding` (1461ms)
  > ai-chat-hub@0.1.0 check:encoding
  > node scripts/check-text-encoding.mjs
  
  Text encoding check passed. No mojibake markers found.
- PASS `npm run build -- --webpack` (84984ms)
  ƒ Proxy (Middleware)
...
- [warning/evidence] components/admin/AdminPromptRefinerShadowPanel.tsx:292-316 (execute): The post-POST status GET (`await readExecution(run!)`, line 311) sits inside the same `try` as the execution POST, so a transport failure on that follow-up *read* is reported as `m.transportUnknown` ("The execution request outcome is unknown. Do not resend it...") even though the POST outcome was received and parsed, and on a `paused` result the lock was already released at line 310 — leaving an enabled "Resume remaining cases" button underneath a "do not resend" alert.
- [nit/judgement] lib/promptRefinerShadowOperatorCore.ts:266-315 (promptRefinerExecutionPreviewProblems): The execution preview is bound to the run only by `runId` and `runContractDigest` — both deployment-independent frozen constants — and `approvalExpiresAt` is accepted as any string, even though the run preview carries `stageApprovalExpiresAt` and the server guarantees `run.approvalExpiresAt === stage.approvalExpiresAt`, so an execution response describing a different authority window is not fail-closed on the client.
- [nit/evidence] tests/promptRefinerShadowOperatorCore.test.mjs:13-95 (fixtures): Every fixture field is derived from `PROMPT_REFINER_SHADOW_OPERATOR_CONTRACT` itself (`timeoutMs: contract.timeoutMs`, `modelId: contract.modelId`, ...), so the suite is self-referential and cannot detect divergence between this duplicated client contract and the server constants it mirrors; a stale client constant would only surface as a fail-closed dead screen in staging.
...
```json
{
  "taskId": "prompt-refiner-shadow-operator-ui-v1",
  "round": 2,
  "reviewedDigest": "sha256:21ed48d0da73a51014390e3c86d8722e9b39189665078cd95ce08a13fdfa8eca",
