...
`main`의 `docs/policy/development-agent-orchestration.md`는 고위험 작업의 승인
원칙만 정의하며, Agent escalation을 확정하는 별도 승인 계약은 아직 없다. 따라서
2인 관리자 `AdminActionApproval`을 단일 사람의 Agent 승인으로 해석하지 않고,
