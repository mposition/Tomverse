...
- **template revision**: `2026-08-24f` — 항목이 바뀌면 이 값을 올리고, 실행
  ...
  다음 회차가 그것을 다시 확인할 자리가 없으면 같은 결함이 조용히 돌아옵니다.

  `2026-08-24d` → `2026-08-24e`: §F-1의 "선택 전에"가 "radio를 누르기 전"으로
  읽혔습니다. 결과 문장은 대상을 고른 직후 같은 화면에 나오며, 그것이 설계
  의도입니다 — wizard 주석이 "stated on the screen that offers it rather than
  at the end"라고 적습니다. 낡은 읽기대로면 의도대로 동작하는 화면이 차단
  항목에서 `fail`이 됩니다.
...
- [ ] **(차단)** 병합을 고르면 무엇이 바뀌는지가 **병합을 제안한 그 화면에서,
      다음 단계로 넘어가기 전에** 나온다 — 새 revision이 이름·설명·지시문·
      모델·문서 목록을 대체하고, 기존 문서는 profile에 남지만 새 revision에는
      들어가지 않는다는 것.

      **radio를 누르기 전에 미리 떠 있어야 한다는 뜻이 아닙니다.** 문장은
      대상을 고른 직후 같은 화면에 나타나며, 그 화면에서 대상을 되돌릴 수
      있고 7단계로 넘어가기 전입니다 — 지키려는 것은 "마지막 확인 화면에서야
      알게 되는 일이 없다"입니다. 6단계에서 문장이 보이면 `pass`입니다
- [ ] 대상을 바꾸면 업로드 경계 동의가 **풀린다** (문장이 create와 merge에서
      다르므로)
- [ ] 손실 보고서의 개수가 정답지와 일치하고, **이름이 붙는 손실은 이름도
      일치한다** — script 경로, 라이선스, 그리고 **이 가져오기가 쓰지 않는
      종류의 파일**. 마지막 것은 한동안 개수만 냈고, 동의 체크박스가 붙은
      화면에서 이름 없이 잃는 것에 동의하게 만들었습니다
