...
- **서로 다른 제출 항목의 반복은 검토자의 반복이다.** 추출 오류가 아니므로 계속
  채점되고 `duplicates`로 센다. v3 이후 `duplicates`가 세는 것은 이것뿐이다.
...
- **왜 굵은 gold가 비싼가:** 같은 검토자의 같은 문장이, 등록 방식만으로 recall
  1.00과 0.50으로 갈린다. 굵은 gold 아래에서는 반만 말한 검토자가 만점을 받는다.
  `sufficiency`로 그 반쪽을 불충분이라 판정할 수는 있지만 **D를 대신하지 못한다**
  — 그 판정은 검토자를 벌할 뿐 놓친 두 번째 요구를 FN으로 세지 않는다.
