...
export function ContinuedConversationWorkspace({
    ...
    const send = useCallback(async () => {
        ...
        const modelId = conversation.selectedModels[0];
        if (!modelId) {
        ...
        try {
            // Pre-save, exactly as the chat workspace does: the user's own
            // message is a row before a provider is asked anything, so a turn
            // that fails still leaves what they wrote.
            ...
            const response = await fetch("/api/chat", {
                ...
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    // The transcript this turn is answered from. The imported
                    // excerpt is NOT in here: it reaches the model as a server
                    // built system block that the server itself priced, and a
                    // client that could put imported text in `messages` could
                    // put anything there (§5).
                    messages: [
            ...
            // The same reader the chat workspace uses, so the out-of-band
            // markers (keepalives, routing signals, the search trailer) are
            // stripped here exactly as they are there rather than painted into
            // the answer.
            await consumeChatStream({
                reader: response.body.getReader(),
                liveness: { noteKeepalive: () => {}, noteVisibleChunk: () => {} },
                onProgress: (progress) => {
                    setConversation((current) =>
                        current
                            ? {
            ...
            // decided the answer's final text, its status and its model.
            void loadAll();
        }
    ...
    return (
        ...
        >
            ...
            <ol className="space-y-3" data-testid="continuation-messages">
