...
// The first is size: the page covers five sections and seven locales. The
// second is the defect this file was created to close -- the previous copy
// table was `{ en } & Partial<Record<Language, LandingCopy>>`, so a key that
// no component read (and a locale that never overrode one) was
// indistinguishable from a key that shipped. `Record<Language, LandingCopy>`
// with no spread means a missing translation is a type error.
//
...
// ## Landing V2 shape
//
// The V1 table described eight sections and told the "one question, several
// answers, one review" story four separate times: the hero demonstration, a
// three-column model row, a workflow diagram, and a numbered step list. The
// duplication was structural, so it is removed here rather than in the
// components. `loop` is now the single place that story is told, and the
// demonstration's own stage captions and disclosure moved under `preview`,
// which is what renders them. `proof`, `compare.steps` and
// `catalogue.modelFinderLead` are gone because a second telling had nowhere
// left to render.
//
// No customer-facing string in this file uses an em dash or an en dash.
...
export type LandingCopy = {
  /** The product label. Tomverse is the brand; this names what is on offer. */
  badge: string;
  brandNote: string;
  title: string;
  description: string;
  primaryCta: string;
  signedInCta: string;
  /** The one text link beside the primary CTA, pointing at the walkthrough. */
  secondaryCta: string;
   ...
   * says about AI Review itself lives here too, because splitting the promise
   * from its limits across two sections is what let the limits drift.
   */
  loop: {
    eyebrow: string;
    title: string;
    ...
    cases: Array<{ title: string; description: string; link: string }>;
  };
  evidence: {
    eyebrow: string;
    title: string;
  ...
  support: {
    title: string;
    description: string;
    /** The four things that carry a comparison forward. */
    items: LandingCard[];
    /** Helpers that must not compete with the four above for attention. */
    secondary: LandingCard[];
    accountNote: string;
    cta: string;
  };
  catalogue: {
...
const en: LandingCopy = {
  ...
  preview: {
    ...
    stages: [
      ...
      {
        title: "AI Review",
        caption: "Common ground, contradictions, missing points, what to verify.",
      },
  ...
  loop: {
    ...
    description: "Choose the models, send the question once, and read the answers next to each other. Two ways to make sense of them: a quick difference summary, or a full AI Review.",
    steps: [
      {
        title: "Ask once",
        description: "Choose up to three models and send one prompt or supported file.",
      },
      {
        title: "Compare the answers",
        description: "Read different strengths side by side without copying between tabs.",
      },
      {
        title: "Run AI Review",
        description: "Structure agreements, conflicts, omissions, and what to verify next.",
      },
    ],
    quickSummary: {
      title: "Quick difference summary",
      description: "Once two or more answers are in, have AI summarise where they diverge, without reading all of them line by line. Built for a fast read, not a full review.",
      condition: "Low credit cost. Usage allowances differ between guest and signed-in use.",
    },
    aiReviewBridge: "When the differences matter enough to work through, AI Review takes the same answers further, section by section, with quotes.",
    ...
    dualReviewer: "When a second reviewer from a different provider is available, the comparison runs twice independently and the review reports where the two reviewers agreed and where they did not.",
    terminologyNote: "AI Review is the product name; in the app the action itself reads “AI answer cross-review”.",
    reviewBoundary: "AI Review compares only the supplied answers. It does not browse the web, prove facts, or declare a correct winner. When an item needs checking, you can run a separate web check on it from the review. Important claims still need current primary sources, testing, or qualified professional review.",
    casesTitle: "Three jobs where a second perspective matters",
    casesDescription: "Each example starts with a concrete artifact and ends with something reviewable. These are controlled product examples, not invented customer testimonials.",
    cases: [
      {
  ...
  evidence: {
    ...
    description: "Comparing several answers narrows the question. These are the tools for the part that comes after: where a claim came from, and whether it still holds.",
    webSearch: {
      title: "Web search",
      description: "Turn web search on for a conversation so supported models answer using current sources and return citations. Choose off, a suggestion when a question looks time-sensitive, or search on every turn.",
      condition: "Support varies by model. Extra credits apply only when a search actually runs.",
    },
    deepResearch: {
      title: "Deep Research",
      description: "Send one question to an extended research run across many web sources. It runs as its own long job rather than a normal chat reply, and asks you to confirm before it starts.",
      condition: "Pro plan and above. Uses credits.",
    },
    sourceGrounding: {
      title: "Source grounding",
      description: "AI Review quotes the answers it is comparing. Each quote is matched back against the answer it came from, and the share that matched exactly is shown with the review.",
    ...
    itemVerification: {
      ...
      description: "For an item the review flagged as needing verification, run a separate web check on just that item and see whether current sources support it, contradict it, or are inconclusive.",
      condition: "A separate opt-in step from the review itself. Signed-in, uses a search model, and is charged separately.",
    },
    footnote: "Credit costs depend on the models involved and the length of the request. The pricing page has the current weights.",
    cta: "See pricing",
  },
  support: {
    title: "Keep the work moving after the comparison.",
    description: "Tomverse keeps the useful context around each answer, so a comparison can become a document, a follow-up, or a result your team can revisit.",
    items: [
      {
  ...
  catalogue: {
    ...
    description: "One catalogue, one selection step. Pick the models that suit the question instead of opening a tab per provider.",
  ...
  trust: {
    ...
    description: "Tomverse makes storage, locks, and sharing behavior visible. AI providers still receive the prompts needed to generate a response.",
    items: [
      {
  ...
  pricing: {
    ...
    description: "The homepage shows only the essentials. Model weights, credit examples, annual billing, add-on credits, and Fair Use details are explained on the pricing page.",
    plans: [
      {
        id: "free",
        title: "Free",
        blurb: "Light everyday use and trying advanced models.",
      },
  ...
  faqs: [
    {
      ...
      answer: "Yes. Without signing in, you can already compare 3 AI models side by side on the same question. Guest use has its own daily and monthly allowance, and a quick verification step before the first message. A Free account unlocks a broader model catalogue, higher usage limits, saved conversations, and other signed-in workflows within the plan limits.",
    },
    {
      question: "Which models can I compare?",
      answer: "The catalogue spans major providers including OpenAI, Anthropic, Google, and Perplexity, and changes as models are added or retired. The models page has the current list, and the live status page is the source of current service state.",
    },
    {
      question: "How is my data handled?",
      answer: "Tomverse applies attachment limits, locked-chat controls, and read-only share snapshots. Selected AI providers still receive the request content needed to answer; review the Safety page for the complete boundaries.",
    },
  ],
...
const ko: LandingCopy = {
  ...
  primaryCta: "무료로 비교 시작하기",
  signedInCta: "Review 열기",
  ...
  loop: {
    ...
    description: "모델을 고르고 질문을 한 번만 보내면 답변을 나란히 읽을 수 있습니다. 정리하는 방법은 두 가지입니다. 빠른 차이 요약, 그리고 본격적인 AI Review입니다.",
    steps: [
      { title: "한 번 질문", description: "최대 3개 모델을 선택하고 질문 또는 지원되는 파일을 보냅니다." },
      { title: "답변 비교", description: "여러 탭에 복사하지 않고 모델별 강점을 나란히 읽습니다." },
      { title: "AI Review", description: "합의, 충돌, 누락과 다음 검증 항목을 구조화합니다." },
    ],
    quickSummary: {
      title: "빠른 차이 요약",
      description: "답변이 두 개 이상 모이면, 전부 정독하지 않고도 어디서 갈라지는지 AI가 요약해 줍니다. 정밀 검토가 아니라 빠르게 훑기 위한 기능입니다.",
      condition: "크레딧 소모가 적습니다. 사용 가능 횟수는 비회원과 로그인 상태에 따라 다릅니다.",
    },
    aiReviewBridge: "차이를 더 깊이 따져야 할 때는 AI Review가 같은 답변을 항목별로, 인용과 함께 더 멀리 끌고 갑니다.",
    ...
    dualReviewer: "다른 공급자의 두 번째 검토 모델을 쓸 수 있을 때는 비교가 독립적으로 두 번 실행되고, 두 검토자가 어디에서 일치했고 어디에서 갈렸는지 함께 표시합니다.",
    terminologyNote: "AI Review는 제품 이름이며, 앱 안에서는 같은 동작이 “AI 답변 교차검토”로 표시됩니다.",
    reviewBoundary: "AI Review는 제공된 답변끼리만 비교합니다. 스스로 웹 검색을 하거나 사실을 확정하거나 정답을 고르지 않습니다. 확인이 필요한 항목은 검토 결과에서 웹 확인을 따로 실행할 수 있습니다. 중요한 주장은 최신 1차 출처, 테스트 또는 자격 있는 전문가를 통해 확인해야 합니다.",
  ...
  evidence: {
    ...
    description: "여러 답변을 비교하면 질문이 좁혀집니다. 그다음이 중요합니다. 그 주장이 어디에서 왔는지, 지금도 유효한지 확인하는 기능들입니다.",
    webSearch: {
      title: "웹 검색",
      description: "대화 단위로 웹 검색을 켜면 지원 모델이 최신 출처를 참고해 답하고 인용을 함께 돌려줍니다. 끄기, 최신 정보가 필요해 보일 때 제안받기, 매 턴 검색하기 중에서 고를 수 있습니다.",
      condition: "지원 범위는 모델마다 다릅니다. 추가 크레딧은 실제로 검색이 실행된 경우에만 사용됩니다.",
    },
    deepResearch: {
      title: "Deep Research",
      description: "하나의 질문을 여러 웹 출처에 걸친 확장 리서치로 보냅니다. 일반 채팅 답변이 아니라 별도의 장시간 작업으로 실행되며, 시작 전에 확인을 요청합니다.",
      condition: "Pro 플랜 이상에서 사용할 수 있고 크레딧을 사용합니다.",
    },
    sourceGrounding: {
      title: "근거 일치율",
      description: "AI Review는 비교 중인 답변을 인용합니다. 각 인용문을 원본 답변과 다시 대조해, 정확히 일치한 비율을 검토 결과와 함께 표시합니다.",
      condition: "인용이 원문과 맞는지를 측정하는 값이며, 사실 정확도나 주장이 참일 확률이 아닙니다.",
    },
    itemVerification: {
      title: "항목별 웹 확인",
      description: "검토 결과가 확인이 필요하다고 표시한 항목은 그 항목만 따로 웹으로 확인해, 최신 출처가 뒷받침하는지·반박하는지·판단 불가인지 볼 수 있습니다.",
      condition: "검토 생성과는 분리된 선택 작업입니다. 로그인 상태에서 검색 모델을 사용하며 크레딧이 별도로 듭니다.",
    },
    footnote: "크레딧 소모량은 사용한 모델과 요청 길이에 따라 달라집니다. 현재 차감 기준은 요금 페이지에 있습니다.",
    cta: "요금 보기",
  },
  support: {
  ...
  trust: {
    ...
    items: [
      ...
      { title: "첨부파일 제한", description: "첨부하는 모든 파일에 형식, 용량, 메시지당 개수 제한이 적용됩니다." },
    ],
    metricPeriod: "최근 30일",
  ...
  faqs: [
    {
      ...
      answer: "네. 로그인 없이도 3개의 AI 모델로 같은 질문에 대한 답변을 바로 비교해볼 수 있습니다. 비회원 이용에는 자체 일간·월간 사용량이 있고, 첫 메시지 전에 간단한 확인 절차를 거칩니다. Free 계정을 만들면 더 넓은 모델 카탈로그, 높은 사용량 한도, 대화 저장 및 로그인 전용 기능을 사용할 수 있습니다.",
    },
    {
      question: "어떤 모델을 비교할 수 있나요?",
      answer: "카탈로그는 OpenAI, Anthropic, Google, Perplexity를 비롯한 주요 공급자를 포함하며, 모델이 추가되거나 종료되면서 바뀝니다. 현재 목록은 모델 페이지에 있고, 실시간 상태 페이지에서 현재 서비스 상태를 확인할 수 있습니다.",
    },
    {
      question: "데이터는 어떻게 처리되나요?",
      answer: "첨부파일 제한, 대화 잠금, 읽기 전용 공유 스냅샷을 적용합니다. 선택한 AI 공급자는 답변에 필요한 요청 내용을 처리하므로 전체 범위는 안전 페이지에서 확인하세요.",
    },
  ],
...
const zh: LandingCopy = {
  ...
  loop: {
    eyebrow: "核心流程",
    title: "一个问题同时发给多个模型。",
    description: "选好模型，只提问一次，就能并排阅读回答。整理方式有两种：快速差异摘要，或完整的 AI Review。",
    steps: [
      { title: "提问一次", description: "最多选择三个模型并发送问题或受支持的文件。" },
      { title: "比较回答", description: "无需在多个标签页复制，即可并排阅读。" },
      { title: "运行 AI Review", description: "整理一致、冲突、遗漏与下一步核实。" },
    ],
    quickSummary: {
      title: "快速差异摘要",
      description: "有两个以上回答后，不必逐行细读，让 AI 概括它们在哪里出现分歧。它为快速浏览而设计，不是完整审查。",
      condition: "积分消耗较低。可用次数在未登录与登录状态下不同。",
    },
    aiReviewBridge: "当差异值得深入处理时，AI Review 会把同样的回答按板块、带引文地推进一步。",
    reviewModesLabel: "选择审查的侧重点",
    reviewModes: ["均衡: 一致、差异、遗漏与实用性并重", "依据: 侧重缺乏依据的说法、冲突与依据缺口", "行动: 侧重可执行的下一步、取舍与风险"],
    dualReviewerLabel: "两位独立审查者",
    dualReviewer: "当可以使用来自不同供应商的第二个审查模型时，比较会独立运行两次，并显示两位审查者在哪里一致、在哪里分歧。",
    terminologyNote: "AI Review 是产品名称；在应用中该操作显示为“AI 回答交叉审查”。",
    reviewBoundary: "AI Review 只比较提供的回答。它不会自行浏览网页、判定事实，也不会选出唯一正确答案。需要核实的项目，可以在审查结果中单独执行网页核实。重要说法仍需通过最新一手来源、测试或专业人员核实。",
    casesTitle: "三个值得获得第二视角的任务",
    casesDescription: "每个示例都从具体材料开始，以可审查结果结束；这是受控产品示例，不是虚构客户评价。",
    cases: [
      { title: "交叉审查决策", description: "比较多个模型对发布、政策或规划的建议。", link: "查看 AI 回答审查" },
      {
        title: "分析 PDF 或文档",
        description: "不必把摘录逐个粘贴给每个模型，直接就同一份 18 页文档向多个模型提问。",
        link: "探索文件分析",
      },
      { title: "审查代码或计划", description: "比较最小补丁、取舍、失败路径和遗漏测试。", link: "比较 AI 模型" },
    ],
  },
  evidence: {
    eyebrow: "依据与时效",
    title: "不只比较，还能核对。",
    description: "比较多个回答会收窄问题。接下来才是关键：这个说法从哪里来，现在是否仍然成立。",
    webSearch: {
      title: "网页搜索",
      description: "为某个对话开启网页搜索，支持的模型会参考当前来源作答并返回引用。可选择关闭、在问题看起来需要最新信息时给出建议，或每轮都搜索。",
      condition: "支持范围因模型而异。只有实际执行搜索时才会额外消耗积分。",
    },
    deepResearch: {
      title: "Deep Research",
      description: "把一个问题交给跨越多个网页来源的扩展研究。它作为独立的长时间任务运行，而不是普通聊天回复，并在开始前请你确认。",
      condition: "Pro 套餐及以上可用，会消耗积分。",
    },
    sourceGrounding: {
      title: "依据匹配率",
      description: "AI Review 会引用它正在比较的回答。每条引文都会与其来源回答重新比对，并随审查结果显示完全匹配的比例。",
      condition: "它衡量的是引文是否与原文一致，而不是事实准确度，也不是某个说法为真的概率。",
    },
    itemVerification: {
      title: "逐项网页核实",
      description: "对审查标记为需要核实的项目，可以只针对该项单独执行网页核实，查看当前来源是支持、反驳还是无法判定。",
  ...
  support: {
    ...
    items: [
      ...
      { title: "定向追问", description: "暂停看完的面板，只向一个模型继续提问，其他回答和原始比较都会保留。" },
      { title: "项目与记录", description: "按项目整理对话并检索历史，避免在多个标签页重复建立上下文。", condition: "需要账户。" },
      {
        title: "分享与导出",
  ...
  trust: {
    ...
    items: [
      { title: "锁定对话", description: "保护敏感对话，并在受保护操作前要求解锁验证。", condition: "需要账户。" },
      { title: "只读分享", description: "分享不会暴露后续对话更新的快照。", condition: "需要账户。分享链接可能会过期。" },
      { title: "附件限制", description: "所有附件都受文件类型、大小和单条消息数量限制。" },
    ],
  ...
  faqs: [
    {
      ...
      answer: "可以。无需登录即可同时比较 3 个 AI 模型对同一问题的回答。未登录使用有各自的每日与每月额度，并在首条消息前需要完成一次快速验证。Free 账户可解锁更广泛的模型库、更高的使用额度、对话保存以及其他登录专属功能。",
    },
    {
      question: "可以比较哪些模型？",
      answer: "模型目录涵盖 OpenAI、Anthropic、Google、Perplexity 等主要供应商，并会随着模型上线或下线而变化。当前列表见模型页，当前服务状态请以实时状态页为准。",
    },
    {
      question: "数据如何处理？",
      answer: "Tomverse 提供附件限制、对话锁和只读分享快照。所选 AI 供应商仍会处理生成回答所需的内容；完整边界请查看安全页。",
    },
  ],
...
const fr: LandingCopy = {
  ...
  primaryCta: "Comparer gratuitement",
  signedInCta: "Ouvrir Review",
  ...
  preview: {
    ...
    stages: [
      ...
      {
        title: "AI Review",
        caption: "Accords, contradictions, omissions, éléments à vérifier.",
      },
      {
  ...
  loop: {
    ...
    description: "Choisissez les modèles, posez la question une seule fois et lisez les réponses côte à côte. Deux façons de les exploiter : un résumé rapide des différences, ou une revue AI Review complète.",
    steps: [
      {
        title: "Une seule question",
        description: "Choisissez jusqu’à trois modèles et envoyez un prompt ou fichier pris en charge.",
      },
      {
        title: "Comparez",
        description: "Lisez les forces de chaque réponse côte à côte.",
      },
      {
        title: "Lancez AI Review",
        description: "Structurez accords, conflits, omissions et vérifications.",
      },
    ],
    quickSummary: {
      title: "Résumé rapide des différences",
      description: "Dès que deux réponses sont là, faites résumer par l’IA les points de divergence, sans tout relire ligne à ligne. Conçu pour une lecture rapide, pas pour une revue complète.",
      condition: "Coût en crédits faible. Les quotas diffèrent entre usage invité et connecté.",
    },
    aiReviewBridge: "Quand les écarts méritent d’être creusés, AI Review reprend les mêmes réponses, section par section et avec citations.",
    ...
    dualReviewer: "Lorsqu’un second relecteur d’un autre fournisseur est disponible, la comparaison est exécutée deux fois de façon indépendante, et la revue indique où les deux relecteurs convergent ou divergent.",
    terminologyNote: "AI Review est le nom produit ; dans l’application, l’action s’appelle « AI answer cross-review ».",
    reviewBoundary: "AI Review compare uniquement les réponses fournies. Il ne navigue pas de lui-même, ne prouve pas les faits et ne désigne pas de gagnant. Pour un élément à vérifier, vous pouvez lancer une vérification web distincte depuis la revue. Les enjeux importants exigent des sources primaires à jour, des tests ou un avis qualifié.",
  ...
  evidence: {
    ...
    description: "Comparer plusieurs réponses resserre la question. Voici les outils pour la suite : d’où vient une affirmation, et tient-elle toujours.",
    webSearch: {
      title: "Recherche web",
      description: "Activez la recherche web pour une conversation : les modèles compatibles répondent à partir de sources actuelles et renvoient des citations. Au choix : désactivée, suggérée quand la question semble dépendre de l’actualité, ou à chaque tour.",
    ...
    deepResearch: {
      title: "Deep Research",
      description: "Envoyez une question vers une recherche étendue sur de nombreuses sources web. Elle s’exécute comme une tâche longue distincte, pas comme une réponse de chat, et demande confirmation avant de démarrer.",
      condition: "À partir du plan Pro. Consomme des crédits.",
    },
    sourceGrounding: {
      title: "Ancrage des citations",
      description: "AI Review cite les réponses qu’il compare. Chaque citation est recomparée à la réponse dont elle provient, et la part qui correspond exactement est affichée avec la revue.",
    ...
    itemVerification: {
      ...
      description: "Pour un élément signalé comme à vérifier, lancez une vérification web sur ce seul élément et voyez si les sources actuelles le soutiennent, le contredisent ou ne tranchent pas.",
      condition: "Étape optionnelle, distincte de la revue elle-même. Compte connecté, modèle de recherche, facturation séparée.",
    },
    footnote: "Le coût en crédits dépend des modèles utilisés et de la longueur de la demande. Les poids actuels figurent sur la page Tarifs.",
    cta: "Voir les tarifs",
  },
  support: {
    ...
    plans: [
      {
        id: "free",
        title: "Free",
        blurb: "Usage quotidien léger et essai de modèles avancés.",
      },
  ...
  faqs: [
    {
      ...
      answer: "Oui. Sans connexion, vous pouvez déjà comparer 3 modèles d’IA côte à côte sur la même question. L’usage invité dispose de ses propres quotas quotidien et mensuel, et d’une vérification rapide avant le premier message. Un compte Free débloque un catalogue plus large, des limites plus élevées, la sauvegarde des conversations et d’autres fonctionnalités réservées aux comptes connectés.",
    },
    {
      question: "Quels modèles puis-je comparer ?",
      answer: "Le catalogue couvre les principaux fournisseurs, dont OpenAI, Anthropic, Google et Perplexity, et évolue au fil des ajouts et retraits. La page Modèles contient la liste actuelle, et la page d’état fait foi pour l’état du service.",
    },
    {
      question: "Comment mes données sont-elles traitées ?",
      answer: "Tomverse applique limites de pièces jointes, verrouillage et instantanés en lecture seule. Les fournisseurs sélectionnés traitent toujours le contenu nécessaire à la réponse.",
    },
  ],
...
const de: LandingCopy = {
  ...
  primaryCta: "Kostenlos vergleichen",
  signedInCta: "Review öffnen",
  ...
  preview: {
    ...
    stages: [
      ...
      {
        title: "AI Review",
        caption: "Gemeinsamkeiten, Widersprüche, Lücken, Prüfpunkte.",
      },
      {
  ...
  loop: {
    ...
    description: "Modelle wählen, die Frage einmal senden, die Antworten nebeneinander lesen. Zwei Wege zur Auswertung: eine schnelle Unterschiedszusammenfassung oder ein vollständiges AI Review.",
    steps: [
      {
        title: "Einmal fragen",
        description: "Bis zu drei Modelle wählen und Prompt oder unterstützte Datei senden.",
      },
      {
        title: "Antworten vergleichen",
        description: "Stärken nebeneinander lesen, ohne Tabs zu kopieren.",
      },
      {
        title: "AI Review starten",
        description: "Übereinstimmungen, Konflikte, Lücken und Prüfbedarf ordnen.",
      },
    ],
    quickSummary: {
      title: "Schnelle Unterschiedszusammenfassung",
      description: "Sobald zwei Antworten vorliegen, fasst die KI zusammen, wo sie auseinandergehen, ohne alles Zeile für Zeile zu lesen. Für den schnellen Überblick gedacht, nicht für die vollständige Prüfung.",
      condition: "Geringer Kreditverbrauch. Die Kontingente unterscheiden sich zwischen Gast- und angemeldeter Nutzung.",
    },
    aiReviewBridge: "Wenn die Unterschiede wirklich zählen, führt AI Review dieselben Antworten weiter, abschnittsweise und mit Zitaten.",
    ...
    dualReviewer: "Steht ein zweiter Prüfer eines anderen Anbieters zur Verfügung, läuft der Vergleich zweimal unabhängig, und die Prüfung zeigt, wo beide übereinstimmten und wo nicht.",
    terminologyNote: "AI Review ist der Produktname; in der App heißt die Aktion „AI answer cross-review“.",
    reviewBoundary: "AI Review vergleicht nur gelieferte Antworten. Es durchsucht von sich aus nicht das Web, beweist keine Fakten und bestimmt keinen Gewinner. Für einen prüfbedürftigen Punkt können Sie aus der Prüfung heraus einen separaten Web-Check starten. Wichtige Aussagen brauchen weiterhin aktuelle Primärquellen, Tests oder qualifizierte Prüfung.",
  ...
  evidence: {
    ...
    description: "Mehrere Antworten zu vergleichen schärft die Frage. Diese Werkzeuge decken den Teil danach ab: Woher stammt eine Aussage, und gilt sie noch?",
    webSearch: {
      title: "Websuche",
      description: "Aktivieren Sie die Websuche für eine Unterhaltung, damit unterstützte Modelle mit aktuellen Quellen antworten und Zitate liefern. Wählbar: aus, ein Hinweis bei zeitkritischen Fragen, oder Suche in jedem Zug.",
      condition: "Die Unterstützung ist je Modell verschieden. Zusätzliche Kredite fallen nur an, wenn tatsächlich gesucht wird.",
    },
    deepResearch: {
      title: "Deep Research",
      description: "Schicken Sie eine Frage in eine erweiterte Recherche über viele Webquellen. Sie läuft als eigener Langzeitauftrag statt als normale Chatantwort und fragt vor dem Start nach einer Bestätigung.",
      condition: "Ab dem Pro-Plan. Verbraucht Kredite.",
    },
    sourceGrounding: {
      title: "Quellenabgleich",
      description: "AI Review zitiert die verglichenen Antworten. Jedes Zitat wird mit der Antwort abgeglichen, aus der es stammt, und der exakt übereinstimmende Anteil wird mit der Prüfung angezeigt.",
    ...
    itemVerification: {
      ...
      description: "Für einen als prüfbedürftig markierten Punkt starten Sie einen separaten Web-Check nur zu diesem Punkt und sehen, ob aktuelle Quellen ihn stützen, widerlegen oder offen lassen.",
      condition: "Ein optionaler Schritt getrennt von der Prüfung selbst. Angemeldet, mit einem Suchmodell und separat abgerechnet.",
    },
    footnote: "Der Kreditverbrauch hängt von den genutzten Modellen und der Anfragelänge ab. Die aktuellen Gewichte stehen auf der Preisseite.",
    cta: "Preise ansehen",
  },
  support: {
    ...
    plans: [
      {
        id: "free",
        title: "Free",
        blurb: "Leichte Alltagsnutzung und Test fortgeschrittener Modelle.",
      },
  ...
  faqs: [
    {
      ...
      answer: "Ja. Ohne Anmeldung können Sie bereits 3 KI-Modelle direkt bei derselben Frage vergleichen. Die Gastnutzung hat eigene Tages- und Monatskontingente und eine kurze Prüfung vor der ersten Nachricht. Ein Free-Konto schaltet einen breiteren Modellkatalog, höhere Nutzungslimits, gespeicherte Unterhaltungen und weitere Funktionen für angemeldete Nutzer frei.",
    },
    {
      question: "Welche Modelle kann ich vergleichen?",
      answer: "Der Katalog umfasst große Anbieter wie OpenAI, Anthropic, Google und Perplexity und ändert sich mit neuen oder abgekündigten Modellen. Die Modellseite führt die aktuelle Liste, die Statusseite den aktuellen Servicestand.",
    },
    {
      question: "Wie werden meine Daten verarbeitet?",
      answer: "Tomverse nutzt Anhangslimits, Chatsperren und schreibgeschützte Snapshots. Ausgewählte KI-Anbieter verarbeiten weiterhin die für Antworten nötigen Inhalte.",
    },
  ],
...
const es: LandingCopy = {
  ...
  primaryCta: "Comparar gratis",
  signedInCta: "Abrir Review",
  ...
  preview: {
    ...
    stages: [
      ...
      {
        title: "AI Review",
        caption: "Acuerdos, contradicciones, omisiones y qué verificar.",
      },
      {
  ...
  loop: {
    ...
    description: "Elige los modelos, envía la pregunta una sola vez y lee las respuestas en paralelo. Dos formas de aprovecharlas: un resumen rápido de diferencias o un AI Review completo.",
    steps: [
      {
        title: "Pregunta una vez",
        description: "Elige hasta tres modelos y envía un prompt o archivo compatible.",
      },
      {
        title: "Compara",
        description: "Lee las fortalezas de cada respuesta lado a lado.",
      },
      {
        title: "Ejecuta AI Review",
        description: "Ordena acuerdos, conflictos, omisiones y verificaciones.",
      },
    ],
    quickSummary: {
      title: "Resumen rápido de diferencias",
      description: "Con dos o más respuestas, deja que la IA resuma dónde divergen sin leerlas línea por línea. Está pensado para una lectura rápida, no para una revisión completa.",
      condition: "Bajo consumo de créditos. Los cupos difieren entre uso sin cuenta y con sesión iniciada.",
    },
    aiReviewBridge: "Cuando las diferencias merecen trabajarse a fondo, AI Review lleva esas mismas respuestas más lejos, sección a sección y con citas.",
    ...
    dualReviewer: "Cuando hay disponible un segundo revisor de otro proveedor, la comparación se ejecuta dos veces de forma independiente y la revisión indica dónde coincidieron y dónde no.",
    terminologyNote: "AI Review es el nombre de producto; dentro de la aplicación la acción aparece como «AI answer cross-review».",
    reviewBoundary: "AI Review solo compara las respuestas dadas. No navega por su cuenta, no demuestra hechos ni elige un ganador. Para un punto que requiera comprobación, puedes ejecutar una verificación web aparte desde la revisión. Las afirmaciones importantes siguen requiriendo fuentes primarias actuales, pruebas o revisión profesional.",
  ...
  evidence: {
    ...
    description: "Comparar varias respuestas acota la pregunta. Estas son las herramientas para lo que viene después: de dónde salió una afirmación y si sigue vigente.",
    webSearch: {
      title: "Búsqueda web",
      description: "Activa la búsqueda web en una conversación para que los modelos compatibles respondan con fuentes actuales y devuelvan citas. Puedes elegir desactivada, sugerida cuando la pregunta parece depender de información reciente, o búsqueda en cada turno.",
      condition: "La compatibilidad varía según el modelo. Solo se consumen créditos extra si la búsqueda se ejecuta realmente.",
    },
    deepResearch: {
      title: "Deep Research",
      description: "Envía una pregunta a una investigación ampliada sobre muchas fuentes web. Se ejecuta como un trabajo largo aparte, no como una respuesta de chat, y pide confirmación antes de empezar.",
      condition: "Desde el plan Pro. Consume créditos.",
    },
    sourceGrounding: {
      title: "Anclaje de citas",
      description: "AI Review cita las respuestas que compara. Cada cita se contrasta con la respuesta de la que procede y se muestra el porcentaje que coincidió exactamente.",
    ...
    itemVerification: {
      ...
      description: "Para un punto marcado como pendiente de verificar, ejecuta una comprobación web solo de ese punto y observa si las fuentes actuales lo respaldan, lo contradicen o no concluyen.",
      condition: "Paso opcional separado de la propia revisión. Con sesión iniciada, usa un modelo de búsqueda y se cobra aparte.",
    },
    footnote: "El consumo de créditos depende de los modelos usados y de la longitud de la solicitud. Los pesos actuales están en la página de precios.",
    cta: "Ver precios",
  },
  support: {
    ...
    plans: [
      {
        id: "free",
        title: "Free",
        blurb: "Uso diario ligero y prueba de modelos avanzados.",
      },
  ...
  faqs: [
    {
      ...
      answer: "Sí. Sin iniciar sesión ya puedes comparar 3 modelos de IA lado a lado en la misma pregunta. El uso sin cuenta tiene sus propios cupos diario y mensual, y una verificación rápida antes del primer mensaje. Una cuenta Free desbloquea un catálogo más amplio, límites más altos, conversaciones guardadas y otras funciones para usuarios con sesión iniciada.",
    },
    {
      question: "¿Qué modelos puedo comparar?",
      answer: "El catálogo incluye proveedores principales como OpenAI, Anthropic, Google y Perplexity, y cambia según se añaden o retiran modelos. La página de modelos tiene la lista actual y la página de estado indica el estado del servicio.",
    },
    {
      question: "¿Cómo se tratan mis datos?",
      answer: "Tomverse aplica límites de archivos, bloqueo de chats e instantáneas de solo lectura. Los proveedores elegidos siguen procesando el contenido necesario para responder.",
    },
  ],
...
const pt: LandingCopy = {
  ...
  primaryCta: "Comparar de graça",
  signedInCta: "Abrir o Review",
  ...
  preview: {
    ...
    stages: [
      ...
      {
        title: "AI Review",
        caption: "Consensos, contradições, omissões e o que verificar.",
      },
      {
        title: "Próxima ação",
  ...
  loop: {
    ...
    description: "Escolha os modelos, envie a pergunta uma única vez e leia as respostas lado a lado. Duas formas de aproveitá-las: um resumo rápido das diferenças ou um AI Review completo.",
    steps: [
      {
        title: "Pergunte uma vez",
        description: "Escolha até três modelos e envie um prompt ou arquivo compatível.",
      },
      {
        title: "Compare",
        description: "Leia os pontos fortes de cada resposta lado a lado.",
      },
      {
        title: "Execute AI Review",
        description: "Organize acordos, conflitos, omissões e verificações.",
      },
    ],
    quickSummary: {
      title: "Resumo rápido das diferenças",
      description: "Com duas ou mais respostas, deixe a IA resumir onde elas divergem, sem ler tudo linha a linha. Foi feito para uma leitura rápida, não para uma revisão completa.",
      condition: "Baixo consumo de créditos. As cotas diferem entre uso sem conta e com login.",
    },
    aiReviewBridge: "Quando as diferenças merecem ser trabalhadas, o AI Review leva as mesmas respostas adiante, seção a seção e com citações.",
    ...
    dualReviewer: "Quando há um segundo revisor de outro provedor disponível, a comparação roda duas vezes de forma independente e a revisão mostra onde os dois concordaram e onde não.",
    terminologyNote: "AI Review é o nome do produto; dentro do aplicativo a ação aparece como “AI answer cross-review”.",
    reviewBoundary: "O AI Review compara apenas as respostas fornecidas. Ele não navega por conta própria, não prova fatos nem escolhe um vencedor. Para um item que precise de checagem, você pode executar uma verificação web separada a partir da revisão. Afirmações importantes continuam exigindo fontes primárias atuais, testes ou revisão profissional.",
  ...
  evidence: {
    ...
    description: "Comparar várias respostas estreita a pergunta. Estas são as ferramentas para o que vem depois: de onde veio uma afirmação e se ela ainda se sustenta.",
    webSearch: {
      title: "Busca na web",
      description: "Ative a busca na web em uma conversa para que os modelos compatíveis respondam com fontes atuais e retornem citações. Escolha entre desligada, sugerida quando a pergunta parece depender de informação recente, ou busca a cada turno.",
      condition: "O suporte varia por modelo. Créditos extras só são usados quando a busca realmente acontece.",
    },
    deepResearch: {
      title: "Deep Research",
      description: "Envie uma pergunta para uma pesquisa estendida por muitas fontes da web. Ela roda como um trabalho longo à parte, não como uma resposta de chat, e pede confirmação antes de começar.",
      condition: "A partir do plano Pro. Usa créditos.",
    },
    sourceGrounding: {
      title: "Ancoragem das citações",
      description: "O AI Review cita as respostas que está comparando. Cada citação é confrontada com a resposta de origem, e a proporção que corresponde exatamente aparece junto da revisão.",
    ...
    itemVerification: {
      ...
      description: "Para um item marcado como a verificar, execute uma checagem web apenas desse item e veja se as fontes atuais o sustentam, contradizem ou são inconclusivas.",
      condition: "Etapa opcional separada da própria revisão. Com login, usa um modelo de busca e é cobrada à parte.",
    },
    footnote: "O consumo de créditos depende dos modelos usados e do tamanho da solicitação. Os pesos atuais estão na página de preços.",
    cta: "Ver preços",
  },
  support: {
    ...
    plans: [
      {
        id: "free",
        title: "Free",
        blurb: "Uso diário leve e teste de modelos avançados.",
      },
  ...
  faqs: [
    {
      ...
      answer: "Sim. Sem entrar, você já pode comparar 3 modelos de IA lado a lado na mesma pergunta. O uso sem conta tem cotas diária e mensal próprias e uma verificação rápida antes da primeira mensagem. Uma conta Free desbloqueia um catálogo mais amplo, limites maiores, conversas salvas e outros recursos para quem está conectado.",
    },
    {
      question: "Quais modelos posso comparar?",
      answer: "O catálogo inclui os principais provedores, como OpenAI, Anthropic, Google e Perplexity, e muda conforme modelos entram ou saem. A página de modelos traz a lista atual e a página de status indica o estado do serviço.",
    },
    {
      question: "Como meus dados são tratados?",
      answer: "O Tomverse aplica limites de anexos, bloqueio e snapshots somente leitura. Os provedores escolhidos continuam processando o conteúdo necessário para responder.",
    },
  ],
