  ...
  return (
    <article className={`rounded-2xl border p-4 ${row.overdue || row.status === "failed" ? "border-amber-500/30 bg-amber-500/10" : "border-zinc-800 bg-zinc-900/60"}`}>
      ...
      <div className="mt-3 grid gap-2 [&>*]:min-w-0">
        ...
        <input type="datetime-local" value={nextDueAt} onChange={(event) => setNextDueAt(event.target.value)} className="h-10 w-full min-w-0 rounded-xl border border-zinc-700 bg-zinc-950 px-3 text-sm text-white" aria-label={`${row.name} next due`} />
    ...
    <section className="rounded-3xl border border-zinc-800 bg-zinc-950/70 p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          <h2 className="mt-2 break-words text-2xl font-black text-white">Recovery and access checkpoints</h2>
