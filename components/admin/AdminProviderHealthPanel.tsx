    ...
    <section className="rounded-2xl border border-zinc-800 bg-zinc-950/70 p-5">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="flex items-start gap-4">
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className={`rounded-full border px-2.5 py-1 text-xs font-semibold ${apiKeyClass(provider.apiKeyConfigured)}`}>
        ...
        <div className="grid grid-cols-2 gap-3 text-sm sm:grid-cols-4 lg:min-w-[520px]">
          <Metric label="Success rate" value={provider.successRate24h === null ? "No traffic" : `${provider.successRate24h}%`} />
          <Metric label="24h calls" value={`${provider.successCount24h} / ${provider.failureCount24h}`} />
          <Metric label="Recent error" value={provider.recentErrorCode || "None"} />
          <Metric label="Budget used" value={`${provider.budgetUsagePercent}%`} valueClass={budgetClass(provider.budgetUsagePercent)} />
        </div>
      </div>
      ...
      <div className="mt-5 grid gap-4 border-t border-zinc-800 pt-5 lg:grid-cols-3">
        <div>
          ...
          <p className="mt-2 text-sm font-semibold text-zinc-200">Today internal {money(provider.todayCostMicroUsd)}</p>
          <p className="mt-1 text-xs text-zinc-500">Month internal {money(provider.monthCostMicroUsd)} of {money(provider.monthBudgetMicroUsd)}</p>
          <p className="mt-1 text-xs text-zinc-500">Provider reported {optionalMoney(provider.providerReportedMonthCostMicroUsd)}</p>
          <p className="mt-1 text-xs text-zinc-500">
            Balance {provider.balanceUsd === null ? "Not configured" : `$${provider.balanceUsd.toFixed(2)}`} ({provider.balanceSource})
          </p>
          ...
          <p className="mt-1 text-xs text-zinc-500">Last usage sync {dateLabel(provider.lastUsageSyncAt)}</p>
        </div>
        <div>
          <PanelLabel>Alerts</PanelLabel>
          <p className="mt-2 text-sm text-zinc-300">
            Alert threshold: {provider.alertLevel === "none" ? "below 50%" : `${provider.alertLevel}% reached`}
          </p>
          <p className="mt-1 text-xs text-zinc-500">Failure surge status is based on recent 24h provider errors.</p>
        </div>
        <div>
          <PanelLabel>Fallback Policy</PanelLabel>
          <p className="mt-2 text-sm text-zinc-300">{provider.fallback.reason}</p>
          <p className="mt-1 text-xs text-zinc-500">{fallbackModels.join(" / ") || "No fallback model configured"}</p>
        </div>
      </div>
          ...
          {provider.recentErrors.length === 0 ? (
            ...
            <div className="mt-2 space-y-2">
              {provider.recentErrors.map((error) => (
                <div key={`${error.code}-${error.updatedAt}`} className="flex items-center justify-between gap-3 rounded-xl border border-zinc-800 bg-zinc-900/60 px-3 py-2 text-xs">
                  <span className="min-w-0 truncate font-semibold text-zinc-200">{error.code}</span>
                  <span className="shrink-0 text-zinc-500">{error.count} / {dateLabel(error.updatedAt)}</span>
                </div>
              ))}
            </div>
          )}
          ...
          {provider.modelIncidents.length === 0 ? (
            ...
            <div className="mt-2 space-y-2">
              {provider.modelIncidents.map((incident) => (
                <div key={incident.modelId} className="rounded-xl border border-red-500/20 bg-red-500/10 px-3 py-2 text-xs">
                  <div className="flex items-center justify-between gap-3">
                    <span className="min-w-0 truncate font-semibold text-red-100">{incident.modelName}</span>
                    <span className="shrink-0 text-red-200">{incident.failureCount5m} failures</span>
                  </div>
                  <p className="mt-1 truncate text-red-200/70">{incident.recentErrorCode || "UNKNOWN"} / {dateLabel(incident.updatedAt)}</p>
                </div>
              ))}
            </div>
          )}
...
function PanelLabel({ children }: { children: React.ReactNode }) {
  return <div className="text-xs font-semibold uppercase tracking-[0.18em] text-zinc-500">{children}</div>;
}

export function AdminProviderHealthPanel({ initialDashboard }: { initialDashboard: ProviderHealthDashboard }) {
  ...
  const refreshDashboard = useCallback(async () => {
    ...
    try {
      ...
      setError(refreshError instanceof Error ? refreshError.message : "Provider data refresh failed.");
    } finally {
      setRefreshing(false);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      window.removeEventListener("tomverse:provider-health-refresh", refreshWhenVisible);
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-3 rounded-2xl border border-zinc-800 bg-zinc-950/70 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-xs text-zinc-500">Admin API refreshes every 30 seconds · Updated {dateLabel(dashboard.generatedAt)}</p>
        </div>
        <button type="button" onClick={() => void refreshDashboard()} disabled={refreshing} className="inline-flex h-10 cursor-pointer items-center justify-center gap-2 rounded-xl border border-zinc-700 bg-zinc-900 px-4 text-sm font-bold text-zinc-200 transition hover:bg-zinc-800 disabled:cursor-wait disabled:opacity-60">
          <RefreshCw className={`h-4 w-4 ${refreshing ? "animate-spin" : ""}`} />
          {refreshing ? "Refreshing" : "Refresh now"}
        </button>
      </div>
      {error && (
        <div role="alert" className="flex items-center gap-2 rounded-xl border border-red-500/30 bg-red-500/10 px-4 py-3 text-sm font-semibold text-red-200">
