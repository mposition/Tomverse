    ...
    <details className={`rounded-2xl border p-4 ${overdue ? "border-red-500/30 bg-red-500/10" : "border-zinc-800 bg-zinc-900/60"}`}>
      <summary className="cursor-pointer list-none">
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex items-center gap-2"><span className="rounded-full border border-zinc-700 px-2 py-1 text-xs font-bold text-zinc-200">{row.status}</span>{row.legalHold ? <span className="rounded-full border border-amber-500/30 px-2 py-1 text-xs font-bold text-amber-200">Legal hold</span> : null}<ChevronRight className="h-4 w-4 text-zinc-500" /></div>
        </div>
      </summary>
...
export function AdminPrivacyRequestsPanel() {
  ...
  return (
    <section className="rounded-3xl border border-zinc-800 bg-zinc-950/70 p-5">
      <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-blue-300">Privacy operations</p><h2 className="mt-2 text-2xl font-black text-white">Data rights request queue</h2><p className="mt-2 text-sm text-zinc-400">Track access, export, deletion, correction, deadlines, and documented legal retention exceptions.</p></div><button type="button" onClick={() => void load()} disabled={loading} className="inline-flex h-10 items-center gap-2 rounded-xl border border-zinc-700 px-3 text-xs font-bold text-zinc-200"><RefreshCw className={`h-4 w-4 ${loading ? "animate-spin" : ""}`} /> Refresh</button></div>
