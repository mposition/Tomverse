...
import { adminFeedbackInboxMessages } from "@/lib/adminMessages/feedbackInbox";
 ...
 */
const traceVerificationLabel = (
  feedback: FeedbackRow,
  labels: FeedbackInboxMessages["verification"]
) => {
  switch (feedback.errorReportVerification) {
    ...
    case "missing_token":
      return feedback.clientErrorCode === "EMPTY_RESPONSE"
        ? labels.clientEmptyResponse
        : labels.missingToken;
    case "expired":
      return labels.expired;
    case "invalid_signature":
      return labels.invalidSignature;
    case "payload_mismatch":
      return labels.payloadMismatch;
    case "unsupported_version":
      return labels.unsupportedVersion;
    case "untrusted_trace_source":
      return labels.untrustedTraceSource;
    default:
      return feedback.traceId ? labels.manualTrace : null;
  }
};

const evidenceAvailabilityLabel = (
  value: string | null,
  labels: FeedbackInboxMessages["evidence"]
) => {
  switch (value) {
    case "recorded":
      return labels.recorded;
    case "intentionally_not_recorded":
      return labels.intentionallyNotRecorded;
    case "existing_limit_event":
      return labels.existingLimitEvent;
    case "existing_provider_event":
      return labels.existingProviderEvent;
    case "not_yet_available":
      return labels.notYetAvailable;
    case "ambiguous_trace":
      return labels.ambiguousTrace;
    default:
      return null;
  }
};
 ...
 * already committed.
 */
const userNotificationSentence = (
  result: UserNotificationResult | undefined,
  sentences: FeedbackInboxMessages["notification"]
) => {
  if (!result) return "";
  if (result.queued) {
  ...
  const updateStatus = async (
    ...
    try {
      ...
      if (!response.ok || !data?.feedback) {
        throw new Error(data?.error || m.toast.updateFailed);
      }
      setItems((current) =>
        current.map((item) =>
          item.id === id
      ...
      dispatchAppToast(
        m.toast.updated(userNotificationSentence(data.userNotification, m.notification)),
        "success"
      );
      return true;
    } catch (error) {
      dispatchAppToast(
        error instanceof Error ? error.message : m.toast.updateFailedFallback,
        "error"
      );
      return false;
    } finally {
            ...
            return (
              ...
              >
                ...
                <div className="mt-3 grid gap-2 rounded-xl border border-zinc-800 bg-zinc-950/70 p-3 text-xs text-zinc-500 md:grid-cols-2 xl:grid-cols-4">
                  <span className="truncate">{m.meta.trace(feedback.traceId || "-")}</span>
                  <span className="truncate">{m.meta.model(feedback.modelId || "-")}</span>
                  <span className="truncate">{m.meta.plan(feedback.plan || "-")}</span>
                  <span>{m.meta.attachments(feedback.attachmentCount)}</span>
                  <span className="truncate">{m.meta.path(feedback.path || "-")}</span>
                  <span className="truncate xl:col-span-3">
                    {m.meta.userAgent(feedback.userAgent || "-")}
                  </span>
                </div>
                ...
                {/*
                  ...
                  is invented here.
                */}
                {traceVerificationLabel(feedback, m.verification) ? (
                  <div
                  ...
                  >
                    ...
                    <span className="truncate">
                      ...
                      {feedback.errorReportVerification === "verified"
                        ? m.traceSourceAuthenticated
                        : m.traceSourceClaim(feedback.traceProvenance || "unknown")}
                    </span>
                    {feedback.clientErrorCode ? (
                      <span className="truncate">
                        {m.clientCode(feedback.clientErrorCode)}
                      </span>
                    ) : null}
                    {evidenceAvailabilityLabel(feedback.evidenceAvailability, m.evidence) ? (
                      <span className="truncate">
                        {evidenceAvailabilityLabel(feedback.evidenceAvailability, m.evidence)}
                      </span>
                    ) : null}
                    ...
                    {feedback.traceEvidence ? (
                      ...
                      >
                        <span className="truncate">
                          {m.evidenceRow.serverCode(
                            feedback.traceEvidence.errorCode || "-",
                            feedback.traceEvidence.classificationSource
                          )}
                        </span>
                        <span className="truncate">
                          {m.evidenceRow.route(feedback.traceEvidence.routeClass)}
                          {feedback.traceEvidence.phase
                            ? ` / ${feedback.traceEvidence.phase}`
                            : ""}
                        </span>
                        <span className="truncate">
                          {m.evidenceRow.release(
                            feedback.traceEvidence.release || "-",
                            feedback.traceEvidence.environment
                          )}
                        </span>
                        <span className="truncate">
                          {m.evidenceRow.provider(
                            feedback.traceEvidence.provider || "-",
                            feedback.traceEvidence.modelId || "-"
                          )}
                        </span>
                        <span className="truncate">
                          {m.evidenceRow.occurred(feedback.traceEvidence.occurredAt)}
                        </span>
                        <span className="truncate">
                          {m.evidenceRow.sentryEvent(feedback.traceEvidence.sentryEventId || "-")}
                        </span>
                      </div>
                    ) : null}
                  </div>
  ...
  return (
    ...
    >
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label={m.dialog.label(m.statuses[status])}
      ...
      >
        ...
        >
          {replyState === "tooShort"
            ? m.dialog.replyTooShort(FEEDBACK_USER_REPLY_MIN_LENGTH)
            : m.dialog.replyHint(userReply.trim().length, FEEDBACK_USER_REPLY_MAX_LENGTH)}
        </p>
        ...
        {notifiable && !alreadyCompleted ? (
          ...
          >
            <p className="text-xs font-bold uppercase tracking-[0.14em] text-zinc-400">
              {m.dialog.preview(feedback.language)}
            </p>
