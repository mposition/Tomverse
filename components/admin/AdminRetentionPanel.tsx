...
export function AdminRetentionPanel() {
  const m = useAdminMessages(adminRetentionMessages);
      ...
      if (!response.ok || !payload || !("items" in payload)) {
        throw new Error(
          payload && "error" in payload && payload.error
            ? payload.error
            : messagesRef.current.toast.loadFailed
        );
      }
      setData(payload);
    } catch (error) {
      dispatchAppToast(
        error instanceof Error ? error.message : messagesRef.current.toast.loadFailed,
        "error"
      );
    } finally {
      setLoading(false);
    ...
    try {
      ...
      if (!response.ok || !payload?.run) {
        ...
        const described = describeAdminApiFailure({
          ...
          error: payload?.error,
          code: payload?.code,
          fallback: m.toast.cleanupFailed,
          locale: apiLocale,
        });
      ...
      setFailure(null);
      dispatchAppToast(
        mode === "execute" ? m.toast.executed : m.toast.dryRunCompleted,
        "success"
      );
      setConfirmText("");
      ...
      dispatchAppToast(
        error instanceof Error ? error.message : m.toast.cleanupFailed,
        "error"
      );
    } finally {
      setRunning(null);
              ...
              <div className="rounded-xl border border-zinc-800 bg-zinc-950 px-3 py-2">
                ...
                */}
                <p className="font-bold text-zinc-500">
                  {item.action === "keep" ? m.beyondFloor : m.cleanupCount}
                </p>
                <p className="mt-1 text-xl font-black text-white">{item.staleCount}</p>
              </div>
              <div className="rounded-xl border border-zinc-800 bg-zinc-950 px-3 py-2">
                <p className="font-bold text-zinc-500">{m.oldestRecord}</p>
        ...
        {preview ? (
          <div className="mt-4 rounded-xl border border-zinc-700 bg-zinc-950/80 p-3">
            <p className="text-xs font-bold uppercase tracking-[0.16em] text-zinc-400">
              {m.dryRunAt(dateLabel(preview.ranAt))}
            </p>
