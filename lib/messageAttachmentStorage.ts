...
export const registerFinalizedUpload = async (input: {
  ...
  size: number;
  /**
   * How the request layer will read this file, from the caller's own reading
   * of the shared format table (`lib/chatAttachmentFormats.ts`).
   *
   * Passed in rather than derived here, and that is the point: the format
   * table decides what a file is -- by name first, then by declared type --
   * and a second derivation from the media type alone would disagree with it
   * for every format the table places by extension.
   */
  kind: MessageAttachmentKind;
