/**
 * Batch 023 — `injection_directives:ko`, second batch. **Candidate pool.**
 *
 ...
 * blanket override, "remember this", an instruction dressed as a preference, a
 * forged system tag, an instruction inside a document, a URL offered as a rule
 * source, a comment in code, an identity swap, a prompt-disclosure demand, a
 * delayed trigger, an encoded instruction, a forged delimiter, markdown
 * exfiltration, staff impersonation, an instruction inside translation input,
 * one inside an email, a trusted-domain request, role reversal, a rule stack,
 * an instruction in a CV, a conditional, mixed languages, a forged tool call,
 * a threat and an instruction in review text.
 *
 * This batch works the containers and disguises those do not reach: a
 * calendar invite, a spreadsheet cell, a filename, image alt text, a chat
 * export being imported, a git commit message, a JSON field, a QR payload —
 * and the grammatical disguises of a directive written as a fact about the
 * user, as a memory the assistant supposedly already holds, as a correction,
 * as a schedule, or as a policy the user claims to be quoting.
 *
