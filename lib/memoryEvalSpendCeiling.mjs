 ...
 *
 *   figures say over                    → over, `derived`
 *   figures say under, flag true        → over, `flag`
 *   figures say under, flag false or none → under, `derived`
 *   figures stated but unusable, flag true → over, `flag`
 *   figures stated but unusable, otherwise → `unknown`
 *   no figures, flag present            → the flag
 *   no figures, no flag                 → `unknown`
 *
 * **A `false` flag never rescues; a `true` flag can always condemn.** That is
 * the whole rule, and both halves are deliberate.
 *
 * An artifact whose own numbers say it went over did go over, and a `false`
 * flag beside them is a claim the numbers contradict. Numbers that cannot be a
 * spend and a ceiling are not rescued by a `false` flag either: it would be
 * vouching for a comparison nobody can reproduce.
 *
 * A `true` flag survives both, and that is not an oversight. The harness wrote
 * it from its own live state at the end of its own run — a first-hand
 * statement that the run went over — and unusable figures make the derivation
 * impossible without making that statement false. Honouring it also records
 * the more specific reason: `exceededCostCeiling` says the run overspent,
 * where `spendComparableToCeiling` says only that nobody can tell. Both
 * discard the run, so the choice is about which fact ends up in the record.
 *
...
export function manifestExceededSpendCeiling(manifest) {
    ...
    // Reached only with no `true` flag, which is the ordering the contract
    // above describes: a `true` flag has already answered by this point.
    //
    // Figures that are there and unusable are worse than figures that are
    // absent, because something wrote them. `unknown` rather than falling
    // through to a `false` flag, so the artifact is discarded instead of
    // vouched for by a comparison nobody can reproduce.
