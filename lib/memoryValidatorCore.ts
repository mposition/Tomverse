...
const IMPERATIVE_PATTERNS: readonly RegExp[] = [
    ...
    // than by negating the verb, so "쓰지 마세요" never contains 하세요 and the
    // affirmative list above misses every prohibition — which is the more
    // directive half of the pair.
 ...
 * instruction wearing a declarative grammar, and none of the imperative
 * patterns above catch it because it has no imperative verb. Demoted rather
 * than rejected: the underlying preference may be real, and a person can
 * approve it once it is rewritten as a statement about the user.
 */
const SECOND_PERSON_ADDRESS_PATTERNS: readonly RegExp[] = [
    // The contraction has no space before it, so it needs its own alternative
    // rather than riding on the `you\s+` prefix.
    /^you(\s+(are|were|will\s+be)|\s*['’]re)\b/i,
    // No `\b` here: JS word boundaries are ASCII-only, so `\b` after a Hangul
    // syllable followed by a space never matches. An explicit space-or-end is
    // the same intent that actually holds.
    /^(너는|넌|당신은|당신이)(\s|$)/,
];
